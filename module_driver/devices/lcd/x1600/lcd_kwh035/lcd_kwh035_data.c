...
static int gpio_lcd_power_en = -1; // GPIO_PD(8)
static int gpio_lcd_rst = -1; // GPIO_PD(14)
static int gpio_lcd_cs = -1; // GPIO_PD(18)
static int gpio_lcd_rd = -1; // -1
...
static struct regulator *kwh035_regulator = NULL;
...
{
    if (kwh035_regulator)
        regulator_enable(kwh035_regulator);
...
static int kwh035_power_off(struct lcdc *lcdc)
{
    ...
    if (kwh035_regulator)
        regulator_disable(kwh035_regulator);
...
static int lcd_kwh035_init(void)
{
    ...
    if (strcmp("-1", lcd_regulator_name) && strlen(lcd_regulator_name)) {
        kwh035_regulator = regulator_get(NULL, lcd_regulator_name);
        if(!kwh035_regulator) {
...
err_rd:
    if (kwh035_regulator)
        regulator_put(kwh035_regulator);
