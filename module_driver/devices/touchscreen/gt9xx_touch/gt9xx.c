...
static u32 gtp_x_min;
static u32 gtp_y_min;
u32 gtp_x_max;
u32 gtp_y_max;
static u32 gtp_max_touch_number;
static u32 gtp_x_coords_flip;
static u32 gtp_y_coords_flip;
static u32 gtp_x_y_coords_exchange;

int gtp_rst_gpio;
int gtp_int_gpio;
int gtp_power_on_gpio;
char *gtp_regulator_name = "";
...
module_param_gpio_named(gtp_power_on_gpio, gtp_power_on_gpio, 0644);
module_param_gpio_named(gtp_reset_gpio, gtp_rst_gpio, 0644);
module_param_gpio_named(gtp_irq_gpio, gtp_int_gpio, 0644);
module_param_named(gtp_max_touch_number, gtp_max_touch_number, int, 0644);

module_param_named(gtp_x_coords_flip, gtp_x_coords_flip, int, 0644);
module_param_named(gtp_y_coords_flip, gtp_y_coords_flip, int, 0644);
module_param_named(gtp_x_y_coords_exchange, gtp_x_y_coords_exchange, int, 0644);

module_param_named(gtp_regulator_name, gtp_regulator_name, charp, 0644);

static struct regulator *gtp_regulator = NULL;
...
static inline int m_gpio_request(int gpio, const char *name)
{
    ...
    return GTP_GPIO_REQUEST(gpio, name);
}
...
static inline void m_gpio_gpio_as_input(int gpio)
{
    if (gpio >= 0)
        GTP_GPIO_AS_INPUT(gpio);
}
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
    ...
    input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_X, gtp_x_min, gtp_x_max, 0, 0);
    input_set_abs_params(ts->pen_dev, ABS_MT_POSITION_Y, gtp_y_min, gtp_y_max, 0, 0);
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    point_data = kmalloc(2 + 1 + 8 * gtp_max_touch_number, GFP_KERNEL);
    ...
    if (touch_num > gtp_max_touch_number)
        goto exit_work_func;

    if (touch_num > 1) {
        u8 *buf = kmalloc(8 * gtp_max_touch_number, GFP_KERNEL);
        buf[0] = (GTP_READ_COOR_ADDR + 10) >> 8;
        buf[1] = (GTP_READ_COOR_ADDR + 10) & 0xff;
        // u8 buf[8 * 2] = {(GTP_READ_COOR_ADDR + 10) >> 8, (GTP_READ_COOR_ADDR + 10) & 0xff};
                ...
                if(!input_w)
                {
                   GTP_DEBUG("Pen touch DOWN(Slot)!");
                    ...
                    if(pre_finger)
                    {
                        dev_active = 1;
                        pre_finger = 0;
                        for (i = 0; i < gtp_max_touch_number; i++)
                        {
                            gtp_touch_up(ts, i);
                        }
                    }
...
void gtp_int_sync(s32 ms)
{
    GTP_GPIO_OUTPUT(gtp_int_gpio, 0 | 0x14);
    msleep(ms);
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    m_gpio_direction_output(gtp_rst_gpio, 0);
    msleep(10);

    if (gtp_regulator)
        regulator_enable(gtp_regulator);
    m_gpio_direction_output(gtp_power_on_gpio, 0);

    msleep(10);
    m_gpio_direction_output(gtp_rst_gpio, 0);   // begin select I2C slave addr
    msleep(10);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    m_gpio_direction_output(gtp_int_gpio, (client->addr == 0x14) & 1);
    msleep(2);                          // T3: > 100us

    m_gpio_direction_output(gtp_rst_gpio, 1);
    msleep(10);                          // T4: > 5ms
    m_gpio_direction_output(gtp_int_gpio, 0);

    msleep(50);
    m_gpio_gpio_as_input(gtp_int_gpio);    // end select I2C slave addr
...
//         GTP_GPIO_OUTPUT(gtp_int_gpio, 1 | 0x14);
//         msleep(5);
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
    ...
    u32 config_x_max = gtp_x_max;
    u32 config_y_max = gtp_y_max;
 ...
 */
...
#if GTP_CUSTOM_CFG
    if (gtp_x_y_coords_exchange) {
        config_x_max = gtp_y_max;
        config_y_max = gtp_x_max;
    }

    config[RESOLUTION_LOC]     = (u8)config_x_max;
    config[RESOLUTION_LOC + 1] = (u8)(config_x_max >> 8);
    config[RESOLUTION_LOC + 2] = (u8)config_y_max;
    config[RESOLUTION_LOC + 3] = (u8)(config_y_max >> 8);
    ...
    if (ret < 0) {
        ...
        ts->abs_x_max = config_x_max;
        ts->abs_y_max = config_y_max;
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    if (strcmp("-1", gtp_regulator_name) && strlen(gtp_regulator_name)) {
        gtp_regulator = regulator_get(NULL, gtp_regulator_name);
        if(!gtp_regulator) {
            printk(KERN_ERR "Failed to get lcd_regulator!\n");
            ret = -EINVAL;
        }
    }

    ret = m_gpio_request(gtp_power_on_gpio, "GTP POWER_ON");
    if (ret < 0) {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d",(s32)gtp_power_on_gpio,ret);
        ret = -ENODEV;
    }
    ...
    ret = m_gpio_request(gtp_rst_gpio, "GTP RST PORT");
    if (ret < 0) {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d",(s32)gtp_rst_gpio,ret);
        ret = -ENODEV;
    }

    m_gpio_gpio_as_input(gtp_rst_gpio);

    gtp_reset_guitar(ts->client, 20);

    if (ret < 0) {
        m_gpio_free(gtp_power_on_gpio);
        m_gpio_free(gtp_rst_gpio);
        GTP_GPIO_FREE(gtp_int_gpio);
        if (gtp_regulator)
            regulator_put(gtp_regulator);
    }
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, gtp_x_max, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, gtp_y_max, 0, 0);
...
Output:
    None.
*******************************************************/
// void goodix_ts_suspend(struct goodix_ts_data *ts)
// {
//     s8 ret = -1;
...
Output:
    None.
*******************************************************/
// void goodix_ts_resume(struct goodix_ts_data *ts)
// {
//     s8 ret = -1;
//     GTP_DEBUG_FUNC();
//     if (ts->enter_update) {
