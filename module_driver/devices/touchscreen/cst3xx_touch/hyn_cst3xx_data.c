...
struct hynitron_fw_array {
    ...
    { "hyn_cst3xx_K1336A_fw", 0x1613,  cst3_fw },	// default
};
...
// #define TPD_MAX_X				    170
// #define TPD_MAX_Y				    320
// #define CST3XX_I2C_ADDR				0x55
// #define CST3XX_I2C_ADDR_BOOT		0x55
...
static u32 cst_x_min;
static u32 cst_y_min;
u32 cst_x_max;
u32 cst_y_max;
static u32 cst_max_touch_number;
static u32 cst_x_coords_flip;
static u32 cst_y_coords_flip;
static u32 cst_x_y_coords_exchange;

int cst_rst_gpio = -1;
int cst_int_gpio = -1;
int cst_i2c_power_en_gpio = -1;
int cst_i2c_power_en_level = -1;
...
/* get ts data */
module_param_named(cst_i2c_bus_num , i2c_bus_num, int, 0644);
module_param_named(cst_i2c_addr, cst_i2c_addr, int, 0644);
// for display coords
module_param_named(cst_x_coords_min, cst_x_min, int, 0644);
module_param_named(cst_y_coords_min, cst_y_min, int, 0644);
module_param_named(cst_x_coords_max, cst_x_max, int, 0644);
module_param_named(cst_y_coords_max, cst_y_max, int, 0644);

// for reset, irq gpio info
module_param_gpio_named(cst_reset_gpio, cst_rst_gpio, 0644);    //GPIO_PB(20)
module_param_gpio_named(cst_irq_gpio, cst_int_gpio, 0644);      //GPIO_PB(21)
module_param_gpio_named(cst_power_en_gpio, cst_i2c_power_en_gpio, 0644);  //GPIO_PB19

module_param_named(cst_power_en_level, cst_i2c_power_en_level, int, 0644);

module_param_named(cst_max_touch_number, cst_max_touch_number, int, 0644);

module_param_named(cst_x_coords_flip, cst_x_coords_flip, int, 0644);
module_param_named(cst_y_coords_flip, cst_y_coords_flip, int, 0644);
module_param_named(cst_x_y_coords_exchange, cst_x_y_coords_exchange, int, 0644);

module_param_named(cst_regulator_name, cst_regulator_name, charp, 0644);
...
static int hyn_cst3xx_power_ctrl(struct cst3xx_ts_data *ts, bool power_on)
{
	...
	if (power_on) {
        m_gpio_direction_output(cst_i2c_power_en_gpio, cst_i2c_power_en_level);
        if(ts->vcc_i2c) {
            ret = regulator_enable(ts->vcc_i2c);
            if (ret) {
                printk("can not enable vcc_i2c 1.8V: %d", ret);
                return ret;
            }
        }
    } else {
        ...
        if(ts->vcc_i2c) {
            ...
            if (ret){
                printk("can not disable vcc_i2c 1.8V: %d", ret);
                return ret;
            }
        }
    }
...
START:
	...
	printk("linc cst3xx [cst3xx] the chip ic_checkcode:0x%xretry:0x%x.\r\n",ic_checkcode,retry);
...
static void cst3xx_touch_report(struct work_struct *work)
{
	...
	if(cnt > cst_max_touch_number) goto OUT_PROCESS;
...
FINGER_PROCESS:
	...
	if (cst_x_y_coords_exchange) {
		input_xy_temp=input_x;
		input_x =input_y ;
		input_y = input_xy_temp;
    }

	if (cst_x_coords_flip)
		input_x = cst_x_max-input_x;

	if (cst_y_coords_flip)
		input_y = cst_y_max-input_y;
...
//	u8 index = 0;
	...
	//input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 0,(5+1),0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, cst_x_min, cst_x_max,0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, cst_y_min, cst_y_max,0, 0);
...
static s32 cst3xx_request_io_port(struct cst3xx_ts_data *ts)
{
    ...
    if (strcmp("-1", cst_regulator_name) && strlen(cst_regulator_name)) {
		ts->vcc_i2c = regulator_get(&ts->client->dev, cst_regulator_name);
        ...
        ret = m_gpio_request(cst_i2c_power_en_gpio, "cst power_enable");
        if(ret < 0) {
            printk(KERN_ERR "cst power enable pin request fail!\n");
            ret = -ENODEV;
        }
    }
...
static int cst3xx_ts_get_dt_coords(struct device *dev, char *name,struct cst3xx_ts_platform_data *pdata)
{
	if (!strcmp(name, "hynitron,panel-coords")) {
		pdata->panel_minx = cst_x_min;
		pdata->panel_miny = cst_y_min;
		pdata->panel_maxx = cst_x_max;
		pdata->panel_maxy = cst_y_max;
	} else if (!strcmp(name, "hynitron,display-coords")) {
		pdata->x_min = cst_x_min;
		pdata->y_min = cst_y_min;
		pdata->x_max = cst_x_max;
		pdata->y_max = cst_y_max;
	} else {
...
static int cst3xx_parse_dt(struct device *dev,struct cst3xx_ts_platform_data *pdata)
{
	...
	pdata->reset_gpio= cst_rst_gpio;
	if (pdata->reset_gpio < 0)
	{
		printk("CST3XX Invalid GPIO, get reset-gpio failed\n");
		return pdata->reset_gpio;
	}
	pdata->irq_gpio = cst_int_gpio;
	if (pdata->irq_gpio < 0)
	{
...
static int cst3xx_tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (cst_max_touch_number > 10)
        cst_max_touch_number = 10;

    if (cst_max_touch_number < 2)
...
//static unsigned short force[] = { 0, cst_i2c_addr, I2C_CLIENT_END, I2C_CLIENT_END };
//static const unsigned short *const forces[] = { force, NULL };

/*
	static struct i2c_board_info __initdata cst3xx_i2c_tpd = {
		I2C_BOARD_INFO(TPD_DEVICE_NAME, cst_i2c_addr)
	};
*/
/*
...
static struct i2c_board_info touch_cst3xx_ts_info = {
...
static int __init cst3xx_ts_init(void)
{
    ...
    touch_cst3xx_ts_info.addr = cst_i2c_addr;
    ...
    g_i2c_client = i2c_register_device(&touch_cst3xx_ts_info, i2c_bus_num);
...
static void __exit cst3xx_ts_exit(void)
{
    ...
    // m_gpio_free(cst_rst_gpio);
    // m_gpio_free(cst_int_gpio);
    m_gpio_free(cst_i2c_power_en_gpio);
