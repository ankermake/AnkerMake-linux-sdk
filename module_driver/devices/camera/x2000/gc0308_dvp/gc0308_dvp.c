...
#define GC0308_DEVICE_NAME              "gc0308"
...
static struct regulator *gc0308_regulator = NULL;
...
static int gc0308_detect(struct i2c_client *i2c)
{
    ...
    printk(KERN_DEBUG "gc0308 get chip id = %02x\n", (int)gc0308_chip_id_regs[1].value);
...
static int init_gpio(void)
{
    ...
    if (strcmp(regulator_name, "-1") && strlen(regulator_name)) {
        gc0308_regulator = regulator_get(NULL, regulator_name);
        if (IS_ERR(gc0308_regulator)) {
...
static void deinit_gpio(void)
{
    ...
    if (gc0308_regulator)
        regulator_put(gc0308_regulator);
}
...
static void gc0308_power_off(void)
{
    ...
    if (gc0308_regulator)
        regulator_disable(gc0308_regulator);
...
static int gc0308_power_on(void)
{
    ...
    if (gc0308_regulator)
        regulator_enable(gc0308_regulator);
...
static struct sensor_attr gc0308_sensor_attr = {
    ...
    .sensor_info = {
        .private_init_setting   = gc0308_init_regs,
        .width                  = GC0308_WIDTH,
        ...
        const struct i2c_device_id *id)
{
    ...
    ret = dvp_init_select_gpio(&gc0308_sensor_attr.dvp, gc0308_sensor_attr.dvp.gpio_mode);
    if (ret) {
        deinit_gpio();
        return ret;
    }

    ret = camera_register_sensor(cam_bus_num, &gc0308_sensor_attr);
    if (ret) {
        dvp_deinit_gpio();
        deinit_gpio();
        return ret;
...
{
    camera_unregister_sensor(cam_bus_num, &gc0308_sensor_attr);
    dvp_deinit_gpio();
    deinit_gpio();
...
static struct i2c_device_id gc0308_id[] = {
...
static __init int init_gc0308(void)
{
    ...
    gc0308_driver.driver.name = sensor_name;
    strcpy(gc0308_id[0].name, sensor_name);
    if (i2c_addr != -1)
        sensor_gc0308_info.addr = i2c_addr;
    strcpy(sensor_gc0308_info.type, sensor_name);
    gc0308_sensor_attr.device_name = sensor_name;
...
MODULE_DESCRIPTION("x2000 gc0308 driver");
MODULE_LICENSE("GPL");
