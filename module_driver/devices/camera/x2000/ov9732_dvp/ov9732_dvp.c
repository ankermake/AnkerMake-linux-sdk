...
static int reset_gpio       = -1;       // GPIO_PE(22)
 ...
 * dvdd=1.500000
 */
static struct regval_list ov9732_init_regs_1280_720_30fps_dvp[] = {
    /*
...
static int ov9732_set_integration_time(int value)
{
    ...
    ret = ov9732_write(i2c_dev, 0x3502, (unsigned char)(expo & 0xff));
    if (ret < 0)
        return ret;
    ret = ov9732_write(i2c_dev, 0x3501, (unsigned char)((expo >> 8) & 0xff));
    if (ret < 0)
        return ret;
    ret = ov9732_write(i2c_dev, 0x3500, (unsigned char)((expo >> 16) & 0xf));
    if (ret < 0)
        return ret;
...
static int ov9732_set_analog_gain(int value)
{
    ...
    /* again */
    ov9732_write(i2c_dev, 0x350b, (unsigned char)(value & 0xff));
#else
    /* again */
    ov9732_write(i2c_dev, 0x350b, (unsigned char)((value >> 8) & 0xff));
    /* dgain */
    ov9732_write(i2c_dev, 0x5181, (unsigned char)(value & 0xff));
    ov9732_write(i2c_dev, 0x5183, (unsigned char)(value & 0xff));
    ov9732_write(i2c_dev, 0x5185, (unsigned char)(value & 0xff));
#endif
...
static int ov9732_set_fps(int fps)
{
    ...
    /* prediv */
    val = 0;
    ret += ov9732_read(i2c_dev, 0x3080, &val);
    val &= 0x7;
    switch (val) {
        case 0:
            pclk = mclk/1;
    ...
    /* mul */
    val = 0;
    ret += ov9732_read(i2c_dev, 0x3081, &val);
    pclk *= val;
    /* sysdiv */
    val = 0;
    ret += ov9732_read(i2c_dev, 0x3082, &val);
    pclk /= ((val&0xf)+1);
    /* pixdiv */
    val = 0;
    ret += ov9732_read(i2c_dev, 0x3083, &val);
    if (val&0x1)
        pclk /= 4;
    else
        pclk /= 2;
    ...
    val = 0;
    ret += ov9732_read(i2c_dev, 0x380c, &val);
    hts = val<<8;
    val = 0;
    ret += ov9732_read(i2c_dev, 0x380d, &val);
    hts |= val;
    if (0 != ret) {
        printk(KERN_ERR "err: ov9732 read err\n");
        return ret;
    }
...
static struct sensor_attr ov9732_sensor_attr = {
    ...
    .sensor_info = {
        .private_init_setting   = ov9732_init_regs_1280_720_30fps_dvp,
        .width                  = OV9732_WIDTH,
        ...
        const struct i2c_device_id *id)
{
    ...
    ret = dvp_init_select_gpio(&ov9732_sensor_attr.dvp,ov9732_sensor_attr.dvp.gpio_mode);
    if (ret) {
        deinit_gpio();
        return ret;
    }
