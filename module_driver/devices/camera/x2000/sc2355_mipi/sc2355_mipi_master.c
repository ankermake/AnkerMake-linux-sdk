...
struct sensor_light_ctl sc2355_light_ctl;
...
static struct regval_list sc2355_regs_master[] = {
    //output fsync
    {0x300b, 0x44},
...
static int sc2355_power_on(void)
{
	...
	ret = aw36515_power_on(AW_MODE_IR, 1, 50);
    if (ret) {
        printk(KERN_ERR "sc2355: failed to power on aw36515\n");
        goto fail;
    }
...
static int sc2355_stream_on(void)
{
    ...
    sc2355_light_ctl.strobe_cnt = 0;
    enable_irq(sc2355_light_ctl.irq);

    ret = sc2355_write_array(i2c_dev, sc2355_regs_master);
    if (ret)
...
static void sc2355_stream_off(void)
{
    ...
    disable_irq(sc2355_light_ctl.irq);
...
static int sc2355_s_register(struct sensor_dbg_register *reg)
{
    ...
    if (reg->reg != SC2355_SET_LED_LEVEL)
        ...
        ret = aw36515_set_current_rate(1, reg->val);
...
int sc2355_frame_start_callback(void)
{
    return sc2355_light_ctl.strobe_cnt;
}
EXPORT_SYMBOL(sc2355_frame_start_callback);
...
static int init_interrupt(void)
{
    ...
    ret = init_aw36515();
    if (ret) {
        printk(KERN_ERR "sc2355: init_aw36515 fail\n");
        return -1;
    }

    if (light_ctl_gpio == -1){
        printk(KERN_ERR "sc2355: Please light_ctl_gpio gpio setting \n");
        exit_aw36515();
        return -1;
    }
    ret = gpio_request(light_ctl_gpio, "light_ctl_gpio");
    if (ret) {
        printk(KERN_ERR "sc2355: failed to request light_ctl_gpio pin: %s\n", gpio_to_str(light_ctl_gpio, gpio_str));
        goto err_light_ctl_gpio;
    }
    gpio_direction_output(light_ctl_gpio, 0);

    if (strobe_gpio == -1){
        printk(KERN_ERR "sc2355: Please strobe_gpio gpio setting \n");
        goto err_strobe_gpio;
    }
    ret = gpio_request(strobe_gpio, "strobe_gpio");
    if (ret) {
        printk(KERN_ERR "sc2355: failed to request strobe_gpio pin: %s\n", gpio_to_str(strobe_gpio, gpio_str));
        goto err_strobe_gpio;
    }
    gpio_direction_input(strobe_gpio);

    sc2355_light_ctl.irq = gpio_to_irq(strobe_gpio);
    ret = request_irq(sc2355_light_ctl.irq, strobe_irq_handler, IRQF_TRIGGER_RISING, "strobe_gpio", (void *)&sc2355_light_ctl);
    if (ret != 0){
        printk(KERN_ERR "sc2355: failed to request_irq  %d\n", sc2355_light_ctl.irq);
