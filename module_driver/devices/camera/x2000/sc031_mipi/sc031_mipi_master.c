...
static int strobe_gpio      = -1;   // GPIO_PB(12)
static int light_ctl_gpio   = -1;   // GPIO_PC(8)

module_param_gpio(power_gpio, 0644);
module_param_gpio(reset_gpio, 0644);
module_param_gpio(pwdn_gpio, 0644);
module_param_gpio(strobe_gpio, 0644);
module_param_gpio(light_ctl_gpio, 0644);
...
#ifdef LIGHT_WITH_PWM
static int light_pwm_id             = -1;
#endif
static int light_pwm                = 50;
static int master_standby = 0;

struct sensor_light_ctl{
    int irq;
    int strobe_cnt;
};
struct sensor_light_ctl sc031_light_ctl;
...
static struct regval_list sc031_init_regs_640_480_30fps_mipi[] = {
 ...
 struct regval_list sc031_regs_master[] = {
...
static int sc031_stream_on(void)
{
    ...
    sc031_light_ctl.strobe_cnt = 0;
    enable_irq(sc031_light_ctl.irq);

    ret = sc031_write_array(i2c_dev, sc031_regs_master);
    if (ret)
        printk(KERN_ERR "sc031: failed to sc031_regs_master\n");
...
{
    disable_irq(sc031_light_ctl.irq);
...
int sc031_frame_start_callback(void)
{
    return sc031_light_ctl.strobe_cnt;
}
EXPORT_SYMBOL(sc031_frame_start_callback);
...
{
    struct sensor_light_ctl *light_ctl = data;
    ...
    light_ctl->strobe_cnt++;
    if ((light_ctl->strobe_cnt % 2) != 0) {
#ifndef LIGHT_WITH_PWM
        gpio_direction_output(light_ctl_gpio, 1);
#else
        pwm2_set_level(light_pwm_id, light_pwm);
#endif
    } else {
#ifndef LIGHT_WITH_PWM
        gpio_direction_output(light_ctl_gpio, 0);
#else
        pwm2_set_level(light_pwm_id, 0);
#endif
	}
    return IRQ_HANDLED;
}
...
static int init_interrupt(void)
{
    ...
    if (light_ctl_gpio == -1){
        printk(KERN_ERR "sc031: please light_ctl_gpio setting \n");
        return -1;
    }
#ifndef LIGHT_WITH_PWM
    ret = gpio_request(light_ctl_gpio, "light_ctl_gpio");
    if (ret) {
        printk(KERN_ERR "sc031: failed to request light_ctl_gpio pin: %s\n", gpio_to_str(light_ctl_gpio, gpio_str));
        goto err_light_ctl_gpio;
    }
    gpio_direction_output(light_ctl_gpio, 0);
#else
    light_pwm_id = pwm2_request(light_ctl_gpio, "light_ctl_gpio");
    if (light_pwm_id < 0) {
        printk(KERN_ERR "sc031: failed to request light_ctl_gpio pwm: %s\n", gpio_to_str(light_ctl_gpio, gpio_str));
        goto err_light_ctl_gpio;
    }
    struct pwm_config_data config = {
        .shutdown_mode = PWM_graceful_shutdown,
        .idle_level = PWM_idle_low,
        .accuracy_priority = PWM_accuracy_freq_first,
        .freq = 50000,
        .levels = 100,
    };
    pwm2_config(light_pwm_id, &config);
    pwm2_set_level(light_pwm_id, 0);
#endif

    if (strobe_gpio == -1){
        printk(KERN_ERR "sc031: please strobe_gpio setting \n");
        goto err_strobe_gpio;
    }
    ret = gpio_request(strobe_gpio, "strobe_gpio");
    if (ret) {
        printk(KERN_ERR "sc031: failed to request strobe_gpio pin: %s\n", gpio_to_str(strobe_gpio, gpio_str));
        goto err_strobe_gpio;
    }
    gpio_direction_input(strobe_gpio);

    sc031_light_ctl.irq = gpio_to_irq(strobe_gpio);
    sc031_light_ctl.strobe_cnt = 0;
    ret = request_irq(sc031_light_ctl.irq, strobe_irq_handler, IRQF_TRIGGER_FALLING, "strobe_gpio", (void *)&sc031_light_ctl);
    if (ret != 0){
        printk(KERN_ERR "sc031: failed to request_irq  %d\n", sc031_light_ctl.irq);
...
err_request_irq:
    gpio_free(strobe_gpio);
err_strobe_gpio:
#ifndef LIGHT_WITH_PWM
    gpio_free(light_ctl_gpio);
#else
    pwm2_release(light_pwm_id);
#endif
err_light_ctl_gpio:
