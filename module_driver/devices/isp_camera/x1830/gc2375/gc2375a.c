...
#if defined MD_X1830_GC2375A_480X800
#define GC2375A_WIDTH                   480
#define GC2375A_HEIGHT                  800
#elif defined MD_X1830_GC2375A_720X1200
#define GC2375A_WIDTH                   720
#define GC2375A_HEIGHT                  1200
#elif defined MD_X1830_GC2375A_1600X1200
...
static struct tx_isp_sensor_attribute gc2375a_attr;
...
static unsigned int gc2375a_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
...
static unsigned int gc2375a_alloc_dgain(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_dgain)
{
    return isp_gain;
}
...
static int gc2375a_set_integration_time(struct tx_isp_subdev *sd, int value)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR "gc2375a: gc2375a_write error  %d\n" ,__LINE__ );
        return ret;
    }
...
static int gc2375a_set_analog_gain(struct tx_isp_subdev *sd, u16 gain)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR "gc2375a_write error  %d" ,__LINE__ );
        return ret;
    }
...
static int gc2375a_set_fps(struct tx_isp_subdev *sd, int fps)
{
    ...
    if (newformat > (max_fps << 8) || newformat < (SENSOR_OUTPUT_MIN_FPS << 8)) {
        printk(KERN_ERR "gc2375a:warn: fps(%x) no in range\n", fps);
        return -1;
    }
    //H Blanking
        ...
        struct tx_isp_chip_ident *chip)
{
    ...
    if (power_gpio != -1) {
        ...
        if (!ret) {
            ...
            printk(KERN_ERR "gc2375a: gpio requrest fail %d\n", power_gpio);
            return -1;
        }
    }

    if (i2c_sel_gpio != -1) {
        int ret = gpio_request(i2c_sel_gpio, "gc2375a_i2c_sel1");
        if (!ret) {
    ...
    if (ret) {
        printk(KERN_ERR "gc2375a: chip found @ 0x%x (%s) is not an gc2375a chip.\n",
                client->addr, client->adapter->name);
        return ret;
    }

    printk(KERN_ERR "gc2375a: gc2375a chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
    if (chip) {
...
static int gc2375a_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    if (IS_ERR_OR_NULL(sd)) {
        printk(KERN_ERR "gc2375a: [%d]The pointer is invalid!\n", __LINE__);
        return -EINVAL;
    }
...
static int gc2375a_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!sensor) {
        printk(KERN_ERR "gc2375a: Failed to allocate sensor subdev.\n");
        return -ENOMEM;
    }

    ret = tx_isp_clk_set(MD_X1830_GC2375A_ISPCLK);
    if (ret < 0) {
        printk(KERN_ERR "gc2375a: Cannot set isp clock\n");
        goto err_get_mclk;
    }
    ...
    if (IS_ERR(sensor->mclk)) {
        printk(KERN_ERR "gc2375a: Cannot get sensor input clock cgu_cim\n");
        goto err_get_mclk;
    }
...
static __init int init_gc2375a(void)
{
    ...
    if (ret) {
        printk(KERN_ERR "gc2375a: Failed to init gc2375a dirver.\n");
        return -1;
    }
...
MODULE_DESCRIPTION("x1830 gc2375a driver depend on isp");
MODULE_LICENSE("GPL");
