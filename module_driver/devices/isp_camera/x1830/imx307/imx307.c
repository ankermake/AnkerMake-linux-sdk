...
// #define SENSOR_SURPORT_1080P
// #define SENSOR_SURPORT_720P
...
static struct tx_isp_sensor_attribute imx307_attr;
 ...
 */
static unsigned int imx307_alloc_again(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_again)
{
...
static unsigned int imx307_alloc_dgain(unsigned int isp_gain, unsigned char shift, unsigned int *sensor_dgain)
{
    *sensor_dgain = 0;
    return 0;
}
...
static struct tx_isp_sensor_attribute imx307_attr = {
    ...
    .max_integration_time_native        = 1211-2,
    .integration_time_limit             = 1211-2,
    .total_width                        = 2200,
    .total_height                       = 1211,
    .max_integration_time               = 1211-2,
...
/*
 *    ( 2200 * 1125 * 30 ) = (74250*1000)
 *    Data rate    ( 2200 * 1125 * 30 * 12 ) / 2 = 445.5M
 *
 *
 *    ( 2200 * 750 * 30 ) = (49500*1000)
 *    Data rate    ( 2200 * 750 * 30 * 12 ) / 2 = 297M
 */
#if 0
static struct regval_list imx307_init_regs_720_1280_30fps[] = {

    {0x3000, 0x01},        // Standby: 0 Operating,1 Standby
    {0x3001, 0x01},        // REGHOLD: 0 Invalid,1 Valid
    {0x3002, 0x01},        // Setting of master mode operation: 0 master start,1 master stop
    {IMX307_REG_DELAY, 0x18},
    {0x3005, 0x01},        // ADBIT  0x3005[0] 0 10bit,1 12bit
    {0x3007, 0x10},        // output fmt      0x3007[0] VREVERSE  0:Normal,1:Inverted,
                           //          0x3007[1] HREVERSE  0:Normal,1:Inverted
                           //         0x3007[4:6] WINMODE 0:Full HD 1080p 1:720p 4:Window cropping from Full HD 1080p
    {0x3009, 0x12},        // FRSEL     0x3009[0:1]     1: 60 frame/s,2: 30 frame/s
                           // FDG_SEL    0x3009[4]     0: LCG mode,1:HCG mode
    {0x300A, 0xF0},
    {0x300B, 0x00},        // 0x300A,0x300B[0] Black level offset value range 000h to 1FFh
    {0x3011, 0x0A},        //def
    {0x3012, 0x64},        //def
    {0x3013, 0x00},        //def
    {0x3014, 0x00},        // Gain setting (0.0 dB to 69.0 dB / 0.3 dB step)

    {0x3018, 0xEE},
    {0x3019, 0x02},
    {0x301A, 0x00},        // VMAX 0x3018 0x3019 0x301A[0:1] 0x2EE

    {0x301C, 0xC8},
    {0x301D, 0x19},        // HMAX 0x301C 0x301D
                           //     30fps 19c8 6600 60fps 1ef0 7920
    {0x3020, 0xFE},
    {0x3021, 0x00},
    {0x3022, 0x00},        // 0x3020 0x3021 0x3022[0] exposure line // 0 < value < VMAX

    {0x3044, 0x01},        // 0x3044[0:1] 0 10bit,1 12bit,interface CSI-2 1h
                           // 0x3044[4:7] output interface CSI-2 0h
    {0x3048, 0x00},        // vsync pulse width 0x3048[4:5] 0:1H
    {0x3049, 0x08},        // hsync pulse width 0x3049[4:5] 0
    {0x304B, 0x0A},        // 0x304B[0:1] 02h VSYNC output,0x304B[2:3] 02h HSYNC output

    {0x305C, 0x20},        // INCKSEL 1
    {0x305D, 0x00},        // INCKSEL 2
    {0x305E, 0x20},        // INCKSEL 3
    {0x305F, 0x01},        // INCKSEL 4
                           // 0x305C 0x305D 0x305E 0x305F 0x315E 0x3164 0x3480
                           // 20 00 20 01 1A 1A 49, 37.125 MHz
                           // 10 00 10 01 1B 1B 92, 74.25 MHz
    {0x309E, 0x4A},        //def
    {0x309F, 0x4A},        //def
    {0x311C, 0x0E},        //def
    {0x3128, 0x04},        //def
    {0x3129, 0x00},        // ADBIT1
    {0x313B, 0x41},        //def
    {0x315E, 0x1A},        // INCKSEL 5
    {0x3164, 0x1A},        // INCKSEL 6
    {0x317C, 0x00},        // ADBIT2
    {0x31EC, 0x0E},        // ADBIT3
                           // A/D Conversion 0x3005[0]
                           // 0x3129[0:7] 0x317C[0:7] 0x31EC[0:7]
    {0x3405, 0x10},        // REPETITION 1h
    {0x3407, 0x01},        // 0x3407[0:1] PHYSICAL_LANE_NUM
    {0x3414, 0x04},        // OPB_SIZE_V
    {0x3418, 0xD9},        // 0x3418 0x3419[0:4] Y_OUT_SIZE    0x2D9
    {0x3419, 0x02},
    {0x3441, 0x0C},        // 0x3441 0x3442 CSI_DT_FMT RAW10:0A0Ah,RAW12:0C0Ch
    {0x3442, 0x0C},
    {0x3443, 0x01},        // 0x3443[0:1] CSI_LANE_MODE 1: 2Lane, 3: 4Lane
    {0x3444, 0x20},        // 0x3444 0x3445 EXTCK_FREQ ,2520 INCK = 37.125MHz,4A40 INCK = 74.25MHz
    {0x3445, 0x25},
    {0x3446, 0x4F},        // 0x3446 0x3447[0] TCLKPOST
    {0x3447, 0x00},
    {0x3448, 0x2F},        // 0x3448 0x3449[0] THSZERO
    {0x3449, 0x00},
    {0x344A, 0x17},        // 0x344A 0x344B[0] THSPREPARE HS_PREPARE //0x1f
    {0x344B, 0x00},
    {0x344C, 0x17},        // 0x344C 0x344D[0] TCLKTRAIL
    {0x344D, 0x00},
    {0x344E, 0x17},        // 0x344E 0x344F[0] THSTRAIL
    {0x344F, 0x00},
    {0x3450, 0x57},        // 0x3450 0x3451[0] TCLKZERO
    {0x3451, 0x00},
    {0x3452, 0x17},        // 0x3452 0x3453[0] TCLKPREPARE
    {0x3453, 0x00},
    {0x3454, 0x17},        // 0x3454 0x3455[0] TLPX
    {0x3455, 0x00},

    {0x3472, 0x1C},        // 0x3472 0x3473[0:4] X_OUT_SIZE    0x51C
    {0x3473, 0x05},

    {0x346a, 0x1C},        // EBD
    {0x346b, 0x05},

    {0x3480, 0x49},        // INCKSEL 7
    {IMX307_REG_DELAY, 0x18},
    {0x3001, 0x00},        // REGHOLD: 0 Invalid,1 Valid
    {0x3002, 0x00},        // Setting of master mode operation: 0 master start,1 master stop
    {0x3000, 0x01},        // Standby: 0 Operating,1 Standby
...
static struct regval_list imx307_init_regs_640_1072_30fps[] = {
    ...
    {IMX307_REG_DELAY, 0x18},
    {0x3005, 0x01},        // ADBIT  0x3005[0] 0 10bit,1 12bit
    {0x3007, 0x40},        // output fmt      0x3007[0] VREVERSE  0:Normal,1:Inverted,
                           //          0x3007[1] HREVERSE  0:Normal,1:Inverted
                           //         0x3007[4:6] WINMODE 0:Full HD 1080p 1:720p 4:Window cropping from Full HD 1080p
    {0x3009, 0x12},        // FRSEL     0x3009[0:1]     1: 60 frame/s,2: 30 frame/s
                           // FDG_SEL    0x3009[4]     0: LCG mode,1:HCG mode
    ...
    {0x3018, 0xBB},
    {0x3019, 0x04},
    {0x301A, 0x00},        // VMAX 0x3018 0x3019 0x301A[0:1]     >= (1072+16+a+13) 1211

    {0x301C, 0x30},
    {0x301D, 0x11},        // HMAX 0x301C 0x301D            2200

    {0x3020, 0x50},
    {0x3021, 0x03},
    {0x3022, 0x00},        // 0x3020 0x3021 0x3022[0] exposure line // 0 < value < VMAX

    {0x303A, 0x0A},        // WINWV_OB     0x303A[0:3]
    {0x303C, 0x04},
    {0x303D, 0x00},        // WINPV      0x303C 0x303D[0:2]    4
    {0x303E, 0x40},
    {0x303F, 0x04},        // WINWV    0x303E 0x303F[0:2]    1088
    {0x3040, 0x80},
    {0x3041, 0x02},        // WINPH    0x3040 0x3041[0:2]    640
    {0x3042, 0x90},
    {0x3043, 0x02},        // WINWH    0x3042 0x3043[0:2]    656

    {0x3046, 0x01},        // 0x3044[0:1] 0 10bit,1 12bit,interface CSI-2 1h
                           // 0x3044[4:7] output interface CSI-2 0h
    {0x3048, 0x00},        // Master Mode // vsync pulse width 0x3048[4:5] 0:1H
    {0x3049, 0x08},        // hsync pulse width 0x3049[4:5] 0
    {0x304B, 0x0A},        // 0x304B[0:1] 02h VSYNC output,0x304B[2:3] 02h HSYNC output

    {0x305C, 0x18},        // INCKSEL 1
    {0x305D, 0x03},        // INCKSEL 2
    {0x305E, 0x20},        // INCKSEL 3
    {0x305F, 0x01},        // INCKSEL 4
                           // 0x305C 0x305D 0x305E 0x305F 0x315E 0x3164 0x3480
                           // 18 03 20 01 1A 1A 49, 37.125 MHz
                           // 10 00 10 01 1B 1B 92, 74.25 MHz
    {0x309E, 0x4A},        //def
    {0x309F, 0x4A},        //def
    {0x311C, 0x0E},        //def
    ...
    {0x3418, 0x40},        // 0x3418 0x3419[0:4] Y_OUT_SIZE= WINWV
    {0x3419, 0x04},
    {0x3441, 0x0C},        // 0x3441 0x3442 CSI_DT_FMT RAW10:0A0Ah,RAW12:0C0Ch
    {0x3442, 0x0C},
    {0x3443, 0x01},        // 0x3443[0:1] CSI_LANE_MODE 1: 2Lane, 3: 4Lane
    {0x3444, 0x20},        // 0x3444 0x3445 EXTCK_FREQ ,2520 INCK = 37.125MHz,4A40 INCK = 74.25MHz
    {0x3445, 0x25},
    {0x3446, 0x57},        // 0x3446 0x3447[0] TCLKPOST
    {0x3447, 0x00},
    {0x3448, 0x37},        // 0x3448 0x3449[0] THSZERO
    {0x3449, 0x00},
    {0x344A, 0x1F},        // 0x344A 0x344B[0] THSPREPARE HS_PREPARE //0x1f
    {0x344B, 0x00},
    {0x344C, 0x1F},        // 0x344C 0x344D[0] TCLKTRAIL
    {0x344D, 0x00},
    {0x344E, 0x1F},        // 0x344E 0x344F[0] THSTRAIL
    {0x344F, 0x00},
    {0x3450, 0x77},        // 0x3450 0x3451[0] TCLKZERO
    {0x3451, 0x00},
    {0x3452, 0x1F},        // 0x3452 0x3453[0] TCLKPREPARE
    {0x3453, 0x00},
    {0x3454, 0x17},        // 0x3454 0x3455[0] TLPX
    {0x3455, 0x00},
    {0x3472, 0x90},        // 0x3472 0x3473[0:4] X_OUT_SIZE
    {0x3473, 0x02},
...
/*
 *   1920*1080 <0x79C*0x455>
 *   1280*720 <0x51C*0x2DF>
 *   640*1072 <0x290*0x44C>
 */
    {
        .width           = 0x290,
        .height          = 0x44C,
        ...
        .regs            = imx307_init_regs_640_1072_30fps,
//      .regs             = imx307_init_regs_720_1280_30fps,
...
static int imx307_read(struct tx_isp_subdev *sd, uint16_t reg,
        unsigned char *value)
{
    int ret;
...
static int imx307_write(struct tx_isp_subdev *sd, uint16_t reg,
         unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
...
static inline int imx307_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
    int ret;
    unsigned char val;
    ...
    while (vals->reg_num != IMX307_REG_END) {
        if (vals->reg_num == IMX307_REG_DELAY) {
            ...
            printk(KERN_ERR "imx307: {0x%0x, 0x%02x}\n",vals->reg_num, val);
        }
        vals++;
    }
...
static int imx307_set_integration_time(struct tx_isp_subdev *sd, int int_time)
{
    ...
    if (0 != ret) {
        printk(KERN_ERR "imx307: err: imx307_write err\n");
        return ret;
    }
...
static int imx307_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR "imx307: imx307_write error  %d\n" ,__LINE__ );
        return ret;
    }
...
static int imx307_set_fps(struct tx_isp_subdev *sd, int fps)
{
    ...
    if (newformat > (SENSOR_OUTPUT_MAX_FPS << 8) || newformat < (SENSOR_OUTPUT_MIN_FPS << 8)) {
        printk(KERN_ERR "imx307: warn: fps(%d) no in range\n", fps);
        return -1;
    }
    ...
    if (0 != ret) {
        printk(KERN_ERR "err: imx307_write err\n");
        return ret;
    }
                   ...
                   struct tx_isp_chip_ident *chip)
{
    ...
    if (power_gpio != -1) {
        ret = gpio_request(power_gpio, "imx307_power");
        if (!ret) {
            private_gpio_direction_output(power_gpio, 1);
            private_msleep(50);
        } else {
            printk(KERN_ERR "imx307: gpio requrest fail %d\n", power_gpio);
            return -1;
        }
    }
    ...
    if (i2c_sel_gpio != -1) {
        int ret = gpio_request(i2c_sel_gpio, "imx307_i2c_sel1");
        if (!ret) {
    ...
    if (ret) {
        printk(KERN_ERR "imx307: chip found @ 0x%x (%s) is not an imx307 chip.\n",
               client->addr, client->adapter->name);
        return ret;
    }
    printk(KERN_ERR "imx307: imx307 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
    if (chip) {
...
static int imx307_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    if (IS_ERR_OR_NULL(sd)) {
        printk(KERN_ERR "imx307: [%d]The pointer is invalid!\n", __LINE__);
        return -EINVAL;
    }
...
struct platform_device imx307_platform_device = {
    .name                   = "imx307",
    .id                     = -1,
    .dev = {
            ...
            const struct i2c_device_id *id)
{
    ...
    if (!sensor) {
        printk(KERN_ERR "imx307: Failed to allocate sensor subdev.\n");
        return -ENOMEM;
    }

    ret = tx_isp_clk_set(MD_X1830_IMX307_ISPCLK);
    if (ret < 0) {
        printk(KERN_ERR "imx307: Cannot set isp clock\n");
        goto err_get_mclk;
    }
    ...
    if (IS_ERR(sensor->mclk)) {
        printk(KERN_ERR "imx307: Cannot get sensor input clock cgu_cim\n");
        goto err_get_mclk;
    }
    ...
    tx_isp_subdev_init(&imx307_platform_device, sd, &imx307_ops);
...
static __init int init_imx307(void)
{
    ...
    if (ret) {
        printk(KERN_ERR "imx307: Failed to init imx307 dirver.\n");
        return -1;
    }
...
MODULE_DESCRIPTION("x1830 imx307 driver depend on isp");
MODULE_LICENSE("GPL");
