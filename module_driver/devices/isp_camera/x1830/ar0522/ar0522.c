...
#if defined MD_X1830_AR0522_2592X1944
#define AR0522_WIDTH                           2592
#define AR0522_HEIGHT                          1944
#define SENSOR_OUTPUT_MAX_FPS                  25
#elif defined MD_X1830_AR0522_2560X1080
...
static int pwdn_gpio = -1; // GPIO_PA(9)
...
static struct tx_isp_sensor_attribute ar0522_attr;
...
static struct again_lut ar0522_again_lut[] = {
    {0x0, 0},
    {0x1, 2794},
    {0x2, 6397},
    {0x3, 9011},
...
static int ar0522_set_fps(struct tx_isp_subdev *sd, int fps)
{
    ...
    if (newformat > (max_fps << 8) || newformat < (SENSOR_OUTPUT_MIN_FPS << 8)) {
        printk(KERN_ERR "ar0522: warn: fps(%x) no in range\n", fps);
        return -1;
    }
        ...
        struct tx_isp_chip_ident *chip)
{
    ...
    if (power_gpio != -1) {
        ...
        if (!ret) {
            ...
            printk(KERN_ERR "ar0522: gpio requrest fail %d\n", power_gpio);
            return -1;
        }
    }
...
//            private_msleep(10);
    ...
    if (i2c_sel_gpio != -1) {
        int ret = gpio_request(i2c_sel_gpio, "ar0522_i2c_sel1");
        if (!ret) {
    ...
    if (ret) {
        printk(KERN_ERR "ar0522: chip found @ 0x%x (%s) is not an ar0522 chip.\n",
                client->addr, client->adapter->name);
        return ret;
    }
    printk(KERN_ERR "ar0522: ar0522 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
    if (chip) {
...
static int ar0522_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    if (IS_ERR_OR_NULL(sd)) {
        printk(KERN_ERR "ar0522: [%d]The pointer is invalid!\n", __LINE__);
        return -EINVAL;
    }
...
static int ar0522_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!sensor) {
        printk(KERN_ERR "ar0522: Failed to allocate sensor subdev.\n");
        return -ENOMEM;
    }

    ret = tx_isp_clk_set(MD_X1830_AR0522_ISPCLK);
    if (ret < 0) {
        printk(KERN_ERR "ar0522: Cannot set isp clock\n");
        goto err_get_mclk;
    }
    ...
    if (IS_ERR(sensor->mclk)) {
        printk(KERN_ERR "ar0522: Cannot get sensor input clock cgu_cim\n");
        goto err_get_mclk;
    }
...
static __init int init_ar0522(void)
{
    ...
    if (ret) {
        printk(KERN_ERR "ar0522: Failed to init ar0522 dirver.\n");
        return -1;
    }
...
MODULE_DESCRIPTION("x1830 ar0522 driver depend on isp");
MODULE_LICENSE("GPL");
