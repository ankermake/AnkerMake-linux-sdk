...
static inline int s5k6a1_read_array(struct tx_isp_subdev *sd, struct regval_list *vals)
{
    ...
    while (vals->reg_num != S5K6A1_REG_END) {
        if (vals->reg_num == S5K6A1_REG_DELAY) {
                ...
                printk(KERN_ERR "private_msleep %d\n",vals->value);
        } else {
            ret = s5k6a1_read(sd, vals->reg_num, &val);
            printk(KERN_ERR "reg_num 0x%2x,value 0x%x\n",vals->reg_num,val);
            if (ret < 0)
                return ret;
        }
        vals++;
        ...
        struct tx_isp_chip_ident *chip)
{
    ...
    if(pwdn_gpio != -1){
        ...
        if(!ret){
            ...
            printk(KERN_ERR "gpio requrest fail %d\n",pwdn_gpio);
        }
    }
    if(reset_gpio != -1){
    ...
    if (i2c_sel_gpio != -1) {
        int ret = gpio_request(i2c_sel_gpio, "s5k6a1_i2c_sel1");
        if (!ret) {
    ...
    if (ret) {
        printk(KERN_ERR "chip found @ 0x%x (%s) is not an s5k6a1 chip.\n",
                client->addr, client->adapter->name);
        return ret;
    }
    printk(KERN_ERR "s5k6a1 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
    if(chip){
...
static int s5k6a1_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    if(IS_ERR_OR_NULL(sd)){
        printk(KERN_ERR "[%d]The pointer is invalid!\n", __LINE__);
        return -EINVAL;
    }
    switch(cmd){
        ...
        const struct i2c_device_id *id)
{
    ...
    if(!sensor){
        printk(KERN_ERR "Failed to allocate sensor subdev.\n");
        return -ENOMEM;
    }
    ...
    if (IS_ERR(sensor->mclk)) {
        printk(KERN_ERR "Cannot get sensor input clock cgu_cim\n");
        goto err_get_mclk;
    }
...
static __init int init_s5k6a1(void)
{
    ...
    if(ret){
        printk(KERN_ERR "Failed to init s5k6a1 dirver.\n");
        return -1;
    }
