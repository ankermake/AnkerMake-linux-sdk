...
static inline int ov9732_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
    int ret;
    unsigned char val;
...
static int ov9732_reset(struct v4l2_subdev *sd, unsigned int val)
{
    return 0;
}
...
static int ov9732_init(struct v4l2_subdev *sd, unsigned int enable)
{
...
static int ov9732_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *chip)
{
    struct i2c_client *client = v4l2_get_subdevdata(sd);
    ...
    if (ret) {
        v4l_err(client,
            "ov9732: chip found @ 0x%x (%s) is not an ov9732 chip.\n",
...
static int ov9732_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ret = tx_isp_clk_set(MD_X1520_OV9732_ISPCLK);
    if (ret < 0) {
...
MODULE_DESCRIPTION("x1520 ov9732 driver depend on isp");
MODULE_LICENSE("GPL");
