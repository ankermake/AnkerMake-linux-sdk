...
#if defined MD_X1520_OV9281_MIPI_1280X800
...
static int ov9281_read(struct v4l2_subdev *sd, u16 reg, u8 *val)
{
    int ret;
...
static int ov9281_write(struct v4l2_subdev *sd, u16 reg, u8 val)
{
    struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static inline int ov9281_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
    int ret;
    u8 val;
...
static int ov9281_reset(struct v4l2_subdev *sd, unsigned int val)
{
    return 0;
}
...
static int ov9281_set_integration_time(struct v4l2_subdev *sd, int value)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR "ov9281_mipi: ov9281_write error  %d\n" ,__LINE__ );
        return ret;
    }
...
static int ov9281_set_analog_gain(struct v4l2_subdev *sd, u16 gain)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR "ov9281_mipi: ov9281_write error  %d" ,__LINE__ );
        return ret;
    }
...
static int ov9281_init(struct v4l2_subdev *sd, unsigned int enable)
{
...
static int ov9281_set_fps(struct tx_isp_sensor *sensor, int fps)
{
    ...
    if(newformat > (max_fps << 8) || newformat < (SENSOR_OUTPUT_MIN_FPS << 8)){
        printk(KERN_ERR "ov9281_mipi: fps(%x) no in range\n", fps);
        return -1;
    }
...
static inline int ov9281_set_vflip(struct tx_isp_sensor *sensor, int enable)
{
    struct tx_isp_notify_argument arg;
    struct v4l2_subdev *sd = &sensor->sd;
    int ret = 0;
        ...
        struct v4l2_dbg_chip_ident *chip)
{
    ...
    if (reset_gpio != -1) {
       ret = gpio_request(reset_gpio,"ov9281_reset");
        if (!ret) {
            ...
            printk(KERN_ERR "ov9281_mipi: gpio requrest fail %d\n", reset_gpio);
            return -1;
        }
    }
    ...
    if (ret) {
        printk(KERN_ERR "ov9281_mipi: chip found @ 0x%x (%s) is not an ov9281 chip.\n",
...
static int ov9281_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(!sensor){
        printk(KERN_ERR "ov9281_mipi: Failed to allocate sensor subdev.\n");
        return -ENOMEM;
    }

    ret = tx_isp_clk_set(MD_X1520_OV9281_ISPCLK);
    if (ret < 0) {
    ...
    if (IS_ERR(sensor->mclk)) {
        printk(KERN_ERR "ov9281_mipi: Cannot get sensor input clock cgu_cim\n");
        goto err_get_mclk;
    }
...
MODULE_DESCRIPTION("x1520 ov9281_mipi driver depend on isp");
MODULE_LICENSE("GPL");
