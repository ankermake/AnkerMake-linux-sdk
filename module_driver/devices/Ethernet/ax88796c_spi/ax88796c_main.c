...
KERN_INFO ADP_NAME ".c:v" DRV_VERSION "\n"
KERN_INFO "  http://www.asix.com.tw\n";

static int comp = 0;
static int ps_level = AX_PS_D0;
#ifndef MD_AX88796C_SPI_DEBUG
...
{
    PAX_PRIVATE ax_local = (PAX_PRIVATE)(arg);
#else
static void ax88796c_watchdog_timer (struct timer_list *t)
{
    PAX_PRIVATE ax_local = from_timer(ax_local, t, watchdog);
#endif
...
ax88796c_open(struct net_device *ndev)
{
    ...
    ax_local->watchdog.data = (unsigned long) ax_local;
#else
...
static int __devinit ax88796c_probe (struct spi_device *spi)
{
    ...
    int gpio_reset;
    u16 temp;
    struct ax88796c_spi_pdata * pdata;

    printk(KERN_INFO "Register AX88796C SPI Ethernet Driver probe.\n");
    ...
    spi_set_drvdata(spi, ax_local);
    dev_set_drvdata(&spi->dev, ax_local);
    ax_local->spi = spi;
    ax_local->ax_spi.spi = spi;
    pdata = spi->dev.platform_data;

    gpio_irq = pdata->gpio_irq;
    if (gpio_irq < 0) {
        printk (KERN_ERR "AX88796C SPI Ethernet: get irq gpio %d faild\n", gpio_irq);
        return -EIO;
    }

    if (gpio_request(gpio_irq, "ax88796c_irq"))
        printk (KERN_ERR "AX88796C SPI Ethernet: request gpio %d faild\n", gpio_irq);

    ndev->irq = gpio_to_irq(gpio_irq);

    /* HARDWARE RESET */
    gpio_reset = pdata->gpio_reset;
    if (gpio_reset >= 0) {
        if (gpio_request(gpio_reset, "ax88796c_reset")) {
            printk (KERN_ERR
                "AX88796C SPI Ethernet: request gpio %d faild\n"
                    ,gpio_reset);

            return -EIO;
        }
        gpio_set_value(gpio_reset, 0);
        usleep_range(5000, 5030);
...
#else
static int
ax88796c_suspend (struct device *dev)
{
    ...
    if(AX_READ (&ax_local->ax_spi, P0_MACCR) & MACCR_RXFC_ENABLE)
        AX_WRITE (&ax_local->ax_spi, AX_READ(&ax_local->ax_spi, P2_POOLCR) &
              ~POOLCR_POLL_FLOWCTRL, P2_POOLCR);
...
#else
static int
ax88796c_resume (struct device *dev)
{
    struct spi_device *spi = container_of(dev, struct spi_device, dev);
#endif
    PAX_PRIVATE ax_local = dev_get_drvdata(&spi->dev);
 ...
 * ----------------------------------------------------------------------------
 */
static struct spi_device *ax88796c_spi_haldle;
...
static int __devexit ax88796c_remove (struct spi_device *spi)
{
    ...
    ax88796c_spi_haldle = spi;

    printk(KERN_INFO "ax88796c-spi: released and freed device, irq %d\n", gpio_irq);

    return 0;
}
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,1,0)
static const struct dev_pm_ops ax88796c_pm_ops = {
    SET_SYSTEM_SLEEP_PM_OPS(ax88796c_suspend, ax88796c_resume)
};
#endif
...
static __init int ax88796c_spi_init(void)
{
    ...
    ret = ax88796c_platform_device_register();
    if (ret < 0) {
        printk(KERN_INFO "Register AX88796C SPI Ethernet Failed.\n");
        return ret;
    }
...
static __exit void ax88796c_spi_exit(void)
{
    ...
    spi_unregister_device(ax88796c_spi_haldle);
}

late_initcall(ax88796c_spi_init);
