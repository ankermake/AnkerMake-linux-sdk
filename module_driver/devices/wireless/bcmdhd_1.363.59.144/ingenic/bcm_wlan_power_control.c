...
int bcm_wlan_init(void)
{
    ...
    if (bt_wifi_power >= 0) {
        if (bt_wifi_power_valid_level)
            gpio_out_level = GPIOF_DIR_OUT|GPIOF_INIT_HIGH;
        else
            gpio_out_level = GPIOF_DIR_OUT|GPIOF_INIT_LOW;

        if (gpio_request_one(bt_wifi_power, gpio_out_level, "bt_wifi_power") < 0) {
            char gpio[10];
...
int ingenic_sdio_wlan_get_irq(void)
{
...
EXPORT_SYMBOL(bcm_wlan_get_oob_irq_flags);