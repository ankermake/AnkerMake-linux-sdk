...
#define FW_CYW43438 "cyw43438-7.46.58.28.bin"
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
    ...
    if (cis_device == BCM43455_CHIP_ID && cis_chipvendor == 0x81) {
        sprintf(fw_path, "%s", MD_BCMDHD_1_363_125_19_FW_PATH);
        sprintf(nv_path, "%s", MD_BCMDHD_1_363_125_19_NVRAM_PATH);
...
extern int bt_power_dev_init(void);
extern int bt_power_dev_exit(void);

extern int bt_power_init(void);
extern int bt_power_exit(void);
...
dhd_tcp_dump(char *ifname, uint8 *pktdata, bool tx)
{
    ...
    unsigned char *opts;
    uint32 ip_id, source, dest, seq, seq_ack, window, tcp_data_len, tcp_option_len, tsval, tsecr;
    unsigned char *sabuf, *dabuf, *deabuf, *seabuf;
