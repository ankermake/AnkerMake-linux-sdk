...
wl_set_key_mgmt(struct net_device *dev, struct cfg80211_connect_params *sme)
{
    ...
    WL_DBG(("[clooney] wpa2_ie ptr:%p rsn_cap 0x%x-0x%x fw mfp support:%d,mfp=%d,current_mfp=%d\n",
         wpa2_ie, rsn_cap[0], rsn_cap[1], fw_support,mfp,current_mfp));
    if (fw_support == false) {
        ...
        if (unlikely(err)) {
            if (!mfp && (err == BCME_UNSUPPORTED)) {
                ...
                {
                    int ret=0;
                    /* setting of mfp_mode requires wl to be down */
                    ret = wldev_ioctl(dev, WLC_DOWN, NULL, 0, true);
                    if (unlikely(ret)) {
                        WL_ERR(("[MFP] WLC_DOWN command failed:[%d]\n", ret));
                        return ret;
                    }
                    err = wldev_iovar_setint(dev, "mfp", mfp);
                    if (unlikely(err)) {
                        WL_ERR(("mfp set failed ret2:%d \n", err));
                        return err;
                    }
                    ...
                    WL_ERR(("[MFP] mfp_mode set to %d successfully \n", mfp));
