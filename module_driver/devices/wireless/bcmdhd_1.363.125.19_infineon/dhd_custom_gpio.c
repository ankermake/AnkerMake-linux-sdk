...
#ifdef CUSTOMER_OOB
extern int md_ingenic_sdio_wlan_get_irq(unsigned long *flag);
...
int dhd_init_gpio_wlan_hostwake(void)
{
    ...
    gpio_wl_host_wake = -1;
    gpio_wl_host_wake = md_ingenic_sdio_wlan_get_irq((unsigned long *)&host_oob_irq_flags);
    ...
    printf("[clooney]%s: WL_HOST_WAKE=%d, oob_irq=%d, oob_irq_flags=0x%x\n", __FUNCTION__,
        gpio_wl_host_wake, host_oob_irq, host_oob_irq_flags);

    return 0;
}

//static void dhd_deinit_gpio_wlan_hostwake(void)
