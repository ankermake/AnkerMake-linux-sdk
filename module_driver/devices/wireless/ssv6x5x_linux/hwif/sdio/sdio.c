...
static int wifi_power_en = -1;
static int wifi_reset = -1;
static int wifi_power_valid_level = -1;
static int wlan_mmc_num = -1;

module_param_gpio_named(gpio_wifi_en, wifi_power_en, 0644);
module_param_gpio_named(gpio_wifi_rst, wifi_reset, 0644);   /* PB04 */
module_param(wifi_power_valid_level, int, 0644);
module_param(wlan_mmc_num, int, 0644);    /* 1 */
...
void sdio_card_detect_change(u32 wlan_mmc_num)
{
    // ingenic_mmc_manual_detect(wlan_mmc_num, 1);
...
static void ssv6x5x_power_enable(void)
{
    int ret;

    ret = m_gpio_request(wifi_power_en, "wifi_power_en");
    if (ret < 0) {
        printk(KERN_ERR "wifi_power_en: failed to request power enable pin: %d\n", wifi_power_en);
        return;
    }

    if(wifi_power_en != -1) {
        gpio_direction_output(wifi_power_en, !wifi_power_valid_level);
        msleep(50);
        gpio_direction_output(wifi_power_en, !!wifi_power_valid_level);
        msleep(50);
    }
...
static void ssv6x5x_reset(void)
{
    int ret;

    ret = m_gpio_request(wifi_reset, "wifi_reset");
    if (ret < 0) {
        printk(KERN_ERR "wifi_reset: failed to request reset enable pin: %d\n", wifi_reset);
        return;
    }
...
static void ssv6x5x_disable(void)
{
    if (wifi_power_en != -1) {
        gpio_set_value(wifi_power_en, !wifi_power_valid_level);
        gpio_free(wifi_power_en);
    }
    if (wifi_reset != -1) {
...
int ssv6xxx_sdio_init(void)
{
    ...
    ssv6x5x_power_enable();
    ssv6x5x_reset();
    sdio_card_detect_change(wlan_mmc_num);
