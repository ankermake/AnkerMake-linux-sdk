...
static int bt_wifi_power_valid_level = -1;

module_param(bt_wifi_power_valid_level, int, 0644);
...
int rtk_wlan_bt_power_ctrl(int is_on)
{
	...
	if (is_on) {
		if (ref_count++ == 0) {
			m_gpio_direction_ouput(bt_wifi_power_en, bt_wifi_power_valid_level);
			msleep(200);
...
int rtk_power_init(void)
{
	...
	if (wl_dis_n < 0) {
		printk(KERN_ERR "ERROR: wl_dis_n must set\n");
		return -1;
	}
	...
	if (bt_wifi_power_valid_level)
		gpio_out_level = GPIOF_OUT_INIT_LOW;
	else
		gpio_out_level = GPIOF_OUT_INIT_HIGH;

	ret = m_gpio_request(bt_wifi_power_en, gpio_out_level, "bt_wifi_power_en");
	if (ret)
		goto err_bt_wifi_en;
