...
static int rtl8189fs_wlan_ctrl(struct rtl_power_data *pfun, int onoff)
{
	int ret = 0;
...
static int rtl8189fs_probe (struct platform_device *pdev)
{
    struct rtl_power_data *pfun = NULL;
    int ret = 0;
    RTW_INFO("rtl8189fs wifi probe function\n");

    pfun = (struct rtl_power_data *)pdev->dev.platform_data;

    ret = rtl8189fs_wlan_ctrl(pfun, NORMAL);
	if (ret) {
		RTW_ERR(" %s:%d rtl8189fs_wlan_ctrl failed\n", __func__, __LINE__);
	}
...
static int rtl8189fs_remove (struct platform_device *pdev)
{
    int ret = 0;
    struct rtl_power_data *pfun = NULL;
	pr_info("INFO: %s fun runing\n", __func__);
...
static struct platform_driver rtl8189fs_driver = {
    .probe = rtl8189fs_probe,
    .remove = rtl8189fs_remove,
    .driver = {
        .name =  "rtl8189fs_wifi",
    },
};

static int rtl8189_driver_init(void)
{
    int ret = 0;
    ret = platform_driver_register(&rtl8189fs_driver);
    if (ret < 0) {
        RTW_ERR("ERROR: rtl8189fs_driver register failed !!\n");
    }
    return ret;
}

static void rtl8189_driver_deinit(void)
{
	platform_driver_unregister(&rtl8189fs_driver);
}
...
static void rtw_init_work_func(struct work_struct *work)
{
	rtl8189_driver_init();
}

static int rtw_module_init(void)
{
	INIT_DELAYED_WORK(&delayed_work, rtw_init_work_func);
	schedule_delayed_work(&delayed_work, msecs_to_jiffies(500));
	return 0;
}

static void rtl8189_driver_exit(void)
{
    platform_driver_unregister(&rtl8189ds_driver);
}
#else
...
static int rtw_module_init(void)
{
	int ret = rtl8189_driver_init();
	if (ret)
		return ret;

	ret = rtk_power_init();
	if (ret) {
		RTW_ERR("ERROR: rtk power init failed !!\n");
		rtl8189_driver_deinit();
		return ret;
	}
...
module_init(rtw_module_init);
// module_init(rtw_drv_entry);
module_exit(rtl8189_driver_exit);
MODULE_LICENSE("GPL");
