...
//	if (!dhd_download_fw_on_driverload) {
#ifdef MD_BCMDHD_FW_PATH
		fw = MD_BCMDHD_FW_PATH;
#endif /* MD_BCMDHD_FW_PATH */
#ifdef MD_BCMDHD_NVRAM_PATH
		nv = MD_BCMDHD_NVRAM_PATH;
#endif /* MD_BCMDHD_NVRAM_PATH */
//	}
...
dhd_os_runtimepm_timer(void *bus, uint tick)
{
	...
	if (tick) {
		...
		if (dhd->rpm_timer_valid == TRUE) {
			dhd->rpm_timer_valid = FALSE;
			DHD_GENERAL_UNLOCK(pub, flags);
			bcm_del_timer_sync(&dhd->rpm_timer);
