...
// extern int ingenic_mmc_manual_detect(int index, int on);
...
module_param_gpio_named(gpio_wifi_int, wifi_int, 0644);   /* PE23 */
...
hi_void sdio_card_detect_change(hi_s32 wlan_mmc_num)
{
#if (_PRE_OS_PLATFORM == _PRE_PLATFORM_HISILICON)
    hisi_sdio_rescan(wlan_mmc_num);
...
hi_s32 oal_sdio_detectcard_to_core(hi_s32 wlan_mmc_num)
{
...
int oal_sdio_interrupt_register(oal_channel_stru *hi_sdio)
{
    ...
    int irq;
#if(_PRE_OS_PLATFORM == _PRE_PLATFORM_JZ)
    jzgpio_set_func(wifi_int / 32, GPIO_INT_RE, 1 << (wifi_int % 32));
    ...
    iounmap(io_mux_base);
#endif
    ret = gpio_request(wifi_int, "SDIO INT");
    if (ret < 0)
        goto gpio_request_fail;

    irq = gpio_to_irq(wifi_int);
...
hi_s32 oal_sdio_func_probe(oal_channel_stru *hi_sdio, hi_s32 wlan_mmc_num)
{
    hi_s32 ret;
    ...
    ret = oal_sdio_detectcard_to_core(wlan_mmc_num);
    if (ret) {
...
/* hi_s32 val卡选择 */
hi_s32 oal_sdio_init(void *data, hi_s32 wlan_mmc_num, struct hcc_bus_ops *bus_ops)
{
    oal_channel_stru *hi_sdio;
    hi_s32 ret;
    ...
    ret = oal_sdio_func_probe(hi_sdio, wlan_mmc_num);
    if (ret != HI_SUCCESS) {
