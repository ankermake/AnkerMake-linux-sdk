...
extern struct regulator_init_data axp216_regl_init_data[];
                  ...
                  int min_uV, int max_uV,unsigned *selector)
{
    ...
    if (check_range(info, min_uV, max_uV)) {
        printk(KERN_ERR "[Axp-regu]: invalid voltage range (%d, %d) uV\n", min_uV, max_uV);
        return -EINVAL;
    }
    ...
    printk(KERN_ERR "[Axp-regu]: axp_regulator_set_voltage reg=0x%x,val=0x%x!\n", info->vol_reg, val);
...
static int axp_enable(struct regulator_dev *rdev)
{
    ...
    constraints->state_mem.disabled = 0;
    constraints->state_mem.enabled = 1;

    printk(KERN_ERR "[Axp-regu]: %s info->enable_reg:0x%2x, info->enable_bit:%d\n",
        __FUNCTION__, info->enable_reg, info->enable_bit);
...
static int axp_disable(struct regulator_dev *rdev)
{
    ...
    struct device *axp_dev = to_axp_dev(rdev);
    struct regulation_constraints *constraints = rdev->constraints;
    constraints->state_mem.disabled = 1;
...
struct axp_regulator_info axp216_regulator_info[] = {
...
static void update_regulator_constraints(struct pmu_constraints_data * data)
{
    int i;
    for (i = 0; i < AXP_REGULATOR_NUM; i++) {
        /* set regulator name */
        if (!data[i].name)
            printk(KERN_ERR "[Axp-regu]: invalid to set null name for %s", axp216_regl_init_data[i].constraints.name);
        else
            axp216_regl_init_data[i].constraints.name = data[i].name;

        /* set regulator boot on*/
        axp216_regl_init_data[i].constraints.boot_on = data[i].boot_status;

        /* set regulator suspend_voltage */
        axp216_regl_init_data[i].constraints.state_mem.uV = data[i].suspend_voltage;
    }
}

static int update_regulator_work_voltage(struct regulator_dev *rdev, int uV)
{
    return axp_set_voltage(rdev, uV, uV,NULL);
}
...
# if LINUX_VERSION_CODE >= KERNEL_VERSION(3,7,0)
    ...
    const struct mfd_cell * axp_regulator_cell = mfd_get_cell(pdev);
    struct pmu_constraints_data *axp_constraints_data = (struct pmu_constraints_data *) axp_regulator_cell->platform_data;
    update_regulator_constraints(axp_constraints_data);

    int i;
    for (i = 0; i < AXP_REGULATOR_NUM; i++) {
        ri = &axp216_regulator_info[i];
        ...
        config.init_data = &axp216_regl_init_data[i];
        config.driver_data = ri;

        rdev = devm_regulator_register(&pdev->dev, &ri->desc, &config);
        if (IS_ERR(rdev)) {
            printk(KERN_ERR "[Axp-regu]: failed to register %s regulator (errno %ld\n)\n",
                    ri->desc.name, PTR_ERR(rdev));
            return PTR_ERR(rdev);
        }

        /* set work_voltage */
        ret = update_regulator_work_voltage(rdev, axp_constraints_data[i].work_voltage);
        if (ret)
            printk(KERN_ERR "[Axp-regu]: axp regulator set work voltage falied\n");
    }
    return 0;
}
