/*
 * linux/drivers/misc/jz_efuse_x1520.c - Ingenic efuse driver
 *
...
int soc_efuse_read(char *seg_id_name, unsigned char *buf, int start, int size)
{
    ...
    if (seg_id < CHIP_ID || seg_id > CUSTOMER_RESV) {
...
static int __init jz_efuse_init(void)
{
    ...
    wr_strobe = 10000 / ns - wr_adj - 1666;
    if (wr_strobe < 0)
        wr_strobe = 0;
    wr_strobe = wr_strobe + 1;
    tmp = (wr_adj + 1666 + wr_strobe) * ns;
    if (tmp > 11000 || tmp < 9000) {
...
MODULE_DESCRIPTION("JZ x1520 EFUSE driver");
MODULE_LICENSE("GPL");
