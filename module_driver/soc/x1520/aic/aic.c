...
static int aic_set_clk_rate(struct aic_data *data)
{
    ...
    aic_dev.clk_freq = sample_rate * div;

    clk_disable(cgu_clk);
    clk_set_rate(cgu_clk, aic_dev.clk_freq);
    clk_enable(cgu_clk);
...
static int aic_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
    ...
    /* x1520 aic only for icodec */
    if (clk_dir != SND_SOC_DAIFMT_CBM_CFM) {
        printk(KERN_ERR "AIC: x1520 aic only for icodec and icodec as master!\n");
        return -EINVAL;
    }
