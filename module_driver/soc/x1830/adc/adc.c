...
/*
 * div         工作时钟/周期          采样率
 *  0          24M/0.041us         1Mbps * 1.1 (1.1: 实际测试adc 速度比手册快约0.1倍)
 *  5 - 1      4.8M/0.208us        200kbps * 1.1
 *  100 - 1    240k/4.1us          10kbps * 1.1
 *  120 - 1    200k/5us            8.3kbps * 1.1
 *
 * div_us      div_ms     工作时钟/周期    repeat时钟/周期(自动重采样)
 * 2 - 1       100 - 1     200k/5us      (200K/2/100=1K)/1毫秒
 *
 *
 * bug:
 *   控制器对启动采样的时间间隔的要求
 *   上次采样结束到下一次重新启动采样的时间 > 4 * 工作周期
 */
...
MODULE_DESCRIPTION("JZ x1830 ADC driver");
MODULE_LICENSE("GPL");
