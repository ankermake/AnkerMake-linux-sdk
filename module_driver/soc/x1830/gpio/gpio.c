...
static const unsigned long gpiobase[] = {
    ...
    [GPIO_PORT_D] = (unsigned long)CKSEG1ADDR(GPIO_IOBASE + 3 * GPIO_PORT_OFF),
...
static enum gpio_function gpio_get_func(int gpio)
{
    ...
    if (pin > 15)
        pull = (gpio_read(port, PXPEH) >> ((pin - 16) * 2)) & 0x3;
    else
        pull = (gpio_read(port, PXPEL) >> (pin * 2)) & 0x3;

    if (pin > 15)
        drive = (gpio_read(port, PXDSH) >> ((pin - 16) * 2)) & 0x3;
    else
        drive = (gpio_read(port, PXDSL) >> (pin * 2)) & 0x3;

    unsigned int rate = !!(gpio_read(port, PXSR) & (1 << pin));

    unsigned int smt = !!(gpio_read(port, PXSMT) & (1 << pin));

    function = (((func << 0) | GPIO_FUNC_0) & 0x1f) | \
               (((pull << 5) | GPIO_PULL_FUNC) & 0xe0) | \
               (((drive << 8) | GPIO_DRIVE_FUNC) & 0xf00) | \
               (((rate << 14) | GPIO_RATE_FUNC) & 0x3000) | \
               (((smt <<  16) | GPIO_SMT_FUNC) & 0xc000);
...
static struct func_name pull_func_names[] = {
    ...
    {GPIO_PULL_UP, "pull_up"},
    {GPIO_PULL_DOWN, "pull_down"},
    {GPIO_PULL_BUSHOLD, "pull_bushold"},
    {0, NULL},
};

static struct func_name drive_func_names[] = {
    {GPIO_DRIVE_2MA, "drive_2ma"},
    {GPIO_DRIVE_4MA, "drive_4ma"},
    {GPIO_DRIVE_8MA, "drive_8ma"},
    {GPIO_DRIVE_12MA, "drive_12ma"},
    {0, NULL},
};

static struct func_name rate_func_names[] = {
    {GPIO_RATE_SLOW, "rate_slow"},
    {GPIO_RATE_FAST, "rate_fast"},
    {0, NULL},
};

static struct func_name smt_func_names[] = {
    {GPIO_SMT_ENABLE, "smt_enable"},
    {GPIO_SMT_DISABLE, "smt_disable"},
    {0, NULL},
};
...
static int str_to_gpio_func(const char *str)
{
    ...
    func = to_gpio_func(str, drive_func_names);
    if (func != -1)
        return func;

    func = to_gpio_func(str, rate_func_names);
    if (func != -1)
        return func;

    func = to_gpio_func(str, smt_func_names);
    if (func != -1)
        return func;
...
static void gpio_dump_func(int gpio, char *buf, unsigned int size)
{
    ...
    size -= len; buf += len;
    len = dump_func(func & 0xf00, buf, size, " ", drive_func_names);
    if (!len)
        return;

    size -= len; buf += len;
    len = dump_func(func & 0x3000, buf, size, " ", rate_func_names);
    if (!len)
        return;

    size -= len; buf += len;
    len = dump_func(func & 0xc000, buf, size, " ", smt_func_names);
    if (!len)
        return;
...
static void gpio_dump_func2(int gpio, char *buf[], unsigned int count, unsigned int size)
{
    ...
    if (count >= 3)
        dump_func(func & 0xf00, buf[2], size, "", drive_func_names);

    if (count >= 4)
        dump_func(func & 0x3000, buf[3], size, "", rate_func_names);

    if (count >= 5)
        dump_func(func & 0xc000, buf[4], size, "", smt_func_names);
}
...
static long gpio_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case CMD_gpio_set_func: {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (!check_func(save_func, func, GPIO_DRIVE_FUNC)) {
                printk(KERN_ERR "gpio: %s func double set\n", "drive");
                return -EINVAL;
            }

            if (!check_func(save_func, func, GPIO_RATE_FUNC)) {
                printk(KERN_ERR "gpio: %s func double set\n", "rate");
                return -EINVAL;
            }

            if (!check_func(save_func, func, GPIO_SMT_FUNC)) {
                printk(KERN_ERR "gpio: %s func double set\n", "smt");
                return -EINVAL;
            }
...
MODULE_DESCRIPTION("JZ x1830 gpio driver");
MODULE_LICENSE("GPL");
