...
void icodec_enable_adc(void)
{
    ...
    codec_set_bit(CACR, adcintf, 0); // 0 stereo 1 mono
    ...
    if (icodec.is_bais) {
        ...
        msleep(1);
        codec_set_bit(CAACR, Micbaisctr, 6);
        msleep(10);
    } else {
...
/********** bias_on 直接输出电压 **********/
void icodec_bias_on_enable(int voltage_level)
{
    if (voltage_level > 7 || voltage_level < 0) {
        pr_err("error:bad voltage_level, bias_on voltage gain rang is 0～7!\n");
        return;
    }
    ...
    codec_set_bit(CAACR, Micbiasen, 1);
    msleep(1);
    codec_set_bit(CAACR, Micbaisctr, voltage_level);
    msleep(10);

}

void icodec_bias_on_disable(void)
{
    codec_set_bit(CAACR, Micbiasen, 0);
    msleep(1);
    // discharge
...
void bias_on_init(void)
{
    struct clk *clk;

    if (keep_bias_on) {

        clk = clk_get(NULL, "aic");
        BUG_ON(IS_ERR(clk));
        clk_enable(clk);

        printk("init enable bias_on\n");
        icodec_bias_on_enable(bias_on_voltage_level);

        clk_put(clk);
    }
}

EXPORT_SYMBOL(icodec_bias_on_enable);
EXPORT_SYMBOL(icodec_bias_on_disable);
