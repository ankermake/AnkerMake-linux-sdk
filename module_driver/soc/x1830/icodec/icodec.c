...
#define ICODEC_FORMATS (SNDRV_PCM_FMTBIT_S8 | SNDRV_PCM_FMTBIT_S16_LE)
...
static int keep_bias_on = 0;
static int bias_on_voltage_level = 0;
...
module_param(keep_bias_on, int ,0644);
module_param(bias_on_voltage_level, int ,0644);
...
{
    int sample_rate[] = {96000, 48000, 44100, 36000, 24000, 16000, 12000, 8000};
    ...
    if (!icodec.is_init) {
        codec_set_bit(CSRR, samplerate, i);
        icodec.rate = i;
    } else {
        if (icodec.rate != i) {
            printk(KERN_ERR "ICODEC: sample rate must be %d\n", sample_rate[icodec.rate]);
            ...
            struct snd_ctl_elem_value *uctl)
{
    ...
    if (icodec.dac_is_enable) {
        ...
        if (mute)
            codec_set_bit(CASR, daclechsrc, 3);
        else
            codec_set_bit(CASR, daclechsrc, 0);
    }
...
static int icodec_init(void)
{
    ...
    ret = platform_driver_register(&icodec_driver);
