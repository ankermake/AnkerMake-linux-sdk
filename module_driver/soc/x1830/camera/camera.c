...
static void init_dvp_irq(void)
{
    ...
    set_bit_field(&irq_en, ISP_FIFO_OVF, 1);
    set_bit_field(&irq_en, VIC_HVF_ERR, 1);
    set_bit_field(&irq_en, VIC_VRES_ERR, 1);
    set_bit_field(&irq_en, VIC_HRES_ERR, 1);
    set_bit_field(&irq_en, VIC_FIFO_OVF, 1);
...
static void vic_dvp_init_setting(struct vic_sensor_config *cfg)
{
    ...
    *(volatile unsigned int*)(0xb00222cc) = 0x1;
    ...
    /*
     * 0x150 这个寄存器据说是配置是否复用mipi相关的gpio作为dvp引脚，这里选择不复用
     */
    vic_write_reg(0x150, 2);
...
static void vic_stream_on(struct vic_sensor_config *cfg)
{
    ...
    if (cfg->vic_interface == VIC_dvp) {
        ...
        panic("not support mipi now\n");
        // clk_enable(vic_data.csi_clk);
        // vic_mipi_init_setting(cfg);
    }
