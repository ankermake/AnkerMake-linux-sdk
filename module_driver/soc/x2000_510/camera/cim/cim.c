...
{
    printk("Dump CIM debug DMA desc\n");
    printk("DBG next_desc_addr       :0x%08x\n", cim_read_reg(CIM_DBG_DES));
    printk("DBG write_back_address   :0x%08x\n", cim_read_reg(CIM_DBG_DES));
    printk("DBG write_back_stride    :0x%08x\n", cim_read_reg(CIM_DBG_DES));
    printk("DBG desc_intc_t          :0x%08x\n", cim_read_reg(CIM_DBG_DES));
    printk("DBG desc_cfg_t           :0x%08x\n", cim_read_reg(CIM_DBG_DES));
    printk("DBG desc_hist_cfg_t      :0x%08x\n", cim_read_reg(CIM_DBG_DES));
    printk("hist_write_back_addr     :0x%08x\n", cim_read_reg(CIM_DBG_DES));
    printk("sf_write_back_addr       :0x%08x\n", cim_read_reg(CIM_DBG_DES));
    printk("DBG current DMA addr     :0x%08x\n", cim_read_reg(CIM_DBG_DMA));
}
...
static inline void cim_dump_desc(void)
{
    ...
    int mem_cnt = drv->mem_cnt;

    printk("Dump CIM DMA desc\n");
    int i = 0;
    for (i = 0; i < mem_cnt; i++) {
        printk("desc index = %d\n", i);
        printk("next_desc_addr       :0x%08lx\n", descs[i].next_desc_addr);
        printk("write_back_address   :0x%08lx\n", descs[i].write_back_address);
        printk("write_back_stride    :0x%08lx\n", descs[i].write_back_stride);
        printk("desc_intc_t          :0x%08x\n", descs[i].desc_intc_t.d32);
        printk("desc_cfg_t           :0x%08x\n", descs[i].desc_cfg_t.d32);
        printk("desc_hist_cfg_t      :0x%08x\n", descs[i].desc_hist_cfg_t.d32);
        printk("hist_write_back_addr :0x%08lx\n", descs[i].hist_write_back_addr);
        printk("sf_write_back_addr   :0x%08lx\n", descs[i].sf_write_back_addr);
        printk("\n");
    }
}
...
static void set_desc_addr(int id, struct frame_data *frm)
{
    ...
    desc->write_back_address = frm->info.paddr;
...
static int cim_alloc_mem(struct sensor_attr *sensor)
{
    ...
    unsigned int frame_align_size = ALIGN(real_frame_size, PAGE_SIZE);
    unsigned int mem_cnt = drv->cam_mem_cnt + 1;

    assert(mem_cnt >= 2);

    void *mem = m_dma_alloc_coherent(drv->dev, mem_cnt * frame_align_size);
    if (mem == NULL) {
    ...
    drv->trans_list = kmalloc(CIM_DMA_DESC_COUNT * sizeof(drv->trans_list[0]), GFP_KERNEL);
    ...
    /* DMA描述符 */
    drv->frame_descs = m_dma_alloc_coherent(drv->dev, CIM_DMA_DESC_COUNT * sizeof(drv->frame_descs[0]));
    ...
    dma_cache_wback((unsigned long)drv->frame_descs, CIM_DMA_DESC_COUNT * sizeof(drv->frame_descs[0]));
    //cim_dump_desc();
...
static void cim_free_mem(void)
{
    ...
    m_dma_free_coherent(drv->dev, drv->mem, drv->frm_size * drv->mem_cnt);
    m_dma_free_coherent(drv->dev, drv->frame_descs, drv->mem_cnt * sizeof(drv->frame_descs[0]));
...
static void cim_init_common_setting(struct sensor_attr *attr)
{
    ...
    default:
        printk(KERN_ERR "cim init mipi setting NOT support format %x\n", attr->info.data_fmt);
        assert(0);
        return;
    }
...
static void cim_init_setting(struct sensor_attr *attr)
{
    ...
    if (attr->dbus_type == SENSOR_DATA_BUS_DVP)
        ...
        panic("cim: not support this type now: %d\n", attr->dbus_type);
}
...
static void frame_format_convert(struct jz_cim_data *drv, void *address)
{
    ...
    case SENSOR_PIXEL_FMT_SRGGB10_1X10:
        ...
        frame_convert_raw10_padding_raw16_forward(
                ...
                address + attr->info.frame_align_size,
                real_frame_size);
        break;
...
static int camera_get_fps(int *fps)
{
    struct jz_cim_data *drv = &jz_cim_dev;
...
static int camera_set_fps(int fps)
{
    struct jz_cim_data *drv = &jz_cim_dev;
    struct sensor_attr *sensor = drv->camera.sensor;
    int ret;
...
static int camera_get_hflip(camera_ops_mode *mode)
{
    struct jz_cim_data *drv = &jz_cim_dev;
    struct sensor_attr *sensor = drv->camera.sensor;
    camera_ops_mode tmp;
...
static int camera_set_hflip(camera_ops_mode mode)
{
    struct jz_cim_data *drv = &jz_cim_dev;
    struct sensor_attr *sensor = drv->camera.sensor;
    int ret;
...
static int camera_get_vflip(camera_ops_mode *mode)
{
    struct jz_cim_data *drv = &jz_cim_dev;
    struct sensor_attr *sensor = drv->camera.sensor;
    camera_ops_mode tmp;
...
static int camera_set_vflip(camera_ops_mode mode)
{
    struct jz_cim_data *drv = &jz_cim_dev;
    struct sensor_attr *sensor = drv->camera.sensor;
    int ret;
...
static long cim_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case CMD_get_fps: {
        if (arg)
            return camera_get_fps((int *)arg);
        else
            return -EINVAL;
    }
    case CMD_set_fps: {
        return camera_set_fps((int)arg);
    }

    case CMD_get_hflip: {
        if (arg)
            return camera_get_hflip((camera_ops_mode *)arg);
        else
            return -EINVAL;
    }
    case CMD_set_hflip: {
        return camera_set_hflip((camera_ops_mode)arg);
    }

    case CMD_get_vflip: {
        if (arg)
            return camera_get_vflip((camera_ops_mode *)arg);
        else
            return -EINVAL;
    }
    case CMD_set_vflip: {
        return camera_set_vflip((camera_ops_mode)arg);
    }
...
static int sensor_attribute_check_init(struct sensor_attr *sensor)
{
    ...
    if (sensor->dbus_type == SENSOR_DATA_BUS_DVP) {
        ...
        default:
            printk(KERN_ERR "attribute check: DVP Unsupported gpio mode(%d) .\n", sensor->dvp.gpio_mode);
            goto unlock;
        }
    }
...
int cim_register_sensor_route(int index, int mem_cnt, struct sensor_attr *sensor)
{
    ...
    if (drv->cam_mem_cnt < 1) {
        drv->cam_mem_cnt = 2;
        printk(KERN_ERR "cim device(camera): mem cnt fix to 2\n");
    }
...
void cim_unregister_sensor_route(int index, struct sensor_attr *sensor)
{
    assert(index == 2);
...
int jz_cim_drv_init(struct device *dev)
{
    struct jz_cim_data *drv = &jz_cim_dev;
    ...
    memset(&drv->cim_mdev, 0, sizeof(struct miscdevice));
    ...
    int ret = request_irq(drv->irq, cim_irq_handler, 0, drv->device_name, drv);
    assert(!ret);
