...
int vic_tiziano_enable_clk(int index)
{
    return vic_enable_clk(index);
}
...
static int dsysfs_vic_tiziano_snap_raw(struct jz_vic_tiziano_data *drv)
{
    ...
    int index = drv->index;
    unsigned int lineoffset = sensor->sensor_info.width * 2;
    unsigned int imagesize = lineoffset * sensor->sensor_info.height;
    void *snap_vaddr;
    ...
    if (snap_paddr){
        ...
        vic_write_reg(index, VIC_DMA_RESOLUTION, sensor->sensor_info.width << 16 | sensor->sensor_info.height);
        ...
        set_fs(KERNEL_DS);
        pos = &(fd->f_pos);
        kernel_write(fd, snap_vaddr, imagesize, pos);
        filp_close(fd, NULL);
        set_fs(old_fs);
