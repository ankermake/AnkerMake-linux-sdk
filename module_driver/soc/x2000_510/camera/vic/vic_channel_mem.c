...
static int vic_alloc_mem(int index, struct sensor_attr *attr)
{
    ...
    if (camera_fmt_is_NV12(attr->info.data_fmt) ) {
        line_length = ALIGN(attr->sensor_info.width, 16);
        frm_size = line_length * attr->sensor_info.height;
        ...
        line_length = ALIGN(attr->sensor_info.width, 8);
        frm_size = line_length * attr->sensor_info.height;
        if (frm_size % VIC_ALIGN_SIZE) {
            printk(KERN_ERR "frm_size not aligned\n");
            return -EINVAL;
        ...
        line_length = ALIGN(attr->sensor_info.width, 8) * 2;
        frm_size = line_length * attr->sensor_info.height;
        if (frm_size % VIC_ALIGN_SIZE) {
            printk(KERN_ERR "frm_size not aligned\n");
            return -EINVAL;
