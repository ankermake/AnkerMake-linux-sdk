...
static struct snd_soc_component_driver dmic_component = {
    ...
    .prepare = dmic_dma_pcm_prepare,

    .pcm_construct    = dmic_dma_pcm_new,
    .pcm_destruct   = dmic_dma_pcm_free,
};
...
static int dmic_probe(struct platform_device *pdev)
{
    ...
    dmic_dev.dma_buffer = m_dma_alloc_coherent(&pdev->dev, MAX_DMA_BUFFERSIZE);
...
{
    m_dma_free_coherent(&pdev->dev, dmic_dev.dma_buffer, MAX_DMA_BUFFERSIZE);
