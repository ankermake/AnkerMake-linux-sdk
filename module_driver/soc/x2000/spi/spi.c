...
#define IRQ_SSI0    (IRQ_INTC_BASE + 9)
#define IRQ_SSI1    (IRQ_INTC_BASE + 8)
...
struct jz_spi_drv *jzspi_handle[2];
...
static struct jz_spi_pin jz_spi1_pin[3] = {
    {
        {
    ...
    {
        {
            {GPIO_PE(16), GPIO_FUNC_1, "spi1_clk"},
            {GPIO_PE(18), GPIO_FUNC_1, "spi1_miso"},
            {GPIO_PE(17), GPIO_FUNC_1, "spi1_mosi"},
        },
    },
};
...
module_param_named(div_ssi_rate, spi_clk_rate, int, 0644);
...
module_param_named(spi1_is_enable, jzspi_dev[1].is_enable, int, 0644);
module_param_gpio_named(spi1_miso, jzspi_dev[1].spi_miso, 0644);
module_param_gpio_named(spi1_mosi, jzspi_dev[1].spi_mosi, 0644);
module_param_gpio_named(spi1_clk, jzspi_dev[1].spi_clk, 0644);
    ...
    void * tx_buf, void * rx_buf, int tlen, int rlen)
{
    ...
    if (tlen > 0) {
        reinit_completion(&hw->done_tx_dma);
    ...
    if (rlen > 0) {
        reinit_completion(&hw->done_rx_dma);
    ...
    void * tx_buf, int tlen, void * rx_buf, int rlen)
{
    ...
    int maxburst;/* unit is words, not bytes */
    if (total_size % 64 == 0) maxburst = 64 / bytes_per_word;
    else if (total_size % 32 == 0) maxburst = 32 / bytes_per_word;
    else if (total_size % 16 == 0) maxburst = 16 / bytes_per_word;
    else                maxburst = 1;
...
static void spi_dma_init(struct jz_spi_drv *hw)
{
    ...
    hw->txchan = dma_request_channel(mask, NULL, hw);
    if(!hw->txchan) {
        printk(KERN_ERR "SPI%d request dma tx channel failed.\n", id);
        BUG();
    }

    hw->rxchan = dma_request_channel(mask, NULL, hw);
    if(!hw->rxchan) {
...
MODULE_DESCRIPTION("JZ x2000 SPI Controller Driver");
MODULE_LICENSE("GPL");
