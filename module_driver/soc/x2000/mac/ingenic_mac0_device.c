...
#define IRQ_GMAC0    (IRQ_INTC_BASE + 55)

#define JZ_MAC0_BASE_ADDR   0x134B0000
#define JZ_MAC0_END_ADDR    0x134BFFFF

#define MAC_INTERFACT_MASK      0x7
#define MAC_INTERFACT_RMII      0x4
#define MAC_INTERFACT_RGMII      0x1
#define CPM_MAC_PHY0            0xe4
#define MAC_CRLT_PORT               2
#define MAC_CRLT_FUNC               GPIO_FUNC_1
#define MAC_CRLT_GPIO(pin)          GPIO_PC(pin)
...
#define MAC_RMII_GPIO_PHY_CLK      0xbcce
#define MAC_RGMII_GPIO_PHY_CLK     0xfffe

#define MAC_TX_CLK_GPIO            0x4000
#define MAC_PHY_CLK_GPIO           0x0002
#define MAC_MDIO_GPIO              0x3000
...
module_param_named(mac0_enable_flag, mac_enable, uint, 0444);
module_param_named(mac0_tx_clk_rate, tx_clk_rate, uint, 0444);
module_param_named(mac0_tx_clk_enable_delay, tx_clk_enable_delay, uint, 0444);
module_param_named(mac0_enable_tx_clk, enable_tx_clk, uint, 0444);
module_param_named(mac0_enable_crystal_clk, enable_crystal_clk, uint, 0444);
module_param_named(mac0_tx_clk_delay, tx_clk_delay, uint, 0444);
module_param_named(mac0_rx_clk_delay, rx_clk_delay, uint, 0444);
module_param_named(mac0_phy_interface, phy_interface, uint, 0444);
module_param_named(mac0_phy_probe_mask, phy_mask, uint, 0444);
module_param_named(mac0_enable_virtual_phy, enable_virtual_phy, uint, 0444);

static int mac0_phy_reset_gpio;
static unsigned int phy_reset_level;
static unsigned int phy_reset_time_us;

module_param_gpio(mac0_phy_reset_gpio, 0444);
module_param_named(mac0_phy_reset_level, phy_reset_level, uint, 0644);
module_param_named(mac0_phy_reset_time_us, phy_reset_time_us, uint, 0644);
...
static int mac_request_pins(unsigned long pins)
{
    ...
    for (i = 0; i < 32; i++) {
        if (pins & (1 << i)) {
            ret = gpio_request(MAC_CRLT_GPIO(i), "mac0_gpio");
            if (ret) {
...
static int mac_enable_tx_clk(void)
{
    ...
    mux_tx_clk = clk_get(NULL, "mux_mactxphy");
    ...
    phy_tx_clk = clk_get(NULL, "div_mactxphy0");
    if (IS_ERR(phy_tx_clk)) {
        clk_put(mux_tx_clk);
        clk_put(epll_clk);
        clk_put(mpll_clk);
...
static int jz_mac_init(struct platform_device *pdev, void *priv)
{
    ...
    cpm_outl(cpm_mphyc, CPM_MAC_PHY0);

    cpm_mphyc = cpm_inl(CPM_MAC_PHY0);
    ...
    if (ret) {
        printk(KERN_ERR "%s: init mac0 gpio failure\n", __func__);
        goto err_request_pins;
    }

    gpio_port_set_func(MAC_CRLT_PORT, mac_crtl_gpio_mask, MAC_CRLT_FUNC | GPIO_PULL_HIZ);

    if (gpio_is_valid(mac0_phy_reset_gpio)) {
        ret = gpio_request(mac0_phy_reset_gpio, "mac0_phy_reset");
        if (ret) {
            printk(KERN_ERR "%s: phy reset gpio request failure\n", __func__);
            goto err_phy_reset_gpio;
        }
        gpio_direction_output(mac0_phy_reset_gpio, !phy_reset_level);
    }
...
static int mac_phy_reset(void *priv)
{
    if (gpio_is_valid(mac0_phy_reset_gpio)) {
        /* reset phy */
        gpio_set_value(mac0_phy_reset_gpio, phy_reset_level);
...
static struct resource jz_mac_resources[] = {
    {
        .start  = JZ_MAC0_BASE_ADDR,
        .end    = JZ_MAC0_END_ADDR,
...
static struct platform_device jz_mac0_device = {
...
int platform_device_register_mac0(void)
{
    if (mac_enable) {
        ...
        return platform_device_register(&jz_mac0_device);
    }
        return 0;
}

void platform_device_unregister_mac0(void)
{
    if (mac_enable)
        platform_device_unregister(&jz_mac0_device);
}