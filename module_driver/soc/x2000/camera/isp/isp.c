...
static void *isp_read_file_data(char *file_name)
{
    ...
    /* alloc data */
    fsize = file->f_inode->i_size;
    data = kmalloc(fsize, GFP_KERNEL);
    if(data == NULL){
        printk(KERN_ERR "%s: failed to alloc %lld KB buffer!\n", __func__, fsize >> 10);
    ...
    old_fs = get_fs();
    set_fs(KERNEL_DS);
    vfs_read(file, data, fsize, &(file->f_pos));
    filp_close(file, NULL);
    set_fs(old_fs);
...
static void isp_free_file_data(void *data)
{
    if (data)
        kfree(data);
}
...
static int tiziano_init(int index)
{
    ...
    snprintf(file_name, sizeof(file_name), "/etc/sensor/%s-x2000.bin", attr->device_name);
    isp_bin_data = isp_read_file_data(file_name);

    ret = tisp_core_init(&drv->core, &iparam, drv, isp_bin_data);
    isp_free_file_data(isp_bin_data);
    if (ret) {
...
static irqreturn_t isp_irq_handler(int irq, void *data)
{
    ...
    //breakfrm & overflow
    if (irqstatus & 0x300) {
        printk(KERN_ERR "%s breakfrm 0x%08x\n",__func__, irqstatus);
        irqstatus = isp_read_reg(index, TOP_CTRL_ADDR_TOP_CON);
        isp_write_reg(index, TOP_CTRL_ADDR_TOP_CON, irqstatus | 0x2);   //[1]:1
        while(retry < 60) {
        ...
        if(retry < 60) {
            //reset isp
            isp_write_reg(index, TOP_CTRL_ADDR_TOP_CON, ((irqstatus & 0xfffffffd) | 0x10)); //[1]:0 [4]:1
            isp_write_reg(index, TOP_CTRL_ADDR_TOP_CON, (irqstatus & 0xffffffed));  //[4]:0
