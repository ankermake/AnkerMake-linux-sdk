...
static irqreturn_t cim_irq_handler(int irq, void *data)
{
    ...
    static unsigned int shutter_count = 0;
        ...
        struct frame_desc *next, struct sensor_attr *sensor)
{
    ...
    desc->desc_intc_t.data.sof_mask = 1;
...
static int cim_alloc_mem(struct sensor_attr *sensor)
{
    ...
    dma_cache_sync(NULL, drv->frame_descs, CIM_DMA_DESC_COUNT * sizeof(drv->frame_descs[0]), DMA_TO_DEVICE);
...
static void cim_free_mem(void)
{
    ...
    m_dma_free_coherent(drv->mem, drv->frm_size * drv->mem_cnt);
    m_dma_free_coherent(drv->frame_descs, drv->mem_cnt * sizeof(drv->frame_descs[0]));
