...
static int wait_mcu_wakeup(unsigned long *timeout)
{
    ...
    ts.tv_nsec = (*timeout) % (1 * 1000 * 1000)*1000;
    ktimeout = timespec_to_ktime(ts);

    int ret = wait_event_hrtimeout(mcu.wait, mcu.msg_status, ktimeout);
    if (ret < 0) {
...
static int __init mcu_init(void)
{
    ...
    assert(!IS_ERR(mcu.clk));

    mcu.intc_clk = clk_get(NULL, "gate_intc");
    assert(!IS_ERR(mcu.intc_clk));
