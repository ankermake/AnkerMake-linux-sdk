...
struct fb_layer_mixer_dev *fb_layer_mixer_create(void)
{
    ...
    mixer->frame.framedesc = m_dma_alloc_coherent(sizeof(struct framedesc));
    memset(mixer->frame.framedesc, 0, sizeof(*mixer->frame.framedesc));

    for(i = 0; i < 4; i++) {
        mixer->frame.layers[i] = m_dma_alloc_coherent(sizeof(struct layerdesc));
...
void fb_layer_mixer_delete(struct fb_layer_mixer_dev *mixer)
{
    ...
    for(i = 0; i < 4; i++)
        m_dma_free_coherent(mixer->frame.layers[i], sizeof(struct layerdesc));

    m_dma_free_coherent(mixer->frame.framedesc, sizeof(struct framedesc));
...
{
    struct fb_layer_mixer_dev *mixer = fb_layer_mixer_create();
    if(!mixer)
        return -1;
...
static int fb_layer_mixer_init(void)
{
    int ret;
    ...
    ret = misc_register(&fb_layer_mixer_mdev);
    if (ret < 0)
        goto misc_err;
...
static void fb_layer_mixer_module_exit(void)
{
    disable_irq(layer_mixer.irq);
    free_irq(layer_mixer.irq, &layer_mixer);
