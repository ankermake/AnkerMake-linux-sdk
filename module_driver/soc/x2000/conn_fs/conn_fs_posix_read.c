...
#define CONN_FS_DRV_TAG                "conn_fs_drv_read:"

/*
 * 该结构体和kernel driver保持一致， 和 RTOS端传递的结构体不一致
 * 用户空间和kernel交互数据buffer 大小固定，需要多次交换数据
 */
struct conn_fs_args_read {
    int argc;
    int fd;
    void *buf;  /* 暂时未使用该变量, 用户空间和kernel 通过mapp地址交互数据  */
    size_t totle_len; /* 总大小 */
    size_t tranfer_len; /* 本次传输大小 */
};

#ifdef CONN_FS_POSIX_CMDS_DEBUG
static void conn_fs_posix_debug_rtos_args_read(struct conn_fs_posix_cmds_arguments *args)
{
...
static void conn_fs_posix_fill_api_args_read(struct conn_fs_posix_data *data, struct conn_fs_args_read *args)
{
    ...
    /*
     * 交互的内容在应用层读取/写入到mmaped_mem映射空间, 驱动中copy到RTOS端指定的buffer
     */
...
static inline void conn_fs_posix_fill_api_args_read_update_buffer(struct conn_fs_posix_data *data, struct conn_fs_args_read *args)
{
    struct conn_fs_posix_cmds_arguments *cmds = data->args;
...
static void *conn_fs_posix_get_rtos_buffer(void **args)
{
    void **argv = args;

    void *buffer = (void *)(*(uint32_t *)argv[1]);

    return buffer;
}

static int conn_fs_drv_service_posix_read(struct conn_fs_posix_data *data, unsigned int arg)
{
    struct conn_fs_args_read args;

    /* 解析计算出要copy的数据大小 */
    conn_fs_posix_fill_api_args_read(data, &args);

    int ret = copy_to_user((void __user *)arg, &args, sizeof(struct conn_fs_args_read));
    if (ret)
        printk(KERN_ERR "conn fs drv service posix read copy to user error\n");

    return ret;
}

/*
 * read的长度超过 mmap映射缓存长度,将read内容更新到RTOS指定的buf中,继续读
 */
static int conn_fs_drv_service_posix_read_update_buf(struct conn_fs_posix_data *data, unsigned int arg)
{
    struct conn_fs_args_read args;
    struct conn_fs_posix_cmds_result *res = data->res;

    int ret = copy_from_user(&args, (void __user *)arg, sizeof(struct conn_fs_args_read));
    if (ret) {
    ...
    /* 主要作用解析出RTOS传递的buf地址 */
    conn_fs_posix_fill_api_args_read_update_buffer(data, &args);

    /*
     * 应用层读取/写入到mmaped_mem映射空间, 驱动中更新到RTOS端指定的buffer中
     */
    int offset = res->result_len; /* 已copy完数据的长度：result_len当全局变量,当return命令执行时,res的内容会更新覆盖 */
    char *buf = args.buf;
    memcpy(&buf[offset], data->mem, args.tranfer_len);
