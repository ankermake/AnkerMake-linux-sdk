...
static int cim_alloc_mem(struct sensor_attr *sensor)
{
    ...
    /* DMA描述符 */
    drv->frame_descs = m_dma_alloc_coherent(CAMERA_DMA_DESC_COUNT * sizeof(drv->frame_descs[0]));
    ...
    dma_cache_sync(NULL, drv->frame_descs, CAMERA_DMA_DESC_COUNT * sizeof(drv->frame_descs[0]), DMA_TO_DEVICE);
...
static void cim_free_mem(void)
{
    ...
    m_dma_free_coherent(drv->frame_descs, CAMERA_DMA_DESC_COUNT * sizeof(drv->frame_descs[0]));
