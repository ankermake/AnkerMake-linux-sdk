...
#define SHADOW                          4
#define GPIO_PULL_FUNC                  GPIO_PULL_HIZ
...
static enum gpio_function gpio_get_func(int gpio)
{
    ...
    unsigned int edg  = !!(gpio_read(port, PXDEG) & (1 << pin));

    unsigned int func = edg << 4 | intr << 3 | mask << 2 | pat1 << 1 | pat0;

    pull = !!(gpio_read(port, PXPEN) & (1 << pin));

    function = (((func << 0) | GPIO_FUNC_0)) | \
               (((pull << 13) | GPIO_PULL_HIZ));
...
static void gpio_dump_func(int gpio, char *buf, unsigned int size)
{
    ...
    len = dump_func(func & 0x1ff, buf, size, "", common_func_names);
    if (!len)
        return;

    size -= len; buf += len;
    len = dump_func(func & 0xe000, buf, size, " ", pull_func_names);
    if (!len)
        return;
...
static void gpio_dump_func2(int gpio, char *buf[], unsigned int count, unsigned int size)
{
    ...
    if (count >= 1)
        dump_func(func & 0x1ff, buf[0], size, "", common_func_names);

    if (count >= 2)
        dump_func(func & 0xe000, buf[1], size, "", pull_func_names);
}
...
static long gpio_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case CMD_gpio_set_func: {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (GPIO_INT_LO <= func && func <= GPIO_INT_MASK_RE_FE) {
