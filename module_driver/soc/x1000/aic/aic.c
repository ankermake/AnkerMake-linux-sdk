...
#define gpio_i2s_mclk       GPIO_PB(0)
#define gpio_i2s_bclk       GPIO_PB(1)
#define gpio_i2s_lr_clk     GPIO_PB(2)
#define gpio_i2s_data_in    GPIO_PB(3)
#define gpio_i2s_data_out   GPIO_PB(4)
...
static int aic_ensure_clk_gpio_request(void)
{
    int ret = 0;

    if (!(gpio_state & CLK_GPIO)) {
        ret = m_gpio_request(gpio_i2s_lr_clk, "i2s-LR-clk", GPIO_FUNC_1);
        if (ret < 0)
            return -EINVAL;

        ret = m_gpio_request(gpio_i2s_bclk, "i2s-bclk", GPIO_FUNC_1);
        if (ret < 0) {
            gpio_free(gpio_i2s_lr_clk);
            return -EINVAL;
        }
        gpio_state |= CLK_GPIO;
    }
...
static int aic_ensure_dac_gpio_request(void)
{
    ...
    ret = aic_ensure_clk_gpio_request();
    if (ret < 0)
        return -EINVAL;

    if (!(gpio_state & DAC_GPIO)) {
        ret = m_gpio_request(gpio_i2s_data_out, "i2s-data-out", GPIO_FUNC_1);
        if (ret < 0)
            return -EINVAL;
...
static int aic_ensure_adc_gpio_request(void)
{
    ...
    if (!(gpio_state & ADC_GPIO)) {
        ret = m_gpio_request(gpio_i2s_data_in, "i2s-data-in", GPIO_FUNC_1);
        if (ret < 0)
            return -EINVAL;
...
static int aic_ensure_mclk_gpio_requeset(void)
{
    ...
    if (!(gpio_state & MCLK_GPIO)) {
        ret = m_gpio_request(gpio_i2s_mclk, "i2s-mclk", GPIO_FUNC_1);
        if (ret < 0)
            return -EINVAL;
...
static void aic_gpio_release(void)
{
    ...
    if (gpio_state & CLK_GPIO) {
        gpio_free(gpio_i2s_lr_clk);
        gpio_free(gpio_i2s_bclk);
    }

    if (gpio_state & ADC_GPIO)
        gpio_free(gpio_i2s_data_in);

    if (gpio_state & DAC_GPIO)
        gpio_free(gpio_i2s_data_out);
...
static int aic_set_clk_rate(struct aic_data *data)
{
    ...
    if (aic_dev.clk_id == SELECT_INNER_CODEC) {
        ...
        if (set_clk_freq == 0) {
            ...
            {
                printk(KERN_ERR "AIC: sample_rate = %d, div can't be %d\n", sample_rate, div);
                return -EINVAL;
            }

            if (div != 256 && div != 384 && div != 512 && div != 768) {
                printk(KERN_ERR "AIC: set_clk_rate failed, div can't be %d.\n", div);
                return -EINVAL;
            }
        }
    }
    ...
    clk_disable(cgu_clk);
    clk_set_parent(aic_dev.cgu_clk, clk_parent);
...
static int aic_probe(struct platform_device *pdev)
{
    ...
    clk_set_rate(aic_dev.cgu_clk, 24*1000*1000);
    clk_enable(aic_dev.cgu_clk);

    clk_ext1 = clk_get(NULL, "ext1");
    BUG_ON(IS_ERR(clk_ext1));

    clk_sclka = clk_get(NULL, "sclka");
    BUG_ON(IS_ERR(clk_sclka));
