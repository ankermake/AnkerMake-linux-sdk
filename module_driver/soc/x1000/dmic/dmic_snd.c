...
#define DMIC_GPIO_CLK  GPIO_PB(21)
#define DMIC_GPIO_DAT0 GPIO_PB(22)
#define DMIC_GPIO_DAT1 GPIO_PB(5)

static struct clk *clk;
static struct clk *clk_cgu;
static struct dmic_data dmic;
static int gpio_state = 0;
static int dmic_gain = 10;
...
static inline int init_gpio(int gpio, const char *name, int func)
{
    int ret;
...
static int dmic_gpio_request(int channels)
{
    ...
    if (!(gpio_state & BIT(0))) {
        ret = init_gpio(DMIC_GPIO_CLK, "dmic-clk", GPIO_FUNC_0);
        if (ret < 0)
            return ret;
        gpio_state |= BIT(0);
    }
    ...
    if (!(gpio_state & BIT(1))) {
        ret = init_gpio(DMIC_GPIO_DAT0, "dmic-dat0", GPIO_FUNC_0);
        if (ret < 0)
            goto err_gpio_dat0;
        gpio_state |= BIT(1);
    }
    ...
    if (channels > 2 && !(gpio_state & BIT(2))) {
        ret = init_gpio(DMIC_GPIO_DAT1, "dmic-dat1", GPIO_FUNC_1);
        if (ret < 0)
            goto err_gpio_dat1;
        gpio_state |= BIT(2);
    }
...
static int snd_dmic_init(struct dmic_data *dmic)
{
    ...
    strcpy(card->longname, "Ingenic x1000 DMIC_PCM");
...
static int dmic_probe(struct platform_device *pdev)
{
    ...
    clk_cgu = clk_get(NULL,"cgu_i2s");
    clk_set_rate(clk_cgu, 24000000);
    clk_enable(clk_cgu);
