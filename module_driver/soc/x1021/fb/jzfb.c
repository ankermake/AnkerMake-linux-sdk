...
static struct {
    ...
    struct lcdc_frame_desc *end_framedesc;
...
static inline unsigned int line_stride_pixels(enum fb_fmt fb_format, unsigned int xres)
{
    unsigned int align = fb_format == fb_fmt_RGB888 ? 2 : 4;
    unsigned int stride = xres + align - 1;

    return stride - stride % align;
}

static void lcdc_stop_frame(void)
{
    jzfb.is_stop = 1;
    lcdc_general_stop();
    wait_event_interruptible(jzfb.wait_queue, jzfb.is_stop == 0);
}

static void lcdc_start_frame(void)
{
    lcdc_set_frame_desc(virt_to_phys(jzfb.data_framedesc));
    lcdc_start_dma();
}
...
static void jzfb_process_slcd_data_table(struct smart_lcd_data_table *table, unsigned int length)
{
    ...
    for (; i < length; i++) {
        ...
        case SMART_CONFIG_CMD:
            jzfb_send_cmd(table[i].value);
            break;
        case SMART_CONFIG_PRM:
        case SMART_CONFIG_DATA:
            jzfb_send_data(table[i].value);
            break;
...
static void jzfb_enable_lcd(void)
{
    ...
    clk_set_rate(jzfb.clk_lpc, jzfb.data->pixclock_when_init);
    clk_enable(jzfb.clk_lpc);

    lcdc_hal_init(jzfb.data);

    lcdc_disable_all_interrupt();
    lcdc_enable_interrupt(lcdc_irq_general_stop);
    lcdc_enable_interrupt(lcdc_irq_frame_end);

    lcdc_disable_slcd_fmt_convert();
    ...
    clk_set_rate(jzfb.clk_lpc, jzfb.data->pixclock);
    clk_enable(jzfb.clk_lpc);

    jzfb_send_cmd(jzfb.data->cmd_of_start_frame);

    lcdc_enable_slcd_fmt_convert();
...
{
    unsigned long flags = lcdc_get_interrupts();

    if (lcdc_check_interrupt(flags, lcdc_irq_frame_end)) {
        lcdc_clear_interrupt(lcdc_irq_frame_end);
        jzfb.display_state = state_display_end;
        wake_up_all(&jzfb.wait_queue);
        return IRQ_HANDLED;
    }

    if (lcdc_check_interrupt(flags, lcdc_irq_general_stop)) {
        lcdc_clear_interrupt(lcdc_irq_general_stop);
        jzfb.is_stop = 0;
        wake_up_all(&jzfb.wait_queue);
        return IRQ_HANDLED;
    }
...
static int jzfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    jzfb.data_framedesc->buffer_addr_rgb = virt_to_phys(jzfb.lcd.fb_mem + next_frm * jzfb.lcd.bytes_per_frame);
    jzfb.data_framedesc->buffer_addr_uv = virt_to_phys(jzfb.lcd.fb_mem + next_frm * jzfb.lcd.bytes_per_frame);
...
static int init_gpio(struct lcdc_data *data)
{
    ...
    case SLCD_8080: {
        int use_cs = 0;
        int use_rdy = data->slcd.enable_rdy_pin;
...
static void auto_calculate_pixel_clock(struct lcdc_data *data)
{
    ...
    if (!data->pixclock_when_init)
        data->pixclock_when_init = data->xres * data->yres * 3;
}
...
{
    struct lcdc_data *data = jzfb.data;
    jzfb.data_framedesc = m_dma_alloc_coherent(sizeof(struct lcdc_frame_desc) * 2);
    jzfb.end_framedesc = jzfb.data_framedesc + 1;

    int line_stride = line_stride_pixels(data->fb_format, data->xres);
    init_frame_end_desc(jzfb.end_framedesc, jzfb.lcd.fb_mem, line_stride);
    init_frame_data_desc(jzfb.data_framedesc, jzfb.lcd.fb_mem, line_stride, 1, jzfb.end_framedesc);
}
...
int jzfb_register_lcd(struct lcdc_data *pdata)
{
    ...
    error_if(pdata->fb_format >= fb_fmt_NV12);
