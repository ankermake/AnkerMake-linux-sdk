...
{
    adc_set_bits(ADENA, AUX0EN, AUX4EN, 0);
}
...
{
    adc_set_bits(ADCTRL, ARDYM0, ARDYM4, 0);
}

inline void adc_hal_mask_all_interrupt(void)
{
    adc_set_bits(ADCTRL, ARDYM0, ARDYM4, ALL_INTERRUPT);
}
...
{
    adc_set_bits(ADSTATE, ARDY0, ARDY4, ALL_INTERRUPT);
}

inline unsigned int adc_hal_get_all_interrupt_flag(void)
{
    return adc_get_bits(ADSTATE, ARDY0, ARDY4);
}
