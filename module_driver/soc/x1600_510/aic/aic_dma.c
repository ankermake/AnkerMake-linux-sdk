...
static dma_addr_t get_dma_addr(struct dma_chan *dma_chan, unsigned long start, unsigned int len, enum dma_transfer_direction direction)
{
    ...
    pr_warn("AIC: DMA address[0x%08x] illegal!\n", aic_dst);
...
int dma_param_config(struct dma_param *dma,
    ...
    if (direction == DMA_DEV_TO_MEM) {
        ...
        slave_config.slave_id = INGENIC_DMA_REQ_ASOC_AIC_RX;
    } else {
        slave_config.dst_addr = dma->dma_addr;
        slave_config.slave_id = INGENIC_DMA_REQ_ASOC_AIC_TX;
    }
...
void dma_param_init(struct dma_param *dma, enum dma_transfer_direction direction)
{
    if (direction == DMA_DEV_TO_MEM)
        dma->dma_type = INGENIC_DMA_TYPE(INGENIC_DMA_REQ_ASOC_AIC_RX);
    else
        dma->dma_type = INGENIC_DMA_TYPE(INGENIC_DMA_REQ_ASOC_AIC_TX);

    dma->dma_addr = AIC_IOBASE + AICDR;
}
...
int aic_dma_invalid_data_cyclic(struct dma_param *dma, enum dma_transfer_direction direction, unsigned char *buf, size_t size)
{
    struct dma_async_tx_descriptor *desc;
    ...
    desc = dma->dma_chan->device->device_prep_dma_cyclic(dma->dma_chan,
            (unsigned long)(void *)virt_to_phys(buf), size, size,
