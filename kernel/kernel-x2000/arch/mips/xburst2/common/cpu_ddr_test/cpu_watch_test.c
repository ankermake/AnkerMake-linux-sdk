...
extern int do_watch_show_stack;
...
//	struct mips3264_watch_reg_state *watches;
	...
	printk("Usage: PID:WatchLo:WatchHi\n");
	printk("%s\n",g_buffer);
	s1 = g_buffer;
	s2 = strchr(s1,':');
	printk("s1 = %p s2 = %p\n",s1,s2);
	if(!s2 || count == 0) {
		...
		do_watch_show_stack = 0;

		return count;
	}

	do_watch_show_stack = 1;
...
static ssize_t cpu_watch_read_trigger(struct file *file, char __user *user_buf,
							size_t count, loff_t *ppos)
{
#if 0
	char *buf = g_buffer;
...
static ssize_t cpu_watch_write_trigger(struct file *file, const char __user *user_buf,
							 size_t count, loff_t *ppos)
{
	unsigned int addr = simple_strtoul(user_buf, 0, 0);

	printk("--[Debug Only]-addr: %x, value: %x\n", addr, *(volatile unsigned int *)addr);

	return count;
}

static const struct file_operations cpu_watch_trigger_ops = {
	.read = cpu_watch_read_trigger,
	.write = cpu_watch_write_trigger,
	.open = simple_open,
	.llseek = default_llseek,
};
...
static int create_debugfs(struct watch_struct *cwt)
{
	...
	d = debugfs_create_file("trigger", S_IWUSR| S_IRUGO, cwt->root, cwt, &cpu_watch_trigger_ops);
	if(IS_ERR_OR_NULL(d)) {
		pr_err("Debugfs create failed!\n");
		goto err_node;
	}
