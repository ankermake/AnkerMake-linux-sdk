...
//#define X2100_IDLE_PD


#ifdef DEBUG_PM
static void x2100_pm_gate_check(void)
{
...
int x2100_pm_enter(suspend_state_t state)
{
	extern volatile u8 *uart_base;

	printk("x2100 pm enter!!\n");
...
static int x2100_pm_begin(suspend_state_t state)
{
	printk("x2100 suspend begin\n");
	return 0;
}

static void x2100_pm_end(void)
{
	printk("x2100 pm end!\n");
}
#if defined CONFIG_KP_AXP
static int x2100_suspend_prepare(void)
{
	return regulator_suspend_prepare(PM_SUSPEND_MEM);
}
static void x2100_suspend_finish(void)
{
...
static const struct platform_suspend_ops x2100_pm_ops = {
	.valid		= ingenic_pm_valid,
	.begin		= x2100_pm_begin,
	.enter		= x2100_pm_enter,
	.end		= x2100_pm_end,
#if defined CONFIG_KP_AXP
	.prepare = x2100_suspend_prepare,
	.finish = x2100_suspend_finish,
#endif
};
