...
int ptrace_getfpxregs(struct task_struct *child, __u32 __user *data)
{
	if (!access_ok(VERIFY_WRITE, data, 33 * 8))
		return -EIO;

	if (tsk_used_math(child)) {
		struct xburst_mxu_struct *mxuregs = get_mxuv3_regs(child);
		copy_to_user(data, mxuregs, sizeof(*mxuregs));
	} else
		return -ENODEV;
		 ...
		 unsigned long addr, unsigned long data)
{
	...
	case PTRACE_GETFPXREGS:
		ret = ptrace_getfpxregs(child, datavp);
		break;
