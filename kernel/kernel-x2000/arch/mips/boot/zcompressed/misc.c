...
{
	volatile unsigned char *base = (volatile unsigned char*)CKSEG1ADDR(UART0_IOBASE);
	int i;
	for(i = 0; i < 10; i++) {
...
static void flush_window(void)
{
    ...
    in = window;
    out = &output_data[output_ptr];
#define IS_ALIGN4(x) (!((unsigned int)x & 3))
	if(IS_ALIGN4(in) && IS_ALIGN4(out)) {
		unsigned int *i_in = (unsigned int *)in;
		unsigned int *i_out = (unsigned int *)out;
		int a_outcnd = (outcnt / 32) * 32;
		for (n = 0; n < a_outcnd / 4; n += 8) {
			i_out[n + 0] = i_in[n + 0];
			i_out[n + 1] = i_in[n + 1];
			i_out[n + 2] = i_in[n + 2];
			i_out[n + 3] = i_in[n + 3];
