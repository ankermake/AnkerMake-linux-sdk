	...
	for(i = 0; i < CONFIG_FB_INGENIC_NR_FRAMES; i++) {
		for(j = 0; j < CONFIG_FB_INGENIC_NR_LAYERS; j++) {
			...
			fbdev->vidmem_phys[j][i] = fbdev->vidmem_phys[0][0] +
				j * fbdev->frm_size * CONFIG_FB_INGENIC_NR_LAYERS +
				i * fbdev->frm_size;
...
//		framedesc[i]->WritebackAddr = (uint32_t)fbdev->sread_vidmem_phys[0] + i*fbdev->frm_size;
	...
	for(i = frm_num_mi; i < frm_num_ma; i++) {
		for(j =0; j < DPU_SUPPORT_MAX_LAYERS; j++) {
			...
			if(lay_cfg[j].uv_addr[i % CONFIG_FB_INGENIC_NR_FRAMES]) {
				fbdev->layerdesc[i][j]->UVBufferAddr =
					lay_cfg[j].uv_addr[i % CONFIG_FB_INGENIC_NR_FRAMES];
			} else {
				fbdev->layerdesc[i][j]->UVBufferAddr =
					fbdev->layerdesc[i][j]->LayerBufferAddr +
...
static void ingenicfb_do_resume(struct ingenicfb_device *fbdev)
{
	...
	if(fbdev->is_suspend) {
		...
		if(fbdev->panel->lcd_type == LCD_TYPE_MIPI_SLCD || fbdev->panel->lcd_type == LCD_TYPE_MIPI_TFT){
		...
		struct device_attribute *attr, const char *buf, size_t n)
{
	...
	lay_cfg->source_w = simple_strtoul(p, NULL, 0);
	lay_cfg->source_h = simple_strtoul(s, NULL, 0);
	lay_cfg->stride = lay_cfg->source_w;

	fb = fbdev->fbs[layer];

	fb->var.xres = lay_cfg->source_w;
	fb->var.yres = lay_cfg->source_h;
	fb->var.xres_virtual = lay_cfg->source_w;
	fb->var.yres_virtual = lay_cfg->source_h;

	printk("layer: %d source_w: %d, source_h: %d\n", layer, lay_cfg->source_w, lay_cfg->source_h);
...
static int ingenicfb_do_probe(struct platform_device *pdev, struct lcd_panel *panel)
{
	...
	struct ingenic_dsi_priv *dsi_phy_priv;
	...
	if (!match)
		        return -ENODEV;
	if(panel->dsi_pdata){
		dsi_phy_priv = (struct ingenic_dsi_priv*)match->data;
		panel->dsi_pdata->dsi_iobase = dsi_phy_priv->dsi_iobase;
		panel->dsi_pdata->dsi_phy_iobase = dsi_phy_priv->dsi_phy_iobase;
	}
	fbdev->panel = panel;
	...
	fb->fix.smem_len = fbdev->vidmem_size / CONFIG_FB_INGENIC_NR_LAYERS + fbdev->sread_vidmem_size;
	...
	for(i = 1; i < CONFIG_FB_INGENIC_NR_LAYERS; i++) {
		...
		fb_layer->fix.smem_len = fbdev->vidmem_size / CONFIG_FB_INGENIC_NR_LAYERS;
...
static struct ingenic_dsi_priv x2000_m300_dsi_priv = {
	.dsi_iobase = 0x10075000,
	.dsi_phy_iobase = 0x10077000,
};

static struct ingenic_dsi_priv x2500_dsi_priv = {
	.dsi_iobase = 0x10003000,
	.dsi_phy_iobase = 0x10004000,
};

static const struct of_device_id ingenicfb_of_match[] = {
	{ .compatible = "ingenic,x2000-dpu",.data = (void*)&x2000_m300_dsi_priv},
	{ .compatible = "ingenic,x2500-dpu",.data = (void*)&x2500_dsi_priv},
	{ .compatible = "ingenic,m300-dpu",.data = (void*)&x2000_m300_dsi_priv},
	{},
};
