...
struct dsi_device * jzdsi_init(struct jzdsi_data *pdata)
{
	...
	if(!dsi->video_config->byte_clock) {
		...
		dsi->real_mipiclk =  dsi->video_config->h_total_pixels * dsi->video_config->v_total_lines * pdata->modes->refresh * pdata->bpp_info / dsi->video_config->no_of_lanes / 2;
	...
	clk_prepare_enable(dsi->clk);
	dsi->address = (unsigned int)ioremap(pdata->dsi_iobase, 0x190);
	if (!dsi->address) {
		pr_err("Failed to ioremap register dsi memory region\n");
		goto err_iounmap;
	}
	dsi->phy_address = (unsigned int)ioremap(pdata->dsi_phy_iobase, 0x200);
	if(!dsi->phy_address) {
