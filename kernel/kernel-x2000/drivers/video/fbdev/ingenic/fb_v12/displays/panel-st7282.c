...
//	printk("--line=%d------>power=%d,panel->power=%d,vdd_en->active_level=%d,vdd_en->gpio =%d\n\n",__LINE__,power,panel->power,vdd_en->active_level,vdd_en->gpio);
	}
	if(!POWER_IS_ON(power) && POWER_IS_ON(panel->power)) {
		gpio_direction_output(vdd_en->gpio, 0);
//	printk("---line=%d----->power=%d,panel->power=%d,vdd_en->gpio =%d\n\n",__LINE__,power,panel->power,vdd_en->gpio);
	}
