...
//#define PANLE_FW050_LCD_READ_DEBUG
...
#ifdef	PANLE_FW050_LCD_READ_DEBUG
static int panel_dev_rdid(struct panel_dev *lcd, unsigned char *buf, int count)
{
	...
	struct dsi_cmd_packet data_to_send1 = {0x14, 0x6C, 0x0};
	struct dsi_cmd_packet data_to_send2 = {0x14, 0x6E, 0x0};
	struct dsi_cmd_packet set_page4 = {0x39, 0x04, 0x00, {0xFF, 0x98, 0x81, 0x04}};
	struct dsi_cmd_packet set_page0 = {0x39, 0x04, 0x00, {0xFF, 0x98, 0x81, 0x00}};
	int ret = 0;

	ops->cmd_write(lcd_to_master(lcd),  set_page4);
	...
	ops->cmd_write(lcd_to_master(lcd),  set_page0);
...
static void panel_dev_panel_init(struct panel_dev *lcd)
{
	...
	{
		ops->cmd_write(dsi,  fitipower_fw050_720_1280_cmd_list[i]);
	}
}
...
static int of_panel_parse(struct device *dev)
{
	...
	if(gpio_is_valid(panel->vdd_en.gpio)) {
	 ...
	 if(gpio_is_valid(panel->lcd_pwm.gpio)) {
	 	...
	 	ret = gpio_direction_output(panel->lcd_pwm.gpio, panel->lcd_pwm.active_level);
	 	if(ret < 0) {
