...
#ifdef CUSTOMER_OOB
extern int ingenic_sdio_wlan_get_irq(unsigned long *flag);
static int gpio_wl_host_wake = -1; // WL_HOST_WAKE is output pin of WLAN module

int dhd_init_gpio_wlan_hostwake(void)
{
	int err = 0;
	int host_oob_irq = -1;
	...
	gpio_wl_host_wake = -1;
	gpio_wl_host_wake = ingenic_sdio_wlan_get_irq((unsigned long *)&host_oob_irq_flags);
...
#ifdef HW_OOB
	if (host_oob_irq_flags & IORESOURCE_IRQ_LOWLEVEL){
		host_oob_irq_flags = IORESOURCE_IRQ | IORESOURCE_IRQ_LOWLEVEL | IORESOURCE_IRQ_SHAREABLE;
	} else if (host_oob_irq_flags & IORESOURCE_IRQ_HIGHLEVEL){
		host_oob_irq_flags = IORESOURCE_IRQ | IORESOURCE_IRQ_HIGHLEVEL | IORESOURCE_IRQ_SHAREABLE;
	} else if (host_oob_irq_flags & IORESOURCE_IRQ_HIGHEDGE){
...
void dhd_deinit_gpio_wlan_hostwake(void)
{
	if (gpio_wl_host_wake >= 0) {
