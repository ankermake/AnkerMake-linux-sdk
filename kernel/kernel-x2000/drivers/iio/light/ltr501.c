...
//ltr559 add
#define LTR559_REG_PART_ID			0x86
...
static int ltr559_parse_dt(struct device *dev, struct ltr501_data *ltr)
{
	...
	ltr->irq_gpio = rc;
	dev_info(dev, "enable read irq gpio:%d\n",rc);

	return 0;
}
static int ltr559_check_device(struct ltr501_data *ltr)
{
	unsigned int part_id;
	int rc;

	rc = regmap_read(ltr->regmap, LTR559_REG_PART_ID, &part_id);
	if (rc) {
		dev_err(&ltr->client->dev, "read reg %d failed,read value = 0x%02x.(%d)\n",
				LTR559_REG_PART_ID,part_id, rc);
		return rc;
	}
	printk("---->part_id=0x%02x\n\n",part_id);
			...
			const struct i2c_device_id *id)
{
	...
	dev_dbg(&client->dev, "probling ltr559...\n");
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dev_err(&client->dev, "ltr559 i2c check failed.\n");
		return -ENODEV;
	}
		dev_info(&client->dev, "ltr559 i2c check ok~~.\n");
	...
	ret = regmap_read(data->regmap, LTR559_REG_PART_ID, &partid);
//	ret = ltr559_check_device(data);
	if (ret) {
	...
	if (ret < 0)
		return ret;
	/* xtang add : configure interrupt */
	if (gpio_is_valid(data->irq_gpio)) {
		ret = gpio_request(data->irq_gpio, "ltr559_interrupt");
		if (ret) {
			dev_err(&client->dev,
		...
		dev_info(&client->dev,"========enable to request interrupt gpio %d\n",data->irq_gpio);
...
static struct i2c_driver ltr501_driver = {
	.driver = {
		...
		.pm	= &ltr501_pm_ops,
		.of_match_table = of_match_ptr(ltr559_match_table),
