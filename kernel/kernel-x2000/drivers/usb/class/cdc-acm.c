		     ...
		     const struct usb_device_id *id)
{
	...
	if(IS_ENABLED(CONFIG_USB_DWC2_DMA_DESCRIPTOR_MODE))
		num_rx_buf = 1;
...
static int __init acm_init(void)
{
	...
	/*Cancel the line feed character NL to CR, and cancel the echo.*/
	acm_tty_driver->init_termios.c_iflag &= ~(ICRNL);
	acm_tty_driver->init_termios.c_lflag &= ~(ECHO | ECHONL);
