...
static int __uvcg_bm_controls(const char *page, size_t page_len, u8 *data, size_t data_len)
{
	char buf[1 + sizeof(u8) * 8 + 1 + 1];
	const char *pg = page;
	int i, ret;

	while (data_len && (pg - page < page_len)) {
		i = 0;
		while (i < sizeof(buf) && (pg - page < page_len) &&
				*pg != '\0' && *pg != '\n')
			buf[i++] = *pg++;
		if (i == sizeof(buf))
			return -EINVAL;
		while ((pg - page < page_len) && (*pg == '\0' || *pg == '\n'))
			++pg;
		buf[i] = '\0';
...
static ssize_t
uvcg_default_processing_bm_controls_store(struct config_item *item,
				const char *page, size_t len)
{
	struct uvcg_default_processing *dp = to_uvcg_default_processing(item);
	struct f_uvc_opts *opts;
	...
	result = __uvcg_bm_controls(page, len, pd->bmControls, sizeof(pd->bmControls));
	if (result)
		goto end;
...
static ssize_t
uvcg_default_camera_bm_controls_store(struct config_item *item,
			   const char *page, size_t len)
{
	struct uvcg_default_camera *dc = to_uvcg_default_camera(item);
	struct f_uvc_opts *opts;
	...
	result = __uvcg_bm_controls(page, len, cd->bmControls, sizeof(cd->bmControls));
	if (result)
		goto end;
