...
static struct notifier_block android_nb;
struct usb_phy			*android_usb_phy;
static int index;
static enum usb_phy_events android_usb_event;
...
static void android_work(struct work_struct *data)
{
	...
	char *event_none[2] = { "USB_EVENT=NONE", NULL };
	char *event_vbus[2] = { "USB_EVENT=VBUS", NULL };
	char *event_id[2] = { "USB_EVENT=ID", NULL };
	char *event_charge[2] = { "USB_EVENT=CHARGE", NULL };
	char *event_headset[2] = { "USB_EVENT=HEADSET", NULL };
	...
	if(android_usb_event == USB_EVENT_NONE)
	{
		kobject_uevent_env(&android_device->kobj,
						   KOBJ_CHANGE, event_none);
		pr_info("%s: sent uevent %s\n", __func__, event_none[0]);
		uevent_sent = true;
	}

	if(android_usb_event == USB_EVENT_VBUS)
	{
		kobject_uevent_env(&android_device->kobj,
						   KOBJ_CHANGE, event_vbus);
		pr_info("%s: sent uevent %s\n", __func__, event_vbus[0]);
		uevent_sent = true;
	}

	if(android_usb_event == USB_EVENT_ID)
	{
		kobject_uevent_env(&android_device->kobj,
						   KOBJ_CHANGE, event_id);
		pr_info("%s: sent uevent %s\n", __func__, event_id[0]);
		uevent_sent = true;
	}

	if(android_usb_event == USB_EVENT_CHARGER)
	{
		kobject_uevent_env(&android_device->kobj,
						   KOBJ_CHANGE, event_charge);
		pr_info("%s: sent uevent %s\n", __func__, event_charge[0]);
		uevent_sent = true;
	}

	if(android_usb_event == USB_EVENT_HEADSET)
	{
		kobject_uevent_env(&android_device->kobj,
						   KOBJ_CHANGE, event_headset);
		pr_info("%s: sent uevent %s\n", __func__, event_headset[0]);
		uevent_sent = true;
	}
...
static ssize_t event_show(struct device *pdev, struct device_attribute *attr,
						  char *buf)
{
	char *state = "none";

	switch(android_usb_event)
	{
	case USB_EVENT_VBUS:
		state = "vbus";
		break;
...
static DEVICE_ATTR(event, S_IRUGO, event_show, NULL);
...
static int android_device_set_drvdata(struct gadget_info *gi)
{
...
static int android_usb_event_nb(struct notifier_block *nb,
							  unsigned long event, void *power)
{

	struct gadget_info *gi = dev_get_drvdata(android_device);
	android_usb_event = event;
	if(gi)
		schedule_work(&gi->work);
	return NOTIFY_DONE;
}

static int android_device_create(void)
{
	...
	android_usb_phy = devm_usb_get_phy(android_device, USB_PHY_TYPE_USB2);
	if (IS_ERR(android_usb_phy)) {
		dev_err(android_device, "Failed to get USB transceiver\n");
	} else {
		int ret;
		android_nb.notifier_call = android_usb_event_nb;
		ret = usb_register_notifier(android_usb_phy, &android_nb);
		if (ret) {
			dev_err(android_device, "Failed to register USB notifier\n");
			/*
			 * If usb_register_notifier() fails, set notifier_call
			 * to NULL, to avoid calling usb_unregister_notifier().
			 */
			android_nb.notifier_call = NULL;
		}
	}
...
static void android_device_destroy(void)
{
	...
	usb_unregister_notifier(android_usb_phy, &android_nb);
	device_destroy(android_device->class, android_device->devt);
}
#else
static inline int android_device_set_drvdata(struct gadget_info *gi)
{
	return 0;
}
#endif
