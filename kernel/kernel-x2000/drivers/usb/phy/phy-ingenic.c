...
static int ingenic_usb_phy_probe(struct platform_device *pdev)
{
	...
	usb_phy->cpm_base = devm_ioremap_resource(dev, res);
	...
	if (res) {
		usb_phy->phy_base = devm_ioremap_resource(dev, res);
	...
	if (usb_phy->vbus_gpiod || usb_phy->id_gpiod) {
		INIT_DELAYED_WORK(&usb_phy->work, gpio_vbus_work);
	}
	if (usb_phy->id_gpiod) {
		usb_phy->id_irq = gpiod_to_irq(usb_phy->id_gpiod);

		err = devm_request_irq(dev, usb_phy->id_irq, gpio_vbus_irq, VBUS_IRQ_FLAGS,
					"id_detect", pdev);
		if (err) {
...
static int ingenic_usb_phy_remove(struct platform_device *pdev)
{
	...
	if (usb_phy->vbus_gpiod)
		cancel_delayed_work_sync(&usb_phy->work);
...
static int ingenic_usb_phy_pm_suspend(struct device *dev)
{
	...
	if (usb_phy->vbus_gpiod && device_may_wakeup(dev))
