...
irq_retry:
	...
	if (gintsts & GINTSTS_INCOMPL_SOIN) {
		...
		for (idx = 1; idx < hsotg->num_of_eps; idx++) {
			...
			/* If using high bandwith fifo, disable ep.
			 *
			 * restart req in handle  ep disable if has req.
			 * */
			epctl = dwc2_readl(hsotg->regs + epctl_reg);
			if(hsotg->dedicated_fifos) {
				epctl |= DXEPCTL_SNAK;
				epctl |= DXEPCTL_EPDIS;

				dwc2_writel(epctl, hsotg->regs + epctl_reg);
			}
		}
...
{
	struct dwc2_hsotg *hsotg = to_hsotg(gadget);
	int ret = hsotg->recv_intr_num;
	hsotg->recv_intr_num = 0;
	return ret;
}
