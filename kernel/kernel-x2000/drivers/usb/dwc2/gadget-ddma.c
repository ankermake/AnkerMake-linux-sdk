			      ...
			      gfp_t gfp_flags)
{
	...
	if (hs->lx_state == DWC2_L2) {
		dev_err(hs->dev, "%s: don't submit request while suspended\n",
				__func__);
		return -EAGAIN;
	}
				      ...
				      struct usb_ctrlrequest *ctrl)
{
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_STANDARD) {
		...
		case USB_REQ_SET_ADDRESS:
			...
			ret = dwc2_hsotg_send_reply(hsotg, ep0, NULL, 0);
			//kobject_uevent(&hsotg->dev->kobj, KOBJ_ONLINE);
			return;
	...
	if (ints & DXEPINT_STSPHSERCVD) {
		...
		if (hsotg->ep0_state == DWC2_EP0_DATA_OUT) {
			...
			if (using_desc_dma(hsotg)) {
				 ...
				 * transfer.
				 */
					__orr32(hsotg->regs + DIEPCTL(0), DXEPCTL_CNAK);
			}
		}
...
irq_retry:
	...
	dev_dbg(hsotg->dev, "%s: %08x %08x (%08x) USBDET:%08x retry %d\n",
			__func__, gintsts, gintsts & gintmsk, gintmsk,
			*(volatile unsigned int *)0xb0000040,retry_count);

	gintsts &= gintmsk;
	/* update frame_number, should be less than 1ms from this point to ISOC process.*/
...
static int dwc2_hsotg_ep_sethalt(struct usb_ep *ep, int value, bool now)
{
	...
	if (hs_ep->dir_in) {
		...
		if (value) {
			if (!(dwc2_readl(hs->regs + GINTSTS) & GINTSTS_GOUTNAKEFF))
				__orr32(hs->regs + DCTL, DCTL_SGOUTNAK);
...
#ifdef CONFIG_USB_DWC2_FORCE_CONNECT
...
{
	struct dwc2_hsotg *hsotg = (struct dwc2_hsotg *)container_of(dev, struct dwc2_hsotg, gadget.dev);
...
int dwc2_gadget_init(struct dwc2_hsotg *hsotg, int irq)
{
	...
	hsotg->g_rx_fifo_sz = 1024;
	hsotg->g_np_g_tx_fifo_sz = 512;
	...
	dwc2_hsotg_dump(hsotg);
	device_create_file(&hsotg->gadget.dev, &dev_attr_connected);
