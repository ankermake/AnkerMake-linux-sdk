...
#define CW2015_DEFAULT_ALEAT_LEVEL   10   // 10% alert
...
static void cw_update_charge_status(struct cw_battery *cw_bat)
{
	...
	ret = power_supply_am_i_supplied(cw_bat->battery);
	if (ret < 0) {
...
static void cw_bat_work(struct work_struct *work)
{
	...
	if (ret) {
		...
		if(cw_bat->charger_attached && cw_bat->soc >= 100 && cw_bat->gpiod_charge_en) {
			gpiod_direction_output(cw_bat->gpiod_charge_en, 0);
		}
...
static irqreturn_t cw2015_irq(int irq, void *dev_id)
{
	struct cw_battery *cw_bat= i2c_get_clientdata(dev_id);
	u32 reg_val;
	int ret;
	mutex_lock(&cw_bat->io_lock);
	ret = regmap_read(cw_bat->regmap, CW2015_REG_CONFIG, &reg_val);
	...
	if(cw_bat->soc > (reg_val >> 3)) {
		ret = regmap_read(cw_bat->regmap, CW2015_REG_RRT_ALERT, &reg_val);
		if(ret < 0){
			dev_err(cw_bat->dev, "Error read ALRT\n");
			mutex_unlock(&cw_bat->io_lock);
			return ret;
		}
		reg_val &= 0x7f;
		ret = regmap_write(cw_bat->regmap, CW2015_REG_RRT_ALERT, reg_val);
		if (ret) {
...
static int cw2015_parse_properties(struct cw_battery *cw_bat)
{
	...
	prop = of_find_property(np,"cellwise,battery-profile",&length);
	length = length / 4;
	...
	if (length < 0) {
		...
		u32 bat_info[CW2015_SIZE_BATINFO];
		int i;
		cw_bat->bat_profile = devm_kzalloc(dev, length, GFP_KERNEL);
		if (!cw_bat->bat_profile)
			return -ENOMEM;

		ret = of_property_read_u32_array(np,"cellwise,battery-profile",bat_info,length);
		if (ret) {
			dev_err(cw_bat->dev, "read battery profile failed!\n");
			return ret;
		}
		for(i = 0;i < length;i++){
			u8 *d = cw_bat->bat_profile;
			d[i] = bat_info[i];
		}
	}
	ret = of_property_read_u32(np, "cellwise,monitor-interval-ms", &cw_bat->poll_interval_ms);
	if (ret) {
		dev_dbg(cw_bat->dev, "Using default poll interval\n");
		cw_bat->poll_interval_ms = CW2015_DEFAULT_POLL_INTERVAL_MS;
	}

	ret = of_property_read_u32(np, "cellwise,alert-level", &cw_bat->alert_level);
	if(ret) {
		dev_dbg(cw_bat->dev, "Using default poll interval\n");
		cw_bat->alert_level = CW2015_DEFAULT_ALEAT_LEVEL;
	}

	cw_bat->gpiod_charge_en = devm_gpiod_get_optional(cw_bat->dev,"cellwise,charge-en-gpio", GPIOD_ASIS);
	if (IS_ERR_OR_NULL(cw_bat->gpiod_charge_en))
		cw_bat->gpiod_charge_en = NULL;
	return 0;
}
...
static int cw_bat_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	cw_bat->battery = devm_power_supply_register(cw_bat->dev,
						    &cw2015_bat_desc,
						    &psy_cfg);
	if (IS_ERR(cw_bat->battery)) {
		dev_err(cw_bat->dev, "Failed to register power supply\n");
		return PTR_ERR(cw_bat->battery);
	}
	mutex_init(&cw_bat->io_lock);
	if (client->irq > 0) {
		int err = devm_request_threaded_irq(&client->dev, client->irq,
										NULL, cw2015_irq,
										IRQF_SHARED|IRQF_ONESHOT|IRQF_TRIGGER_FALLING,
										"cw2015", client);
		if (err) {
			dev_err(cw_bat->dev, "unable to request IRQ %d\n",
					client->irq);
			return err;
		}
	}
	cw_bat->battery_workqueue = create_singlethread_workqueue("battery_work");
