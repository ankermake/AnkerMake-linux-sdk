...
{
	if ((!gpio_is_valid(icodec_spk_power->spk_gpio)) || (!gpio_is_valid(icodec_spk_power->sgm8903_gpio)))
		return;

	if (enable) {
		gpio_direction_output(icodec_spk_power->spk_gpio, icodec_spk_power->spk_en_level);
		gpio_direction_output(icodec_spk_power->sgm8903_gpio, icodec_spk_power->spk_en_level);
	} else {
		gpio_direction_output(icodec_spk_power->spk_gpio, !icodec_spk_power->spk_en_level);
		gpio_direction_output(icodec_spk_power->sgm8903_gpio, !icodec_spk_power->spk_en_level);
	}
...
static int snd_x2500_probe(struct platform_device *pdev)
{
	...
	x2500->sgm8903_gpio = of_get_named_gpio_flags(card->dev->of_node, "ingenic,sgm8903en-gpio", 0, &flags);
	icodec_spk_power = x2500;
	if (gpio_is_valid(x2500->spk_gpio) && gpio_is_valid(x2500->sgm8903_gpio)) {
		...
		ret = devm_gpio_request_one(card->dev, x2500->sgm8903_gpio, init_flags, "Sgm8903_en");
		if (ret)
			printk("SGM8903 enable pin(%d) request failed\n",x2500->sgm8903_gpio);
		else
			printk("SGM8903 enable pin(%d) request ok\n", x2500->sgm8903_gpio);
