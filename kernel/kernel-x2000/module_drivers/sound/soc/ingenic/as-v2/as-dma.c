		 ...
		 dma_addr_t dma_addr, struct snd_pcm_hw_params *params)
{
	...
	if(prtd->substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if(dummy_time) {
			...
			/* fix dummy_bytes: maybe cause data mismatch when playback S24_3LE audio file */
			dummy_bytes = dummy_bytes - (dummy_bytes % (ts_size * 3)) + ts_size * 3;
...
static int ingenic_as_dma_dmic_and_amic_sync_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct ingenic_as_dma *as_dma = snd_soc_platform_get_drvdata(rtd->platform);
	int ret;
	...
	case SNDRV_PCM_TRIGGER_RESUME:	/*FIXME*/
		...
		if((substream->stream == SNDRV_PCM_STREAM_CAPTURE) &&
			(strcmp("DMA5", rtd->cpu_dai->name) == 0 || strcmp("DMA6", rtd->cpu_dai->name) == 0)) {
			snd_pcm_stream_unlock_irq(substream);
		}
		if(substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
			if(!(strcmp("DMA5", rtd->cpu_dai->name))) {
			...
			if(!(strcmp("DMA6", rtd->cpu_dai->name))) {
		...
		ret = ingenic_as_dma_start(as_dma, rtd->cpu_dai->id);
		if((substream->stream == SNDRV_PCM_STREAM_CAPTURE) &&
			(strcmp("DMA5", rtd->cpu_dai->name) == 0 || strcmp("DMA6", rtd->cpu_dai->name) == 0)) {
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		DMA_DEBUG_MSG("enter %s[dai%d substream = %s] stop\n", __func__,
				...
				(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) ?
				"playback" : "capture");
		ret = ingenic_as_dma_stop(as_dma, rtd->cpu_dai->id);
		return ret;
...
static int ingenic_as_dma_resume(struct platform_device *pdev)
{
	...
	regmap_write(as_dma->dma_regmap, DGER, DGER_DMA_EN);
	for (i = 0; i < as_dma->chan_cnts; i++) {
		regmap_write(as_dma->fifo_regmap, FAS(i), as_dma->fifo_depth[i]);
	}
