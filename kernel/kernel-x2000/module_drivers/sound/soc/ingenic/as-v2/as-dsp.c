...
static int ingenic_as_dsp_suspend(struct platform_device *pdev, pm_message_t state)
{

	return 0;
}

static int ingenic_as_dsp_resume(struct platform_device *pdev)
{
	struct ingenic_as_dsp *as_dsp = platform_get_drvdata(pdev);
...
//		.pm = &ingenic_as_dsp_pm_ops,
	...
	.suspend = ingenic_as_dsp_suspend,
	.resume = ingenic_as_dsp_resume,
};
