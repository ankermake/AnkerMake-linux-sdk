...
static void inno_poweron(struct snd_soc_codec *codec)
{
...
static int inno_dac_configure(struct snd_soc_codec *codec,
...
static void inno_dac_enable (struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
	unsigned int reg = 0x00;
...
static void inno_dac_disable (struct snd_soc_codec *codec)
{
	unsigned int reg = 0x00;
...
static void inno_poweroff(struct snd_soc_codec *codec)
{
	unsigned int reg = 0x00;
	int i ;
...
static int inno_adc_configure(struct snd_soc_codec *codec,
			struct snd_pcm_hw_params *params)
{
	int fmt_width = snd_pcm_format_width(params_format(params));
	unsigned int reg = 0x00;
...
static void inno_adc_enable (struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
	unsigned int reg = 0x00;
...
static void inno_adc_disable (struct snd_soc_codec *codec)
{
/***1*/
	unsigned int reg = 0x00;
			    ...
			    struct snd_soc_dai *dai)
{
	...
	*(volatile unsigned int *)(INNO_CODEC_BASE + 0x03*4) = reg;
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ret = inno_dac_configure(codec, params);
	} else {
		ret = inno_adc_configure(codec, params);
	}
		...
		struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
			...
			inno_dac_enable(codec);

			if(icodec_mute == 1){
				inno_dac_disable(codec);
			}
		} else {
			inno_adc_enable(codec);
		}
		break;
...
static int icodec_spk_power(struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol, int event)
{
	...
	if (!gpio_is_valid(icodec->spk_gpio))
		return 0;
...
static const struct snd_kcontrol_new icodec_controls[] = {
	...
	SOC_SINGLE_EXT_TLV("Capture Volume", 0, 0, 0x1f, 0 , icodec_controls_get, icodec_controls_put, alcl_gain_tlv),
	SOC_SINGLE_EXT_TLV("Speaker Playback Volume", 0, 0, 0x1f, 0 , icodec_controls_get, icodec_controls_put, hpoutl_gain_tlv),
