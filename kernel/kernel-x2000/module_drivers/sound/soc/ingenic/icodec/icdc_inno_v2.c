...
static unsigned int icodec_read(struct snd_soc_codec *codec , unsigned int reg)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
	int value = readl(icodec->io_base + reg);
	return value;
}

static int icodec_write(struct snd_soc_codec *codec , unsigned int reg , unsigned value)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static void icodec_poweron(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
	int i = 0;
	char value = 0;
...
static int icodec_dac_configure(struct snd_soc_codec *codec,
			struct snd_pcm_hw_params *params)
{
	...
	if(fmt_width == 32) {
		...
		dev_err(codec->dev, "icodec not support format width\n");
		return -EINVAL;
	}
...
static void icodec_dac_enable(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static void icodec_dac_disable(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static int icodec_adc_configure(struct snd_soc_codec *codec,
			struct snd_pcm_hw_params *params)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
	...
	if(channel == 2)
		...
		if(icodec->channel == 0 || icodec->channel == STEREO){
			icodec->channel = MONO_LEFT;
		}
/* #ifdef CONFIG_SND_ASOC_INGENIC_MONO_LEFT */
/* 		icodec->channel = MONO_LEFT; */
/* #elif CONFIG_SND_ASOC_INGENIC_MONO_RIGHT */
/* 		icodec->channel = MONO_RIGHT; */
/* #endif */
	} else {
		dev_err(codec->dev, "channel is not support !\n");
	}
	...
	if(samplerate > 96000 || samplerate < 8000) {
		dev_err(codec->dev, "icodec not support samplerate %d \n", samplerate);
		return -EINVAL;
	}
...
static int icodec_enable_left_record(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static int icodec_disable_left_record(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static int icodec_enable_right_record(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static int icodec_disable_right_record(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static int icodec_enable_stereo_record(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static int icodec_disable_stereo_record(struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static void icodec_adc_enable (struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static void icodec_adc_disable (struct snd_soc_codec *codec)
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
...
static void icodec_poweroff(struct snd_soc_codec *codec)
{
	unsigned int reg = 0x00;
	int i ;
			    ...
			    struct snd_soc_dai *dai)
{
	...
	*(volatile unsigned int *)(INNO_CODEC_BASE + 0x03*4) = reg;
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ret = icodec_dac_configure(codec, params);
	} else {
		ret = icodec_adc_configure(codec, params);
	}
		...
		struct snd_soc_dai *dai)
{
	...
	if (!icodec->powered || snd_soc_codec_is_active(codec))
		return;
...
#ifdef CONFIG_PM
static int icodec_suspend(struct snd_soc_codec *codec)
{
	return 0;
}

static int icodec_resume(struct snd_soc_codec *codec)
{
	return 0;
}
#endif
...
{
	struct icodec *icodec = snd_soc_codec_get_drvdata(codec);
	dev_info(codec->dev, "icodec probe enter\n");
...
{
	dev_info(codec->dev, "codec icodec remove enter\n");
	return 0;
}
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if(!strcmp(kcontrol->id.name, "Left/Right channel select")) {
		ucontrol->value.integer.value[0] = icodec->channel;
	}
...
//	SOC_SINGLE_EXT_TLV("Right ADC Digital Volume", TS_CODEC_CGAINLR_09, 0, 0xff, 0 , icodec_controls_get, icodec_controls_put, micr_dgain_tlv),
	...
	SOC_SINGLE_EXT("Left/Right channel select", 0, 0, 3, 0 , icodec_controls_get, icodec_controls_put),
#else
...
static struct snd_soc_codec_driver soc_codec_dev_icodec_codec = {
	.probe = 	icodec_probe,
...
static int icodec_platform_probe(struct platform_device *pdev)
{
	...
	icodec->micbias = 0;
	icodec->channel = STEREO;		// stereo default
