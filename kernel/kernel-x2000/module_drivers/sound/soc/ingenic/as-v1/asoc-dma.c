...
static inline void *pcm_alloc_coherent(int size)
{
...
int ingenic_pcm_invalid_data_cyclic(struct ingenic_pcm_runtime_data *prtd, unsigned char *dma_buf,
				unsigned int size, struct snd_pcm_hw_params *params)
{
...
static void ingenic_pcm_adapt_sequence(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	void *dma_buf;
	dma_addr_t dma_handle;
	struct snd_soc_pcm_runtime *rtd = snd_pcm_substream_chip(substream);
	struct device *aic = rtd->platform->dev;
	struct ingenic_pcm_runtime_data *prtd = substream_to_prtd(substream);
	struct snd_pcm *pcm = substream->pcm;
	int period_size = params_period_size(params);

	aic_if_send_invalid_data = SEND_INVALID_DATA;
	aic_if_send_invalid_data_everytime = SEND_INVALID_DATA_EVERYTIME;
	aic_send_invalid_data_time_ms = CONFIG_SND_X1600_AIC_SEND_INVALID_DATA_TIME_MS;

	if (!aic_if_send_invalid_data){
		printk("!aic_if_send_invalid_data\n");
		return;
	}
	aic_clk_ctrl(aic, true);
	dma_buf = pcm_alloc_coherent(period_size);
	...
	dma_free_coherent(NULL, period_size, (void *)CKSEG1ADDR(dma_buf), dma_handle);
		...
		struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		ingenic_pcm_adapt_sequence(pcm_substream, params);
	return 0;
}
