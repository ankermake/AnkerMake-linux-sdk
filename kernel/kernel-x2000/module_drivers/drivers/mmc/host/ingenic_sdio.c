...
struct wifi_data_atbm {
	int sdio_index;
	uint         wifi_reset;
	uint         wifi_reset_flags;
    uint         wifi_pwron;
    uint         wifi_pwron_flags;
};
#define RESET  0
#define NORMAL 1

struct wifi_data wifi_data;
struct wifi_data_atbm wifi_data_atbm;
...
int ingenic_sdio_wlan_init(struct device *dev, int index)
{
	...
	for_each_child_of_node(np, cnp) {
        ...
        if(of_device_is_compatible(cnp, "atbm,atbm_wlan")) {
			printk("----atbm,atbm_wlan\n");
			wifi_data_atbm.wifi_reset = of_get_named_gpio_flags(cnp, "ingenic,sdio-reset", 0, &flags);
			wifi_data_atbm.wifi_reset_flags = flags;
            wifi_data_atbm.wifi_pwron = of_get_named_gpio_flags(cnp, "ingenic,wifi_pwron", 0, &flags);
			wifi_data_atbm.wifi_pwron_flags = flags;

            gpio = wifi_data_atbm.wifi_reset;
            if (devm_gpio_request(dev, gpio, "wifi_reset")) {
                printk("ERROR: no wifi_reset pin available !!\n");
                return -EINVAL;
            } else {
                gpio_direction_output(gpio, !wifi_data_atbm.wifi_reset_flags);
            }
            wifi_data_atbm.wifi_reset = gpio;

            gpio= wifi_data_atbm.wifi_pwron;
            if (devm_gpio_request(dev, gpio, "wifi_pwron")) {
                printk("ERROR: no wifi_pwron pin available !!\n");
                return -EINVAL;
            } else {
                gpio_direction_output(gpio, !wifi_data_atbm.wifi_pwron_flags);
            }
            wifi_data_atbm.wifi_pwron = gpio;

            wifi_data_atbm.sdio_index = index;
            return 0;
        }
    }
...
int ingenic_sdio_wlan_atbm_power_onoff(int onoff, int flag)
{
	int err = 0;
	int reset = wifi_data_atbm.wifi_reset;
    int pwron = wifi_data_atbm.wifi_pwron;

	if(onoff) {
		switch(flag) {
		case RESET:
			ingenic_mmc_clk_ctrl(wifi_data_atbm.sdio_index, 1);
            gpio_set_value(pwron, 1);
			msleep(10);
			gpio_set_value(reset, 0);
			msleep(10);
			gpio_set_value(reset, 1);
		...
		case NORMAL:
			...
			gpio_set_value(reset, 1);
			msleep(10);
			ingenic_mmc_manual_detect(wifi_data_atbm.sdio_index, 1);
			break;
		}
	} else {
		printk("wlan power off:%d\n", flag);
...
EXPORT_SYMBOL(ingenic_sdio_wlan_atbm_power_onoff);
