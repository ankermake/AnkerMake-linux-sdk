...
static int dpu_queue_setup(struct vb2_queue *vq, const void *parg,
			   ...
			   unsigned int sizes[], void *alloc_ctxs[])
{
	...
	*nplanes = 1;
	sizes[0] = frame->size;
	alloc_ctxs[0] = ctx->dpu_dev->alloc_ctx;
...
void * hw_comp_v4l2_init(struct device *dev)
{
	...
	ret = video_register_device(vfd, VFL_TYPE_GRABBER, INGENIC_DPU_COMP_VIDEO_NR);
	if (ret) {
