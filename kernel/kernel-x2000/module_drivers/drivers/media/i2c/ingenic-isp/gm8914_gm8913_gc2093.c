...
int gc2093_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *param)
{
	struct gc2093_info *info = to_state(sd);
	if(info->win->fps){
		param->parm.capture.timeperframe.numerator = info->win->fps & 0xffff;
		param->parm.capture.timeperframe.denominator = info->win->fps >> 16;
		return 0;
	}
	return -EINVAL;
}
