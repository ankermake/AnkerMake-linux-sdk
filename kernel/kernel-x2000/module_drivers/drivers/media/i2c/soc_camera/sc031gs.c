		...
		struct v4l2_subdev_format *format)
{
	...
	mf->width = SC031GS_DEFAULT_WIDTH;//priv->win->width;
	mf->height = SC031GS_DEFAULT_HEIGHT;//priv->win->height;
...
static int sc031gs_set_params(struct i2c_client *client, u32 *width, u32 *height, u32 code)
{
	...
	/* select win */
	priv->win = sc031gs_select_win(width, height);
		...
		struct v4l2_subdev_format *format)
{
	 ...
	 */
	sc031gs_select_win(&mf->width, &mf->height);

	mf->field	= V4L2_FIELD_NONE;
	mf->colorspace = V4L2_COLORSPACE_JPEG;

	if (format->which == V4L2_SUBDEV_FORMAT_ACTIVE)
		return sc031gs_set_params(client, &mf->width,
					 &mf->height, mf->code);
	cfg->try_fmt = *mf;
	return 0;
}
...
{
	if (code->pad || code->index >= ARRAY_SIZE(sc031gs_codes))
		return -EINVAL;

	code->code = sc031gs_codes[code->index];
	return 0;
}
		       ...
		       struct v4l2_subdev_frame_size_enum *fse)
{
	...
	j = ARRAY_SIZE(sc031gs_codes);
	while(--j)
		if(fse->code == sc031gs_codes[j])
			break;

	for (i = 0; i < N_WIN_SIZES; i++) {
		if (index == ++num_valid) {
			fse->code = sc031gs_codes[j];
...
static int sc031gs_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
	a->c.left	= 0;
	a->c.top	= 0;
	a->c.width	= SC031GS_DEFAULT_WIDTH;
	a->c.height	= SC031GS_DEFAULT_HEIGHT;
	a->type		= V4L2_BUF_TYPE_VIDEO_CAPTURE;

	return 0;
}

static int sc031gs_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	a->bounds.left			= 0;
	a->bounds.top			= 0;
	a->bounds.width			= SC031GS_DEFAULT_WIDTH;
	a->bounds.height		= SC031GS_DEFAULT_HEIGHT;
	a->defrect			= a->bounds;
	a->type				= V4L2_BUF_TYPE_VIDEO_CAPTURE;
...
static struct v4l2_subdev_video_ops sc031gs_subdev_video_ops = {
	...
	.cropcap	= sc031gs_cropcap,
	.g_crop		= sc031gs_g_crop,
...
/* OF probe functions */
static int sc031gs_hw_power(struct device *dev, int on)
{
...
static int sc031gs_hw_reset(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct sc031gs_priv *priv = to_sc031gs(client);
...
no_supply:
	...
	priv->ssdd_dt.power = sc031gs_hw_power;
	priv->ssdd_dt.reset = sc031gs_hw_reset;
