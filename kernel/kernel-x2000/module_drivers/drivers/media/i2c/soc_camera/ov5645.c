...
struct ov5645_priv {
	...
	struct ov5645_gpio reset;
	struct ov5645_gpio pwdn;
	struct ov5645_gpio vcc_en;
		...
		struct v4l2_subdev_format *format)
{
	...
	mf->width = OV5645_DEFAULT_WIDTH;//priv->win->width;
	mf->height = OV5645_DEFAULT_HEIGHT;//priv->win->height;
		...
		struct v4l2_subdev_format *format)
{
	 ...
	 */
	ov5645_select_win(&mf->width, &mf->height);

	mf->field	= V4L2_FIELD_NONE;
	mf->colorspace = V4L2_COLORSPACE_JPEG;

	if (format->which == V4L2_SUBDEV_FORMAT_ACTIVE)
		return ov5645_set_params(client, &mf->width,
					 &mf->height, mf->code);
	cfg->try_fmt = *mf;
	return 0;
}
...
static int ov5645_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
	a->c.left	= 0;
	a->c.top	= 0;
	a->c.width	= OV5645_DEFAULT_WIDTH;
	a->c.height	= OV5645_DEFAULT_HEIGHT;
...
static int ov5645_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	a->bounds.left			= 0;
	a->bounds.top			= 0;
	a->bounds.width			= OV5645_DEFAULT_WIDTH;
	a->bounds.height		= OV5645_DEFAULT_HEIGHT;
	a->defrect			= a->bounds;
	a->type				= V4L2_BUF_TYPE_VIDEO_CAPTURE;
...
static int ov5645_g_mbus_config(struct v4l2_subdev *sd,
...
static struct v4l2_subdev_video_ops ov5645_subdev_video_ops = {
	...
	.g_mbus_config	= ov5645_g_mbus_config,
};
...
/* OF probe functions */
static int ov5645_hw_power(struct device *dev, int on)
{
	...
	if(priv->vcc_en.pin) {
		gpio_direction_output(priv->vcc_en.pin, priv->vcc_en.active_level);
	}

	if (priv->pwdn.pin) {
		gpio_direction_output(priv->pwdn.pin, priv->pwdn.active_level);
	}
...
static int ov5645_hw_reset(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct ov5645_priv *priv = to_ov5645(client);
		...
		struct ov5645_priv *priv)
{
	...
	if(gpio_is_valid(gpio)) {
		priv->pwdn.pin = gpio;
		priv->pwdn.active_level = (flags == OF_GPIO_ACTIVE_LOW) ? 0 : 1;
	}

	/* Request the reset GPIO asserted */
	gpio = of_get_named_gpio_flags(client->dev.of_node, "ingenic,rst-gpio", 0, &flags);
	if(gpio_is_valid(gpio)) {
		priv->reset.pin = gpio;
		priv->reset.active_level = (flags == OF_GPIO_ACTIVE_LOW) ? 0 : 1;
	}

	/* Request the power enable GPIO asserted */
	gpio = of_get_named_gpio_flags(client->dev.of_node, "ingenic,pwen-gpio", 0, &flags);
	if(gpio_is_valid(gpio)) {
		priv->vcc_en.pin = gpio;
		priv->vcc_en.active_level = (flags == OF_GPIO_ACTIVE_LOW) ? 0 : 1;
	}

	/* Initialize the soc_camera_subdev_desc */
	priv->ssdd_dt.power = ov5645_hw_power;
	priv->ssdd_dt.reset = ov5645_hw_reset;
