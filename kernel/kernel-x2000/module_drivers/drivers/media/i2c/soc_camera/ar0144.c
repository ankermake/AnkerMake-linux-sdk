 ...
 * General functions
 */
static struct ar0144_priv *to_ar0144(const struct i2c_client *client)
{
...
{
	/*
	struct ar0144_priv *priv = to_ar0144(client);
...
static const struct ar0144_win_size *ar0144_select_win(u32 *width, u32 *height)
{
	...
	*width = ar0144_supported_win_sizes[default_size].width;
	*height = ar0144_supported_win_sizes[default_size].height;
...
static int ar0144_set_params(struct i2c_client *client, u32 *width, u32 *height, u32 code)
{
	...
	/* select win */
	priv->win = ar0144_select_win(width, height);
...
//	ret = ar0144_write_array(client, ar0144_init_regs);
//	if (ret < 0)
//		goto err;
		...
		struct v4l2_subdev_format *format)
{
	 ...
	 */
	ar0144_select_win(&mf->width, &mf->height);

	mf->field	= V4L2_FIELD_NONE;
	mf->colorspace = V4L2_COLORSPACE_JPEG;

	if (format->which == V4L2_SUBDEV_FORMAT_ACTIVE)
		return ar0144_set_params(client, &mf->width,
					 &mf->height, mf->code);
	cfg->try_fmt = *mf;
	return 0;
}
...
{
	if (code->pad || code->index >= ARRAY_SIZE(ar0144_codes))
		return -EINVAL;

	code->code = ar0144_codes[code->index];
	return 0;
}
		       ...
		       struct v4l2_subdev_frame_size_enum *fse)
{
	...
	j = ARRAY_SIZE(ar0144_codes);
	while(--j)
		if(fse->code == ar0144_codes[j])
			break;

	for (i = 0; i < N_WIN_SIZES; i++) {
		if (index == ++num_valid) {
			fse->code = ar0144_codes[j];
...
static int ar0144_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
	a->c.left	= 0;
	a->c.top	= 0;
	a->c.width	= ar0144_DEFAULT_WIDTH;
	a->c.height	= ar0144_DEFAULT_HEIGHT;
	a->type		= V4L2_BUF_TYPE_VIDEO_CAPTURE;

	return 0;
}

static int ar0144_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	a->bounds.left			= 0;
	a->bounds.top			= 0;
	a->bounds.width			= ar0144_DEFAULT_WIDTH;
	a->bounds.height		= ar0144_DEFAULT_HEIGHT;
	a->defrect			= a->bounds;
	a->type				= V4L2_BUF_TYPE_VIDEO_CAPTURE;
...
static int ar0144_video_probe(struct i2c_client *client)
{
	struct ar0144_priv *priv = to_ar0144(client);
	int ret;

	ret = ar0144_s_power(&priv->subdev, 1);
	if (ret < 0)
		return ret;
	/*
...
static int ar0144_g_mbus_config(struct v4l2_subdev *sd,
...
static struct v4l2_subdev_video_ops ar0144_subdev_video_ops = {
	...
	.cropcap	= ar0144_cropcap,
	.g_crop		= ar0144_g_crop,
	.g_mbus_config	= ar0144_g_mbus_config,
};
...
/* OF probe functions */
static int ar0144_hw_power(struct device *dev, int on)
{
...
static int ar0144_hw_reset(struct device *dev)
{
	/*
	struct i2c_client *client = to_i2c_client(dev);
	struct ar0144_priv *priv = to_ar0144(client);
...
no_supply:
	...
	priv->ssdd_dt.power = ar0144_hw_power;
	priv->ssdd_dt.reset = ar0144_hw_reset;
