...
#include <dt-bindings/media/ingenic-cim.h> 
 ...
 * General functions
 */
static struct ov772x_priv *i2c_to_ov772x(const struct i2c_client *client)
{
...
static int ov772x_hw_reset(struct device *dev)
{
	...
	struct ov772x_priv *priv = i2c_to_ov772x(client);

	dev_info(&client->dev, "%s: the camera\n", __func__);
...
static int ov772x_hw_power(struct device *dev, int on)
{
	...
	dev_info(&client->dev, "%s: %s the camera\n",
			...
			const struct i2c_device_id *did)
{
	...
	ret = v4l2_clk_set_rate(priv->clk, 24000000);
...
/* 		v4l2_ctrl_handler_free(&priv->hdl); */
	...
	dev_info(&adapter->dev, "OV772X Probed OK\n");

	printk("@@@@@@@@@@@@@%s@@@@@@@@@@@\n", __func__);
	return 0;

	dev_info(&adapter->dev, "OV772X Probed Failed\n");
eclkget:
err_videoprobe:
	v4l2_ctrl_handler_free(&priv->hdl);
	v4l2_clk_put(priv->clk);

	printk("ov7725 probe err!!!!!!!!\n");
	return ret;
}
