...
static int devmem_alloc(devmem_ctx_t *ctx, devmem_info_t *di)
{
	...
	if(di->coherent) {
		...
		di->kva = (unsigned long)dma_alloc_noncoherent(ctx->dev, di->length, &dma_addr, GFP_KERNEL);
	}
...
static void devmem_free(devmem_ctx_t *ctx, devmem_info_t *di)
{
	if(di->coherent) {
		...
		dma_free_noncoherent(ctx->dev, di->length, (void *)di->kva, (dma_addr_t)di->paddr);
	}
}
