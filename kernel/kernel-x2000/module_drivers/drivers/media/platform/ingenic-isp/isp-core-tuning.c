...
static inline int flicker_value_v4l2_to_tuning(struct isp_device *isp, int val)
{
	int ret = 0;
	switch (val) {
...
static inline int apical_isp_flicker_s_control(tisp_core_t *core)
{
	...
	ret = flicker_value_v4l2_to_tuning(isp, isp->ctrls.flicker);
	if(ret < 0)
		return ret;
	ret = tisp_s_antiflick(&core->core_tuning, ret);
	if(ret != 0)
