...
static inline void csi_core_write_io(unsigned int iobase, unsigned int address, unsigned int data, unsigned char shift, unsigned char width)
{
        unsigned int mask = (1 << width) - 1;
...
{
	struct csi_device *csi = v4l2_get_subdevdata(sd);
	struct v4l2_of_bus_mipi_csi2 *mipi_csi2 = ispcam_get_bus_mipi_csi2(csi->ispcam);
                              ...
                              void *master_data)
{
	...
	ret = media_entity_init(&sd->entity, CSI_NUM_PADS, csi->pads, 0);
