...
static int _rot_vidioc_expbuf(struct file *file, void* prv,
		  struct v4l2_exportbuffer *eb)
{
	struct vb2_queue *vq;
	struct ingenic_rot_ctx *ctx = fh2ctx(prv);
	if(ctx->m2m_ctx){
...
static void device_run(void *prv)
{
	...
	rot_set_src_desc(dev, ingenic_vb2_dma_contig_plane_dma_addr(src, 0));

	rot_set_dst_cfg(dev, &ctx->out);
	rot_set_dst_desc(dev, ingenic_vb2_dma_contig_plane_dma_addr(dst, 0));
	...
	if (!dev->irq_is_request) {
		ret = devm_request_irq(dev->dev, dev->irq, rot_irq_handler,
						IRQF_SHARED, dev->name, dev);
		BUG_ON(ret);
		dev->irq_is_request = 1;
	}
	rot_start(dev);

	spin_unlock_irqrestore(&dev->ctrl_lock, flags);
	ret = wait_for_completion_interruptible_timeout(&ctx->ctx_done, msecs_to_jiffies(3000));
	if(ret < 0){
...
static const struct v4l2_ioctl_ops rot_ioctl_ops = {
	...
	.vidioc_expbuf          = _rot_vidioc_expbuf,
...
static int ingenic_rot_probe(struct platform_device *pdev)
{
	...
	*vfd = rot_videodev;
	...
	ret = video_register_device(vfd, VFL_TYPE_GRABBER, INGENIC_ROTATE_VIDEO_NR);
	if (ret) {
