...
static int mscaler_subdev_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if(val == 0)
		mscaler->iobase = (void __iomem *)0xb3316000;
	else if(val == 1)
		mscaler->iobase = (void __iomem *)0xb3317000;
	return 0;
}
...
static int mscaler_get_selection(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_selection *sel)
{
	struct mscaler_device *mscaler = v4l2_get_subdevdata(sd);
	struct isp_video_device *ispvideo = v4l2_get_subdev_hostdata(sd);
	...
	output_fmt = &mscaler->formats[ch_to_pad[ch]];
...
static int mscaler_set_selection(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_selection *sel)
{
	struct mscaler_device *mscaler = v4l2_get_subdevdata(sd);
	struct isp_video_device *ispvideo = v4l2_get_subdev_hostdata(sd);
	int ch = 0;
...
//	memset(&mscaler->attr[ch], 0, sizeof(tisp_channel_attr_t));
	mscaler->state[ch] = 0;
	spin_unlock_irqrestore(&mscaler->lock, flags);
