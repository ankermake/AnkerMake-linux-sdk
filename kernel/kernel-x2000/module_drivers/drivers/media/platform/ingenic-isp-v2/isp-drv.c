...
void isp_prestart_work(struct work_struct *work){
	struct ispcam_device *ispcam = container_of(work, struct ispcam_device, prestart_work);
	ispcam->prestart_thread = kthread_run(isp_prestart, ispcam, "isp_prestart");
	if(IS_ERR_OR_NULL(ispcam->prestart_thread)){
		dev_info(ispcam->dev, "%s[%d] kthread_run was failed!\n",__func__,__LINE__);
	}
}

#ifdef CONFIG_X2500_RISCV
extern int register_riscv_notifier(struct notifier_block *nb);
int ispdrv_riscv_notifier(struct notifier_block *nb, unsigned long action, void *data)
{
	struct ispcam_device *ispcam = container_of(nb, struct ispcam_device, nb);
	struct message *message = (unsigned int)action | 0x80000000;
	unsigned int *data_paddr;
	unsigned int *vaddr;
	struct isp_prestart_message *p_message;

	while(message->type != MESSAGE_END){
		if(message->type == MESSAGE_ISP_PRESTART){
			data_paddr = message->data;
			vaddr = (unsigned int)data_paddr | 0x80000000;
			p_message = vaddr;
			if(p_message->dev_nr == ispcam->dev_nr)
			{
				ispcam->p_info = kzalloc(sizeof(struct isp_prestart_info), GFP_KERNEL);
				if(IS_ERR_OR_NULL(ispcam->p_info)) {
					dev_err(ispcam->dev, "Failed to alloc p_info\n");
					return -ENOMEM;
				}
				ispcam->p_info->p_message = p_message;
				schedule_work(&ispcam->prestart_work);
				break;
			}
		}
		message++;
...
#ifdef CONFIG_X2500_RISCV
	ispcam->nb.notifier_call = ispdrv_riscv_notifier;
	register_riscv_notifier(&ispcam->nb);
#endif
	INIT_WORK(&ispcam->prestart_work, isp_prestart_work);
