...
struct isp_prestart_info {
	atomic_t usr_streaming;
	atomic_t driver_streaming;
	int buf_cnt;
	int frame_cnt;
	wait_queue_head_t wq;
...
struct ispcam_device {
	...
	struct task_struct *prestart_thread;
	struct work_struct prestart_work;
	struct isp_prestart_info *p_info;
};
...
extern int isp_prestart(void *data);
