...
static int vic_subdev_core_init(struct v4l2_subdev *sd, u32 ispcam_enabled)
{
	...
	if((ispcam_enabled == 0 && vic->iobase == (unsigned int *)0xb3380000) || (ispcam_enabled == 1 && vic->iobase == (unsigned int *)0xb3390000))
		mux_out = 0;
	else
		       ...
		       struct v4l2_subdev_format *format)
{
	...
	vic->sensor_info = (struct sensor_info *)*(unsigned int *)remote_subdev_fmt.format.reserved;
	...
	if(enable) {
		...
		if(ispvideo->bypass == 1) {
			...
			case V4L2_PIX_FMT_NV12 :
				regval |= 6<<0; /* mode 6: NV12 */
				y_stride = width;
				uv_stride = width;
				break;
			case V4L2_PIX_FMT_NV21 :
				regval |= 7<<0; /* mode 7: NV12 */
				y_stride = width;
				uv_stride = width;
				break;
		...
		if(vic->iobase == (unsigned int *)0xb3390000 && *(volatile unsigned int *)0xb33801c0 == 0)
...
int bypass_video_g_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	ret = v4l2_subdev_call(sensor->isd->sd, core, g_ctrl, a);
	if(ret < 0) {
...
int bypass_video_s_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	ret = v4l2_subdev_call(sensor->isd->sd, core, s_ctrl, a);
	if(ret < 0) {
...
static irqreturn_t vic_irq_handler(int irq, void *data)
{
	...
	struct ispcam_device *ispcam = vic ->ispcam;
...
//	printk("VIC_INT_STA2=%x\n", status2);
	...
	if((status & (1 << 0)) && ispcam->p_info) {
		ispcam->p_info->frame_cnt++;
	}
