...
#if 0
int isp_prestart_load_params(tisp_init_param_t *iparam, struct isp_prestart_message *p_message)
{
	unsigned int ret = 0;
	void *paddr = 0x80000000 | (p_message->b_addr);

	iparam->tuned_params = paddr;
	if(iparam->tuned_params == NULL){
		printk("%s[%d]: Failed to get prestart bin file!\n",__func__,__LINE__);
		return -1;
	}
	iparam->tuned_params_size = 2048000;
	return 0;
}
...
#define PRESTART "prestart"
	if(ispcam->p_info){
		memcpy(isp->bpath.path, PRESTART, sizeof(PRESTART));
		memcpy(isp->bpath.path + sizeof(PRESTART), &ispcam->p_info->p_message->b_addr, sizeof(unsigned int));
	}
...
int isp_set_wdr(struct isp_device *isp)
{
	...
	wdr_size = tsize;
	wdr_vaddr = dma_alloc_noncoherent(isp->dev, tsize, &wdr_paddr, GFP_KERNEL);
	if(!wdr_vaddr)
		return -ENOMEM;
...
int isp_set_mdns(struct isp_device *isp)
{
	...
	isp->buf_info[MDNS_BUF].vaddr = dma_alloc_noncoherent(isp->dev, tsize, &isp->buf_info[MDNS_BUF].paddr, GFP_KERNEL);
	if(!isp->buf_info[MDNS_BUF].vaddr)
		return -ENOMEM;
...
int isp_set_dualsensor_buf(struct isp_device *isp)
{
	...
	if(!dualsensor_vaddr) {
		dualsensor_vaddr = dma_alloc_noncoherent(isp->dev, tsize, &dualsensor_paddr, GFP_KERNEL);
		if(!dualsensor_vaddr)
			return -ENOMEM;
	}
...
static int isp_subdev_streamon(struct v4l2_subdev *sd) {
	...
	if(isp->sensor_id == 0) {
		...
		g_isp_device[1]->process_thread = kthread_run(isp_fw_process1, isp, "isp_fw_process1");
...
#ifndef CONFIG_INGENIC_ISP_V2_SENSOR_NUM_ONE
	if(dualsensor_vaddr) {
		dma_free_noncoherent(g_isp_device[MAIN_ISP_INDEX]->dev, dualsensor_size, dualsensor_vaddr, dualsensor_paddr);
	}
#endif
set_dualsensor_buf_err:
	dualsensor_vaddr = 0;
	dualsensor_paddr = 0;
	if(isp->sensor_info->wdr_en && wdr_vaddr){
		dma_free_noncoherent(isp->dev, wdr_size, wdr_vaddr, wdr_paddr);
	}
set_wdr_err:
	wdr_vaddr = 0;
	wdr_paddr = 0;
	if(isp->buf_info[MDNS_BUF].vaddr)
		dma_free_noncoherent(isp->dev, isp->buf_info[MDNS_BUF].size, isp->buf_info[MDNS_BUF].vaddr, isp->buf_info[MDNS_BUF].paddr);
set_mdns_err:
...
static int isp_subdev_streamoff(struct v4l2_subdev *sd) {
	...
	if(isp->sensor_id == 0) {
		...
		spin_unlock_irqrestore(&isp->lock, flags);
		kthread_stop(g_isp_device[1]->process_thread);
		tisp_deinit(1);
	}
