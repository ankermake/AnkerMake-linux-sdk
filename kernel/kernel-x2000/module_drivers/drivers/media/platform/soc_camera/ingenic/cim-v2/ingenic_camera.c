...
#include <uapi/linux/ingenic-media-bus-format.h>
...
static int ingenic_videobuf_setup(struct vb2_queue *vq, const void *parg,
	...
	else if(pcdev->lumi_en){
		size = icd->sizeimage + CIM_LUMI_SIZE;
	} else
		size = icd->sizeimage;
...
static int ingenic_init_desc(struct vb2_buffer *vb2) {
	...
	if(pcdev->cim_priv->version_num == 2 && pcdev->output_y == 1)
	...
	if(pcdev->cim_priv->version_num == 2) {
		if(!pcdev->lumi_en) {
			...
			desc_v2[index].LUMI_WRBK_ADDR
				= desc_v2[index].WRBK_ADDR + icd->sizeimage;
		}

		if(!pcdev->interlace) {
			desc_v2[index].WRBK_STRD = icd->user_width;
			desc_v2[index].SF_WRBK_ADDR = 0;
		} else {
			desc_v2[index].WRBK_STRD = icd->user_width * 2;
			desc_v2[index].SF_WRBK_ADDR =
				desc_v2[index].WRBK_ADDR + icd->user_width * 2;
		}
	} else {
		if(!pcdev->hist_en) {
			...
			desc_v1[index].DES_HIST_CFG_t.GAIN_MUL = pcdev->hist_gain_mul;
			desc_v1[index].DES_HIST_CFG_t.GAIN_ADD = pcdev->hist_gain_add;
			desc_v1[index].HIST_WRBK_ADDR
				= desc_v1[index].WRBK_ADDR + icd->sizeimage;
		}

		if(!pcdev->interlace) {
			desc_v1[index].WRBK_STRD = icd->user_width;
			desc_v1[index].SF_WRBK_ADDR = 0;
		} else {
			desc_v1[index].WRBK_STRD = icd->user_width * 2;
			desc_v1[index].SF_WRBK_ADDR =
				desc_v1[index].WRBK_ADDR + icd->user_width * 2;
		}
	}
...
static int ingenic_camera_init_videobuf2(struct vb2_queue *q, struct soc_camera_device *icd) {
	...
	q->mem_ops = &ingenic_vb2_dma_contig_memops;
...
static int ingenic_camera_set_host_crop(struct soc_camera_device *icd, const struct v4l2_crop *a)
{
	...
	/*Close scale function*/
	cim_writel(pcdev, 0, RESIZE_CFG);
	cim_writel(pcdev, 0, RESIZE_COEF_X);
	cim_writel(pcdev, 0, RESIZE_COEF_Y);
...
static int ingenic_camera_set_fmt(struct soc_camera_device *icd, struct v4l2_format *f) {
	...
	if(xlate->host_fmt->fourcc == V4L2_PIX_FMT_GREY && pcdev->cim_priv->version_num == 2)
		pcdev->output_y = 1;
	else
		pcdev->output_y = 0;
	...
	if(unlikely(img_sz->rsz_way == IMG_RESIZE_CROP)) {
		...
		img_sz->rsz_way = IMG_RESIZE_SCALE;
		img_sz->scl_w	= pix->width;
		img_sz->scl_h	= pix->height;
	}
	pix->field       = mf->field;
	pix->colorspace  = mf->colorspace;
...
static int ingenic_camera_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case INGENIC_CID_SCALE_SHARP:
		img_sz->sharp = ctrl->val;
		tmp = cim_readl(pcdev,RESIZE_CFG);
		tmp &= ~CIM_SHARPL_MASK;
		tmp |= img_sz->sharp << CIM_SHARPL_LBIT;
	...
	case INGENIC_CID_HIST_GAIN_ADD:
		...
		desc_v1 = (struct ingenic_camera_desc_v1 *) pcdev->desc_vaddr;
		if(!pcdev->hist_en || !desc_v1) {
			dev_dbg(icd->parent, "setting hist_add are not satisfied!\n");
			return 0;
		}
		for(i = 0; i < pcdev->buf_cnt; i++)
			desc_v1[i].DES_HIST_CFG_t.GAIN_ADD = pcdev->hist_gain_add;
		break;
	case INGENIC_CID_HIST_GAIN_MUL:
		...
		for(i = 0; i < pcdev->buf_cnt; i++)
			desc_v1[i].DES_HIST_CFG_t.GAIN_MUL = pcdev->hist_gain_mul;
		break;
	default:
...
struct soc_camera_format_xlate *ingenic_camera_add_xlate(struct soc_camera_device *icd,
		const struct soc_mbus_pixelfmt *fmt, struct soc_camera_format_xlate *xlate, int *formats, __u32 code)
{
	int buswidth;
	if (!fmt) {
		dev_dbg(icd->parent,
				  ...
				  struct soc_camera_format_xlate *xlate)
{
	...
	if (pcdev->cim_priv->version_num == 2) {
		switch (code.code) {
			...
			case MEDIA_BUS_FMT_SBGGR12_1X12:
				...
				fmt = soc_mbus_get_fmtdesc(MEDIA_BUS_FMT_SBGGR8_1X8);
				xlate = ingenic_camera_add_xlate(icd, fmt, xlate, &formats, code.code);
				/*8/10/12 bit raw RGB -> 8 bit y*/
				fmt = soc_mbus_get_fmtdesc(MEDIA_BUS_FMT_Y8_1X8);
				xlate = ingenic_camera_add_xlate(icd, fmt, xlate, &formats, code.code);
				break;
			...
			case MEDIA_BUS_FMT_SGBRG12_1X12:
				/*8/10/12 bit raw RGB -> 8 bit raw RGB*/
				fmt = soc_mbus_get_fmtdesc(MEDIA_BUS_FMT_SGBRG8_1X8);
				xlate = ingenic_camera_add_xlate(icd, fmt, xlate, &formats, code.code);
				/*8/10/12 bit raw RGB -> 8 bit y*/
				fmt = soc_mbus_get_fmtdesc(MEDIA_BUS_FMT_Y8_1X8);
				xlate = ingenic_camera_add_xlate(icd, fmt, xlate, &formats, code.code);
			...
			case MEDIA_BUS_FMT_SGRBG12_1X12:
				/*8/10/12 bit raw RGB -> 8 bit raw RGB*/
				fmt = soc_mbus_get_fmtdesc(MEDIA_BUS_FMT_SGRBG8_1X8);
				xlate = ingenic_camera_add_xlate(icd, fmt, xlate, &formats, code.code);
				/*8/10/12 bit raw RGB -> 8 bit y*/
				fmt = soc_mbus_get_fmtdesc(MEDIA_BUS_FMT_Y8_1X8);
				xlate = ingenic_camera_add_xlate(icd, fmt, xlate, &formats, code.code);
			...
			case MEDIA_BUS_FMT_SRGGB12_1X12:
				/*8/10/12 bit raw RGB -> 8 bit raw RGB*/
				fmt = soc_mbus_get_fmtdesc(MEDIA_BUS_FMT_SRGGB8_1X8);
				xlate = ingenic_camera_add_xlate(icd, fmt, xlate, &formats, code.code);
				/*8/10/12 bit raw RGB -> 8 bit y*/
				fmt = soc_mbus_get_fmtdesc(MEDIA_BUS_FMT_Y8_1X8);
				xlate = ingenic_camera_add_xlate(icd, fmt, xlate, &formats, code.code);
	...
	if (!icd->host_priv) {
		...
		if (pcdev->cim_priv->version_num == 2) {
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &lumi_en, NULL);
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &output_y, NULL);
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &point1_x, NULL);
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &point1_y, NULL);
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &point2_x, NULL);
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &point2_y, NULL);
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &snapshot_en, NULL);
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &exp_pulse_w, NULL);
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &delay_t, NULL);
		} else {
			v4l2_ctrl_new_custom(&icd->ctrl_handler, &img_scale_sharp, NULL);
...
static int ingenic_camera_set_bus_param(struct soc_camera_device *icd) {
	...
	unsigned long common_flags;
	unsigned long csi2_flags;
	unsigned long resize_coef_x;
	unsigned long resize_coef_y;
...
//		}
		...
		ret = csi_phy_start(version, csi2_lanes);
		if(ret < 0) {
	...
	if(pcdev->dat_if != V4L2_MBUS_BT656) {
		...
		if(icd->current_fmt->code != MEDIA_BUS_FMT_YUYV8_2X8 &&
			icd->current_fmt->code != MEDIA_BUS_FMT_YVYU8_2X8 &&
			icd->current_fmt->code != MEDIA_BUS_FMT_UYVY8_2X8 &&
			icd->current_fmt->code != MEDIA_BUS_FMT_VYUY8_2X8
			) {
			dev_err(icd->parent, "Providing code %d not support\n",
					icd->current_fmt->code);
		}
	...
	if(img_sz->rsz_way == IMG_NO_RESIZE) {
		...
		/*Close scale function*/
		cim_writel(pcdev, 0, RESIZE_CFG);
	} else if(img_sz->rsz_way == IMG_RESIZE_SCALE) {
		tmp = (img_sz->src_w << CIM_CROP_WIDTH_LBIT) |
			(img_sz->src_h << CIM_CROP_HEIGHT_LBIT);
		cim_writel(pcdev, tmp, FRM_SIZE);
		cim_writel(pcdev, 0, CROP_SITE);

		resize_coef_x = (img_sz->src_w * 512) / img_sz->scl_w;
		resize_coef_y = (img_sz->src_h * 512) / img_sz->scl_h;
		cim_writel(pcdev, resize_coef_x, RESIZE_COEF_X);
		cim_writel(pcdev, resize_coef_y, RESIZE_COEF_Y);

		/*set scale width*/
		tmp = img_sz->sharp << CIM_SHARPL_LBIT;
		tmp |= img_sz->scl_w << CIM_TAR_WIDTH_LBIT;
		tmp |= img_sz->scl_h << CIM_TAR_HEIGHT_LBIT;
		tmp |= CIM_SCALE_EN;
		cim_writel(pcdev, tmp, RESIZE_CFG);
...
static irqreturn_t ingenic_camera_irq_handler(int irq, void *data) {
	...
	if(status & CIM_INT_FLAG_EOW) {
		...
		if (pcdev->cim_priv->version_num == 2) {
			...
			pcdev->desc_v2_head =
				(struct ingenic_camera_desc_v2 *)UNCAC_ADDR(phys_to_virt(regval));
		} else {
			desc_v1_p = pcdev->desc_v1_paddr;
			regval = (unsigned long)(pcdev->desc_v1_head->next);
			pcdev->desc_v1_head =
				(struct ingenic_camera_desc_v1 *)UNCAC_ADDR(phys_to_virt(regval));
		}
	...
	if(status & CIM_INT_FLAG_EOF) {
		...
		if (pcdev->cim_priv->version_num == 2) {
			pcdev->desc_v2_head =
				(struct ingenic_camera_desc_v2 *)UNCAC_ADDR(phys_to_virt(pcdev->desc_v2_head->next));
		} else
			pcdev->desc_v1_head =
				(struct ingenic_camera_desc_v1 *)UNCAC_ADDR(phys_to_virt(pcdev->desc_v1_head->next));
...
static int ingenic_camera_probe(struct platform_device *pdev) {
	...
	pcdev->alloc_ctx = ingenic_vb2_dma_contig_init_ctx(&pdev->dev);
	...
	pcdev->soc_host.video_nr	= INGENIC_CIM_VIDEO_NR;

	pcdev->dat_if = V4L2_MBUS_PARALLEL;
...
err_soc_camera_host_register:
	ingenic_vb2_dma_contig_cleanup_ctx(pcdev->alloc_ctx);
err_alloc_ctx:
