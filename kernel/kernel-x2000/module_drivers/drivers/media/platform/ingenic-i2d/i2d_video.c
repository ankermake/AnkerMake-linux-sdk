...
static int i2d_queue_setup(struct vb2_queue *vq, const void *parg,
			   ...
			   unsigned int sizes[], void *alloc_ctxs[])
{
	...
	*nplanes = 1;
	sizes[0] = frame->size;
	alloc_ctxs[0] = ctx->i2d_dev->alloc_ctx;
...
static void device_run(void *prv)
{
	...
	/* clk_prepare_enable(dev->clk); */
	i2d_safe_reset(dev);
	srcaddr = ingenic_vb2_dma_contig_plane_dma_addr(src,0);
	dstaddr = ingenic_vb2_dma_contig_plane_dma_addr(dst,0);
