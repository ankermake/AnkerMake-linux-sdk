		...
		char *buf)
{
	...
	unsigned char *func_map[] = {"F0", "F1", "F2", "F3", "Out0", "Out1", "In0", "In1"};
	/* Pretty dump.*/
	for (idx = 0; idx < pctl->num_chips; idx++) {
		...
		for(i = 0; i < 32; i++) {
			...
			func = func < 6 ? func: 6 + pin;
			if(func < 4) {
				len += sprintf(buf + len, "P%c%02d:%s,%s\n", 'A' + idx, i, func_map[func], pin ? "High":"Low");
			} else {
				len += sprintf(buf + len, "P%c%02d:%s\n", 'A' + idx, i, func_map[func]);
			}
		}
	}
...
unsigned long ingenic_pinctrl_lock(int port)
{
	...
	pctl = gpctl->gpio_chips[port].pctl;
...
{
	const struct ingenic_priv *priv = jzgc->pctl->priv;
	u32 pm_pull = 0, pm_high = 0, pm_low = 0, pm_nopull = 0, pm_pullup = 0, pm_pulldown = 0, pm_hiz = 0;
...
static const struct of_device_id ingenic_pinctrl_dt_match[] = {
	...
	{ .compatible = "ingenic,x2000-v12-pinctrl", .data = (void *)x2000_priv_data},
