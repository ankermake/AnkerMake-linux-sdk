...
static int ingenic_get_settings(struct net_device *netdev,
...
static int ingenic_set_settings(struct net_device *netdev,
			      struct ethtool_cmd *ecmd)
{
	struct ingenic_mac_local *lp = netdev_priv(netdev);
	struct phy_device *phydev = lp->phydev;
		...
		struct ethtool_pauseparam *pp)
{
	...
	if (pp->autoneg) {
		lp->phydev->advertising |= ADVERTISED_Pause;
		lp->phydev->advertising |= ADVERTISED_Asym_Pause;
	} else {
		lp->phydev->advertising &= ~ADVERTISED_Pause;
		lp->phydev->advertising &= ~ADVERTISED_Asym_Pause;
...
static const struct ethtool_ops ingenic_mac_ethtool_ops = {
	.get_settings		= ingenic_get_settings,
	.set_settings		= ingenic_set_settings,
