...
inline int adc_hal_enable_channel(unsigned int channel, int try_count)
{
    unsigned int val;
    int count = try_count;
...
unsigned int adc_hal_enable_bits_channels(unsigned int bits_channels, int try_count)
{
    ...
    do {
        adc_set_bits(ADENA, AUX0EN, AUX3EN, bits_channels);
        val = adc_get_bits(ADENA, AUX0EN, AUX3EN);
        if (count > 0)
            count--;
    } while ((val != bits_channels) && count);

    spin_unlock_irqrestore(&lock, flags);

    if (count == 0 && (val != bits_channels)) {
        printk(KERN_ERR "adc enable bits_channel %1x timeout\n", bits_channels);
        return -1;
    }
