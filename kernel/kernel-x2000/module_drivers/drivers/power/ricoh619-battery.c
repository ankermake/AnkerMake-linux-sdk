...
#define CH_ICHG	  	 0x0e	/* ICHG		= 0 - 0x1D (100mA - 3000mA) */
#define CH_ILIM_ADP 	 0x11	/* ILIM_ADP	= 0 - 0x1D (100mA - 3000mA) */
#define CH_ILIM_USB 	 0x11	/* ILIM_USB	= 0 - 0x1D (100mA - 3000mA) */
...
#define JT_EN  		 1	/* JEITA Enable	  = 0 or 1 (1:enable, 0:disable) */
#define JT_HW_SW 	 0	/* JEITA HW or SW = 0 or 1 (1:HardWare, 0:SoftWare) */
#define JT_TEMP_H 	 45	/* degree C */
#define	JT_TEMP_L 	 15	/* degree C */
...
#define	JT_ICHG_H 	 0x0e	/* ICHG Hi   	= 0 - 0x1D (100mA - 3000mA) */
#define JT_ICHG_L 	 0x03	/* ICHG Low   	= 0 - 0x1D (100mA - 3000mA) */
/* ****************************BATTERY INIT PARAMETER END******************** */

static struct ricoh61x_battery_info *mInfo = NULL;
...
//		cc_stop_flag = 0;
	...
	if (cc_stop_flag == 1)
	{
		...
		/* disable charging */
		err = ricoh61x_clr_bits(info->dev->parent, RICOH619_CHGCTL1, 0x03);
		if (err < 0)
			goto out;
	}
...
*/
{
	...
	*is_jeita_updated = false;
	...
	if (temp <= 0 || 50 <= temp) {
		/* 1st and 5th temperature ranges (~0, 50~) */
		...
		*is_jeita_updated = true;
		...
		/* 4th temperature range (45~50) */
		if (vfchg != info->jt_vfchg_l) {
			pr_info("PMU: %s *** 45<Temp<50, update to vfchg==%d ******\n", __func__, info->jt_vfchg_l);
			...
			*is_jeita_updated = true;
			...
			pr_info("JEITA: %s *** 45<Temp<50, already set vfchg==%d, so no need to update ******\n",
					__func__, info->jt_vfchg_l);
...
static int ricoh61x_init_charger(struct ricoh61x_battery_info *info)
{
	...
	err = ricoh61x_set_bits(info->dev->parent, RICOH619_PWRFUNC, 0x20);
	if (err < 0) {
...
//	return NULL;
	...
	of_property_read_u32(np, "alarm_vol_mv", &alarm_vol_mv);
	of_property_read_u32(np, "multiple", &multiple);
	of_property_read_u32(np, "monitor_time",&monitor_time);
	...
	info->monitor_time = monitor_time * HZ;
	info->alarm_vol_mv = alarm_vol_mv;
	...
	info->ch_vfchg = CH_VFCHG;
	info->ch_vrchg = CH_VRCHG;
	info->ch_vbatovset = CH_VBATOVSET;
	info->ch_ichg = CH_ICHG;
	info->ch_ilim_adp = CH_ILIM_ADP;
	info->ch_ilim_usb = CH_ILIM_USB;
	info->ch_icchg = CH_ICCHG;
	info->fg_rsense_val = FG_RSENSE_VAL;
	info->fg_target_vsys = FG_TARGET_VSYS;
	info->fg_target_ibat = FG_TARGET_IBAT * info->fg_rsense_val / 20;
	info->fg_poff_vbat = FG_POFF_VBAT;
	info->jt_en = JT_EN;
	info->jt_hw_sw = JT_HW_SW;
	info->jt_temp_h = JT_TEMP_H;
	info->jt_temp_l = JT_TEMP_L;
	info->jt_vfchg_h = JT_VFCHG_H;
	info->jt_vfchg_l = JT_VFCHG_L;
	info->jt_ichg_h = JT_ICHG_H;
	info->jt_ichg_l = JT_ICHG_L;
	...
	*/
	...
	info->battery_desc.name = "battery";
	info->battery_desc.type = POWER_SUPPLY_TYPE_BATTERY;
	info->battery_desc.properties = ricoh61x_batt_props;
	info->battery_desc.num_properties = ARRAY_SIZE(ricoh61x_batt_props);
	info->battery_desc.get_property = ricoh61x_batt_get_prop;
	info->battery_desc.set_property = NULL;
	info->battery_desc.external_power_changed
	...
	ret = power_supply_register(&pdev->dev, &info->battery, &psy_cfg);

	if (ret)
		info->battery.dev.parent = &pdev->dev;


	ret = power_supply_register(&pdev->dev, &powerac, &psy_cfg);
	ret = power_supply_register(&pdev->dev, &powerusb, &psy_cfg);
	...
	printk("RICOH61x_IRQ_FCHGCMPINT1,RICOH61x_IRQ_FONCHGINT,------------ %d %d %d %d %d\n",RICOH61x_IRQ_FONCHGINT,RICOH61x_IRQ_FCHGCMPINT,RICOH61x_IRQ_FVUSBDETSINT,RICOH61x_IRQ_FVADPDETSINT,RICOH61x_IRQ_VSYSLIR);
	//charger_irq = of_get_named_gpio_flags(np, "ingenic,ricoh-irq", 0, &flags);
	charger_irq = 0;//IRQ_RESERVED_BASE;//INT_PMIC_BASE;//gpio_to_irq(charger_irq);
	ret = request_threaded_irq(charger_irq + RICOH61x_IRQ_FCHGCMPINT1, NULL,
	...
	ret = request_threaded_irq(charger_irq + RICOH61x_IRQ_FVADPDETSINT1,
	...
	printk("----------PMU:ricoh61x_battery_probe success! \n");
	mInfo = info;
#ifdef CONFIG_BOARD_CIRCLEV2
	ricoh61x_read(mInfo->dev->parent, 0x98, &val);
	val &= 0xF8;
	val |= 0x6;
	ricoh61x_write(mInfo->dev->parent, 0x98, val);
#endif
	//mdelay(1000);
	return 0;
...
static int  ricoh61x_battery_remove(struct platform_device *pdev)
{
	...
	free_irq(charger_irq + RICOH61x_IRQ_FCHGCMPINT1, &info);
	free_irq(charger_irq + RICOH61x_IRQ_FONCHGINT, &info);
	free_irq(charger_irq + RICOH61x_IRQ_FVUSBDETSINT, &info);
	free_irq(charger_irq + RICOH61x_IRQ_FVADPDETSINT1, &info);
#ifdef ENABLE_LOW_BATTERY_DETECTION
...
#define INGENIC_LED_FLICKER_SLOW  2
#define INGENIC_LED_FLICKER_FAST  3
#define INGENIC_LED_FLICKER_OFF   4

ssize_t set_pmu_led_status(const char *name, const char *buf, size_t count)
{
	int value = -1;
	uint8_t val;
	sscanf(buf, "%d", &value);
	if(!mInfo)return count;
	ricoh61x_read(mInfo->dev->parent, 0x98, &val);
	val &= 0xF8;
	switch (value) {
	...
	case INGENIC_LED_ON :
		...
		ricoh61x_write(mInfo->dev->parent, 0x98, val);
		break;
	case INGENIC_LED_FLICKER_SLOW :
		val |= 0x5;
		ricoh61x_write(mInfo->dev->parent, 0x98, val);
		break;
	case INGENIC_LED_FLICKER_FAST :
		val |= 0x6;
		ricoh61x_write(mInfo->dev->parent, 0x98, val);
		break;
	default :
...
EXPORT_SYMBOL(set_pmu_led_status);
...
{
	return set_pmu_led_status(attr->attr.name, buf, count);
}
...
{
	leds_kobj = kobject_create_and_add("pmu_leds", NULL);
	if (!leds_kobj)
		return -ENOMEM;
#if 0
