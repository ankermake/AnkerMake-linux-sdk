...
#else
u32 ingenic_dma_read_dda(struct dma_chan *chan) { return 0;};
int ingenic_dma_get_desc_index(struct dma_chan *chan, unsigned int doa, int desc_num) { return 0;};
u32 ingenic_dma_read_dtc(struct dma_chan *chan) { return 0;};
#endif
...
static void ingenic_dma_tx_complete(void *arg)
{
	...
	dma_unmap_sg(up->port.dev, &up->sg_tx, 1, DMA_MEM_TO_DEV);
...
static void ingenic_dma_tx(unsigned long data)
{
	...
	dma_map_sg(up->port.dev,&up->sg_tx,1,DMA_MEM_TO_DEV);
...
static int serial_ingenic_probe(struct platform_device *pdev)
{
	...
	if(up->use_dma){
		...
		up->chan_rx = dma_request_slave_channel_reason(&pdev->dev, "rx");
		if (up->chan_rx < 0){
		...
		up->chan_tx = dma_request_slave_channel_reason(&pdev->dev, "tx");
		if (up->chan_tx < 0){
