...
//    disable_irq(info->irqdh);
	...
	/*Saving EEPROM used for debugging modified on  20190702
	mdelay(10);
	write_reg(client, 0xc003, 0x0001);
	write_reg(client, 0xc104, 0x0001);
	
	write_cmd(client,BT541_SAVE_STATUS_CMD);
...
static bool ts_set_touchmode(u16 value)
{
    ...
    if (write_cmd(misc_info->client, BT541_SWRESET_CMD) != I2C_SUCCESS) {
		printk(KERN_INFO "Failed to write reset command\n");
	}
...
//		pdata->tsp_en_gpio = of_get_named_gpio(np, "ingenic,tp-en-gpio", 0); //lcd have init tp-en-gpio.
//		if (pdata->tsp_en_gpio < 0) {
//			pr_err("%s: of_get_named_gpio failed: tp-en-gpio %d\n", __func__,
//				pdata->tsp_en_gpio);
//			return -EINVAL;
//		}
#if 0
...
//		pdata->tsp_en_gpio = of_get_named_gpio(np, "ingenic,tp-en-gpio", 0);
//		if (pdata->tsp_en_gpio < 0) {
...
#endif	
	pr_err("%s:gpio_int :%d, vcc_en :%d\n",__func__, pdata->gpio_int, pdata->tsp_en_gpio);
