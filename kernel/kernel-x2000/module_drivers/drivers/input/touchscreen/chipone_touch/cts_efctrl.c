...
{
	int ret;
	/**** chip erase config default****/
	/*if (flags & EFCTRL_CMD_CE_FLAGS) {
	   efctrl_write_reg_check_ret(writel, EFCTRL_ET_OP_CNT, EFCTRL_EF_OP_CNT_CE_CFG);
	   } */
   /*** sector erase config default***/
	/* if (flags & EFCTRL_CMD_SE_FLAGS) {
	   efctrl_write_reg_check_ret(writel, rEFCTRL_ET_OP_CNT, EFCTRL_EF_OP_CNT_SE_CFG);
	   } */

	efctrl_write_reg_check_ret(writeb, EFCTRL_CMD_SEL, cmd);

	if (flags & EFCTRL_OPFLAG_SRAM_DATA_XCHG) {
		efctrl_write_reg_check_ret(writel, EFCTRL_SRAM_ADDR,
					   sram_addr);
	...
	if (wait_efctrl_xfer_comp(cts_dev) != 0) {
		cts_err("Wait efctrl ready failed %d", ret);
		return ret;
	}
...
{
	cts_err("ICN_EMBEDDED_FLASH not support block earse");
	return 0;
}
