...
//#include "cts_firmware_new.h"
#include <linux/path.h>
#include <linux/mount.h>
#include <linux/namei.h>
#include <linux/vmalloc.h>

unsigned int cts_fw_crc_calc(unsigned crc_in, char *buf, int len)
{
	...
	cts_info("cts_fw_crc_calc,crc:%0x ", crc);
	return crc;
}
					      ...
					      int curr_version)
{
	...
	if (IS_ERR(file)) {
		cts_err("Open file '%s' failed %ld", filepath,
			   PTR_ERR(file));
		return PTR_ERR(file);
	}
...
#endif
	if (read_size < 0) {
		cts_err("Read version from offset 0x114 failed");
		ret = -EIO;
		goto err_close_file;
	}
	...
	if (version <= curr_version) {
		cts_info
		    ("File '%s' size: %zu version: %04x <= %04x(curr_version),no need update",
	...
	if (read_size < 0 || read_size != firmware->size) {
		cts_err("Request from fs read whole file failed %d",
			   read_size);
		ret = -EIO;
...

	{
		...
		if (r)
			cts_info("Close file '%s' failed %d", filepath, r);
	}
			       ...
			       u32 Length)
{
	...
	if (ret) {
		cts_err("read crc section to sram  failed %d", ret);
		return ret;
	}
	...
	ret =
	    cts_sram_readsb(cts_dev, Length, temp_buf,
			    FIRMWARE_CRC_SECTION_SIZE);
	if (ret) {
		cts_err("read crc section from sram  failed %d", ret);
		return ret;
	}
	for (j = 0; j < 16; j++) {
		if (temp_buf[j] != temp[j]) {
			cts_err(" cts firmware check  error");
			return R_VERIFY_ERR;
		}
	}

	cts_info(" firmware  check  ok");
	return 0;
}
	...
	if (ret) {
		cts_err
		    ("Calc data in flash from 0x%06x size %zu crc failed %d, "
		     "try to validate by direct readback", flash_addr, size,
		     ret);
	} else {
						   ...
						   *cts_dev, u32 Length)
{
	...
	cts_info("Program firmware from sram to flash, size %zu", Length);
			...
			const struct cts_firmware *firmware, bool to_flash)
{
	...
	if (ret) {
		cts_info("Prepare flash operation failed %d", ret);
		goto out;
	}
	/*** calc fw crc ***/
	crc_fw = cts_fw_crc_calc(crc_fw, firmware->data, firmware->size);
	cts_info("Update firmware , fw crc: 0x%08x, size:", crc_fw,
		    firmware->size);
	...
	do {
		...
		if (ret) {
			cts_err("Validate flash crc  failed %d", ret);
			continue;
		}
		...
		if (ret) {
			cts_err(" crc write check failed %d ", ret);
			continue;
		}
