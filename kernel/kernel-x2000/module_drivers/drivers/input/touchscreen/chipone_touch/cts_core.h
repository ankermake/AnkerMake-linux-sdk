...
struct cts_device_gesture_info {
    u8    gesture_id;
#define CTS_GESTURE_UP                  (0x03)
#define CTS_GESTURE_DOWN             (0x04)
#define CTS_GESTURE_D_TAP            (0x02)
...
struct chipone_ts_data {
	...
	int self_deviation_test_min;
	int self_deviation_test_max;
...
#define cts_round_up(x, y)  ((((x)-1) | __round_mask(x, y))+1)
#define cts_round_down(x, y)  ((x) & ~__round_mask(x, y))*/
...
extern int cts_prog_get_project_id(struct cts_device *cts_dev);
extern int cts_normal_get_project_id(struct cts_device *cts_dev);
