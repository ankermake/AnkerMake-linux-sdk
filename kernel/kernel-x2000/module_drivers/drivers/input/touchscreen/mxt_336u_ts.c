...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (active) {
		...
		input_mt_report_slot_state(input_dev, tool, 1);
		input_report_abs(input_dev, ABS_MT_POSITION_X, x); //cljiang add
		input_report_abs(input_dev, ABS_MT_POSITION_Y, y); //cljiang add
//		input_report_abs(input_dev, ABS_MT_POSITION_X, (x * 2 / 3)); //cljiang add
//		input_report_abs(input_dev, ABS_MT_POSITION_Y, (y * 3 / 4)); //cljiang add
...
static int mxt_acquire_irq(struct mxt_data *data)
{
	...
	if (!data->irq) {
        data->client->irq = gpio_to_irq(data->client->irq); //cljiang add
...
static int mxt_probe_regulators(struct mxt_data *data)
{
    ...
    gpio_request(data->pdata->gpio_reset, "reset");
...
static const struct mxt_platform_data *mxt_parse_dt(struct i2c_client *client)
{
    ...
    client->irq = of_get_named_gpio_flags(np, "atmel,irq-gpio", 0, NULL);
    ret = gpio_request(client->irq, "atmel,irq-gpio");
    if(ret){
    }
    ...
    pdata->suspend_mode = MXT_SUSPEND_REGULATOR;
