#ifndef _DT_BINDINGS_INTERRUPT_CONTROLLER_X1630_IRQ_H
#define _DT_BINDINGS_INTERRUPT_CONTROLLER_X1630_IRQ_H

#include <dt-bindings/interrupt-controller/mips-irq.h>

#define IRQ_RESERVED0_0		(0)
#define IRQ_AIC		(1)
#define IRQ_RESERVED0_2		(2)
#define IRQ_RESERVED0_3		(3)
#define IRQ_MIPI_RX		(4)
#define IRQ_LCDC		(5)
#define IRQ_IPU			(6)
#define IRQ_SFC			(7)
#define IRQ_SSI1		(8)
#define IRQ_SSI0		(9)
#define IRQ_PDMA		(10)
#define IRQ_RESERVED0_11	(11)
#define IRQ_RESERVED0_12	(12)
#define IRQ_RESERVED0_13	(13)
#define IRQ_GPIO3		(14)
#define IRQ_GPIO2		(15)
#define IRQ_GPIO1		(16)
#define IRQ_GPIO0		(17)
#define IRQ_SADC		(18)
#define IRQ_MSCALER	(19)
#define IRQ_LDC	(20)
#define IRQ_USB			(21)
#define IRQ_RESERVED0_22	(22)
#define IRQ_AES			(23)
#define IRQ_RESERVED0_24	(24)
#define IRQ_TCU2		(25)
#define IRQ_TCU1		(26)
#define IRQ_TCU0		(27)
#define IRQ_RESERVED0_28	(28)
#define IRQ_ISP			(29)
#define IRQ_RESERVED0_30	(30)
#define IRQ_RESERVED0_31	(31)

#define IRQ_RTC			(32 + 0)
#define IRQ_NCU		(32 + 1)
#define IRQ_DTRNG		(32 + 2)
#define IRQ_NCU_P		(32 + 3)
#define IRQ_MSC1		(32 + 4)
#define IRQ_MSC0		(32 + 5)
#define IRQ_RESERVED1_6		(32 + 6)
#define IRQ_RESERVED1_7		(32 + 7)
#define IRQ_DMIC		(32 + 8)
#define IRQ_RESERVED1_9		(32 + 9)
#define IRQ_RESERVED1_10	(32 + 10)
#define IRQ_RESERVED1_11	(32 + 11)
#define IRQ_HARB2		(32 + 12)
#define IRQ_RADIX	(32 + 13)
#define IRQ_HARB0		(32 + 14)
#define IRQ_CPM			(32 + 15)
#define IRQ_RESERVED1_16	(32 + 16)
#define IRQ_RESERVED1_17	(32 + 17)
#define IRQ_UART1		(32 + 18)
#define IRQ_UART0		(32 + 19)
#define IRQ_DDR			(32 + 20)
#define IRQ_RESERVED1_21	(32 + 21)
#define IRQ_EFUSE		(32 + 22)
#define IRQ_GMAC			(32 + 23)
#define IRQ_RESERVED1_24	(32 + 24)
#define IRQ_RESERVED1_25	(32 + 25)
#define IRQ_I2C2		(32 + 26)
#define IRQ_I2C1		(32 + 27)
#define IRQ_I2C0		(32 + 28)
#define IRQ_PDMAM		(32 + 29)
#define IRQ_HELIX		(32 + 30)
#define IRQ_RESERVED1_31	(32 + 31)

#endif
