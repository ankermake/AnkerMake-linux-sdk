...
struct jz_gpio_func_def platform_devio_array[] = {
	...
	SSI0_PORTC_NO_CE,
#elif defined(CONFIG_JZ_SPI0_PC)
	SSI0_PORTC,
#endif
#endif
	...
	DVP_PORTA_LOW_8BIT,
#elif defined CONFIG_MIPI_SENSOR
    MCLK_PORTA,
#elif defined CONFIG_SOC_CAMERA
	DVP_PORTA_LOW_8BIT,
#endif
...
struct platform_device snd_alsa_device = {
 ...
 /* end of ALSA platform devices */
#elif defined(CONFIG_JZ_MICCHAR) || defined(CONFIG_X1520_MIC)
...
#if defined(CONFIG_JZ_VIC_CORE) || defined(CONFIG_SOC_CAMERA)
static struct resource tx_isp_vic_resource[] = {
	[0] = {
		.name = "isp_vic",
		.start = ISP_VIC_IOBASE,
		.end = ISP_VIC_IOBASE + 0x30000,
		.flags = IORESOURCE_MEM,
	},
	[1] = {
		.name = "isp_irq_id",
...
extern struct vic_sensor_config* get_sensor_data(void);
struct platform_device tx_isp_vic_platform_device = {
	.name = VIC_ONLY_NAME,
	.id = 0,
	.dev = {
		.coherent_dma_mask = 0xffffffff,
		.platform_data = get_sensor_data,
	},
...
#ifdef CONFIG_JZ_AXI_MONITOR
static u64 jz_axi_monitor_dmamask = ~(u64) 0;
static struct resource jz_axi_monitor_resources[] = {
	[0] = {
		.start = CH0_MON_IOBASE,
		.end = CH0_MON_IOBASE + 0x80 - 1,
		.flags = IORESOURCE_MEM,
	},
	[1] = {
		.start = IRQ_CH0_MON,
	...
	[2] = {
		.start = CH4_MON_IOBASE,
		.end = CH4_MON_IOBASE + 0x80 - 1,
		.flags = IORESOURCE_MEM,
	},
	[3] = {
		.start = IRQ_CH4_MON,
	...
	[4] = {
		.start = CH6_MON_IOBASE,
		.end = CH6_MON_IOBASE + 0x80 - 1,
		.flags = IORESOURCE_MEM,
	},
	[5] = {
		.start = IRQ_CH6_MON,
...
struct platform_device jz_axi_monitor_device = {
	.name = "jz_axi_monitor",
	.id = 0,
	.dev = {
		.dma_mask = &jz_axi_monitor_dmamask,
		.coherent_dma_mask = 0xffffffff,
	},
	.num_resources = ARRAY_SIZE(jz_axi_monitor_resources),
	.resource = jz_axi_monitor_resources,
};
#endif
