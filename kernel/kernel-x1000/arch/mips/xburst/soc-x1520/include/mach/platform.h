...
#define SSI0_PORTC_NO_CE					\
	{ .name = "ssi0-pc",	       .port = GPIO_PORT_C, .func = GPIO_FUNC_0, .pins = 0xB800, }
/* SFC : the function of hold and reset not use, hardware pull up */
#define SFC_PORTA_QUAD							\
	{ .name = "sfc",		.port = GPIO_PORT_A, .func = GPIO_FUNC_1 | GPIO_PULL_ENABLE, .pins = (0x3f << 23), }
#define SFC_PORTA							\
	{ .name = "sfc",		.port = GPIO_PORT_A, .func = GPIO_FUNC_1 | GPIO_PULL_ENABLE, .pins = (0x3 << 23) | (0x3 << 27), }
...
extern struct platform_device jz_axi_monitor_device;
#endif
