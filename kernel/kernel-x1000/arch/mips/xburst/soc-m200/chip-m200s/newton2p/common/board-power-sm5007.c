...
SM_PDATA_INIT(buck2,     0, 1200,  1200, 0, BUCK2_ALWAYS_ON, BUCK2_BOOT_ON, 1,
         BUCK2_INIT_UV, BUCK2_INIT_ENABLE, 1, 0, 0, 0, BUCK2_LPM);
...
SM_PDATA_INIT(ps5,  0,  1800,   2800, 0, PS5_ALWAYS_ON,  PS5_BOOT_ON,  1,
...
static struct sm5007_fuelgauge_platform_data sm5007_fuelgauge_data = {
    ...
    /* 注意: battery_type 这仅仅是一个标记，不对sm5007的软件和硬件起作用, 所以不设置了
     * 对设置sm5007的电池电压真正起作用的是 sm5007_charger_data.type.chg_batreg
     */
    .type = {
        .id                 = 0,
...
static struct sm5007_charger_platform_data sm5007_charger_data = {
    ...
    .type = {
#if   defined(CONFIG_SM5007_BATTERY_FULL_VOLTAGE_4200MV)
        .chg_batreg     = 4200, /* mV */
#elif defined(CONFIG_SM5007_BATTERY_FULL_VOLTAGE_4350MV)
        .chg_batreg     = 4350,
#else
#error "select your battery full voltage, or add a new one !"
#endif
        .chg_fastchg    = 200,   /* mA */
...
static int sm5007_fuelgauge_mask_all_irqs(struct i2c_client *client_fg){
    int ret;
    uint8_t val = 0x000F;

    ret = i2c_smbus_write_i2c_block_data(client_fg, SM5007_REG_INTFG_MASK, 2, &val);
    if (ret < 0) {
        dev_err(&client_fg->dev, "failed writings to 0x%02x\n", SM5007_REG_INTFG_MASK);
    }

    //read and clear fg irq
    ret = i2c_smbus_read_i2c_block_data(client_fg, SM5007_REG_INTFG, 2, &val);
    if (ret < 0) {
        dev_err(&client_fg->dev, "failed to read 0x%02x\n", SM5007_REG_INTFG);
    }
...
static int __init pmu_dev_init(void)
{
    ...
    sm5007_fuelgauge_mask_all_irqs(client_fg);

    sm5007_fuelgauge_data.type.battery_type = sm5007_charger_data.type.chg_batreg;
#endif
    ...
    //slpt_set_pmu_name("sm5007");
    //slpt_set_pmu_i2c_bus_num(busnum);
...
//arch_initcall(pmu_dev_init); // branch Indroid5.1-master
