...
unsigned long clk_get_rate(struct clk *clk)
{
	...
	if(clk->source)
		clk = clk->source;
	if(!clk->rate && clk->ops && clk->ops->get_rate)
...
static struct clk **proc_clk = NULL;
static int enable_write(struct file *file, const char __user *buffer,size_t count, loff_t *data)
{
	...
	if(clk) {
		int index = get_clk_id(clk);
		struct clk *op = proc_clk[index];

		if(count && (buffer[0] == '1')) {
			if (!op) {
				op = clk_get(NULL, clk->name);
				if (IS_ERR(op)) {
					pr_err("clk(%s) not found!!!!(check source code)\n", clk->name);
					return count;
				}
				proc_clk[index] = op;
			}
...
static int enable_show(struct seq_file *m, void *v)
{
	...
	int index = get_clk_id(clk);
	return seq_printf(m,"%s(hw%s)\n", proc_clk[index] ? "enabled": "disabled",
...
static int __init init_clk_proc(void)
{
	...
	proc_clk = kzalloc(get_clk_sources_size() * sizeof(struct clk*), GFP_KERNEL);
	if (!proc_clk)
		return -ENOMEM;
	for(i = 0; i < get_clk_sources_size(); i++) {
		if (clk_srcs[i].name == NULL)
