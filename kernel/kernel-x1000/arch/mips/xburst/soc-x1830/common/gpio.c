			      ...
			      struct gpio_reg_func *rfunc, unsigned int pins)
{
	...
	{
		unsigned int pins_tmp = pins;
		if (pins_tmp & 0xffff) {
			unsigned int pin, mask = 0;
			comp = readl(chip->reg + PXPEL);
			while (!!(pin = fls((pins_tmp & 0xffff)))) {
				pin = pin - 1;
				pins_tmp &= ~(BIT(pin));
				mask |= 0x3 << (pin << 1);
			}
			comp &= ~mask;
			comp |= (rfunc->save[4] & mask);
			//printk("=====> %s %d %lx:%x %x %ld\n", __func__, __line__, comp, mask, pins, grp);
			writel(comp, chip->reg + PXPEL);
		}
		if (pins_tmp & 0xffff0000) {
			unsigned int pin, mask = 0;
			comp = readl(chip->reg + PXPEH);
			while (!!(pin = fls((pins_tmp)))) {
				pin = pin - 1;
				pins_tmp &= ~(BIT(pin));
				mask |= 0x3 << ((pin - 16) << 1);
			}
			comp &= ~mask;
			comp |= (rfunc->save[5] & mask);
			//printk("=====> %s %d %lx:%x:%x %ld\n", __func__, __line__, comp, mask, pins,  grp);
...
int jzgpio_ctrl_pull(enum gpio_port port, int enable_pull,unsigned long pins)
{
	...
	if (!enable_pull)
	{
        ...
        gpio_set_func(jz,GPIO_PULL_UP,BIT(pins));
    }
	return 0;
}
			...
			unsigned offset, int value)
{
	...
	if (!test_bit(offset, jz->out_map)) {
		pr_err("BAD set to input gpio. %d %d\n", jz - jz_gpio_chips, offset);
		return;
	}
...
static const char *get_platform_pin_descption(int port, int pin)
{
	int i;
...
static int jz_gpio_request(struct gpio_chip *chip, unsigned offset)
{
	...
	if(jz->dev_map[0] & (1 << offset)) {
		int port = jz - jz_gpio_chips;
		char P = 'A' + port;
		const char *desc = get_platform_pin_descption(port, offset);
		unsigned int io_mode = readl(jz->reg + PXMSK);
		if (io_mode & (1 << offset))
			return 0;
		dump_stack();
		printk("gpio: GPIO_P%c%d functions has redefinition\n", P, offset);
		printk("gpio: platform defined as [%s], used pins: %lx\n", desc, jz->dev_map[0]);
		return 0;
	}
...
static void jz_gpio_free(struct gpio_chip *chip, unsigned offset)
{
	...
	gpio_set_func(jz, GPIO_INPUT|GPIO_PULL_HIZ, BIT(offset));
...
static int gpio_set_type(struct irq_data *data, unsigned int flow_type)
{
	...
	case IRQ_TYPE_LEVEL_HIGH:	func = GPIO_INT_HI|GPIO_PULL_HIZ;	break;
	case IRQ_TYPE_LEVEL_LOW:	func = GPIO_INT_LO|GPIO_PULL_HIZ;	break;
	case IRQ_TYPE_EDGE_RISING:	func = GPIO_INT_RE|GPIO_PULL_HIZ;	break;
	case IRQ_TYPE_EDGE_FALLING:	func = GPIO_INT_FE|GPIO_PULL_HIZ;	break;
	case IRQ_TYPE_EDGE_BOTH:
		if (gpio_pin_level(jz, pin))
			func = GPIO_INT_LO | GPIO_PULL_HIZ;
		else
			func = GPIO_INT_HI | GPIO_PULL_HIZ;
		break;
