...
#elif defined(CONFIG_SOC_T30) || defined(CONFIG_SOC_X1830) || defined(CONFIG_SOC_X1630)
/* T30 */
#define TX_ISP_EXIST_FR_CHANNEL 1
#define TX_ISP_EXIST_DS2_CHANNEL 0
...
enum tx_isp_notification {
	...
	TX_ISP_EVENT_SENSOR_DUAL_CAMERA_SWITCH,
...
#define VIDIOC_RELEASE_RESOURCE  	 		_IOW('V', BASE_VIDIOC_PRIVATE + 20, int)
...
/* camera channel type */
enum camera_chn_type{
	CAMERA_CHN_NORMAL = 0,
	CAMERA_CHN_IR,
};

struct dual_camera_frame_info {
	unsigned int camera1_frame_rate;
	unsigned int camera2_frame_rate;
	enum camera_chn_type camera1_chn_type;
	enum camera_chn_type camera2_chn_type;
...
struct dual_camera_param {
	struct dual_camera_frame_info frame_info;
	long frame_cnt;
	char frame_cnt_comp;
...
struct tx_isp_video_in {
	...
	struct dual_camera_param *dc_param;
...
int tx_isp_clk_set(unsigned long rate);
