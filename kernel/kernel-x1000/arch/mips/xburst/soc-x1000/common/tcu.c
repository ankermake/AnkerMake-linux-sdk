						    ...
						    unsigned int half_num)
{
	...
	/* spinlock_t counter_lock;
	   spin_lock_init(&tcu->counter_lock); */

	spin_lock_irqsave(&tcu->counter_lock, flags);
	if (tcu->enable_cnt) {
		full = regr(CH_TDFR(id));
	...
	spin_unlock_irqrestore(&tcu->counter_lock, flags);
	return ;
}
...
int tcu_as_timer_config(struct tcu_device *tcu)
{
	...
	if (tcu->enable_cnt) {
		set_tcu_full_half_value_wait_period_end(tcu, tcu->id, tcu->full_num, tcu->half_num);
	}
	else {
		set_tcu_full_half_value(tcu->id, tcu->full_num, tcu->half_num);
	}
