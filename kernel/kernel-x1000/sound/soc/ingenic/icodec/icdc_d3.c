						   ...
						   struct snd_soc_dai *dai)
{
	...
	if(icdc_d3->VIC_count==1){
		...
		snd_soc_update_bits(codec, SCODA_REG_CR_VIC, SCODA_CR_VIC_SB_SLEEP_MASK, 0<<SCODA_CR_VIC_SB_SLEEP_SHIFT);
	}
...
#ifndef CONFIG_JZ_ASOC_CODEC_INNER_REFERENCE_SOUND
	icdc_d3_hw_write_extend (icdc_d3, SCODA_MIX_2, (0<<6)| (1<<4));
#else
        snd_soc_write(codec, SCODA_REG_CR_MIX, 0x80);
        icdc_d3_hw_write_extend (icdc_d3, SCODA_MIX_0, 0);  /*AIDAC normal*/
        icdc_d3_hw_write_extend (icdc_d3, SCODA_MIX_1, 0);  /*MIXDAC normal*/
        icdc_d3_hw_write_extend (icdc_d3, SCODA_MIX_2, 1);  /*AIADC normal DAC+INPUT*/
        icdc_d3_hw_write_extend (icdc_d3, SCODA_MIX_3, (0x3 << 6)|  (0x3 << 4));  /*MIXADC 0 input*/
        snd_soc_write(codec, SCODA_REG_GCR_MIXDACL, 0x2);
        snd_soc_write(codec, SCODA_REG_GCR_MIXDACR, 0x2);
#endif
