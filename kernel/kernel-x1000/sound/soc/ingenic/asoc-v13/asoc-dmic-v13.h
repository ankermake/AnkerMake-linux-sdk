...
#define __dmic_test_full(dev) \
	(dmic_read_reg(dev, DMICINTCR) & DMIC_FULL_FLAG_MASK)
#define __dmic_clear_full(dev)	\
	dmic_set_reg(dev, DMICINTCR, 1,  DMIC_FULL_FLAG_MASK, DMIC_FULL_FLAG)
#define __dmic_test_empty(dev) \
	(dmic_read_reg(dev, DMICINTCR) & DMIC_EMPTY_FLAG_MASK)
...
#define __dmic_fifo_level(dev)	\
		(dmic_read_reg(dev, DMICFSR) & DMIC_FIFO_LVL_MASK)
