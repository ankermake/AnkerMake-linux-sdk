				...
				struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	int trigger, fifo_size, fifo_buswidth;
...
#if 0
	if (jz_dmic->fifo_pack_enable) {
		buswidth = jz_dmic->intf_unpack_disable ? DMA_SLAVE_BUSWIDTH_4_BYTES :
			DMA_SLAVE_BUSWIDTH_2_BYTES;
		switch (channels) {
		case 3:
			fifo_buswidth = jz_dmic->intf_unpack_disable ? 8 : 6;
			break;
		default:
			fifo_buswidth = 8;
			break;
		...
		__dmic_enable_pack(dev);
		__dmic_disable_unpack_msb(dev);
		if (jz_dmic->intf_unpack_disable)
			__dmic_enable_unpack_dis(dev);
		else
			__dmic_disable_unpack_dis(dev);
	} else {
		__dmic_disable_pack(dev);
		buswidth = DMA_SLAVE_BUSWIDTH_2_BYTES;
		fifo_buswidth = buswidth * channels;
	}
	fifo_size = DMIC_FIFO_DEPTH * fifo_buswidth;
#else
	buswidth = DMA_SLAVE_BUSWIDTH_2_BYTES;
	if (channels == 3)
		fifo_buswidth = 6;
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		jz_dmic->rx_dma_data.max_burst = (fifo_size / 2)  > 128 ? 128 : (fifo_size / 2);
		trigger = jz_dmic->rx_dma_data.max_burst / fifo_buswidth;
		...
		struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		__dmic_reset(dai->dev);
		while(__dmic_get_reset(dai->dev));
		...
		struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		if (__dmic_is_enable_rdms(dev)) {
			...
			timeout_jiffies = jiffies + msecs_to_jiffies((1000 * DMIC_FIFO_DEPTH * 4) / 8000) * 110 / 100;
			printk("dmic timeout %lu timeout_jiffies %lu jiffies %lu\n",
					msecs_to_jiffies((1000 * DMIC_FIFO_DEPTH * 4) / 8000) * 110 / 100,
					timeout_jiffies, jiffies);
			__dmic_clear_full(dev);
			while((!__dmic_test_full(dev)) && time_before(jiffies, timeout_jiffies));
			printk("dmic stop ok\n");
		}
	} else {
		dev_err(dai->dev, "DMIC is a capture device\n");
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	struct jz_dmic *jz_dmic = snd_soc_codec_get_drvdata(codec);
	unsigned int value;

	if(jz_dmic->en){
		value = dmic_read_reg(jz_dmic->dev, DMICGCR);
	}else{
		clk_enable(jz_dmic->clk_gate_dmic);
		value = dmic_read_reg(jz_dmic->dev, DMICGCR);
		clk_disable(jz_dmic->clk_gate_dmic);
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	if(jz_dmic->en){
		dmic_write_reg(jz_dmic->dev, DMICGCR, value);
	}else{
		clk_enable(jz_dmic->clk_gate_dmic);
		dmic_write_reg(jz_dmic->dev, DMICGCR, value);
		clk_disable(jz_dmic->clk_gate_dmic);
...
static int jz_dmic_platfrom_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(jz_dmic->clk_gate_dmic)) {
		...
		dev_err(&pdev->dev, "Failed to get dmic clock: %d\n", ret);
		return ret;
	}
	...
	if (IS_ERR_OR_NULL(jz_dmic->dmic_enable)) {
		...
		dev_err(&pdev->dev, "Failed to get dmic_enable clock: %d\n", ret);
		return ret;
	}
