...
{
	return jz_speaker_enable(is_on);
}

static int pd_x1830_audio_spk_power(struct snd_soc_dapm_widget *w,
...
static const struct snd_soc_dapm_widget pd_x1830_audio_dapm_widgets[] = {
	SND_SOC_DAPM_SPK("Speaker", pd_x1830_audio_spk_power),
	SND_SOC_DAPM_MIC("Buildin-mic", NULL),
	SND_SOC_DAPM_MIC("Digital-mic", NULL),
};

/* pd_x1830_audio machine audio_map */
...
static int pd_x1830_audio_dlv_dai_link_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if (err)
		return err;
        err = snd_soc_dapm_new_controls(dapm, pd_x1830_audio_dapm_widgets,
			ARRAY_SIZE(pd_x1830_audio_dapm_widgets));
	if (err){
		printk("pd_x1830_audio dai add controls err!!\n");
		return err;
	}
	...
	if (err){
		printk("add pd_x1830_audio dai routes err!!\n");
		return err;
	}
...
static int snd_pd_x1830_audio_probe(struct platform_device *pdev)
{
	...
	/*
	 * pd x1830 audio board's SD card cd mode pin and speaker en pin
