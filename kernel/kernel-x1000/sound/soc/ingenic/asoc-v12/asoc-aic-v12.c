...
void aic_disable_gate_clk(struct device *aic)
{
       struct jz_aic *jz_aic = dev_get_drvdata(aic);

       clk_disable(jz_aic->clk_gate);
}
EXPORT_SYMBOL_GPL(aic_disable_gate_clk);

void aic_enable_gate_clk(struct device *aic)
{
       struct jz_aic *jz_aic = dev_get_drvdata(aic);

       clk_enable(jz_aic->clk_gate);
}
EXPORT_SYMBOL_GPL(aic_enable_gate_clk);

void aic_disable_clk(struct device *aic)
{
       struct jz_aic *jz_aic = dev_get_drvdata(aic);

       clk_disable(jz_aic->clk);
}
EXPORT_SYMBOL_GPL(aic_disable_clk);

void aic_enable_clk(struct device *aic)
{
       struct jz_aic *jz_aic = dev_get_drvdata(aic);

       clk_enable(jz_aic->clk);
}
EXPORT_SYMBOL_GPL(aic_enable_clk);
...
#if defined(CONFIG_SND_ASOC_JZ_AIC_I2S_V12) || defined(CONFIG_SND_ASOC_JZ_AIC_I2S_V13)
        AIC_REGISTER_SUBDEV(i2s);
#endif
#if defined(CONFIG_SND_ASOC_JZ_AIC_SPDIF_V12) || defined(CONFIG_SND_ASOC_JZ_AIC_SPDIF_V13)
        AIC_REGISTER_SUBDEV(spdif);
#endif
#if defined(CONFIG_SND_ASOC_JZ_AC97)
...
static irqreturn_t jz_aic_irq_thread(int irq, void *dev_id)
{
	...
	if ((jz_aic->mask & 0x8) && __aic_test_ror(jz_aic->dev)) {
		...
		dev_printk(KERN_DEBUG, jz_aic->dev,
				"recieve fifo [overrun] interrupt time [%d] %x\n",
				jz_aic->ror, jz_aic_read_reg(jz_aic->dev, AICSR));
	}
