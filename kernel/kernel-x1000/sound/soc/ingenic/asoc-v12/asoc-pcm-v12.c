		...
		struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (__pcm_transmit_dma_is_enable(dev)) {
			__pcm_disable_transmit_dma(dev);
			__pcm_clear_tur(dev);
			timeout_jiffies = jiffies + msecs_to_jiffies((1000 * PCM_TFIFO_DEPTH)/8000) * 110/100;
			while(!__pcm_test_tur(dev) && time_before(jiffies, timeout_jiffies));
		}
		__pcm_disable_replay(dev);
		__pcm_clear_tur(dev);
	} else {
		if (__pcm_receive_dma_is_enable(dev)) {
			...
			__pcm_clear_ror(dev);
			timeout_jiffies = jiffies + msecs_to_jiffies((1000 * PCM_TFIFO_DEPTH)/8000) * 110/100;
			while(!__pcm_test_ror(dev) && time_before(jiffies, timeout_jiffies));
		}
		__pcm_disable_record(dev);
		__pcm_clear_ror(dev);
	}
