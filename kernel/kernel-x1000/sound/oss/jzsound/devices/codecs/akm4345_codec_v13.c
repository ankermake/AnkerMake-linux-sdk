...
static struct extcodec_platform_data *akm4345_spi = NULL;
...
static int codec_init(void)
{
	...
	/* reset PDN pin */
	if (akm4345_spi->power->gpio != -1){
		gpio_set_value(akm4345_spi->power->gpio, akm4345_spi->power->active_level);
		mdelay(20);
		gpio_set_value(akm4345_spi->power->gpio, !akm4345_spi->power->active_level);
		mdelay(1);
	}
...
static int akm4345_spi_probe(struct platform_device *pdev)
{
	...
	if (akm4345_spi->power->gpio != -1) {
                ret = gpio_request_one(akm4345_spi->power->gpio,
                                GPIOF_DIR_OUT, "akm4345-pdn");
                if (ret != 0) {
                        printk("Can't request pdn pin = %d\n",
                                        akm4345_spi->power->gpio);
                        return ret;
                } else
                        gpio_set_value(akm4345_spi->power->gpio, akm4345_spi->power->active_level);
        }
...
static int akm4345_spi_remove(struct platform_device *pdev)
{
	if (akm4345_spi->power->gpio != -1){
		gpio_set_value(akm4345_spi->power->gpio, akm4345_spi->power->active_level);
		gpio_free(akm4345_spi->power->gpio);
	}
	if (akm4345_spi->csn->gpio != -1)
