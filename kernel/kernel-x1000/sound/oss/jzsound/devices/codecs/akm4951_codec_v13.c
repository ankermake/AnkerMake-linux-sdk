...
static int codec_dac_setup(void)
{
	...
	struct akm4951_arch_data *akm4951_data =
		(struct akm4951_arch_data *)codec_platform_data->priv;

	if (akm4951_data && akm4951_data->register_array) {
		for (i = 0; i < akm4951_data->array_length; i++)
			ret |= akm4951_i2c_write_regs(akm4951_data->register_array[i][0],
					&akm4951_data->register_array[i][1], 1);
	} else {
		/* Init 0x02 ~ 0x4F registers */
		for (i = 0; i < sizeof(akm4951_registers) / sizeof(akm4951_registers[0]); i++)
			ret |= akm4951_i2c_write_regs(akm4951_registers[i][0],
					&akm4951_registers[i][1], 1);
	}
...
static int codec_init(void)
{
	int ret = 0;
	struct device *dev;
	struct extcodec_platform_data *akm4951;
	...
	/* reset PDN pin */
	if (akm4951->power->gpio != -1){
		gpio_set_value(akm4951->power->gpio, akm4951->power->active_level);
		msleep(20);
		gpio_set_value(akm4951->power->gpio, !akm4951->power->active_level);
		msleep(1);
	}
...
static int codec_set_replay_volume(int *val)
{
	...
	if (*val) {
		...
		if (akm4951_data && akm4951_data->volume_to_gain) {
			data = akm4951_data->volume_to_gain(*val);
		} else {
			/* use 0dB ~ -50dB */
			data = 100 - *val + 24;
		}
...
static int akm4951_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
        int ret = 0;
        struct device *dev = &client->dev;
        struct extcodec_platform_data *akm4951 = dev->platform_data;
	...
	if (akm4951->power->gpio != -1){
		ret = gpio_request_one(akm4951->power->gpio,
