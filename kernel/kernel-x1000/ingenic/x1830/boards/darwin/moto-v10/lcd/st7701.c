...
static int st7701s_power_on_begin(struct lcdc_gpio_struct *gpio)
{
...
static int st7701s_power_off_begin(struct lcdc_gpio_struct *gpio)
{
    pr_debug("%s:%s =====> %d\n",__FILE__,__func__,__LINE__);
...
static struct lcd_callback_ops st7701s_ops = {
    .lcd_power_on_begin     = (void*)st7701s_power_on_begin,
    .lcd_power_off_begin    = (void*)st7701s_power_off_begin,
};

#define ILI6122_LCD_X_RES   (480)
#define ILI6122_LCD_Y_RES   (854)

static struct fb_videomode jzfb_st7701s_videomode = {
    .name                   = "480x854",
    .refresh                = 45,
...
static struct jzfb_tft_config st7701s_cfg = {
    .pix_clk_inv            = 0,
    .de_dl                  = 0,
    .sync_dl                = 0,
    .color_even             = TFT_LCD_COLOR_EVEN_RGB,
    .color_odd              = TFT_LCD_COLOR_EVEN_RGB,
 ...
 * Power and Backlight
 */
struct st7701s_power {
    struct regulator *vlcdio;
    struct regulator *vlcdvcc;
    int inited;
};

static struct st7701s_power lcd_power = {
    NULL,
    NULL,
    0
};

static int st7701s_power_init(void)
{
    int ret = 0;
...
static int st7701s_power_reset(struct lcd_device *ld)
{
    pr_debug("%s:%s =====> %d\n",__FILE__,__func__,__LINE__);
...
static int st7701s_power_on(struct lcd_device *ld, int enable)
{
    if (!lcd_power.inited && st7701s_power_init())
        return -EFAULT;
    if (enable) {
        /* PowerON */
        if (GPIO_LCD_POWER_EN != -1) {
...
struct lcd_platform_data st7701s_pdata = {
    .reset                  = st7701s_power_reset,
...
{
    pr_debug("======lcd st7701s=====backlight==init=================\n");
