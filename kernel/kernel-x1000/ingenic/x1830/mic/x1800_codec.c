...
static int spk_gpio =  -1;//GPIO_PC(16);
...
static int delay_ms_1 = 10;
module_param(delay_ms_1, int, 0644);

static int delay_ms_2 = 10;
module_param(delay_ms_2, int, 0644);

static int delay_ms_3 = 10;
module_param(delay_ms_3, int, 0644);

static int delay_ms_4 = 10;
module_param(delay_ms_4, int, 0644);

static int delay_ms_5 = 10;
module_param(delay_ms_5, int, 0644);

static int delay_ms_6 = 10;
module_param(delay_ms_6, int, 0644);

static int delay_ms_7 = 10;
module_param(delay_ms_7, int, 0644);

static int delay_ms_8 = 10;
module_param(delay_ms_8, int, 0644);

static int delay_ms_9 = 10;
module_param(delay_ms_9, int, 0644);

static int delay_ms_10 = 180;
module_param(delay_ms_10, int, 0644);

static int enable_pa_power_before_hpout = 0;
module_param(enable_pa_power_before_hpout, int, 0644);
...
int codec_reg_get(unsigned int reg, int start, int end)
{
    int ret = 0;
    int i = 0, mask = 0;
    unsigned int oldv = 0;
...
int init_codec(void)
{
    ...
    codec_reg_set(TS_CODEC_CMCR_03, 5, 5, 1);

    /* Choose ADC,DAC I2S Master Mode */
    codec_reg_set(TS_CODEC_CMCR_03, 0, 7, 0x3e);
    codec_reg_set(TS_CODEC_CDCR1_04, 0, 7, 0x40 | 0x10);//DAC I2S interface is I2S Mode.
    // enable signal to control the I2S LRC Polarity of the DAC interface
    codec_reg_set(TS_CODEC_CDCR2_05, 0, 7, 0xe);

    /* codec precharge */
    codec_reg_set(TS_CODEC_CHCR_27, 6, 7, 1);
    msleep(10);
#ifdef CONFIG_SOC_X1800
	...
	codec_reg_set(TS_CODEC_CCR_28, 0, 6, 1);
#else
#error "no init codec, please add your code"
#endif
    msleep(20);
...
void codec_set_gain(int gain) {
    if (gain > 31)
        gain = 31;
    if (gain < 0)
       gain = 0;

    codec_reg_set(TS_CODEC_CACR2_23, 0, 4, gain);
    msleep(10);
}

static int codec_set_buildin_mic(void)
{
    /* ADC valid data length: 24bit */
    codec_reg_set(TS_CODEC_CACR_02, 5, 6, 0x2);

    /* Choose ADC I2S interface mode */
    codec_reg_set(TS_CODEC_CACR_02, 3, 4, 0x2);

    /* swap left-right channel data of adc i2s */
    codec_reg_set(TS_CODEC_CACR_02, 1, 1, 0x1);

    /* adc i2s mono type */
    codec_reg_set(TS_CODEC_CACR_02, 0, 0, 0x1);

    /* enable signal to control the I2S LRC Polarity of the ADC interface */
    ...
    msleep(10);
    /* record ALC gain 6dB */
    codec_reg_set(TS_CODEC_CACR2_23, 0, 4, 12);
    msleep(10);
    /* record fix alg gain 0dB */
    codec_reg_set(TS_CODEC_CMICCR_22, 5, 5, 0);
    msleep(10);

    codec_reg_set(TS_CODEC_CMICCR_22, 3, 3, 1);
    msleep(10);

    // codec_reg_set(TS_CODEC_CMICCR_22, 2, 2, 1);
    // msleep(10);

    codec_reg_set(TS_CODEC_CGAINR_0a, 0, 7, 0x08);
    msleep(10);

    return 0;
}

void codec_enable_pa_power(void)
{
...
void codec_disable_pa_power(void)
{
    if (codec_platform_data->gpio_spk_en.gpio > 0)
        gpio_direction_output(codec_platform_data->gpio_spk_en.gpio,
                              !codec_platform_data->gpio_spk_en.active_level);
...
#define mmsleep(x) if (x) msleep(x);
...
void codec_enable_hpout(void)
{
    ...
    if (enable_pa_power_before_hpout)
        codec_enable_pa_power();

    volume1 = codec_get_play_volume();

    volume2 = 0;
    codec_set_play_volume(&volume2);

    codec_reg_set(TS_CODEC_CAR_25, 5, 5, 0x1);
    mmsleep(delay_ms_1);
...
{
    codec_disable_pa_power();
    codec_reg_set(TS_CODEC_CHR_26, 7, 7, 0);
    codec_reg_set(TS_CODEC_CHR_26, 6, 6, 0);
    codec_reg_set(TS_CODEC_CHR_26, 5, 5, 0);
    mmsleep(10);

    codec_reg_set(TS_CODEC_CAR_25, 2, 2, 0x0);
    codec_reg_set(TS_CODEC_CAR_25, 1, 1, 0x0);
    codec_reg_set(TS_CODEC_CAR_25, 0, 0, 0x0);
    mmsleep(10);
}
...
static int codec_set_speaker(void)
{
    ...
    codec_reg_set(TS_CODEC_CAACR_21, 6, 6, 0x1);
    msleep(10);
    ...
    /* set replay volume */
    codec_reg_set(TS_CODEC_CHCR_27, 0, 4, codec_platform_data->hp_default_volume_regval);
#if 0
    if (codec_platform_data->gpio_spk_en.gpio > 0) {
...
void codec_set_play_volume(int * vol)
{
    ...
    if (volume == codec_platform_data->hp_min_volume_regval || volume == 0)
        codec_reg_set(TS_CODEC_CASR, 6, 7, 3);
    else
        codec_reg_set(TS_CODEC_CASR, 6, 7, 0);
}

int codec_get_play_volume(void)
{
    return codec_reg_get(TS_CODEC_CHCR_27, 0, 4);
}
...
{
    return codec_reg_get(TS_CODEC_CACR2_23, 0, 4);
}
...
void init_codec_playback(void)
{
    ...
    codec_set_buildin_mic();

    ret = codec_set_sample_rate(48000);
...
{
    *min_vol = codec_platform_data->hp_min_volume_regval;
    *max_vol = codec_platform_data->hp_max_volume_regval;
}
...
static int jz_codec_register(struct platform_device *pdev)
{
    ...
    if (!codec_platform_data->hp_max_volume_regval)
        codec_platform_data->hp_max_volume_regval = 0x1f;
    
    if (!codec_platform_data->hp_default_volume_regval)
        codec_platform_data->hp_default_volume_regval = 13;
