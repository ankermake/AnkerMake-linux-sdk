/* drivers/input/touchscreen/gt917s_extents.c
*
...
static ssize_t gt917s_gesture_data_read(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	s32 ret = -1;
	GTP_DEBUG("visit gt917s_gesture_data_read. ppos:%d", (int)*ppos);
	if (*ppos) {
		return 0;
	}
	if (size == 4) {
		ret = copy_to_user(((u8 __user *) page), "GT917S", 4);
		return 4;
	}
...
static ssize_t gt917s_gesture_data_write(struct file *filp, const char __user * buff, size_t len, loff_t * off)
{
	s32 ret = 0;
...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	/** package: -head 4B + track points + extra info-
		...
		*/
	ret = gt917s_i2c_read(GTP_REG_WAKEUP_GESTURE, doze_buf, 4);
	if (ret < 0) {
		return 0;
	}
	...
	if (extra_len >= 0) {
		...
		ret = gt917s_i2c_read(GTP_REG_WAKEUP_GESTURE + 4,
	...
	if (ges_type == 0 || !QUERYBIT(gestures_flag, ges_type)) {
		...
		doze_buf[0] = 0x00;
		gt917s_i2c_write(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);
		gesture_enter_doze();
		return 0;
	}
	...
	if (len > 0) { /* coor num * 4 + chksum 2*/
		...
		ret = gt917s_i2c_read(GES_BUFFER_ADDR, ges_data, len * 4);
		if (ret < 0) {
		...
		ret = gt917s_i2c_read(0x819F, &ges_data[len * 4], 2);
		if (ret < 0) {
			GTP_ERROR("Read gesture data failed.");
			return 0;
		}
...
void gt917s_gesture_debug(int on)
{
	if (on) {
		gesture_enabled = 1;
...
#define HOTKNOT_VERSION  "GOODIX,GT917S"
#define HN_AUTH_HEADER	14
#define GT917S_HN_JUMP_FW	"gt917s_hotknot_jump_fw.bin"
#define GT917S_HN_AUTH_FW	"gt917s_hotknot_auth_fw.bin"
...
static s32 hotknot_enter_transfer_mode(void)
{
	...
	gt917s_irq_disable();
	gt917s_send_cmd(GTP_CMD_HN_TRANSFER, 0);
	msleep(100);
	gt917s_irq_enable();

	ret = gt917s_i2c_read(0x8140, buffer, sizeof(buffer));
	if (ret) {
...
static int gt917s_load_auth_subsystem(void)
{
	const struct firmware *jump_fw, *auth_fw;
	int ret;

	GTP_INFO("Request %s", GT917S_HN_JUMP_FW);
	ret = request_firmware(&jump_fw, GT917S_HN_JUMP_FW,
			&gt917s_i2c_client->dev);
	if (ret < 0) {
		GTP_ERROR("Failed to get %s", GT917S_HN_JUMP_FW);
		return ret;
	...
	GTP_INFO("Request %s", GT917S_HN_AUTH_FW);
	ret = request_firmware(&auth_fw, GT917S_HN_AUTH_FW,
			&gt917s_i2c_client->dev);
	if (ret < 0) {
		GTP_ERROR("Failed to get %s", GT917S_HN_AUTH_FW);
		goto out1;
	...
	ret = gt917s_load_patch((u8 *)jump_fw->data, SZ_4K, 0, SZ_8K);
	if (ret < 0) {
	...
	ret = gt917s_load_patch((u8 *)&auth_fw->data[HN_AUTH_HEADER],
			SZ_4K, 4096, SZ_8K);
	if (ret < 0) {
...
static s32 hotknot_load_authentication_subsystem(void)
{
	...
	ret = gt917s_hold_ss51_dsp_no_reset();
	if (ret < 0) {
	...
	if (gt917s_chip_type == CHIP_TYPE_GT917S)
		ret = gt917s_load_auth_subsystem();
	else /* GT2X */
	...
	ret = gt917s_startup_patch();
	if (ret < 0) {
		GTP_ERROR("Startup auth system fail!");
		return ret;
	}

	ret = gt917s_i2c_read(GTP_REG_VERSION, buffer, 4);
	if (ret < 0) {
...
s32 hotknot_event_handler(u8 * data)
{
	...
	if (hotknot_paired_flag) {
		s32 ret = -1;
		ret = gt917s_i2c_read(GTP_REG_HN_STATE, hn_state_buf, 6);
		if (ret < 0) {
...
static s32 io_iic_read(u8 * data, void __user * arg)
{
	...
	err = gt917s_i2c_read(addr, &data[CMD_HEAD_LENGTH], data_length);
	if (!err) {
...
static s32 io_iic_write(u8 * data)
{
	...
	err = gt917s_i2c_write(addr, &data[CMD_HEAD_LENGTH], data_length);
	if (!err) {
		err = CMD_HEAD_LENGTH + data_length;
	}
...
//         error otherwise.
static long gt917s_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	u32 value = 0;
...
#ifdef CONFIG_COMPAT
static long gt917s_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static const struct file_operations gt917s_fops = {
	.owner = THIS_MODULE,
#ifdef CONFIG_GTP_GESTURE_WAKEUP
	.read = gt917s_gesture_data_read,
	.write = gt917s_gesture_data_write,
#endif
	.unlocked_ioctl = gt917s_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = gt917s_compat_ioctl,
#endif
};
...
s32 gt917s_init_node(void)
{
	...
	proc_entry = proc_create(GESTURE_NODE, 0666, NULL, &gt917s_fops);
	if (proc_entry == NULL) {
