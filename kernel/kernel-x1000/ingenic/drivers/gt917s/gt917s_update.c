/* drivers/input/touchscreen/gt917s_update.c
*
...
#define GT917S_FW_NAME				"gt917s_fw.bin"
#define UPDATE_FILE_PATH_1          "/data/_goodix_update_.bin"
#define UPDATE_FILE_PATH_2          "/sdcard/_goodix_update_.bin"

#define CONFIG_FILE_PATH_1          "/data/_gt917s_config_.cfg"
#define CONFIG_FILE_PATH_2          "/sdcard/_gt917s_config_.cfg"
...
1.  firmware structure
	...
	25               1              chip type                             0x91: GT917S,   0x92: GT2X
	26               6              reserved
...
int gt917s_update_prepare(char *filename);
int gt917s_check_firmware(void);
u8 *gt917s_get_fw_data(u32 offset, int length);
int gt917s_update_judge(void);
int gt917s_run_ss51_isp(u8 * ss51_isp, int length);
int gt917s_burn_subsystem(struct fw_subsystem_info *subsystem);
u16 gt917s_calc_checksum(u8 * fw, u32 length);
int gt917s_recall_check(u8 * chk_src, u16 start_rd_addr, u16 chk_length);
void gt917s_update_cleanup(void);
int gt917s_check_subsystem_in_flash(struct fw_subsystem_info *subsystem);
int gt917s_read_flash(u32 addr, int length);
int gt917s_error_erase(void);
void dump_to_file(u16 addr, int length, char *filepath);
int gt917s_update_firmware(void *filename);
int gt917s_auto_update_proc(void *data);
static int gt917s_search_update_files(void);

int gt917s_hold_ss51_dsp(void);
void gt917s_leave_update_mode(void);

/**
 * @return: return 0 if success, otherwise return a negative number
 *          which contains the error code.
 */
s32 gt917s_check_fs_mounted(char *path_name)
{
...
int gt917s_i2c_write_with_readback(u16 addr, u8 * buffer, int length)
{
	u8 buf[100];
	int ret = gt917s_i2c_write(addr, buffer, length);
	if (ret) {
		return ret;
	}
	ret = gt917s_i2c_read(addr, buf, length);
	if (ret) {
		return ret;
	}
...
int gt917s_auto_update_proc(void *data)
{
	...
	ret = gt917s_search_update_files();
	if (ret & (FOUND_FW_PATH_1 | FOUND_FW_PATH_2)) {
		...
		gt917s_update_firmware(filename);
	}
	...
	if (ret & (FOUND_CFG_PATH_1 | FOUND_CFG_PATH_2)) {
		...
		if (gt917s_parse_config(filename, config) > 0) {
			ret = gt917s_i2c_write(GTP_REG_CONFIG_DATA, config, GTP_CONFIG_ORG_LENGTH);
			if (ret < 0) {
...
static int gt917s_search_update_files(void)
{
	...
	while (retry-- > 0) {
		...
		if (gt917s_check_fs_mounted("/data")) {
...
void gt917s_enter_update_mode(void)
{
...
int gt917s_update_firmware(void *filename)
{
	...
	gt917s_enter_update_mode();

	ret = gt917s_update_prepare(filename);
	if (ret) {
		update_info.status = UPDATE_STATUS_ABORT;
		goto gt917s_update_exit;
	}

	ret = gt917s_check_firmware();
	if (ret) {
		update_info.status = UPDATE_STATUS_ABORT;
		goto gt917s_update_exit;
	}
	...
	p = gt917s_get_fw_data(update_info.firmware_info->subsystem[0].offset, update_info.firmware_info->subsystem[0].length);
	if (p == NULL) {
	...
	ret = gt917s_run_ss51_isp(p, update_info.firmware_info->subsystem[0].length);
	if (ret) {
	...
	for (i = 1; i < update_info.firmware_info->subsystem_count; i++) {
		...
		ret = gt917s_burn_subsystem(&(update_info.firmware_info->subsystem[i]));
		if (ret) {
	...
	for (i = 1; i < update_info.firmware_info->subsystem_count; i++) {
		...
		ret = gt917s_check_subsystem_in_flash(&(update_info.firmware_info->subsystem[i]));
		if (ret) {
			gt917s_error_erase();
			break;
		}
...
gt917s_update_exit:
	gt917s_update_cleanup();
	gt917s_leave_update_mode();
	gt917s_read_version(NULL);
	if (ret) {
		update_info.progress = 2 * update_info.max_progress;
		GTP_ERROR("Update firmware failed!");
		return ret;
	} else if (gt917s_init_failed) {
		gt917s_read_version(&gt917s_version);
		gt917s_init_panel();
	#ifdef CONFIG_GTP_CHARGER_SWITCH
		gt917s_parse_chr_cfg(gt917s_version.sensor_id);
	#endif
...
int gt917s_update_prepare(char *filename)
{
	...
	if (filename == NULL) {
		...
		update_info.fw = NULL;
		ret = request_firmware(&update_info.fw, GT917S_FW_NAME, &gt917s_i2c_client->dev);
		if (ret < 0) {
			GTP_ERROR("Request firmware failed - %s (%d)", GT917S_FW_NAME, ret);
			return ERROR_FW;
		}
	...
	if (retry <= 0) {
		ret = ERROR_RETRY;
		goto gt917s_update_pre_fail0;
	}

	return 0;

gt917s_update_pre_fail0:
	kfree(update_info.firmware_info);
gt917s_update_pre_fail1:
...
int gt917s_check_firmware(void)
{
	...
	// parse firmware
	p = gt917s_get_fw_data(0, FW_HEAD_SIZE);
	if (p == NULL) {
		return ERROR_FW;
	}
...
int gt917s_update_judge(void)
{
	...
	u8 reg_val[2] = {0};
	u8 retry = 2;
	struct gt917s_version_info ver_info;
	struct gt917s_version_info fw_ver_info;
	...
	do {
		...
		ret = gt917s_i2c_read_dbl_check(GTP_REG_FW_CHK_SUBSYS, &reg_val[1], 1);
		if (ret < 0) {
			goto _reset;
		} else if (ret > 0) {
			continue;
...
_reset:
	...
	ret = gt917s_read_version(&ver_info);
	if (ret < 0) {
...
int __gt917s_hold_ss51_dsp_20(void)
{
	int ret = -1;
	int retry = 0;
	u8 buf[1];
	...
	while (retry++ < 30) {
		...
		// Hold ss51 & dsp
		buf[0] = 0x0C;
		ret = gt917s_i2c_write(_rRW_MISCTL__SWRST_B0_, buf, 1);
		if (ret) {
		...
		// Confirm hold
		buf[0] = 0x00;
		ret = gt917s_i2c_read(_rRW_MISCTL__SWRST_B0_, buf, 1);
		if (ret) {
			GTP_ERROR("Hold ss51 & dsp I2C error,retry:%d", retry);
			continue;
		}
...
int gt917s_hold_ss51_dsp(void)
{
	...
	usleep_range(20000, 20010);
		ret =  gt917s_i2c_read(0x4220, buffer, 1);
	} while (retry-- && ret < 0);
	...
	//hold ss51_dsp
	ret = __gt917s_hold_ss51_dsp_20();
	if (ret) {
		return ret;
	}
	// enable dsp & mcu power
	buffer[0] = 0x00;
	ret = gt917s_i2c_write_with_readback(_bRW_MISCTL__DSP_MCU_PWR_, buffer, 1);
	if (ret) {
	...
	// disable watchdog
	buffer[0] = 0x00;
	ret = gt917s_i2c_write_with_readback(_bRW_MISCTL__TMR0_EN, buffer, 1);
	if (ret) {
	...
	// clear cache
	buffer[0] = 0x00;
	ret = gt917s_i2c_write_with_readback(_bRW_MISCTL__CACHE_EN, buffer, 1);
	if (ret) {
	...
	// soft reset
	buffer[0] = 0x01;
	ret = gt917s_i2c_write(_bWO_MISCTL__CPU_SWRST_PULSE, buffer, 1);
	if (ret) {
	...
	// set scramble
	buffer[0] = 0x00;
	ret = gt917s_i2c_write_with_readback(_rRW_MISCTL__BOOT_OPT_B0_, buffer, 1);
	if (ret) {
...
int gt917s_run_ss51_isp(u8 * ss51_isp, int length)
{
	...
	ret = gt917s_hold_ss51_dsp();
	if (ret) {
		return ret;
	}
	// select bank4
	buffer[0] = 0x04;
	ret = gt917s_i2c_write_with_readback(_bRW_MISCTL__SRAM_BANK, buffer, 1);
	if (ret) {
	...
	buffer[0] = 0x01;
	ret = gt917s_i2c_write_with_readback(_bRW_MISCTL__PATCH_AREA_EN_, buffer, 1);
	if (ret) {
		GTP_ERROR("enable patch area access fail!");
		return ret;
	}

	GTP_INFO("ss51_isp length: %d, checksum: 0x%04X", length, gt917s_calc_checksum(ss51_isp, length));
	// load ss51 isp
	ret = gt917s_i2c_write(0xC000, ss51_isp, length);
	if (ret) {
		GTP_ERROR("load ss51 isp fail!");
		return ret;
	}
	// recall compare
	ret = gt917s_recall_check(ss51_isp, 0xC000, length);
	if (ret) {
	...
	ret = gt917s_i2c_write_with_readback(0x8140, buffer, 10);
	...
	ret = gt917s_i2c_write_with_readback(0x8006, buffer, 8);
	if (ret) {
	...
	// release ss51
	buffer[0] = 0x08;
	ret = gt917s_i2c_write_with_readback(_rRW_MISCTL__SWRST_B0_, buffer, 1);
	if (ret) {
	...
	msleep(100);
	// check run state
	ret = gt917s_i2c_read(0x8006, buffer, 2);
	if (ret) {
...
int gt917s_recall_check(u8 * chk_src, u16 start_addr, u16 chk_length)
{
	u8 rd_buf[PACK_SIZE];
	s32 ret = 0;
	u16 len = 0;
	...
	while (chk_length > 0) {
		...
		ret = gt917s_i2c_read(start_addr + compared_length, rd_buf, len);
		if (ret) {
...
int gt917s_burn_subsystem(struct fw_subsystem_info *subsystem)
{
	...
	while (length > 0 && retry > 0) {
		...
		fw = gt917s_get_fw_data(subsystem->offset + burn_len, block_len);
		if (fw == NULL) {
			return ERROR_FW;
		}
		...
		checksum += block_len;
		checksum += cur_addr;
		checksum += gt917s_calc_checksum(fw, block_len);
		checksum = (0 - checksum);
		...
		ret = gt917s_i2c_write_with_readback(0x8100, buffer, 4);
		if (ret) {
			GTP_ERROR("write length & address fail!");
			continue;
		}

		ret = gt917s_i2c_write(0x8100 + 4, fw, block_len);
		if (ret) {
		...
		ret = gt917s_i2c_write_with_readback(0x8100 + 4 + block_len, buffer, 2);
		if (ret) {
		...
		buffer[0] = 0;
		ret = gt917s_i2c_write_with_readback(0x8022, buffer, 1);
		if (ret) {
		...
		ret = gt917s_i2c_write_with_readback(0x8020, buffer, 2);
		if (ret) {
		...
		while (wait_time-- > 0) {
			...
			ret = gt917s_i2c_read(0x8022, buffer, 1);
			if (ret < 0) {
				continue;
			}
			msleep(5);
			ret = gt917s_i2c_read(0x8022, &confirm, 1);
			if (ret < 0) {
				continue;
			}
			if (buffer[0] != confirm) {
...
int gt917s_check_subsystem_in_flash(struct fw_subsystem_info *subsystem)
{
	...
	while (length > 0) {
		...
		fw = gt917s_get_fw_data(subsystem->offset + checked_len, block_len);
		if (fw == NULL) {
			return ERROR_FW;
		}
		ret = gt917s_read_flash(subsystem->address + checked_len, block_len);
		if (ret) {
			check_state |= ret;
		}

		ret = gt917s_recall_check(fw, 0x8100, block_len);
		if (ret) {
...
int gt917s_read_flash(u32 addr, int length)
{
	...
	buffer[0] = 0xAA;
	buffer[1] = 0xAA;
	ret |= gt917s_i2c_write(0x8020, buffer, 2);
	if (ret) {
	...
	while (wait_time > 0) {
		wait_time--;
		msleep(5);
		ret = gt917s_i2c_read_dbl_check(0x8022, buffer, 1);
		if (ret) {
			continue;
		}
		if (buffer[0] == 0xBB) {
...
int gt917s_error_erase(void)
{
	...
	fw = gt917s_get_fw_data(update_info.firmware_info->subsystem[0].offset,
		update_info.firmware_info->subsystem[0].length);
	if (fw == NULL) {
		GTP_ERROR("get isp fail");
		return ERROR_FW;
	}
	ret = gt917s_run_ss51_isp(fw, update_info.firmware_info->subsystem[0].length);
	if (ret) {
		GTP_ERROR("run isp fail");
		return ERROR_PATH;
	}
	...
	while (retry-- > 0) {
		...
		ret = gt917s_recall_check(fw, 0x8100 + 4, block_len);
		if (ret) {
			continue;
		}
...
void dump_to_file(u16 addr, int length, char *filepath)
{
	...
	while (length > 0) {
		...
		memset(buf, 0x33, len);
		if (gt917s_i2c_read(addr + read_length, buf, len)) {
			memset(buf, 0x33, len);
		}
...
int gt917s_hold_ss51_dsp_no_reset(void)
{
	int ret = ERROR;
	u8 buffer[2];
...
#define GT917S_LOAD_PACKET_SIZE (1024 * 2)

int gt917s_load_patch(u8 * patch, u32 patch_size, int offset, int bank_size)
{
	s32 loaded_length = 0;
	s32 len = 0;
	s32 ret = 0;
	...
	GTP_INFO("Load patch code(size: %d, checksum: 0x%04X, position: 0x%04X, bank-size: %d", patch_size, gt917s_calc_checksum(patch, patch_size), 0xC000 + offset, bank_size);
	while (loaded_length != patch_size) {
		if (loaded_length == 0 || (loaded_length + offset) % bank_size == 0) {
			...
			ret = gt917s_i2c_write(_bRW_MISCTL__SRAM_BANK, &bank, 1);
			if (ret) {
			...
			// enable patch area access
			tmp = 0x01;
			ret = gt917s_i2c_write_with_readback(_bRW_MISCTL__PATCH_AREA_EN_ + bank - 4, &tmp, 1);
			if (ret) {
				GTP_ERROR("enable patch area access fail!");
				return ret;
			}
		...
		len = patch_size - loaded_length > GT917S_LOAD_PACKET_SIZE ? GT917S_LOAD_PACKET_SIZE : patch_size - loaded_length;
		address = 0xC000 + (loaded_length + offset) % bank_size;

		ret = gt917s_i2c_write(address, &patch[loaded_length], len);
		if (ret) {
			GTP_ERROR("load 0x%04X, %dbytes fail!", address, len);
			return ret;
		}
		ret = gt917s_recall_check(&patch[loaded_length], address, len);
		if (ret) {
...
int gt917s_startup_patch(void)
{
	s32 ret = 0;
	u8 buffer[8] = { 0x55 };
	buffer[0] = 0x00;
	buffer[1] = 0x00;
	ret |= gt917s_i2c_write(_bRW_MISCTL__PATCH_AREA_EN_, buffer, 2);

	memset(buffer, 0x55, 8);
	ret |= gt917s_i2c_write(GTP_REG_FLASH_PASSBY, buffer, 8);
	ret |= gt917s_i2c_write(GTP_REG_VERSION, buffer, 5);

	buffer[0] = 0xAA;
	ret |= gt917s_i2c_write(GTP_REG_CMD, buffer, 1);
	ret |= gt917s_i2c_write(GTP_REG_ESD_CHECK, buffer, 1);

	buffer[0] = 0x00;
	ret |= gt917s_i2c_write(_rRW_MISCTL__SWRST_B0_, buffer, 1);
