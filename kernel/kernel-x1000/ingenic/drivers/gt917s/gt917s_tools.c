...
static ssize_t gt917s_tool_read(struct file *filp, char __user * buffer, size_t count, loff_t * ppos);
static ssize_t gt917s_tool_write(struct file *filp, const char *buffer, size_t count, loff_t * ppos);


static int gt917s_tool_release(struct inode *inode, struct file *filp);
static int gt917s_tool_open(struct inode *inode,struct file *file);
...
static struct proc_dir_entry *gt917s_tool_proc_entry;
static struct file_operations gt917s_tool_fops = {
	.read = gt917s_tool_read,
	.write = gt917s_tool_write,
.open = gt917s_tool_open,
.release = gt917s_tool_release,
.owner = THIS_MODULE,
};
...
int gt917s_init_tool_node(void)
{
	...
	gt917s_tool_proc_entry = proc_create(procname, 0666, NULL, &gt917s_tool_fops);
	if (gt917s_tool_proc_entry == NULL) {
...
void gt917s_deinit_tool_node(void)
{
...
static s32 tool_i2c_read(u8 * buf, u16 len)
{
	...
	if (!gt917s_i2c_read(addr, &buf[2], len)) {
		return 1;
	}
	return -1;
}
...
{
	u16 addr = (buf[0] << 8) + buf[1];
	if (!gt917s_i2c_write(addr, &buf[2], len - 2)) {
		return 1;
	}
	return -1;
}
...
Output:
	Return write length.
********************************************************/
static ssize_t gt917s_tool_write(struct file *filp, const char __user * buff, size_t len, loff_t * data)
{
	u64 ret = 0;
	GTP_DEBUG_FUNC();
	...
	*/
	...
	if (1 == cmd_head.wr) {
	...
	} else if (3 == cmd_head.wr) {	//gt917s unused
		...
		if (cmd_head.data[GTP_ADDR_LENGTH]) {
			...
			gt917s_rawdiff_mode = true;
		} else {
			gt917s_rawdiff_mode = false;
		...
		GTP_DEBUG("update firmware, filename: %s", cmd_head.data);
			thrd = kthread_run(gt917s_update_firmware, (void *)cmd_head.data, "GT917S FW Update");
			if (IS_ERR(thrd)) {
					return PTR_ERR(thrd);
			}
	}
