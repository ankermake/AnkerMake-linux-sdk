/* drivers/input/touchscreen/gt917s_generic.c
*
...
extern struct gt917S_platform_data *gt917S_pdata;
static struct workqueue_struct *gt917s_workqueue;
struct i2c_client *gt917s_i2c_client;
static u8 gt917s_config[GTP_CONFIG_ORG_LENGTH + GTP_CONFIG_EXT_LENGTH];
static u32 gt917s_cfg_length = GTP_CONFIG_MAX_LENGTH;
gt917s_chip_type_t gt917s_chip_type = CHIP_TYPE_NONE;
struct gt917s_version_info gt917s_version = {{0}};
#ifdef CONFIG_GTP_HAVE_TOUCH_KEY
const u16 gt917s_touch_key_array[] = GTP_KEY_TAB;
#endif
#if defined(CONFIG_GTP_WITH_STYLUS) && defined(CONFIG_GTP_HAVE_STYLUS_KEY)
static const u16 gt917s_stylus_key_array[] = GTP_STYLUS_KEY_TAB;
#endif
#define GOODIX_SYSFS_DIR		"goodix"
static struct kobject *sysfs_rootdir = NULL;

volatile int gt917s_rawdiff_mode = 0;
static u8 gt917s_wakeup_level = 0;
u8 gt917s_init_failed = 0;
u8 gt917s_int_type = 0;
u32 gt917s_abs_x_max = 0;
u32 gt917s_abs_y_max = 0;
int gt917s_halt = 0;

static ssize_t gt917s_debug_read_proc(struct file *, char __user *, size_t,loff_t *);
static ssize_t gt917s_debug_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *gt917s_debug_proc_entry = NULL;
static const struct file_operations gt917s_debug_fops = {
	.owner = THIS_MODULE,
	.read = gt917s_debug_read_proc,
	.write = gt917s_debug_write_proc,
};

static s32 gt917s_init_debug_node(void)
{
	gt917s_debug_proc_entry = proc_create(GT917S_DEBUG_PROC_FILE,
			0660, NULL, &gt917s_debug_fops);
	if (gt917s_debug_proc_entry == NULL) {
		GTP_ERROR("Create proc entry /proc/%s FAILED!",
				GT917S_DEBUG_PROC_FILE);
		return -1;
	}
	GTP_INFO("Created proc entry /proc/%s.", GT917S_DEBUG_PROC_FILE);
	return 0;
}

static void gt917s_deinit_debug_node(void)
{
	if (gt917s_debug_proc_entry != NULL) {
		remove_proc_entry(GT917S_DEBUG_PROC_FILE, NULL);
	}
}

static ssize_t gt917s_debug_read_proc(struct file *file, char __user * page,
		size_t size, loff_t * ppos)
{
	char *ptr = page;
	char temp_data[gt917s_cfg_length];
	...
	ptr += sprintf(ptr, "==== GT917S default config setting in driver====\n");

	for (i = 0; i < gt917s_cfg_length; i++) {
		ptr += sprintf(ptr, "0x%02X,", gt917s_config[i]);
	...
	ptr += sprintf(ptr, "==== GT917S config read from chip====\n");
	i = gt917s_i2c_read(GTP_REG_CONFIG_DATA, temp_data,
			GTP_CONFIG_ORG_LENGTH);
	if (i == 0 && gt917s_cfg_length == GTP_CONFIG_ORG_LENGTH +
			GTP_CONFIG_EXT_LENGTH) {
		i = gt917s_i2c_read(GTP_REG_EXT_CONFIG,
	...
	ptr += sprintf(ptr, "\n");
	/* Touch PID & VID */
	ptr += sprintf(ptr, "==== GT917S Version Info ====\n");

	gt917s_i2c_read(GTP_REG_VERSION, temp_data, 12);
	...
	irq_desc = irq_to_desc(gt917s_i2c_client->irq);
	if (irq_desc) {
		ptr += sprintf(ptr, "IRQ: %d, irq_desc->disable-depth:%d\n",
				gt917s_i2c_client->irq, irq_desc->depth);
	}
	*ppos += ptr - page;
	return (ptr - page);
}

static ssize_t gt917s_debug_write_proc(struct file *file,
		const char *buffer, size_t count, loff_t * ppos)
{
	s32 ret = 0;
	u8 buf[gt917s_cfg_length];
	char mode_str[50] = { 0 };
	int mode;
	int cfg_len;
	char arg1[50] = { 0 };
	u8 temp_config[gt917s_cfg_length];

	GTP_DEBUG("write count %ld\n", (unsigned long)count);

	if (count > gt917s_cfg_length) {
		GTP_ERROR("Too much data, buffer size: %d, data:%ld",
				gt917s_cfg_length, (unsigned long)count);
		return -EFAULT;
	}
	...
	// send config
	if (count == gt917s_cfg_length) {
		memcpy(gt917s_config, buf, count);
		ret = gt917s_send_cfg(gt917s_config, gt917s_cfg_length);
		if (ret < 0) {
			GTP_ERROR("send gt917s_config failed.");
			return -EFAULT;
		}
		gt917s_abs_x_max = (gt917s_config[RESOLUTION_LOC + 1] << 8) +
				gt917s_config[RESOLUTION_LOC];
		gt917s_abs_y_max = (gt917s_config[RESOLUTION_LOC + 3] << 8) +
				gt917s_config[RESOLUTION_LOC + 2];
	...
	//force clear gt917s_config
	if (strcmp(mode_str, "clear_config") == 0) {
		GTP_INFO("Force clear gt917s_config");
		gt917s_send_cmd(GTP_CMD_CLEAR_CFG, 0);
		return count;
	}
	...
	if (strcmp(mode_str, "poweron") == 0) {
		gt917s_power_switch(1);
		return count;
	}

	if (strcmp(mode_str, "poweroff") == 0) {
		gt917s_power_switch(0);
		return count;
	}

	if (strcmp(mode_str, "version") == 0) {
		gt917s_read_version(NULL);
		return count;
	}
	...
	if (strcmp(mode_str, "update") == 0) {
		if(strcmp(arg1, "request") == 0){
			...
			gt917s_update_firmware(NULL);
		}else{
			gt917s_update_firmware(arg1);
		}
		return count;
	}

	if (strcmp(mode_str, "sendconfig") == 0) {
		cfg_len = gt917s_parse_config(arg1, temp_config);
		if (cfg_len < 0) {
			return -1;
		}
		gt917s_send_cfg(temp_config, gt917s_cfg_length);
		return count;
	}

	if (strcmp(mode_str, "debug_gesture") == 0) {
#ifdef CONFIG_GTP_GESTURE_WAKEUP
		gt917s_gesture_debug(!!mode);
#endif
	}
	...
	return gt917s_debug_proc(buf, count);
}
...
int gt917s_parse_config(char *filename, u8 * config)
{
	mm_segment_t old_fs;
	struct file *fp = NULL;
	u8 *buf;
	...
	if (len > gt917s_cfg_length * 6 || len < gt917s_cfg_length) {
	...
	for (i = 0, cur_len = 0; i < len && cur_len < gt917s_cfg_length;) {
...
s32 _do_i2c_read(struct i2c_msg * msgs, u16 addr, u8 * buffer, s32 len)
{
	...
	while (pos != data_length) {
		...
		ret = i2c_transfer(gt917s_i2c_client->adapter, msgs, 2);
		if (ret != 2) {
...
s32 _do_i2c_write(struct i2c_msg * msg, u16 addr, u8 * buffer, s32 len)
{
	...
	while (pos != data_length) {
		...
		ret = i2c_transfer(gt917s_i2c_client->adapter, msg, 1);
		if (ret != 1) {
			GTP_ERROR("I2c transfer error! (%d)", ret);
			kfree(data);
			return ERROR_IIC;
...
/**
 * gt917s_i2c_read_dbl_check - read twice and double check
 * @addr: register address
 * @buffer: data buffer
 * @len: bytes to read
 * Return <0: i2c error, 0: ok, 1:fail
 */
s32 gt917s_i2c_read_dbl_check(u16 addr, u8 * buffer, s32 len)
{
	u8 buf[16] = { 0 };
	u8 confirm_buf[16] = { 0 };
	int ret;
	...
	ret = gt917s_i2c_read(addr, buf, len);
	if (ret < 0) {
		return ret;
	}
	...
	ret = gt917s_i2c_read(addr, confirm_buf, len);
	if (ret < 0) {
		return ret;
	}
...
/**
 * gt917s_get_info - Get information from ic, such as resolution and
 * int trigger type
 * Return    <0: i2c failed, 0: i2c ok
 */
s32 gt917s_get_info(void)
{
	u8 opr_buf[4] = { 0 };
	s32 ret = 0;

	ret = gt917s_i2c_read(GTP_REG_CONFIG_DATA + 1, opr_buf, 4);
	if (ret < 0) {
		return ret;
	}

	gt917s_abs_x_max = (opr_buf[1] << 8) + opr_buf[0];
	gt917s_abs_y_max = (opr_buf[3] << 8) + opr_buf[2];

	ret = gt917s_i2c_read(GTP_REG_CONFIG_DATA + 6, opr_buf, 1);
	if (ret < 0) {
		return ret;
	}
	gt917s_int_type = opr_buf[0] & 0x03;

	GTP_INFO("X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x",
			gt917s_abs_x_max, gt917s_abs_y_max, gt917s_int_type);

	return 0;
}

/**
 * gt917s_send_cfg - Send gt917s_config Function.
 * @config: pointer of the configuration array.
 * @cfg_len: length of configuration array.
 * Return 0--success,non-0--fail.
 */
s32 gt917s_send_cfg(u8 * config, int cfg_len)
{
	...
	if (config[GTP_REG_EXT_CFG_FLAG - GTP_REG_CONFIG_DATA] & 0x40) {
		...
		*/
		...
		do {
			ret = gt917s_i2c_write(GTP_REG_EXT_CONFIG,
	...
	retry = 0;
	while (retry++ < 3) {
		ret = gt917s_i2c_write(GTP_REG_CONFIG_DATA, config, cfg_len);
		if (!ret) {
...
#ifdef CONFIG_OF
/**
 * gt917s_parse_tp_config - get config data of touch panel
 * @of_node: device node
 ...
 */
int gt917s_find_tp_config(struct device *dev,
...
/**
 * gt917s_init_panel - Prepare config data for touch ic,
 * don't call this function after initialization.
 *
 * Return 0--success,<0 --fail.
 */
s32 gt917s_init_panel(void)
{
	u16 cfg_len = 0;
	s32 ret = 0;
...
#else
	struct device *dev = &gt917s_i2c_client->dev;
#endif

	cfg_len = gt917s_find_tp_config(dev, "default-config",
			gt917s_version.sensor_id, gt917s_config);
	if (cfg_len < 0) {
	...
	* the_config of driver. */
	gt917s_config[0] &= 0x7F;

#ifdef CONFIG_GTP_CUSTOM_CFG
	gt917s_config[RESOLUTION_LOC] = (u8) gt917S_pdata->x;
	gt917s_config[RESOLUTION_LOC + 1] = (u8) (gt917S_pdata->x >> 8);
	gt917s_config[RESOLUTION_LOC + 2] = (u8) gt917S_pdata->y;
	gt917s_config[RESOLUTION_LOC + 3] = (u8) (gt917S_pdata->y >> 8);

	if (GTP_INT_TRIGGER == 0) {	/* RISING  */
		gt917s_config[TRIGGER_LOC] &= 0xfe;
	} else if (GTP_INT_TRIGGER == 1) {	/* FALLING */
		gt917s_config[TRIGGER_LOC] |= 0x01;
	}
	set_reg_bit(gt917s_config[MODULE_SWITCH3_LOC], 5, !gt917s_wakeup_level);
#endif /* END GTP_CUSTOM_CFG */

#else /* DRIVER NOT SEND CONFIG */
	cfg_len = GTP_CONFIG_ORG_LENGTH;
	ret = gt917s_i2c_read(GTP_REG_CONFIG_DATA, gt917s_config, cfg_len);
	if (ret < 0)
		return ret;

	if (gt917s_config[GTP_REG_EXT_CFG_FLAG  - GTP_REG_CONFIG_DATA] & 0x40) {
		ret = gt917s_i2c_read(GTP_REG_EXT_CONFIG,
			&gt917s_config[cfg_len], GTP_CONFIG_EXT_LENGTH);
		if (ret < 0)
			return ret;
	...
	/* match resolution when gt917s_abs_x_max & gt917s_abs_y_max
	* have been set already */
	if ((gt917s_abs_x_max == 0) && (gt917s_abs_y_max == 0)) {
		gt917s_abs_x_max = (gt917s_config[RESOLUTION_LOC + 1] << 8) +
				gt917s_config[RESOLUTION_LOC];
		gt917s_abs_y_max = (gt917s_config[RESOLUTION_LOC + 3] << 8) +
			gt917s_config[RESOLUTION_LOC + 2];
		gt917s_int_type = (gt917s_config[TRIGGER_LOC]) & 0x03;
		gt917s_wakeup_level = !(gt917s_config[MODULE_SWITCH3_LOC] & 0x20);
	} else {
		gt917s_config[RESOLUTION_LOC] = (u8) gt917s_abs_x_max;
		gt917s_config[RESOLUTION_LOC + 1] = (u8) (gt917s_abs_x_max >> 8);
		gt917s_config[RESOLUTION_LOC + 2] = (u8) gt917s_abs_y_max;
		gt917s_config[RESOLUTION_LOC + 3] = (u8) (gt917s_abs_y_max >> 8);
		set_reg_bit(gt917s_config[MODULE_SWITCH3_LOC], 5, !gt917s_wakeup_level);
		gt917s_config[TRIGGER_LOC] = (gt917s_config[TRIGGER_LOC] & 0xFC) | gt917s_int_type;
	}

	GTP_INFO("X_MAX=%d,Y_MAX=%d,TRIGGER=0x%02x,WAKEUP_LEVEL=%d",
			gt917s_abs_x_max, gt917s_abs_y_max, gt917s_int_type, gt917s_wakeup_level);

	gt917s_cfg_length = cfg_len;
	ret = gt917s_send_cfg(gt917s_config, gt917s_cfg_length);
	return ret;
}

void gt917s_select_addr(void)
{
	GTP_GPIO_OUTPUT(gt917S_pdata->gpio_shutdown, 0);
#ifdef CONFIG_GTP_INT_SEL_SYNC
	GTP_GPIO_OUTPUT(gt917S_pdata->gpio_int, gt917s_i2c_client->addr == 0x14);
#endif
	usleep_range(3000, 3030);
	GTP_GPIO_OUTPUT(gt917S_pdata->gpio_shutdown, 1);
	usleep_range(2000, 2030);
}

static s32 gt917s_set_reset_status(void)
{
	/* 0x8040 ~ 0x8043 */
	u8 value[] = {0xAA, 0x00, 0x56, 0xAA};
	int ret;

	ret = gt917s_i2c_write(GTP_REG_CMD + 1, &value[1], 3);
	if (ret < 0)
		return ret;

	return gt917s_i2c_write(GTP_REG_CMD, value, 1);
}

#ifdef CONFIG_GTP_INCELL_PANEL
int gt917s_write_and_readback(u16 addr, u8 * buffer, s32 len)
{
	int ret;
	u8 d[len];

	ret = gt917s_i2c_write(addr, buffer, len);
	if (ret < 0)
		return -1;

	ret = gt917s_i2c_read(addr, d, len);
...
#define RST_RETRY       5
	...
	do {
		...
		/* test i2c */
		ret = gt917s_i2c_read(0x4220, d, 1);
	...
	do {
		d[0] = 0x0C;
		ret = gt917s_write_and_readback(0x4180, d, 1);
	} while (--retry && ret < 0);
	...
	do {
		d[0] = 0x00;
		ret = gt917s_write_and_readback(0x4305, d, 1);
		if (ret < 0)
			continue;

		d[0] = 0x2B;
		d[1] = 0x24;
		ret = gt917s_write_and_readback(0x42c4, d, 2);
		if (ret < 0)
			continue;

		d[0] = 0xE1;
		d[1] = 0xD3;
		ret = gt917s_write_and_readback(0x42e4, d, 2);
		if (ret < 0)
			continue;
...
*/
#ifdef CONFIG_GTP_INT_SEL_SYNC
	GTP_GPIO_OUTPUT(gt917S_pdata->gpio_int, 0);
	msleep(50);
	GTP_GPIO_AS_INT(gt917S_pdata->gpio_int);
#else
	/* firmware init */
	msleep(50);
#endif

	ret = gt917s_set_reset_status();
	return ret;
}

/**
 * gt917s_read_version - Read gt917s version info.
 * @ver_info: address to store version info
 * Return 0-succeed.
 */
s32 gt917s_read_version(struct gt917s_version_info * ver_info)
{
	s32 ret = -1;
	u8 buf[12] = { 0 };
	u32 mask_id = 0;
	...
	while (retry--) {
		ret = gt917s_i2c_read_dbl_check(GTP_REG_VERSION, buf, sizeof(buf));
		if (!ret) {
			checksum = 0;
...
/**
 * gt917s_get_chip_type - get chip type .
 *
 * different chip synchronize in different way,
 */
s32 gt917s_get_chip_type(void)
{
	u8 opr_buf[4] = { 0x00 };
	u8 gt917s_data[] = { 0x02, 0x08, 0x90, 0x00 };
	u8 gt9l_data[] = { 0x03, 0x10, 0x90, 0x00 };
	s32 ret = -1;

	/* chip type already exist */
	if (gt917s_chip_type != CHIP_TYPE_NONE) {
		return 0;
	}

	/* read hardware */
	ret = gt917s_i2c_read_dbl_check(GTP_REG_HW_INFO, opr_buf, sizeof(opr_buf));
	if (ret) {
	...
	/* find chip type */
	if (!memcmp(opr_buf, gt917s_data, sizeof(gt917s_data))) {
		gt917s_chip_type = CHIP_TYPE_GT917S;
	} else if (!memcmp(opr_buf, gt9l_data, sizeof(gt9l_data))) {
		gt917s_chip_type = CHIP_TYPE_GT2X;
	}
...
/**
 * gt917s_enter_sleep - Eter sleep function.
 *
 * Returns  0--success,non-0--fail.
 */
static s32 gt917s_enter_sleep(void)
{
...
#else
	{
		...
		while (retry++ < 3) {
			if (!gt917s_send_cmd(GTP_CMD_SLEEP, 0)) {
...
/**
 * gt917s_wakeup_sleep - wakeup from sleep mode Function.
 *
 * Return: 0--success,non-0--fail.
 */
static s32 gt917s_wakeup_sleep(void)
{
	...
	while (retry++ < 2) {
		...
		if (gesture_enabled) {
			...
			disable_irq_wake(gt917s_i2c_client->irq);
#endif
...
#endif
		{
			...
			GTP_GPIO_OUTPUT(gt917S_pdata->gpio_int, gt917s_wakeup_level);
			msleep(5);
 ...
 */
s32 gt917s_send_cmd(u8 cmd, u8 data)
{
	s32 ret;
	...
	ret = gt917s_i2c_write(GTP_REG_CMD + 1, &buffer[1], 2);
	ret |= gt917s_i2c_write(GTP_REG_CMD, &buffer[0], 1);
	msleep(50);
...
void gt917s_power_reset(void)
{
	...
	gt917s_power_switch(SWITCH_OFF);
	msleep(30);
	gt917s_power_switch(SWITCH_ON);

	for (i = 0; i < 5; i++) {
		if(gt917s_reset_guitar()) {
			continue;
		}
		if(gt917s_send_cfg(gt917s_config, gt917s_cfg_length)) {
			msleep(500);
			continue;
		}
		break;
...
s32 gt917s_request_event_handler(void)
{
	s32 ret = -1;
	u8 rqst_data = 0;

	ret = gt917s_i2c_read(GTP_REG_RQST, &rqst_data, 1);
	if (ret) {
	...
	case GTP_RQST_CONFIG:
		...
		ret = gt917s_send_cfg(gt917s_config, gt917s_cfg_length);
		if (ret) {
			GTP_ERROR("Send gt917s_config error.");
		} else {
			GTP_INFO("Send gt917s_config success.");
			rqst_data = GTP_RQST_RESPONDED;
			gt917s_i2c_write(GTP_REG_RQST, &rqst_data, 1);
		}
		break;
	case GTP_RQST_RESET:
...
/**
 * gt917s_touch_event_handler - handle touch event
 * (pen event, key event, finger touch envent)
 * @data:
 * Return    <0: failed, 0: succeed
 */
s32 gt917s_touch_event_handler(u8 * data, struct input_dev * dev,
		struct input_dev * pen_dev)
{
	...
	s32 i = 0;
	s32 ret = -1;
	s32 fw_resolution_x = 0, fw_resolution_y = 0;
	u8 re_buf[4] = { 0 };
	s32 Ret = 0;

	Ret = gt917s_i2c_read(GTP_REG_CONFIG_DATA + 1, re_buf, 4);
	if (Ret < 0) {
		return Ret;
	}

	fw_resolution_x = (re_buf[1] << 8) + re_buf[0];
	fw_resolution_y = (re_buf[3] << 8) + re_buf[2];
	...
	/* read the remaining coor data
		...
		*/
	if (touch_num > 1) {
		ret = gt917s_i2c_read((GTP_READ_COOR_ADDR + 11),
	...
	if (check_sum) { /* checksum error*/
		ret = gt917s_i2c_read(GTP_READ_COOR_ADDR,
...
/* start handle current event and pre-event */
	...
	if (CHK_BIT(cur_event, BIT_STYLUS_KEY) || CHK_BIT(pre_event, BIT_STYLUS_KEY)) {
		...
		*/
		...
		for (i = 4; i < 6; i++) {
			input_report_key(pen_dev, gt917s_stylus_key_array[i - 4],
					temp & (0x01 << i));
		}
	...
	if (CHK_BIT(cur_event, BIT_STYLUS)) {
		...
		input_x = GTP_WARP_X(gt917s_abs_x_max, input_x);
		input_y = GTP_WARP_Y(gt917s_abs_y_max, input_y);

		GTP_DEBUG("Pen touch DOWN.");
		gt917s_pen_down(input_x, input_y, input_w, 0);
	...
	if (CHK_BIT(cur_event, BIT_TOUCH_KEY) || CHK_BIT(pre_event, BIT_TOUCH_KEY)) {
		for (i = 0; i < GTP_MAX_KEY_NUM; i++) {
			input_report_key(dev, gt917s_touch_key_array[i],
					key_value & (0x01 << i));
		}
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				input_x = GTP_WARP_X(gt917s_abs_x_max, input_x);
				input_y = GTP_WARP_Y(gt917s_abs_y_max, input_y);
				input_x = input_x * gt917S_pdata->x / fw_resolution_x;
				input_y = input_y * gt917S_pdata->y / fw_resolution_y;

				GTP_DEBUG("(%d)(%d,%d)[%d]", id, input_x, input_y, input_w);
				gt917s_touch_down(input_x, input_y, input_w, i);
...
static void gt917s_pen_init(void)
{
	s32 ret = 0;
	...
	input_set_abs_params(pen_dev, ABS_MT_POSITION_X, 0, gt917s_abs_x_max, 0, 0);
	input_set_abs_params(pen_dev, ABS_MT_POSITION_Y, 0, gt917s_abs_y_max, 0, 0);
...
void gt917s_pen_down(s32 x, s32 y, s32 size, s32 id)
{
...
static struct gt917s_ps_device *gt917s_ps_dev;

void gt917s_ps_report(int state)
{
#ifdef PLATFORM_MTK
	s32 ret = -1;
...
#else
	input_report_abs(gt917s_ps_dev->input_dev, ABS_DISTANCE, !!state);
	input_sync(gt917s_ps_dev->input_dev);
...
static s32 gt917s_ps_enable(s32 enable)
{
	u8 state;
	s32 ret = -1;
	...
	state = enable ? 1 : 0;
	if (gt917s_chip_type == CHIP_TYPE_GT917S)
		ret = gt917s_i2c_write(GTP_REG_PROXIMITY_ENABLE, &state, 1);
	else if (gt917s_chip_type == CHIP_TYPE_GT2X)
		ret = gt917s_send_cmd(state ? 0x12 : 0x13, 0);
	if (ret) {
	...
	if (!ret && enable) {
		gt917s_ps_dev->enabled = 1;
	} else {
		gt917s_ps_dev->enabled = 0;
	}
	gt917s_ps_dev->state = PS_FARAWAY;
	GTP_INFO("Proximity function %s %s.",
			state ? "enable" : "disable", ret ? "fail" : "success");
	return ret;
}

int gt917s_prox_event_handler(u8 * data)
{
	u8 ps = 0;

	if (gt917s_ps_dev && gt917s_ps_dev->enabled) {
		ps = (data[0] & 0x60) ? 0 : 1;
		if (ps != gt917s_ps_dev->state) {
			gt917s_ps_report(ps);
			gt917s_ps_dev->state = ps;
...
#ifdef PLATFORM_MTK
static inline s32 gt917s_get_ps_value(void)
{
	return gt917s_ps_dev->state;
}

static s32 gt917s_ps_operate(void *self, u32 command, void *buff_in,
		s32 size_in, void *buff_out, s32 size_out, s32 * actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(sensor_size))) {
			...
			sensor_data->values[0] = gt917s_get_ps_value();
			sensor_data->value_divide = 1;
			sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
		}
...
#ifndef PLATFORM_MTK
static ssize_t gt917s_ps_enable_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d",
			gt917s_ps_dev->enabled);
}

static ssize_t gt917s_ps_enable_store(struct kobject *kobj,
...
static ssize_t gt917s_ps_state_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d", gt917s_ps_dev->state);
}

static ssize_t gt917s_ps_state_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	if(sscanf(buf, "%u", &input) != 1) {
	...
	if(input == 1) {
		...
		gt917s_ps_dev->state = PS_NEAR;
	} else {
		return -EINVAL;
	}

	gt917s_ps_report(gt917s_ps_dev->state);
	return count;
}

static struct kobj_attribute ps_attrs[] = {
	__ATTR(enable, S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP,
			gt917s_ps_enable_show, gt917s_ps_enable_store),
	__ATTR(state, S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP,
			gt917s_ps_state_show, gt917s_ps_state_store)
};

#endif /* End PLATFORM_MTK */

static int gt917s_ps_init(void)
{
	int err;

	gt917s_ps_dev = kzalloc(sizeof(struct gt917s_ps_device), GFP_KERNEL);
	if (!gt917s_ps_dev) {
		return  -ENOMEM;
	}
...
#ifdef PLATFORM_MTK
	gt917s_ps_dev->obj_ps.polling = 0;
	gt917s_ps_dev->obj_ps.sensor_operate = gt917s_ps_operate;

	if ((err = hwmsen_attach(ID_PROXIMITY, &gt917s_ps_dev->obj_ps))) {
	...
	return 0;
#else
	gt917s_ps_dev->input_dev = input_allocate_device();
	if(!gt917s_ps_dev->input_dev) {
		GTP_ERROR("Failed to alloc inpput device for proximity!");
		err = -ENOMEM;
		goto err_exit;
	}

	gt917s_ps_dev->input_dev->name = GTP_PS_DEV_NAME;
	gt917s_ps_dev->input_dev->phys = "goodix/proximity";
	gt917s_ps_dev->input_dev->id.bustype = BUS_I2C;
	gt917s_ps_dev->input_dev->id.vendor = 0xDEED;
	gt917s_ps_dev->input_dev->id.product = 0xBEEF;
	gt917s_ps_dev->input_dev->id.version = 1;
	set_bit(EV_ABS, gt917s_ps_dev->input_dev->evbit);
	input_set_abs_params(gt917s_ps_dev->input_dev, ABS_DISTANCE, 0, 1, 0, 0);

	err = input_register_device(gt917s_ps_dev->input_dev);
	if(err) {
		GTP_ERROR("Failed to register proximity input device: %s!",
				gt917s_ps_dev->input_dev->name);
	...
	gt917s_ps_dev->kobj = kobject_create_and_add("proximity", sysfs_rootdir);
	if(!gt917s_ps_dev->kobj){
	...
	//  create sysfs files
	{
		...
		for(i = 0; i < sizeof(ps_attrs)/sizeof(ps_attrs[0]); i++) {
			if((err = sysfs_create_file(gt917s_ps_dev->kobj, &ps_attrs[i].attr))) {
	...
	return 0;
err_create_file:
	kobject_put(gt917s_ps_dev->kobj);
err_register_dev:
	input_free_device(gt917s_ps_dev->input_dev);
#endif  /* End PLATFROM_MTK */
...
static void gt917s_ps_deinit(void)
{
	if(gt917s_ps_dev) {
#ifndef PLATFORM_MTK
		int i = 0;
		for(; i < sizeof(ps_attrs) / sizeof(ps_attrs[0]); i++) {
			sysfs_remove_file(gt917s_ps_dev->kobj, &ps_attrs[i].attr);
		}
		kobject_del(gt917s_ps_dev->kobj);
		input_free_device(gt917s_ps_dev->input_dev);
#endif
		kfree(gt917s_ps_dev);
	}
...
static void gt917s_esd_check_func(struct work_struct *);

void gt917s_init_esd_protect(void)
{
	...
	INIT_DELAYED_WORK(&esd_check_work, gt917s_esd_check_func);
	mutex_init(&esd_lock);
}

static void gt917s_deinit_esd_protect(void)
{
	gt917s_esd_switch(SWITCH_OFF);
}

void gt917s_esd_switch(s32 on)
{
	mutex_lock(&esd_lock);
	if (SWITCH_ON == on) {	/* switch on esd check */
		if (!esd_running) {
			...
			queue_delayed_work(gt917s_workqueue,
...
static void gt917s_esd_check_func(struct work_struct *work)
{
	s32 i = 0;
	s32 ret = -1;
	u8 esd_buf[4] = { 0 };
	...
	for (i = 0; i < 3; i++) {
		ret = gt917s_i2c_read(GTP_REG_CMD, esd_buf, 4);
	...
	mutex_lock(&esd_lock);
	if (esd_running) {
		queue_delayed_work(gt917s_workqueue, &esd_check_work, esd_work_cycle);
	} else {
		GTP_INFO("Esd protector suspended!");
	}
	mutex_unlock(&esd_lock);
...
static struct smart_cover_device *gt917s_sc_dev;

/**
 * gt917s_smart_cover_update_state - update smart cover config
 */
static int gt917s_smart_cover_update_state(void)
{
	int ret = 0;
	struct smart_cover_device *dev = gt917s_sc_dev;
	...
	if(!dev->suspended) {
		if(dev->state) {  /* near */
			ret = gt917s_send_cfg(dev->config, dev->cfg_len);
		} else {
		#ifdef CONFIG_GTP_CHARGER_SWITCH
			gt917s_charger_config(1); // charger detector module check and
									// send a config
		#else
			ret = gt917s_send_cfg(gt917s_config, gt917s_cfg_length);
		#endif
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	dev->state = s;
	gt917s_smart_cover_update_state();

	return count;
}

/**
 * gt917s_parse_sc_cfg - parse smart cover config
 *  @sensor_id: sensor id of the hardware
 */
int gt917s_parse_sc_cfg(int sensor_id)
{
#ifdef CONFIG_MTK_PLATFORM
	struct device *dev = tpd->tpd_dev;
#else
	...
	if (!gt917s_sc_dev)
		return -ENODEV;
	cfg = gt917s_sc_dev->config;

	size = gt917s_find_tp_config(dev, "smartcover-config",
			sensor_id, cfg);
	if (size < 0) {
	...
	cfg[0] &= 0x7F;
	set_reg_bit(cfg[TRIGGER_LOC], 0, gt917s_int_type);
	set_reg_bit(cfg[MODULE_SWITCH3_LOC], 5, !gt917s_wakeup_level);
	gt917s_sc_dev->cfg_len = size;
	return 0;
}
...
static int gt917s_smart_cover_init(void)
{
	int err = 0;

	gt917s_sc_dev = kzalloc(sizeof(struct smart_cover_device), GFP_KERNEL);
	if (!gt917s_sc_dev) {
		GTP_ERROR("SmartCover init failed in step: 1.");
		return -ENOMEM;
	}

	gt917s_sc_dev->enabled = 1;
	gt917s_parse_sc_cfg(gt917s_version.sensor_id);
	...
	if (!gt917s_sc_dev->kobj)
		gt917s_sc_dev->kobj = kobject_create_and_add("smartcover",
				sysfs_rootdir);
		if (!gt917s_sc_dev->kobj) {
			err = -3;
			goto exit_free_mem;
	...
	if(sysfs_create_file(gt917s_sc_dev->kobj, &sc_attr.attr)) {
		err = -4;
		goto exit_put_kobj;
	}
...
exit_put_kobj:
	kobject_put(gt917s_sc_dev->kobj);
exit_free_mem:
	kfree(gt917s_sc_dev);
	gt917s_sc_dev = NULL;
...
static void gt917s_smart_cover_deinit(void)
{
	if (!gt917s_sc_dev) {
		return;
	}

	kobject_del(gt917s_sc_dev->kobj);
	kfree(gt917s_sc_dev);
	gt917s_sc_dev = NULL;
}
#endif
 ...
 */
#ifdef CONFIG_GTP_CHARGER_SWITCH
static u8 gt917s_config_charger[GTP_CONFIG_ORG_LENGTH +
...
static void gt917s_charger_work_func(struct work_struct *);

/**
 * gt917s_parse_chr_cfg - parse  charger config
 *  @sensor_id: sensor id of the hardware
 * Return:  0: succeed, <0 error
 */
int gt917s_parse_chr_cfg(int sensor_id)
{
#ifdef CONFIG_MTK_PLATFORM
	struct device *dev = tpd->tpd_dev;
#else
	struct device *dev = &gt917s_i2c_client->dev;
#endif
	u8 *cfg = gt917s_config_charger;
	int len;

	len = gt917s_find_tp_config(dev, "charger-config",
			sensor_id, cfg);
	if (len < 0) {
	...
	cfg[0] &= 0x7F;
	cfg[RESOLUTION_LOC] = (u8) gt917s_abs_x_max;
	cfg[RESOLUTION_LOC + 1] = (u8) (gt917s_abs_x_max >> 8);
	cfg[RESOLUTION_LOC + 2] = (u8) gt917s_abs_y_max;
	cfg[RESOLUTION_LOC + 3] = (u8) (gt917s_abs_y_max >> 8);
	set_reg_bit(cfg[TRIGGER_LOC], 0, gt917s_int_type);
	set_reg_bit(cfg[MODULE_SWITCH3_LOC], 5, !gt917s_wakeup_level);
	return 0;
}
...
static void gt917s_init_charger(void)
{
	...
	INIT_DELAYED_WORK(&charger_switch_work, gt917s_charger_work_func);
	spin_lock_init(&charger_lock);

	if (gt917s_parse_chr_cfg(gt917s_version.sensor_id) < 0) {
		GTP_ERROR("Error occured when parse charger config.");
	}
}

/**
 * gt917s_charger_switch - switch states of charging work thread
 *
 * @on: SWITCH_ON - start work thread, SWITCH_OFF: stop .
 *
 */
void gt917s_charger_switch(s32 on)
{
...
/**
 * gt917s_charger_config - check and update charging status configuration
 * @dir_update
 * 	 0: check before send charging status configuration
 *  	 1: directly send charging status configuration
 *
 */
void gt917s_charger_config(s32 dir_update)
{
	static u8 chr_pluggedin = 0;

#ifdef CONFIG_GTP_SMART_COVER
	if (gt917s_sc_dev && gt917s_sc_dev->enabled
		&& gt917s_sc_dev->state) {
		return;
	}
#endif

	if (gt917s_get_charger_status()) {
		if (!chr_pluggedin || dir_update) {
			GTP_INFO("Charger Plugin.");
			if (gt917s_send_cfg(gt917s_config_charger, gt917s_cfg_length)) {
				GTP_ERROR("Send config for Charger Plugin failed!");
			}
			if (gt917s_send_cmd(GTP_CMD_CHARGER_ON, 0)) {
		...
		if (chr_pluggedin || dir_update) {
			...
			if (gt917s_send_cmd(GTP_CMD_CHARGER_OFF, 0)) {
...
static void gt917s_charger_work_func(struct work_struct *work)
{
	if (!charger_running) {
...
int gt917s_suspend(void)
{
...
#else
		ret = gt917s_i2c_read_dbl_check(GTP_REG_HN_PAIRED, buf, sizeof(buf));
...
#endif
#ifdef CONFIG_GTP_CHARGER_SWITCH
	gt917s_charger_switch(SWITCH_OFF);
#endif
	gt917s_irq_disable();
	...
	if (gesture_enabled) {
		...
		gt917s_irq_enable();
#ifndef CONFIG_MTK_PLATFORM
		enable_irq_wake(gt917s_i2c_client->irq);
#endif
		gt917s_halt = 0;
	} else
#endif
	{
		ret = gt917s_enter_sleep();
		if (ret < 0) {
...
int gt917s_resume(void)
{
	...
	if (!gesture_enabled && !gt917s_halt){
			return 0;
		}
#else
	if (!gt917s_halt){
	...
	ret = gt917s_wakeup_sleep();
	if (ret < 0) {
		GTP_ERROR("Resume failed.");
	}
#ifdef CONFIG_GTP_HOTKNOT
	if (!hotknot_enabled) {
		gt917s_send_cmd(GTP_CMD_HN_EXIT_SLAVE, 0);
	}
#endif

#ifdef CONFIG_GTP_CHARGER_SWITCH
	gt917s_charger_config(0);
	gt917s_charger_switch(SWITCH_ON);
#endif
...
s32 gt917s_init(void)
{
	...
	while (retry++ < GTP_RETRY_3) {
		gt917s_init_failed = 0;
			/* check main system firmware */
		ret = gt917s_i2c_read_dbl_check(GTP_REG_FW_CHK_MAINSYS, reg_val, 1);
		if (ret != 0) {
			gt917s_init_failed = 1;
			gt917s_reset_guitar();
			continue;
		...
		/* debug info  */
		ret = gt917s_i2c_read_dbl_check(GTP_REG_FW_CHK_SUBSYS, reg_val, 1);
	...
	if (gt917s_init_failed) {
		GTP_ERROR("Init failed, use default setting");
		gt917s_abs_x_max = gt917S_pdata->x;
		gt917s_abs_y_max = gt917S_pdata->y;
		gt917s_int_type = GTP_INT_TRIGGER;
		gt917s_wakeup_level = GTP_WAKEUP_LEVEL;
	}

	/* get chip type */
	ret = gt917s_get_chip_type();
	if (ret != 0)
		GTP_ERROR("Get chip type failed!");

	/* read version information */
	ret = gt917s_read_version(&gt917s_version);
	if (ret != 0)
		GTP_ERROR("Get verision failed!");

	/* init and send configs */
	ret = gt917s_init_panel();
	if (ret != 0)
		GTP_ERROR("Init panel failed.");

	gt917s_workqueue = create_singlethread_workqueue("gt917s_workthread");
	if (gt917s_workqueue == NULL)
		GTP_ERROR("Create workqueue failed!");

	/* init auxiliary node and functions */
	gt917s_init_debug_node();

#ifdef CONFIG_GTP_CREATE_WR_NODE
	gt917s_init_tool_node();
#endif
...
#ifdef CONFIG_GTP_CHARGER_SWITCH
	gt917s_init_charger();
	gt917s_charger_config(1);
	gt917s_charger_switch(SWITCH_ON);
#endif

#ifdef CONFIG_GTP_SMART_COVER
	gt917s_smart_cover_init();
#endif
...
void gt917s_deinit(void)
{
	gt917s_deinit_debug_node();
...
#ifdef CONFIG_GTP_CREATE_WR_NODE
	gt917s_deinit_tool_node();
#endif

#ifdef CONFIG_GTP_ESD_PROTECT
	gt917s_deinit_esd_protect();
#endif
	...
	if (gt917s_workqueue) {
		destroy_workqueue(gt917s_workqueue);
	}
