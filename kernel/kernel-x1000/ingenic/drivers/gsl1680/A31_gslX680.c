...
#define DEBUG_X_Y_INFO (1 << 0)
#define DEBUG_INT_INFO (1 << 1)
#define DEBUG_SUSPEND  (1 << 2)
#define DEBUG_INIT     (1 << 3)
...
static void gslX680_ts_worker(struct work_struct *work)
{
	...
	dprintk(DEBUG_X_Y_INFO,"tp-gsl  finger_num = %d\n",cinfo.finger_num);
	for(i = 0; i < (touches < MAX_CONTACTS ? touches : MAX_CONTACTS); i ++)
	{
		...
		dprintk(DEBUG_X_Y_INFO,"tp-gsl  before: x[%d] = %d, y[%d] = %d, id[%d] = %d \n",i,cinfo.x[i],i,cinfo.y[i],i,cinfo.id[i]);
	}
	cinfo.finger_num=(ts->touch_data[3]<<24)|(ts->touch_data[2]<<16)
		|(ts->touch_data[1]<<8)|(ts->touch_data[0]);
	gsl_alg_id_main(&cinfo);
	tmp1=gsl_mask_tiaoping();
	dprintk(DEBUG_X_Y_INFO,"[tp-gsl] tmp1=%x\n",tmp1);
	if(tmp1>0&&tmp1<0xffffffff)
	{
		...
		dprintk(DEBUG_X_Y_INFO,"tmp1=%08x,buf[0]=%02x,buf[1]=%02x,buf[2]=%02x,buf[3]=%02x\n",
...
static u32 gsl_ts_irq(int irq, void *data)
{			
...
static void glsX680_init_events (struct work_struct *work)
{
	...
	ret = request_irq(CTP_IRQ_NUMBER, gsl_ts_irq, IRQF_DISABLED | IRQ_TYPE_EDGE_RISING, "tp-irq", ts_init);
	printk("irq-request: %d %d\n", gpio_get_value(gsl1680_pdata->gpio_int), ret);
			...
			const struct i2c_device_id *id)
{
	...
	gsl1680_pdata = client->dev.platform_data;
	...
	gpio_request(gsl1680_pdata->gpio_shutdown, "tp-shutdown");
	gpio_request(gsl1680_pdata->gpio_int, "tp-int");

	gpio_direction_input(gsl1680_pdata->gpio_int);
	CTP_IRQ_NUMBER = gpio_to_irq(gsl1680_pdata->gpio_int);
...
static int __exit gsl_ts_remove(struct i2c_client *client)
{
	...
	free_irq(CTP_IRQ_NUMBER, ts_init);
	destroy_workqueue(ts->wq);
