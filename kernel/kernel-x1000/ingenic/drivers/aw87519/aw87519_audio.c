...
static int aw87519_gpio_rest = 0;
 ...
 */
static void aw87519_hvload_cfg_loaded(const struct firmware *cont, void *context)
{
    int i = 0;
...
{
    int *pdata = (int *)client->dev.platform_data;
    int ret = -1;
...
static int aw87519_probe(struct platform_device *pdev)
{
    int ret;
    ...
    aw87519_gpio_rest = pdev->dev.platform_data;
    adapter = i2c_get_adapter(2);
    if (!adapter) {
        pr_err("wrong i2c adapter: %d\n", bus_type);
        return -ENODEV;
    }

    struct i2c_board_info board_info = {
        I2C_BOARD_INFO(AW87519_I2C_NAME, 0X58),
    };
    client = i2c_new_device(adapter, &board_info);
    if (!client) {
        pr_err("%s allocate i2c_client failed\n", __func__);
        return -ENODEV;
    }
    client->dev.platform_data = &aw87519_gpio_rest;
...
static int aw87519_remove(struct platform_device *pdev)
{
...
static struct platform_driver aw87519_driver = {
    .probe = aw87519_probe,
    .remove = aw87519_remove,
    .driver = {
        .name = "AW87519_AUDIO",
    },
    .id_table = id_tables,
...
static int __init aw87519_pa_init(void)
{
    ...
    ret = platform_driver_register(&aw87519_driver);
    if (ret) {
        pr_err("%s unable to register driver %d.\n", __func__, ret);
        return ret;
    }
    pr_info("success to register aw87519 driver.\n");
    return 0;
}

static void __exit aw87519_pa_exit(void)
{
    platform_driver_unregister(&aw87519_driver);
}
