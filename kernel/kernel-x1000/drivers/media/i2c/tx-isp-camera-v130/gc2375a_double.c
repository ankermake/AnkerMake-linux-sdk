...
#if defined CONFIG_GC2375A_DOUBLE_480X800
#define GC2375A_WIDTH                   480
#define GC2375A_HEIGHT                  800
#elif defined CONFIG_GC2375A_DOUBLE_720X1200
#define GC2375A_WIDTH                   720
#define GC2375A_HEIGHT                  1200
#elif defined CONFIG_GC2375A_DOUBLE_1600X1200
#define GC2375A_WIDTH                   1600
#define GC2375A_HEIGHT                  1200
#endif
#define GC2375A_CAMERA1_RATE            1
#define GC2375A_CAMERA2_RATE            1
#define GC2375A_CAMERA1_CHN_TYPE        CAMERA_CHN_NORMAL
#define GC2375A_CAMERA2_CHN_TYPE        CAMERA_CHN_IR
#define SENSOR_VERSION                  "H20190717"
...
#endif
struct dual_camera_param gc2375a_dc_param;
...
static struct camera_reg_op gc2375a_init_regs_1600_1200_30fps_mipi[] = {
    ...
    {CAMERA_REG_OP_DATA,0x91,(GC2375A_MAX_HEIGHT-GC2375A_HEIGHT)/2/256}, //win_y_h
    {CAMERA_REG_OP_DATA,0x92,8+(GC2375A_MAX_HEIGHT-GC2375A_HEIGHT)/2%256}, //win_y_l
    {CAMERA_REG_OP_DATA,0x93,(GC2375A_MAX_WIDTH-GC2375A_WIDTH)/2/256}, //win_x_h
    {CAMERA_REG_OP_DATA,0x94,8+(GC2375A_MAX_WIDTH-GC2375A_WIDTH)/2%256}, //win_x_l
    {CAMERA_REG_OP_DATA,0x95,GC2375A_HEIGHT/256}, //win_height_h
    {CAMERA_REG_OP_DATA,0x96,GC2375A_HEIGHT%256}, //win_height_l
    {CAMERA_REG_OP_DATA,0x97,GC2375A_WIDTH/256}, //win_width_h
    {CAMERA_REG_OP_DATA,0x98,GC2375A_WIDTH%256}, //win_width_l
    ...
    {CAMERA_REG_OP_DATA,0x17,0xc0}, //mirror & flip
    ...
    {CAMERA_REG_OP_DATA,0x12,(GC2375A_WIDTH*5/4)%256}, //LWC_set_l :RAW10:win_width*5/4
    {CAMERA_REG_OP_DATA,0x13,(GC2375A_WIDTH*5/4)/256}, //LWC_set_h
    {CAMERA_REG_OP_DATA,0x15,0x00},
    {CAMERA_REG_OP_DATA,0x42,GC2375A_WIDTH%256}, //buf_win_width_l
    {CAMERA_REG_OP_DATA,0x43,GC2375A_WIDTH/256}, //buf_win_width_h
...
static struct camera_reg_op gc2375a_stream_on[] = {
    {CAMERA_REG_OP_DATA,0xfe,0x00},
    {CAMERA_REG_OP_DATA,0xef,0x90},
    {CAMERA_REG_OP_END,0x0,0x0},    /* END MARKER */
};

static struct camera_reg_op gc2375a_stream_off[] = {
    //{CAMERA_REG_OP_DATA,0xfe,0x00},
    {CAMERA_REG_OP_DATA,0xef,0x00},
    {CAMERA_REG_OP_DATA,0xfe,0x20},
    {CAMERA_REG_OP_END,0x0,0x0},    /* END MARKER */
};


static int gc2375a_read(struct tx_isp_subdev *sd, unsigned char reg,
        unsigned char *value)
{
...
static int gc2375a_write(struct tx_isp_subdev *sd, unsigned char reg,
        unsigned char value)
{
    struct i2c_client *client = tx_isp_get_subdevdata(sd);
    unsigned long flag;
...
static inline int gc2375a_read_array(struct tx_isp_subdev *sd, struct camera_reg_op *vals)
{
    int ret;
    unsigned char val;
...
static void gc2375a_switch_camera(struct tx_isp_subdev *sd)
{
    ...
    if (last_chn != info->frame_array[info->frame_cursor]) {
        gc2375a_write_array(&sensor->sd, gc2375a_stream_off);

        dual_camera_i2c_switch(info->frame_array[info->frame_cursor]);

        gc2375a_write_array(&sensor->sd, gc2375a_stream_on);
    }
}

static unsigned char gc2375a_get_camera_channel(struct tx_isp_subdev *sd)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    struct dual_camera_frame_info *info = &sensor->video.dc_param->frame_info;
...
static int gc2375a_init(struct tx_isp_subdev *sd, int enable)
{
    ...
    dual_camera_do(ret, sensor, 0, gc2375a_write_array, sd, wsize->regs);
    dual_camera_do(ret, sensor, 0, gc2375a_write_array, sd, gc2375a_stream_off);
    if (ret)
        return ret;
...
static int gc2375a_s_stream(struct tx_isp_subdev *sd, int enable)
{
    ...
    if (enable) {
        ret = gc2375a_write_array(sd, gc2375a_stream_on);

        pr_debug("gc2375a stream on\n");
    }
    else {
        ret = gc2375a_write_array(sd, gc2375a_stream_off);
        ...
        struct tx_isp_chip_ident *chip)
{
    ...
    if (i2c_sel1_gpio != -1) {
        int ret = gpio_request(i2c_sel1_gpio, "gc2375a_i2c_sel1");
        if (!ret) {
            private_gpio_direction_output(i2c_sel1_gpio, 1);
        } else {
            printk("gpio requrest fail %d\n", i2c_sel1_gpio);
    ...
    if (i2c_sel2_gpio != -1) {
        int ret = gpio_request(i2c_sel2_gpio, "gc2375a_i2c_sel2");
        if (!ret) {
            private_gpio_direction_output(i2c_sel2_gpio, 0);
        } else {
            printk("gpio requrest fail %d\n", i2c_sel2_gpio);
...
static int gc2375a_sensor_ops_ioctl(struct tx_isp_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    switch(cmd){
        case TX_ISP_EVENT_SENSOR_INT_TIME:
            if(arg) {
#if 1
                if (gc2375a_get_camera_channel(sd) == 0) {
                    ret = gc2375a_set_integration_time(sd, *(int*)arg);
                }
#else
                dual_camera_do(ret, sensor, 0, gc2375a_set_integration_time, sd, *(int*)arg);
#endif
            }
            break;
...
#if 1
                if (gc2375a_get_camera_channel(sd) == 0) {
                    ret = gc2375a_set_analog_gain(sd, *(int*)arg);
                }
#else
                dual_camera_do(ret, sensor, 0, gc2375a_set_analog_gain, sd, *(int*)arg);
#endif
            break;
        ...
        case TX_ISP_EVENT_SENSOR_PREPARE_CHANGE:
            dual_camera_do_lock(ret, sensor, 0, gc2375a_write_array, sd, gc2375a_stream_off);
            break;
        case TX_ISP_EVENT_SENSOR_FINISH_CHANGE:
            dual_camera_do_lock(ret, sensor, 0, gc2375a_write_array, sd, gc2375a_stream_on);
            break;
        case TX_ISP_EVENT_SENSOR_FPS:
            if(arg)
                dual_camera_do_lock(ret, sensor, 0, gc2375a_set_fps, sd, *(int*)arg);
            break;
        case TX_ISP_EVENT_SENSOR_VFLIP:
            if(arg)
                dual_camera_do_lock(ret, sensor, 0, gc2375a_set_vflip, sd, *(int*)arg);
            break;
        case TX_ISP_EVENT_SENSOR_DUAL_CAMERA_SWITCH:
            gc2375a_switch_camera(sd);
            break;
        default:
            break;;
    }
...
static int gc2375a_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("probe ok ----start-w %d h %d -->gc2375a\n",GC2375A_WIDTH, GC2375A_HEIGHT);
    ...
    if (gc2375a_board_info) {
        ...
        i2c_sel1_gpio = gc2375a_board_info->gpios.gpio_i2c_sel1;
        i2c_sel2_gpio = gc2375a_board_info->gpios.gpio_i2c_sel2;
    }
#endif
    ...
    sensor->video.dc_param = &gc2375a_dc_param;
    sensor->video.dc_param->frame_info.camera1_frame_rate = GC2375A_CAMERA1_RATE;
    sensor->video.dc_param->frame_info.camera2_frame_rate = GC2375A_CAMERA2_RATE;
    sensor->video.dc_param->frame_info.camera1_chn_type = GC2375A_CAMERA1_CHN_TYPE;
    sensor->video.dc_param->frame_info.camera2_chn_type = GC2375A_CAMERA2_CHN_TYPE;
    ...
    tx_isp_subdev_init(&gc2375a_platform_device, sd, &gc2375a_ops);
...
static int gc2375a_remove(struct i2c_client *client)
{
    ...
    interpolation_frame_deinit(&sensor->video.dc_param->frame_info);
    kfree(sensor);
