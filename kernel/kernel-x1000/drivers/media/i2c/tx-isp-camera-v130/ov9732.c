...
#elif defined CONFIG_OV9732_640X480
...
MODULE_PARM_DESC(dvp_gpio_func, "Sensor DVP function");


struct tx_isp_sensor_attribute ov9732_attr;
#ifndef MODULE
static struct sensor_board_info *ov9732_board_info;
#endif
...
#if defined CONFIG_OV9732_640X480
        .regs           = ov9732_init_regs_640_480_25fps,
#elif defined CONFIG_OV9732_1280X720
...
static int ov9732_set_vflip(struct tx_isp_subdev *sd, int enable)
{
    struct tx_isp_sensor *sensor = sd_to_sensor_device(sd);
    struct tx_isp_sensor_win_setting *wsize = &ov9732_win_sizes[0];
    int ret = ISP_SUCCESS;
    ...
    ret += ov9732_write(sd, 0x3820, val);
                   ...
                   struct tx_isp_chip_ident *chip)
{
    ...
    if(power_gpio != -1){
        ret = gpio_request(power_gpio,"ov9732_pwdn");
        if(!ret){
    ...
    if (i2c_sel1_gpio != -1) {
        int ret = gpio_request(i2c_sel1_gpio, "ov9732_i2c_sel1");
        if (!ret) {
    ...
    if (ret) {
        printk("chip found @ 0x%x (%s) is not an ov9732 chip.\n",
               client->addr, client->adapter->name);
        return ret;
    }
    printk("ov9732 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
    if(chip){
...
static int ov9732_sensor_ops_ioctl(struct tx_isp_subdev *sd,  unsigned int cmd, void *arg)
{
    ...
    case TX_ISP_EVENT_SENSOR_VFLIP:
        if(arg)
            ret = ov9732_set_vflip(sd, *(int*)arg);
        break;
    default:
        break;;
    }
...
#ifndef MODULE
    ov9732_board_info = get_sensor_board_info(ov9732_platform_device.name);
    if (ov9732_board_info) {
        power_gpio    = ov9732_board_info->gpios.gpio_power;
        reset_gpio    = ov9732_board_info->gpios.gpio_sensor_rst;
        i2c_sel1_gpio = ov9732_board_info->gpios.gpio_i2c_sel1;
        dvp_gpio_func = ov9732_board_info->dvp_gpio_func;
    }
#endif
