...
#if defined CONFIG_AR0230_640X1072
#define AR0230_WIDTH                    640
#define AR0230_HEIGHT                   1072
#elif defined CONFIG_AR0230_1920X1080
...
static int dvp_gpio_func = DVP_PA_12BIT;
...
#ifndef MODULE
static struct sensor_board_info *ar0230_board_info;
#endif
...
struct tx_isp_sensor_attribute ar0230_attr={
    ...
    .min_integration_time_native = 2*CONFIG_AR0230_RATIO_T1_T2,
...
static int ar0230_set_analog_gain(struct tx_isp_subdev *sd, int value)
{
    ...
    if (again_mode != again_mode_last) {
        if (again_mode == LCG){
            ...
            printk("Do not support this Again: %d(%x)\n", again_mode, value);
        }
    }
#endif
...
#ifdef CONFIG_AR0230_HDR
    ret += ar0230_write(sd, 0x3082, (private_log2_fixed_to_fixed(CONFIG_AR0230_RATIO_T1_T2, 0, 0)-2) << 2);
#endif
    if (ret)
        return ret;
    ...
    if (enable){
        ...
        switch(dvp_gpio_func){
            ...
            case DVP_PA_12BIT:
                ...
                break;
            default:
                printk("vfilp fail, unsupport code\n");
        }
    } else {
        val &= 0xfd;
        switch(dvp_gpio_func){
        ...
        struct tx_isp_chip_ident *chip)
{
    ...
    if (i2c_sel1_gpio != -1) {
        int ret = gpio_request(i2c_sel1_gpio, "ar0230_i2c_sel1");
        if (!ret) {
    ...
    if (ret) {
        printk("chip found @ 0x%x (%s) is not an ar0230 chip.\n",
                client->addr, client->adapter->name);
        return ret;
    }
    printk("ar0230 chip found @ 0x%02x (%s)\n", client->addr, client->adapter->name);
    if(chip){
...
#ifndef MODULE
    ar0230_board_info = get_sensor_board_info(ar0230_platform_device.name);
    if (ar0230_board_info) {
        power_gpio    = ar0230_board_info->gpios.gpio_power;
        reset_gpio    = ar0230_board_info->gpios.gpio_sensor_rst;
        i2c_sel1_gpio = ar0230_board_info->gpios.gpio_i2c_sel1;

        dvp_gpio_func = ar0230_board_info->dvp_gpio_func;
    }
#endif
...
static __init int init_ar0230(void)
{
    ...
    if(ret){
        printk("Failed to init ar0230 dirver.\n");
        return -1;
    }
