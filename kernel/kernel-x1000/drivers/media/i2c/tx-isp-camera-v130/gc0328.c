        ...
        struct tx_isp_chip_ident *chip)
{
    ...
    if (power_gpio != -1) {
        ret = gpio_request(power_gpio, "gc0328_pwron");
        if (!ret) {
...
struct platform_device gc0328_platform_device = {
    .name = "gc0328",
    .id = -1,
    .dev = {
            ...
            const struct i2c_device_id *id)
{
...
#ifndef MODULE
    gc0328_board_info = get_sensor_board_info(gc0328_platform_device.name);
    if (gc0328_board_info) {
        power_gpio    = gc0328_board_info->gpios.gpio_power;
    ...
    tx_isp_subdev_init(&gc0328_platform_device, sd, &gc0328_ops);
