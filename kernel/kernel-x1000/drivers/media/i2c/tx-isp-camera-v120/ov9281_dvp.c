...
#define OV9281_720P_SUPPORT
//#undef OV9281_720P_SUPPORT
#ifdef OV9281_720P_SUPPORT
#define OV9281_SCLK                     (100000000)
#define SENSOR_OUTPUT_MAX_FPS           72
#else
...
struct tx_isp_sensor_attribute ov9281_attr = {
    ...
    .max_integration_time_native    = 0x38e - 4,
    .one_line_expr_in_us            = 8,    //0x2d8/96M
    .integration_time_limit         = 0x38e - 4,
    .total_width                    = 0x2d8,
    .total_height                   = 0x38e,
    .max_integration_time           = 0x38e - 4,
...
static struct regval_list ov9281_init_regs_1280_800_72fps_dvp[] = {
    {0x0103,0x01},
    {0x0302,0x30},
    {0x030d,0x60},
    {0x030e,0x06},
...
static struct tx_isp_sensor_win_setting ov9281_win_sizes[] = {
    {
        ...
        .regs       = ov9281_init_regs_1280_800_72fps_dvp,
    },
};
...
static int ov9281_set_integration_time(struct v4l2_subdev *sd, int value)
{
    ...
    ret = ov9281_write(sd, 0x3502, (unsigned char)(expo & 0xff));
    if (ret < 0)
        return ret;

    ret = ov9281_write(sd, 0x3501, (unsigned char)((expo >> 8) & 0xff));
    if (ret < 0)
        return ret;

    ret = ov9281_write(sd, 0x3500, (unsigned char)((expo >> 16) & 0xf));
    if (ret < 0)
        return ret;
...
static int ov9281_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(!sensor){
        ...
        if (ov9281_board_info) {
            ...
            sensor_gpio_func= ov9281_board_info->dvp_gpio_func;
        }
#endif
    ...
    ov9281_attr.dvp.gpio = sensor_gpio_func;
