...
#if defined CONFIG_SC123GS_1072X1280_60FPS
#define SC132GS_HTS                0x535
#define SC132GS_VTS                0x546
#define SENSOR_OUTPUT_MAX_FPS    60
#elif defined CONFIG_SC123GS_1072X1280_30FPS
#define SC132GS_HTS                0x5dc
#define SC132GS_VTS                0x960
...
static struct sensor_board_info *sc132gs_board_info;
...
struct tx_isp_sensor_attribute sc132gs_attr={
    ...
    .max_integration_time_native        = SC132GS_VTS - 8,
    .integration_time_limit             = SC132GS_VTS - 8,
    .total_width                        = SC132GS_HTS,
    .total_height                       = SC132GS_VTS,
    .max_integration_time               = SC132GS_VTS - 8,
...
static struct regval_list sc132gs_init_regs_1072_1280_60fps[] = {
...
#if defined CONFIG_SC123GS_1072X1280_60FPS
        .regs                   = sc132gs_init_regs_1072_1280_60fps,
#elif defined CONFIG_SC123GS_1072X1280_30FPS
...
int sc132gs_read(struct v4l2_subdev *sd, uint16_t reg, unsigned char *value)
{
    int ret;
...
int sc132gs_write(struct v4l2_subdev *sd, uint16_t reg, unsigned char value)
{
    int ret;
...
static int sc132gs_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
    int ret;
    unsigned char val;
...
static int sc132gs_reset(struct v4l2_subdev *sd, u32 val)
{
    return 0;
}
...
static int sc132gs_init(struct v4l2_subdev *sd, u32 enable)
{
    ...
    sensor->video.attr->again   = 0;
    sensor->video.attr->integration_time = 4;
...
static int sc132gs_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *chip)
{
    ...
    if (ret) {
        v4l_err(client,
            "chip found @ 0x%x (%s) is not an sc132gs chip.\n",
...
#ifndef MODULE
    sc132gs_board_info = get_sensor_board_info(sc132gs_platform_device.name);
    if (sc132gs_board_info) {
        pwdn_gpio       = sc132gs_board_info->gpios.gpio_power;
        reset_gpio      = sc132gs_board_info->gpios.gpio_sensor_rst;
        gpio_i2c_sel1   = sc132gs_board_info->gpios.gpio_i2c_sel1;
        gpio_i2c_sel2   = sc132gs_board_info->gpios.gpio_i2c_sel2;
    }
#endif
