...
//#define OV5693_BINNING_960P_SUPPORT
#undef OV5693_BINNING_960P_SUPPORT
...
static struct sensor_board_info *ov5693_board_info;
...
int ov5693_read(struct v4l2_subdev *sd, u16 reg, u8 *val)
{
    int ret;
...
int ov5693_write(struct v4l2_subdev *sd, u16 reg, u8 val)
{
    struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int ov5693_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
    int ret;
    u8 val;
...
static int ov5693_reset(struct v4l2_subdev *sd, int val)
{
    return 0;
}
...
static int ov5693_init(struct v4l2_subdev *sd, int enable)
{
...
static int ov5693_set_vflip(struct tx_isp_sensor *sensor, int enable)
{
    struct tx_isp_notify_argument arg;
    struct v4l2_subdev *sd = &sensor->sd;
    int ret = 0;
        ...
        struct v4l2_dbg_chip_ident *chip)
{
    ...
    if (ret) {
        printk("chip found @ 0x%x (%s) is not an ov5693 chip.\n",
...
static int ov5693_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ov5693_board_info = get_sensor_board_info(sensor_platform_device.name);
    if (ov5693_board_info) {
        pwdn_gpio       = ov5693_board_info->gpios.gpio_power;
        reset_gpio      = ov5693_board_info->gpios.gpio_sensor_rst;
        gpio_i2c_sel1   = ov5693_board_info->gpios.gpio_i2c_sel1;
        gpio_i2c_sel2   = ov5693_board_info->gpios.gpio_i2c_sel2;
    }
...
MODULE_DESCRIPTION("A low-level driver for Gcoreinc ov5693 sensors");
MODULE_LICENSE("GPL");
