...
//假如定义 CONFIG_VIDEO_TX_ISP_60M_V120,则isp时钟为60M,那么不能选择SC031_120FPS_SUPPORT
#ifdef SC031_120FPS_SUPPORT
...
static struct sensor_board_info *sc031_board_info;
...
static enum v4l2_mbus_pixelcode sc031_mbus_code[] = {
...
int sc031_read(struct v4l2_subdev *sd, uint16_t reg, unsigned char *value)
{
    int ret;
...
int sc031_write(struct v4l2_subdev *sd, uint16_t reg, unsigned char value)
{
    int ret;
    struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int sc031_read_array(struct v4l2_subdev *sd, struct regval_list *vals)
{
    int ret;
    unsigned char val;
...
static int sc031_reset(struct v4l2_subdev *sd, u32 val)
{
    return 0;
}
...
static int sc031_init(struct v4l2_subdev *sd, u32 enable)
{
...
static int sc031_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *chip)
{
    ...
    if (ret) {
        v4l_err(client,
            "chip found @ 0x%x (%s) is not an sc031 chip.\n",
...
static int sc031_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if 1
#ifndef MODULE
    sc031_board_info = get_sensor_board_info(sc031_platform_device.name);
    if (sc031_board_info) {
        pwdn_gpio       = sc031_board_info->gpios.gpio_power;
        reset_gpio      = sc031_board_info->gpios.gpio_sensor_rst;
        gpio_i2c_sel1   = sc031_board_info->gpios.gpio_i2c_sel1;
        gpio_i2c_sel2   = sc031_board_info->gpios.gpio_i2c_sel2;
        sensor_gpio_func= sc031_board_info->dvp_gpio_func;
    }
#endif
#endif
    ...
    sc031_attr.dvp.gpio = sensor_gpio_func;
...
MODULE_DESCRIPTION("A low-level driver for SmartSens sc031 sensors");
MODULE_LICENSE("GPL");
