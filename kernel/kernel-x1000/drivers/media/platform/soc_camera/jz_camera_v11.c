...
static int jz_init_dma(struct videobuf_queue *vq, struct videobuf_buffer *vbuf) {
	...
	if(icd->current_fmt->host_fmt->fourcc == V4L2_PIX_FMT_YUYV ||
			icd->current_fmt->host_fmt->fourcc == V4L2_PIX_FMT_RGB565) {
		dma_desc[vbuf->i].cmd = icd->sizeimage >> 2 |
			...
			icd->current_fmt->host_fmt->fourcc == V4L2_PIX_FMT_SRGGB8 ||
			icd->current_fmt->host_fmt->fourcc == V4L2_PIX_FMT_GREY) {
		dma_desc[vbuf->i].cmd = icd->sizeimage >> 2 |
			CIM_CMD_EOFINT | CIM_CMD_OFRCV;
	} else {
...
static int jz_camera_set_bus_param(struct soc_camera_device *icd) {
	...
	ctrl2_reg |= CIM_CTRL2_APM | CIM_CTRL2_EME | CIM_CTRL2_OPE |
		(0 << CIM_CTRL2_OPG_BIT) | CIM_CTRL2_FSC | CIM_CTRL2_ARIF;
...
static int camera_sensor_gpio_init(struct platform_device *pdev) {
	...
	for (i = 0; i < ARRAY_SIZE(pdata->cam_sensor_pdata); i++) {
		if(pdata->cam_sensor_pdata[i].gpio_pw_en > 0) {
			sprintf(gpio_name, "sensor_pw_en%d", i);
			printk("sensor_pw_en%d,%d\n", i,pdata->cam_sensor_pdata[i].gpio_pw_en);
			ret = gpio_request(pdata->cam_sensor_pdata[i].gpio_pw_en,
					gpio_name);
			if(ret) {
				dev_err(&pdev->dev, "%s:request cim%d power en gpio fail\n",
						__func__, i);
				goto err_request_gpio;
			}
			gpio_direction_output(pdata->cam_sensor_pdata[i].gpio_pw_en, 1);
			cnt++;
		}
		if(pdata->cam_sensor_pdata[i].gpio_rst > 0) {
		...
		if(pdata->cam_sensor_pdata[i].gpio_power > 0) {
...
err_request_gpio:
	for (i = 0; i < ARRAY_SIZE(pdata->cam_sensor_pdata); i++) {
		if(pdata->cam_sensor_pdata[i].gpio_pw_en > 0)
			gpio_free(pdata->cam_sensor_pdata[i].gpio_pw_en);
		if(pdata->cam_sensor_pdata[i].gpio_rst > 0)
