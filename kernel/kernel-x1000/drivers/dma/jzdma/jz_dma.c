...
#endif //endif CONFIG_XBURST_PDMA
...
static void dump_dma_desc(struct jzdma_channel *dmac)
{
	...
	dev_info(chan2dev(&dmac->chan), "dmac->desc_nr = %d phys = %p\n", dmac->desc_nr, (void *)dmac->desc_phys);
	for (i = 0; i < dmac->desc_nr; i++)
		dev_info(chan2dev(&dmac->chan),
			 "[%d]DESC: %p, DSA: %x, DTA: %x, DCM: %lx, DTC:%lx DRT:%lx\n",
			 i, &desc[i], desc[i].dsa, desc[i].dta, desc[i].dcm, desc[i].dtc,
			 desc[i].drt);
...
#if defined(CONFIG_XBURST_PDMA) && defined(CONFIG_NAND)
	if (dmac->id != 1)
		dcm |= DCM_LINK;
#else
        dcm |= DCM_LINK;
						      ...
						      direction, int flag)
{
	...
	if (flag & 0x2) {
		if (build_one_dymic_desc(dmac, src, dst,
                                        dcm, cnt / tsz, type, flag, direction)) {
			return &dmac->tx_desc;
		}
	} else {
			 ...
			 unsigned long arg)
{
	...
	case DMA_SLAVE_CONFIG:
		...
		if (!dmac->config) {
			dmac->config = kzalloc(sizeof(struct dma_slave_config), GFP_ATOMIC);
			if (!dmac->config) {
				return -ENOMEM;
			}
		}
...
static int __init jzdma_probe(struct platform_device *pdev)
{
	...
	int i, ret = 0;
#if defined(CONFIG_XBURST_PDMA)
        short irq_pdmam; /* irq_pdmam for MCU irq */
