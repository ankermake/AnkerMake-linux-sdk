...
static int sm5007_read_irq_status(struct sm5007 *sm5007)
{
    ...
    ret = sm5007_read(sm5007->dev, SM5007_INTSOURCE, &intsource_val);
    if (ret < 0) {
    ...
    ret = sm5007_read(sm5007->dev, SM5007_INT1, &int_val[0]);
    if (ret < 0) {
        dev_err(sm5007->dev, "Failed on reading irq1 status\n");
        return ret;
    }
    ...
    ret = sm5007_read(sm5007->dev, SM5007_INT2,
            &int_val[1]);
    if (ret < 0) {
        dev_err(sm5007->dev, "Failed on reading irq2 status\n");
        return ret;
    }
    sm5007->irq_status_cache[1] = int_val[1];
    pr_debug("irq2 = 0x%x\n", int_val[1]);

    ret = sm5007_read(sm5007->dev, SM5007_INT3, &int_val[2]);
    if (ret < 0) {
        dev_err(sm5007->dev, "Failed on reading irq3 status\n");
        return ret;
    }
    sm5007->irq_status_cache[2] = int_val[2];
    pr_debug("irq3 = 0x%x\n", int_val[2]);

    ret = sm5007_read(sm5007->dev, SM5007_INT4, &int_val[3]);
    if (ret < 0) {
        dev_err(sm5007->dev, "Failed on reading irq4 status\n");
        return ret;
    }
    sm5007->irq_status_cache[3] = int_val[3];
    pr_debug("irq4 = 0x%x\n", int_val[3]);

    ret = sm5007_read(sm5007->dev, SM5007_INT5, &int_val[4]);
    if (ret < 0) {
        dev_err(sm5007->dev, "Failed on reading irq5 status\n");
        return ret;
    }
    sm5007->irq_status_cache[4] = int_val[4];
    pr_debug("irq5 = 0x%x\n", int_val[4]);

    ret = sm5007_read(sm5007->dev, SM5007_INT6, &int_val[5]);
    if (ret < 0) {
        dev_err(sm5007->dev, "Failed on reading irq6 status\n");
        return ret;
    }
...
static uint8_t sm5007_irqs_ctrl_mask_all_val[] = {
    0xFF,
    0xFF, //CHGRSTF, DONE, TOPOOF,VBUSPOK, VBUSUVLO
    0xFF,
    0xFF,
    0xFF,
    0xFF,
...
int sm5007_irq_init(struct sm5007 *sm5007, int irq,int irq_base)
{
    int i, ret;
