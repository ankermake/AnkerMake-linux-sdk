...
#else
    src_rate = clk_get_rate(hw->clk)/2;
#endif
...
#ifdef CONFIG_JZ_SPI_SELECT_EXT_CLK
    hw->clk = clk_get(&pdev->dev, "ext1");
#else
	hw->clk = clk_get(&pdev->dev, "cgu_ssi");
#endif
	if (IS_ERR(hw->clk)) {
		dev_err(&pdev->dev, "Cannot get cgu_ssi clock\n");
		hw->clk = NULL;
