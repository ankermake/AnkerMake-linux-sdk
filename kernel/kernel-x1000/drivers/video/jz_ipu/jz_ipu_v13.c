...
static int _ipu_set_bg_route(struct jz_ipu *ipu, struct ipu_param *ipu_param)
{
	...
	unsigned int in_stride  = ipu_param->in_stride;
	unsigned int out_stride = ipu_param->out_stride;
	unsigned int in_uv_stride  = ipu_param->in_sub_stride;
	unsigned int out_uv_stride = ipu_param->out_sub_stride;
    unsigned int inwstride = 0;
	...
	IPU_DEBUG("ipu: %s, bgrgb = %d, srcw = %d, srch = %d, outw = %d, outh = %d, " \
		  "input stride = %d, input uv stride = %d, out stride = %d, out uv stride = %d\n",
		  __func__, bgrgb, srcw, srch, outw, outh,
		  in_stride, in_uv_stride, out_stride, out_uv_stride);
    ...
    IPU_DEBUG("%d %s infmt %x outfmt %x\n",__LINE__,__func__,infmt,outfmt);
    switch(infmt) {
        case IN_FMT_RGB_888: /* BGRA need srcw<<2,register Y_STRIDE & OUT_STRIDE don't need srcw<<2 */
            inwstride= srcw<<2;
            break;
        default:
            inwstride = srcw;
	...
	reg_write(ipu, IPU_IN_FM_GS, (IN_FM_W(inwstride) | IN_FM_H(srch)));


   if(ip->cmd & IPU_CMD_OSD)
    {
      if (ip->bg_bak_argb)
			reg_write(ipu, IPU_OSD_CH_B_BAK_ARGB, ip->bg_bak_argb);
		else
    ...
    /****** Add HSV *******/
        if((outfmt == OUT_FMT_HSV)){/* out format */
            reg_val = 0x4;
        }else if(outfmt == OUT_FMT_BGR888){
            reg_val = 0x68;
        }else if(outfmt == OUT_FMT_RGB888){
            reg_val = 0x40;
        }else if(outfmt == OUT_FMT_NV21){
            reg_val = 0x3;
        }else if(outfmt == OUT_FMT_NV12){
            reg_val = 0x2;
        } else {
            reg_val = 0x1;
        }
	...
	*/
	...
	switch (infmt) {
		...
		case IN_FMT_YUV411:
			...
			break;
		default:
		        reg_write(ipu, IPU_UV_STRIDE, 0);
	}

	if (in_stride)
	  reg_write(ipu, IPU_Y_STRIDE, in_stride);
	if(out_stride)
	  reg_write(ipu, IPU_OUT_STRIDE, out_stride);
...
//		reg_val |= CH_BK_ARGB_TYPE_BGRA;	//Maybe it's have an other choice, but here I fix it.You can change it.
	...
	{
	    if(ip->osd_ch0_para)
	      reg_val = ip->osd_ch0_para;
		else{
            unsigned int mask = (0x3 << CH_BK_PIC_TYPE) | (0xf << CH_BK_ARGB_TYPE);
            reg_val &= mask;
            if(bgrgb == 0){
                reg_val |= 0x1 << 24;
            }
	    ...
	    reg_write(ipu, IPU_OSD_CH0_PARA, reg_val);
	  }
...
static int _ipu_set_bg_buffer(struct jz_ipu *ipu, struct ipu_param *ipu_param)
{
    ...
    IPU_DEBUG("ipu: ip->bg_w %d ip->bg_h %d\n", ip->bg_w,ip->bg_h);
    if(ip->cmd & IPU_CMD_CSC) {
        /* OUT HSV */
    ...
    IPU_DEBUG("ipu: bg_y_pbuf = 0x%08x, bg_u_pbuf = 0x%08x, bg_v_pbuf = 0x%08x out_nv21_y = 0x%08x\n", bg_y_pbuf, bg_u_pbuf, 
    bg_v_pbuf,out_nv21_y);
    if(ip->bg_fmt == HAL_PIXEL_FORMAT_NV12) {
    ...
    if(ip->cmd & IPU_CMD_CSC) {/* out addr */
        ...
        }else if((ip->out_fmt == HAL_PIXEL_FORMAT_NV21)||(ip->out_fmt == HAL_PIXEL_FORMAT_NV12)){
...
static int ipu_start_async(struct jz_ipu *ipu, struct ipu_param *ipu_param)
{
	int ret = 0;
    int isrgb = 0;
	struct ipu_param *ip = ipu_param;
	if((ipu->async_state & 1) == 0)
	{
		dev_err(ipu->dev, "ipu: ipu's clk is not enabled.\n");
		return ret;
	}
	...
	isrgb = _ipu_set_bg_route(ipu, ip);
    ...
    if (ip->cmd & IPU_CMD_CSC) {
        ...
        if(isrgb){
            reg_write(ipu, IPU_CH0_CSC_C0_COEF, (0x106 << 0) | (0x202 << 20));
            reg_write(ipu, IPU_CH0_CSC_C1_COEF, (0x64 << 0) | (0x97 << 20));
            reg_write(ipu, IPU_CH0_CSC_C2_COEF, (0x129 << 0) | (0x1c0 << 20));
            reg_write(ipu, IPU_CH0_CSC_C3_COEF, (0x1c0 << 0) | (0x177 << 20));
            reg_write(ipu, IPU_CH0_CSC_C4_COEF, 0x49);
...
static int ipu_wait_finish(struct jz_ipu *ipu)
{
	int ret = 0;
	if(ipu->async_state == 3){
#ifdef DEBUG
		ipu_dump_info(ipu);
#endif
		IPU_DEBUG("ipu_start\n");
		ret = wait_for_completion_interruptible_timeout(&ipu->done_ipu, msecs_to_jiffies(200));
		ipu->async_state = 1;
		if (ret < 0) {
...
static int ipu_async_start(struct jz_ipu *ipu, struct ipu_param *ipu_param)
{
	int ret = 0;
	if((ipu->async_state & 1) == 0)
	{
	...
	ret = ipu_start_async(ipu,ipu_param);
	return ret;
}

static void ipu_async(struct jz_ipu *ipu)
{
	ipu_wait_finish(ipu);
	ipu->async_state &= ~(1 << 1);
}

static void ipu_sync(struct jz_ipu *ipu){
	ipu_wait_finish(ipu);
	if(ipu->async_state & 1){
...
static long ipu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case IOCTL_IPU_ASYNC_START:
		ret = ipu_async_start(ipu, (struct ipu_param *)arg);
		if (ret) {
			printk("ipu: error ipu async start ret = %d\n", ret);
		}
		break;
	case IOCTL_IPU_ASYNC:
		ipu_async(ipu);
		break;
	case IOCTL_IPU_SYNC:
		ipu_sync(ipu);
		...
		case IOCTL_IPU_START:
			ret = ipu_start(ipu, (struct ipu_param *)arg);
			if (ret) {
...
#if defined(CONFIG_SOC_X1630) || defined(CONFIG_SOC_X1830)
