...
struct ipu_param
{
	...
	unsigned int            out_stride;
  	/* stride for input UV(U) (reserved) */
	unsigned int            in_sub_stride;
	/* stride for outputed hsv data or for nv12/nv21 (reserved)*/
	unsigned int            out_sub_stride;
        /* background param and ARGB */
        unsigned int 		bg_para;
        unsigned int            bg_bak_argb;
...
#define IOCTL_IPU_SYNC          _IO(JZIPU_IOC_MAGIC, 119)
#define IOCTL_IPU_ASYNC         _IO(JZIPU_IOC_MAGIC, 120)
#define IOCTL_IPU_ASYNC_START   _IO(JZIPU_IOC_MAGIC, 121)
