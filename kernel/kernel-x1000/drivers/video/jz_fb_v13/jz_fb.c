...
static int jzfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if (var->bits_per_pixel != jzfb_get_controller_bpp(jzfb) &&
	    var->bits_per_pixel != jzfb->fb_bpp)
		return -EINVAL;
...
static void jzfb_slcd_mcu_init(struct fb_info *info)
{
	...
	if(pdata->bpp / pdata->smart_config.bus_width != 1 ) {
		...
		tmp |=  ((pdata->bpp + (pdata->smart_config.bus_width - 1)) /
			pdata->smart_config.bus_width) == 2 ?
...
static int jzfb_display_logo(struct fb_info *info)
{
	int height;
	...
	/* from fbmem.c */
	height = fb_prepare_logo(info, FB_ROTATE_UR);
	if (!height) {
		dev_warn(info->dev, "prepare logo height: %d\n", height);
		return -EINVAL;
	}
	/* from fbmem.c */
	height = fb_show_logo(info, FB_ROTATE_UR);
	if (!height) {
		dev_warn(info->dev, "show logo height: %d\n", height);
		return -EINVAL;
	}
...
#endif /* CONFIG_FB_JZ_LOGO */

static void jzfb_notify_conblank(struct fb_info *fb)
{
	int blank = 0;
...
static int jzfb_probe(struct platform_device *pdev)
{
	...
	*(volatile unsigned int*)0xb0000020 &= ~(1 << 23);*/
	...
	fb->var.bits_per_pixel = jzfb->fb_bpp;
...
#ifdef CONFIG_FB_JZ_LOGO
		jzfb_display_logo(fb);
#endif

		/* Nofity fb unblank backlight */
		jzfb_notify_conblank(fb);
