...
static void jzmac_set_multicast_list(struct net_device *dev)
{
	...
	/* Toggle On/Off */
	if((dev->flags & IFF_PROMISC) && !(lp->promisc)) {
		/* Accept any kinds of packets */
		synopGMAC_promisc_enable(gmacdev);
		lp->promisc = 1;
		printk("%s: Enter promisc mode!\n",dev->name);
	} else if(!(dev->flags & IFF_PROMISC) && lp->promisc){
		synopGMAC_promisc_disable(gmacdev);
		lp->promisc = 0;
		printk("%s: Exit promisc mode!\n",dev->name);
	}

	if((dev->flags & IFF_ALLMULTI) && !(lp->mcast_all)) {
		...
		lp->mcast_all = 1;
	} else if(!(dev->flags & IFF_ALLMULTI) && lp->mcast_all) {
		printk("%s: Exit allmulticast mode! %d\n", dev->name, dev->mc.count);
		synopGMAC_multicast_disable(gmacdev);
		lp->mcast_all = 0;
	}

	if(((dev->flags & IFF_MULTICAST) || !netdev_mc_empty(dev)) && !(lp->mcast_all)) {
		synopGMAC_multicast_enable(gmacdev);
...
static void jz_mac_change_rx_flags(struct net_device *dev, int flags)
{
...
//	.ndo_change_rx_flags = jz_mac_change_rx_flags,
