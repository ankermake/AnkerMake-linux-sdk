...
static int rtl88x2cs_wlan_ctrl(struct rtl_power_data *pfun, int onoff)
{
    int ret = 0;
    switch (onoff)
    {
...
static int rtl88x2cs_probe(struct platform_device *pdev)
{
    struct rtl_power_data *pfun = NULL;
    int ret = 0;
    printk("######rtl88x2cs_probe.\n");
    RTW_INFO("rtl88x2cs wifi probe function.\n");
    pfun = (struct rtl_power_data *)pdev->dev.platform_data;
    ret = rtl88x2cs_wlan_ctrl(pfun, NORMAL);
    if (ret) {
        RTW_ERR("%s:%d rtl88x2cs_wlan_ctrl failed\n", __func__, __LINE__);
    }
...
static int rtl88x2cs_remove (struct platform_device *pdev)
{
    int ret = 0;
    struct rtl_power_data *pfun = NULL;
...
static struct platform_driver rtl88x2cs_driver = {
    .probe = rtl88x2cs_probe,
    .remove = rtl88x2cs_remove,
    .driver = {
        .name = DEVNAME,
    }
};

static int __init rtl88x2cs_driver_init(void)
{
    int ret = 0;
    ret = platform_driver_register(&rtl88x2cs_driver);
    if (ret < 0) {
        RTW_ERR("ERROR: rtl88x2cs_driver register failed !!\n");
    }
    return ret;
}

static void __exit rtl88x2cs_driver_exit(void)
{
    platform_driver_unregister(&rtl88x2cs_driver);
}

//late_initcall_sync(rtw_drv_entry);
late_initcall_sync(rtl88x2cs_driver_init);
module_exit(rtl88x2cs_driver_exit);
MODULE_LICENSE("GPL");