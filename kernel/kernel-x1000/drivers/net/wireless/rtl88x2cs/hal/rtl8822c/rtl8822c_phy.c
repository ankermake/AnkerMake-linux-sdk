...
u8 rtl8822c_get_tx_power_index(PADAPTER adapter, enum rf_path rfpath, u8 rate, u8 bandwidth, u8 channel, struct txpwr_idx_comp *tic)
{
    ...
    s16 power_idx;
    u8 pg = 0;
    s8 by_rate_diff = 0, limit = 0, tpt_offset = 0, extra_bias = 0, by_btc_diff = 0, dis_dpd_rate_diff = 0;
    ...
    if (tic) {
        ...
        tic->btc = by_btc_diff;
        tic->dpd_rate= dis_dpd_rate_diff;
    }

    by_rate_diff = by_rate_diff > limit ? limit : by_rate_diff;
    power_idx = pg + by_rate_diff + tpt_offset + extra_bias + by_btc_diff + dis_dpd_rate_diff;
