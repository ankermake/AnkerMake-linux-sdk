...
#if (ATBM_WIFI_PLATFORM == PLATFORM_INGENIC)
#define PLATFORMINF	"ingenic"
//#define CARD_INSERT_GPIO 23
#define CARD_INSERT_GPIO GPIO_PC(16)
...
static int atbm_platform_power_ctrl(const struct atbm_platform_data *pdata,bool enabled)
{
	int ret = 0; 
#ifndef USB_BUS	
	#if (ATBM_WIFI_PLATFORM == PLATFORM_INGENIC) ||(ATBM_WIFI_PLATFORM == PLATFORM_FRIENDLY)
	{
	...
	enabled = 1;
		...
		if (gpio_request(pdata->power_gpio, "WIFI_POWERON")!=0) {
			...
			gpio_direction_output(pdata->power_gpio, enabled);/* WLAN_CHIP_PWD */
			gpio_set_value(pdata->power_gpio, enabled);
			mdelay(100);
			//gpio_free(pdata->power_gpio);
			printk("[altobeam] + %s :Ingenic power pin=%d wlan powerwew %s\n",__func__, pdata->power_gpio, enabled?"on":"off");
		}
    
        /* reset */
		if (gpio_request(pdata->reset_gpio, "WIFI_RESET")!=0) {
			printk("[altobeam] ERROR:Cannot request WIFI_RESET\n");
		} else {
			...
			gpio_direction_output(pdata->reset_gpio, 1);
			mdelay(100);
			printk("[altobeam] + %s :Ingenic reset-pin=%d wlan reset \n",__func__, pdata->reset_gpio);
		}
#endif
	}
	#endif //(ATBM_WIFI_PLATFORM == PLATFORM_INGENIC) ||(ATBM_WIFI_PLATFORM == PLATFORM_FRIENDLY)
...
static int atbm_platform_insert_crtl(const struct atbm_platform_data *pdata,bool enabled)
{
	...
	#if (ATBM_WIFI_PLATFORM == PLATFORM_INGENIC)
	{
		...
		printk("============platform insert crtl====== enable=%d\n", enabled);
	}
	#endif
