...
#endif
#if ((ATBM_WIFI_PLATFORM == PLATFORM_INGENIC)||(ATBM_WIFI_PLATFORM == PLATFORM_AMLOGIC_905))
...
#else
	printk("atbm power on and reset\n");
	if (pdata->power_ctrl) {
		ret = pdata->power_ctrl(pdata, true);
		if (ret)
			goto err_power;
...
late_initcall(apollo_sdio_module_init);
