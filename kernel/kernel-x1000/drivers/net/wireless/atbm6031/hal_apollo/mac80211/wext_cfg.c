...
SPECIAL_CH_FREQ spec_recd[SPECIAL_NUM] = {{0,0,0},{0,0,0}};
...
static const struct iw_priv_args atbm_privtab[] = {
		...
		#endif
		{SIOCIWFIRSTPRIV + 25, IW_PRIV_TYPE_CHAR | 10, 0, "getSigmstarEfuse"},	
		{SIOCIWFIRSTPRIV + 26, IW_PRIV_TYPE_CHAR | 95, 0, "setSigmstarEfuse"},
};
...
static int atbm_ioctl_command_help(struct net_device * dev,struct iw_request_info * ifno,union iwreq_data * wrqu,char * ext)
{
	int ret = 0;
		...
		printk("setSigmstarEfuse		   <%02x,%02x,...,%02x>	set sigmstar 256bits efuse \n");
...
void get_test_threshold(char *param)
{
	...
	if(strlen(param) != 0)
	{
		sscanf(param, "cfg:%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", 
			&Freq, &txEvm, &rxEvm, &txEvmthreshold,&rxEvmthreshold,&Txpwrmax, &Txpwrmin, &Rxpwrmax, &Rxpwrmin, &rssifilter, &cableloss);
	...
	printk("Freq:%d,txEvm:%d,rxEvm:%d,txevmthreshold:%d,rxevmthreshold:%d,Txpwrmax:%d,Txpwrmin:%d,Rxpwrmax:%d,Rxpwrmin:%d,rssifilter:%d,cableloss:%d\n",
...
static int atbm_ioctl_set_SIGMSTAR_256BITSEFUSE(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	for (i = 0; i < 95; i+= 3)
	{
		sscanf(extra + i, "%02", (int *)&efuseBuff[index]);
		index++;
	}
...
static int atbm_ioctl_get_wifi_state(struct net_device *dev, struct iw_request_info *info, void *wrqu, char *extra)
{
	...
	printk("atbm_ioctl_get_wifi_state(), status %d\n", wifi_status);
...
static char wifi_freq_buf[64]={0};
static char *wifi_freq = "NULL";
module_param(wifi_freq,charp,0644);
MODULE_PARM_DESC(wifi_freq,"wifi freq");
void atbm_set_freq(struct atbm_common *hw_priv, int channel, int freq)
{
	memset(wifi_freq_buf, 0, sizeof(wifi_freq_buf));
	sprintf(wifi_freq_buf, "ch:%d, freq:%d", channel, freq);
...
static int atbm_ioctl_set_freq(struct net_device *dev, struct iw_request_info *info, void *wrqu, char *extra)
{
	...
	for(i=0; i<SPECIAL_NUM; i++){
		if(spec_recd[i].flag == 0){
			CmdLine_GetInteger(&pspec, &spec_recd[i].special_ch);
			CmdLine_GetInteger(&pspec, &spec_recd[i].special_freq);
			printk("atbm: [%d] ch %d, freq %d\n", i, spec_recd[i].special_ch, spec_recd[i].special_freq);
			
			atbm_set_freq(hw_priv, spec_recd[i].special_ch, spec_recd[i].special_freq);

			if(spec_recd[i].special_ch > 14 || spec_recd[i].special_ch < 1){
				atbm_kfree(ptr);
				atbm_kfree(free_pp);
				mutex_unlock(&sdata->local->iflist_mtx);
				return -1;
			...
			if(spec_recd[i].special_freq < 2412 || spec_recd[i].special_freq > 2484){
				spec_recd[i].flag = 1;//valid freq
			}
			else{
				spec_recd[i].flag = 0;//valid freq
				printk("atbm: invalid freq\n");
			}
			break;
		}
	}
...
static int best_ch_scan_flag = 0;
...
static int atbm_ioctl_best_ch_start(struct net_device *dev, struct iw_request_info *info, void *wrqu, char *extra)
{
	int ret = 0;
...
static int atbm_ioctl_best_ch_end(struct net_device *dev, struct iw_request_info *info, void *wrqu, char *extra)
{
	int i,j;
	int ret = 0;
	...
	for(i=0; i<CHANNEL_NUM; i++){
		...
		if(ch_ave_ratio[i] <= 60){
			...
			for(j=0; j<SPECIAL_NUM; j++){
				if((spec_recd[j].flag != 0) && ((i+1) == spec_recd[j].special_ch)){
					spec_ch = spec_recd[j].special_ch;
				}
			}
...
static int atbm_ioctl_best_ch_scan_result(struct net_device *dev, struct iw_request_info *info, void *wrqu, char *extra)
{
	int ret = 0;
	unsigned char *ptr = NULL;
	union iwreq_data *wdata = (union iwreq_data *)wrqu;
...
const iw_handler atbm_private_handler[]={
	...
	#endif
	[25] = (iw_handler)atbm_ioctl_get_SIGMSTAR_256BITSEFUSE,
	[26] = (iw_handler)atbm_ioctl_set_SIGMSTAR_256BITSEFUSE,
...
{	
	if((gthreshold_param.txpwrmax == 0) && (gthreshold_param.txpwrmin == 0))
	{
		...
		printk("Use default Txrssimin threshold:%d\n", gthreshold_param.txpwrmin);
	}
