...
static int atbm_usb_wait_anchor_empty_timeout(struct sbus_priv *self,int timeout)
{
	...
	urb_unlink = self->drvobj->rx_urb;
	for(urb_index=0;urb_index<RX_URB_NUM;urb_index++){
		usb_unlink_urb(urb_unlink[urb_index].test_urb);
	}
	ret = usb_wait_anchor_empty_timeout(&hw_priv->sbus_priv->drvobj->rx_submitted,timeout);
	if(ret == 0){
		printk(KERN_ERR "%s:rx_submitted cancle timeout \n",__func__);
		goto exit;
	}
	...
	if(ret == 0){
		printk(KERN_ERR "%s:tx_submitted cancle timeout \n",__func__);
	}
exit:
	return ret;
}
static int _atbm_lmac_restart(struct sbus_priv *self)
{
	...
	*/
	...
	ret = atbm_usb_wait_anchor_empty_timeout(self,msecs_to_jiffies(500));
	if(ret == 0){
		ret = atbm_usb_device_reset(self);
		if(ret){
			printk(KERN_ERR "Reset Usb Err\n");
		...
		ret = atbm_usb_wait_anchor_empty_timeout(self,msecs_to_jiffies(800));
