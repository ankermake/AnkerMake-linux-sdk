...
extern int rtk_power_down(void);
extern int rtk_power_on(void);
...
extern int rtk_wlan_power_on(int flag);
extern int rtk_wlan_init(void);
extern int jz_gpio_set_func(int gpio, int func);
...
static int rtl8723ds_wlan_ctrl(struct rtl_power_data *pfun, int onoff)
{
	int ret = 0;
	...
	switch (onoff) 
	{
		case 0:
			if(poweroff_num){
                ...
                if (pfun->rtk_power_off)
                    pfun->rtk_power_off(RESET);
				else
					ret = -1;
			} else {
		...
		case 1:
			if(poweron_num){
                ...
                if (pfun->rtk_power_on)
                    pfun->rtk_power_on(RESET);
				else
					ret = -1;
			} else {
...
static int rtl8723ds_probe (struct platform_device *pdev)
{
    struct rtl_power_data *pfun = NULL;
    int ret = 0;
    RTW_INFO("rtl8723ds wifi probe function\n");
    pfun = (struct rtl_power_data *)pdev->dev.platform_data;

    ret = rtl8723ds_wlan_ctrl(pfun, NORMAL);
	if (ret) {
		RTW_ERR(" %s:%d rtl8723ds_wlan_ctrl failed\n", __func__, __LINE__);
	}
...
static int rtl8723ds_remove (struct platform_device *pdev)
{
    int ret = 0;
    struct rtl_power_data *pfun = NULL;
	pr_info("INFO: %s fun runing\n", __func__);
    pfun = (struct rtl_power_data *)pdev->dev.platform_data;
    rtw_drv_halt();
    ret = pfun->rtk_power_off(NORMAL);
    if (ret < 0) {
        RTW_ERR("rtk_power_off fun failed\n");
    }
    if (ret < 0) {
        RTW_ERR("rtk_power_down fun failed\n");
    }
    return ret;
}

static struct platform_driver rtl8723ds_driver = {
    .probe = rtl8723ds_probe,
    .remove = rtl8723ds_remove,
    .driver = {
        .name =  DEVNAME,
    },
};

static int __init rtl8723_driver_init(void) 
{
    int ret = 0;
    ret = platform_driver_register(&rtl8723ds_driver);
    if (ret < 0) {
        RTW_ERR("ERROR: rtl8723ds_driver register failed !!\n");
    }
    return ret;
}

static void __exit rtl8723_driver_exit(void)
{
    platform_driver_unregister(&rtl8723ds_driver);
}

late_initcall_sync(rtl8723_driver_init);
module_exit(rtl8723_driver_exit);
MODULE_LICENSE("GPL");
