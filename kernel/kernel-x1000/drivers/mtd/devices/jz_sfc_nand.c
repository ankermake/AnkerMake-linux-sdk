...
static int32_t is_split_page(const void *buf, size_t len) {
...
static int32_t __init jz_sfc_nand_dev_init(struct sfc_flash *flash)
{
	int32_t ret;
	/*release protect*/
...
#ifdef CONFIG_JZ_SPINAND_FMW
	flash->mtd.size = nand_info->param.flashsize - (CONFIG_SN_SIZE * 1024 * 1024) - (CONFIG_MAC_SIZE * 1024 * 1024) - (CONFIG_LICENSE_SIZE * 1024 * 1024);
#else
