...
static void dwc2_ep0_start_transfer(struct dwc2 *dwc,
	...
	DWC2_EP0_DEBUG_MSG("req 0x%p is_in = %d transfering = %d length = %d dma_mapped = %d\n",
			req, dep->is_in, req->transfering, r->length, req->dma_mapped);

	if (dep->is_in) {
		if (!req->transfering) {
			if (( r->length != 0) && !req->dma_mapped) {
				...
				DWC2_EP0_DEBUG_MSG("req 0x%p dma_mapped to 0x%08x, length = %d\n", req, r->dma, r->length);
