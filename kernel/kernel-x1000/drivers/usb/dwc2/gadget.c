...
//#endif
	...
	if (likely(r->length && req->dma_mapped)) {
	...
	if(req->saved_req_buf) {
		if(!real_dep->is_in && !status)
			memcpy(req->saved_req_buf, r->buf, r->actual);

		kfree(r->buf);

		r->buf = req->saved_req_buf;
		req->saved_req_buf = NULL;
	}

	dev_dbg(dwc->dev, "request %p from %s completed %d/%d ===> %d\n",
			req, real_dep->name, r->actual,
			r->length, status);

	dwc2_spin_unlock(dwc);
	r->complete(&dep->usb_ep, r);
	dwc2_spin_lock(dwc);
}
...
static int __dwc2_gadget_ep_queue(struct dwc2_ep *dep, struct dwc2_request *req)
{
	...
	if (likely(r->length)) {
		if(((unsigned int)(r->buf)) % 4) {
			...
			req_buf = kzalloc(r->length, GFP_ATOMIC);
			if(!req_buf) {
				dev_err(dwc->dev, "unable to allocate memory for bounce buffer\n");
				return -ENOMEM;
			}

			if(dep->is_in)
				memcpy(req_buf, r->buf, r->length);

			req->saved_req_buf = r->buf;
			r->buf = req_buf;
		}
		...
		DWC2_GADGET_DEBUG_MSG("%s_queue: req = 0x%p len = %d dma_mapped=0x%08x\n",
...
int dwc2_gadget_init(struct dwc2 *dwc)
{
	...
	dwc->ep0out_shadow_buf = (u8 *)kmalloc(DWC_EP0_MAXPACKET, GFP_ATOMIC);
	dwc->ep0out_shadow_uncached = UNCAC_ADDR((void *)dwc->ep0out_shadow_buf);

	memset(dwc->ep0out_shadow_uncached, 0, DWC_EP0_MAXPACKET);
	dwc->ep0out_shadow_dma = virt_to_phys(dwc->ep0out_shadow_buf);
