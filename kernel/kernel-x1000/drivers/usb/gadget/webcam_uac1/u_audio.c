		...
		struct snd_pcm_hw_params *hw_params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (p_srate != params_rate(hw_params))
			goto err_param_invalid;
		if (num_channels(p_chmask) != params_channels(hw_params))
			goto err_param_invalid;
	} else {
		if (c_srate != params_rate(hw_params))
			goto err_param_invalid;
		if (num_channels(c_chmask) != params_channels(hw_params))
			goto err_param_invalid;
	}
...
err_param_invalid:
	printk(KERN_ERR "%s:%d Error, param invalid!\n", __func__, __LINE__);
	return -EINVAL;
}
