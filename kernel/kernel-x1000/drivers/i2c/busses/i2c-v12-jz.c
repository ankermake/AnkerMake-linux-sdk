...
static inline int xfer_read_poll(struct i2c_jz *i2c, unsigned char *buf, int len,
...
static inline int xfer_write_poll(struct i2c_jz *i2c, unsigned char *buf, int len,
		enum msg_end_type end_type)
{
	int ret = 0;
	...
	for (i = 0; i < count; i++, msg++) {
		...
		if (msg->flags & I2C_M_RD) {
			if (!i2c->poll_flags)
				...
				ret = xfer_read_poll(i2c, msg->buf, msg->len, end_type);
		} else {
			if (!i2c->poll_flags)
				ret = xfer_write(i2c, msg->buf, msg->len, end_type);
			else
				ret = xfer_write_poll(i2c, msg->buf, msg->len, end_type);
		}
...
void i2c_jz_select_poll_mode(struct i2c_adapter *adap)
{
	struct i2c_jz *i2c = adap->algo_data;
	i2c->poll_flags = 1;
}
EXPORT_SYMBOL(i2c_jz_select_poll_mode);

void i2c_jz_select_normal_mode(struct i2c_adapter *adap)
{
	struct i2c_jz *i2c = adap->algo_data;
	i2c->poll_flags = 0;
}
EXPORT_SYMBOL(i2c_jz_select_normal_mode);
...
static int i2c_jz_probe(struct platform_device *dev)
{
	...
	//some slave devices may cause spikes in SCL, make the controller unable to send
	//so set the filter
