...
static int wlan_power_on(void)
{
	...
	if (-1 != bcm_power->power_en){
		gpio_direction_output(bcm_power->power_en, bcm_power->power_en_level);
	}
...
static int _bcm_power_on(void)
{
	...
	if (bcm_power->use_count == 0) {
		clk_32k_on();
		udelay(100);  //two sleep cycles,  by snmu for android-6.0 speed up,warn:it is probably cause has bug.
		ret = wlan_power_on();
		if (ret < 0) {
...
static int bcm_power_probe(struct platform_device *pdev)
{
	...
	bcm_power->power_en_level = pdata->wlan_pwr_en_level;
