...
static int inline __tcu_as_timer_config(struct tcu_device *tcu)
{
	printk("__tcu_as_timer_config dummy function\n");
	return 0;
}

int tcu_as_timer_config(struct tcu_device *) __attribute__((weak,alias("__tcu_as_timer_config")));
...
static int jz_pwm_request(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	pr_info("%s, request pwm channel %d successfully\n", __func__, id);
	return 0;
}
			     ...
			     int duty_ns, int period_ns)
{
	...
	tmp = (unsigned long long)JZ_EXTAL * period_ns;
	...
	if (prescaler == 6) {
		pr_err("%s, period_ns(%d) period(%lu) too big\n",
				__func__, period_ns, period);
		return -EINVAL;
	}
