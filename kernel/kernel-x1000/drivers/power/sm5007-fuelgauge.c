...
void fg_vbatocv_check(struct i2c_client *client)
{
	...
	if((abs(fuelgauge->info.batt_current)<30) //30mA under
		&& (abs(fuelgauge->info.p_batt_current-fuelgauge->info.batt_current)<10))
	{
		if(abs(fuelgauge->info.batt_ocv-fuelgauge->info.batt_voltage)>5) // 5mV over
		{
	...
	if(fuelgauge->info.iocv_error_count > 5)
	{
		...
		else
		{
		    ...
		    DEBUG_PRINT("%s: mode change to mix RS manual mode\n", __func__);
		    // RS manual value write
		    sm5007_fg_i2c_write_word(client, SM5007_REG_RS_MAN, fuelgauge->info.rs_value[0]);
		    // run update
	...
	else
	{
		...
		{
		    DEBUG_PRINT("%s: mode change to mix RS auto mode\n", __func__);
...
//	int ta_exist;
	...
	fuelgauge->is_charging = (fuelgauge->info.batt_current > 5) ? true : false;
	...
	DEBUG_PRINT("%s: temp_std = %d, temperature = %d, temp_offset = %d, temp_offset_cal = 0x%x, temp_offset_cal_ofs = 0x%x,  curr_cal = 0x%x\n",
...
#endif
	DEBUG_PRINT("%s: batt_avgvoltage = %d\n",
...
#endif
	DEBUG_PRINT("%s: batt_avgcurrent = %d\n",
...
static int fg_get_temp(struct i2c_client *client)
{
	...
	DEBUG_PRINT("%s: read = 0x%x, temperature = %d\n", __func__, ret, temp);
...
static bool sm5007_fg_reg_init(struct i2c_client *client, int ocv_level)
{
	int i, j, value, ret;
	uint8_t table_reg;
	struct sm5007_fuelgauge_info *fuelgauge = i2c_get_clientdata(client);
	...
	/* CAL write*/
	sm5007_fg_i2c_write_word(client,
		SM5007_REG_VOLT_CAL, fuelgauge->info.volt_cal);
	sm5007_fg_i2c_write_word(client,
	...
	/*Manual OCV Enable Check*/
	if(ocv_level != 0)
		value |= ENABLE_IOCV_MAN_MODE;
	...
	{
		value = 0;
		value |= (ocv_level / 1000) * 2048;
		value |= ((ocv_level - ((value/256)*125))*256/125);
		sm5007_fg_i2c_write_word(client, SM5007_REG_IOCV_MAN, value);
	}
...
bool sm5007_fg_fuelalert_init(struct i2c_client *client, struct sm5007_fuelgauge_platform_data *pdata)
{
	...
	ret |= ((fuelgauge->info.alarm_vol_mv - ((ret/32) * 125))*32/125 );
	//ret = 0x0300;//3000mV
...
bool sm5007_en_lp_mode(struct device *dev, bool isEnable)
{
	...
	if(isEnable) {
		...
		sm5007_fg_i2c_write_word(fuelgauge->client, SM5007_REG_RS_MAN, fuelgauge->info.rs_value[0]>>3);
		// run update
...
/*Maulal OCV write with FG reset. if OCV is 4.321V then ocv_level will be used 4321*/
bool sm5007_fg_reset_maual_ocv(struct i2c_client *client, int ocv_level)
{
    pr_debug("%s: sm5007_fg_reset_maual_ocv\n", __func__);
	...
	msleep(200);
	/* init code*/
	sm5007_fg_reg_init(client, ocv_level);
			...
			const struct i2c_device_id *id)
{
	...
	/* get rce*/
	fuelgauge->info.rce_value[0] = pdata->type.rce_value[0];
	fuelgauge->info.rce_value[1] = pdata->type.rce_value[1];
	fuelgauge->info.rce_value[2] = pdata->type.rce_value[2];
	pr_debug("rce_value = <0x%x 0x%x 0x%x>\n",
		fuelgauge->info.rce_value[0], fuelgauge->info.rce_value[1],
		fuelgauge->info.rce_value[2]);

	/* get dtcd_value*/
	fuelgauge->info.dtcd_value = pdata->type.dtcd_value;
	pr_debug("dtcd_value = <0x%x>\n", fuelgauge->info.dtcd_value);

	/* get rs_value*/
	fuelgauge->info.rs_value[0] = pdata->type.rs_value[0];
	fuelgauge->info.rs_value[1] = pdata->type.rs_value[1];
	fuelgauge->info.rs_value[2] = pdata->type.rs_value[2];
	fuelgauge->info.rs_value[3] = pdata->type.rs_value[3];
	...
	/* get vit_period*/
	fuelgauge->info.vit_period = pdata->type.vit_period;
	pr_debug("vit_period = <0x%x>\n", fuelgauge->info.vit_period);

	/* get mix_value*/
	fuelgauge->info.mix_value[0] = pdata->type.mix_value[0];
	fuelgauge->info.mix_value[1] = pdata->type.mix_value[1];
	pr_debug("mix_value = <0x%x 0x%x>\n",
		fuelgauge->info.mix_value[0], fuelgauge->info.mix_value[1]);

	/* battery_type*/
	fuelgauge->info.battery_type = pdata->type.battery_type;
	pr_debug("battery_type = <0x%d>\n", fuelgauge->info.battery_type);

	/* VOL & CURR CAL*/
	fuelgauge->info.volt_cal = pdata->type.volt_cal;
	fuelgauge->info.curr_cal = pdata->type.curr_cal;
	pr_debug("volt_cal = <0x%x>\n", fuelgauge->info.volt_cal);
	pr_debug("curr_cal = <0x%x>\n", fuelgauge->info.curr_cal);

	/* temp_std*/
	fuelgauge->info.temp_std = pdata->type.temp_std;
	pr_debug("temp_std = <0x%x>\n", fuelgauge->info.temp_std);

	/* temp_offset*/
	fuelgauge->info.temp_offset = pdata->type.temp_offset;
	pr_debug("temp_offset = <0x%x>\n", fuelgauge->info.temp_offset);

	/* temp_offset_cal*/
	fuelgauge->info.temp_offset_cal = pdata->type.temp_offset_cal;
	pr_debug("temp_offset_cal = <0x%x>\n", fuelgauge->info.temp_offset_cal);

	/* temp_offset_cal_ofs*/
	fuelgauge->info.temp_offset_cal_ofs = pdata->type.temp_offset_cal_ofs;
	pr_debug("temp_offset_cal_ofs = <0x%x>\n", fuelgauge->info.temp_offset_cal_ofs);

	/* charge_offset_cal*/
	fuelgauge->info.charge_offset_cal = pdata->type.charge_offset_cal;
	pr_debug("charge_offset_cal = <0x%x>\n", fuelgauge->info.charge_offset_cal);

	/*TOPOFF_ENABLE*/
	fuelgauge->info.enable_topoff_soc= pdata->type.topoff_soc[0];

	/*TOPOFF_SOC*/
	fuelgauge->info.topoff_soc = pdata->type.topoff_soc[1];
	/**************************************************/
