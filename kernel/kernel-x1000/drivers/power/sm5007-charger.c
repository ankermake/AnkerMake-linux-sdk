...
static void sm5703_chg_delay_work(struct work_struct *work) {
    struct sm5007_charger_info *info = container_of(to_delayed_work(work),
                struct sm5007_charger_info, delay_work);
    uint8_t status;
    int ret;
    ...
    printk("%s: USB %s\n", __func__, (status & SM5007_STATE_VBUSPOK) ? "connect" : "disconnect");
...
static irqreturn_t sm5703_chg_vbuspok_irq_handler(int irq, void *data)
{
    ...
    if (status & SM5007_STATE_VBUSPOK) {
		...
		cancel_delayed_work(&info->delay_work);
		schedule_delayed_work(&info->delay_work, DELAY_WORK_TIME);
	}
...
static int sm5007_charger_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&info->delay_work, sm5703_chg_delay_work);
