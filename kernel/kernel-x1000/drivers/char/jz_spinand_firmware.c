...
static int fmw_open(struct inode *inode, struct file *filp)
{
    ...
    if(!driver_find(JZ_SFC_NAND, &platform_bus_type)) {
        printk("%s %s %d: %s driver module init failed!\n",
...
static int32_t fwm_buf_compare(uint8_t *wbuf, uint8_t *rbuf, uint32_t len)
{
    ...
    if (ret) {
        printk("compare err wbuf do not equal to rbuf.\n");
        return -EIO;
    }
    return 0;
}
...
static int32_t sn_read_config(struct fmw_dev *fmw)
{
    ...
    for (i = 0; i < CONFIG_SN_FLASH_SIZE / mtd->erasesize; i++) {
        ret  = fmw_read(mtd->size + CONFIG_MAC_FLASH_SIZE + flash_off, &fmw->sn_config, sizeof(fmw->sn_config));
    ...
    if(fmw->sn_config.sn_len == -1 ||
        ...
        printk("%s %s %d: flash don`t save sn value!\n",
...
static int32_t mac_read(struct fmw_dev *fmw)
{
    ...
    if(mac_read_config(fmw)) {
        printk("%s %s %d:read mac config failed!\n",
            __FILE__, __func__, __LINE__);
        return -EIO;
    }
    ...
    if(i == CONFIG_MAC_FLASH_SIZE / blocksize) {
        printk("%s %s %d: mac flash all blocks ecc error!\n",
            __FILE__, __func__, __LINE__);
            return -EIO;
    }
...
static int32_t license_read(struct fmw_dev *fmw)
{
    ...
    if(i == CONFIG_LICENSE_FLASH_SIZE / blocksize) {
        printk("%s %s %d: license flash all blocks ecc error!\n",
            __FILE__, __func__, __LINE__);
        return -EIO;
    }
    ...
    switch(cmd) {
        case SN_GET_CONFIG:
            if((ret = sn_read_config(fmw))) {
                printk("%s %s %d: sn get config failed, ret = %d\n",
        ...
        case MAC_GET_CONFIG:
            if((ret = mac_read_config(fmw))) {
                printk("%s %s %d: mac get config failed, ret = %d\n",
                    __FILE__, __func__, __LINE__, ret);
                break;
            }
        ...
        case MAC_READ:
            if((ret = mac_read(fmw))) {
                printk("%s %s %d: mac_read failed, ret = %d\n",
                    __FILE__, __func__, __LINE__, ret);
                break;
            }
        ...
        case LICENSE_WRITE:
            if((copy_from_user(&fmw->license_config, (void *)args, sizeof(fmw->license_config)))) {
                printk("%s %s %d: get license_config error, ret = %d\n",
                    __FILE__, __func__, __LINE__, ret);
                break;
            }
            ...
            if((ret = copy_from_user(fmw->buf, (char*)args+sizeof(struct license_config), fmw->license_config.license_len))) {
                printk("%s %s %d: get license data error, ret = %d\n",
                    __FILE__, __func__, __LINE__, ret);
                break;
            }
            if((ret = license_write(fmw))) {
                printk("%s %s %d: license write failed, ret = %d\n",
                    __FILE__, __func__, __LINE__, ret);
                break;
            }
            break;
        ...
        default:
            printk("%s %s %d: don`t support this cmd!\n", __FILE__, __func__, __LINE__);
            ret = -EINVAL;
    }
    return ret;
}
...
static int __init fmw_init(void)
{
    ...
    if(!fmw->buf) {
        printk("%s %s %d: alloc sn buf failed!\n", __FILE__, __func__, __LINE__);
        kfree(fmw);
        return -ENOMEM;
    }
    ...
    if(alloc_chrdev_region(&dev, fmw->minor, fmw->nr_devs, "jz-spinand-fmw")) {
        ...
        printk("%s %s %d: alloc chrdev failed!\n",
            __FILE__, __func__, __LINE__);
        ret = -ENODEV;
        goto failed;
    }
    ...
    if(IS_ERR_OR_NULL(fmw->dev)) {
        printk("%s %s %d :creates a device and register sysfs failed !\n",
            __FILE__, __func__, __LINE__);
        ret = -ENODEV;
        goto free_all;
    }
