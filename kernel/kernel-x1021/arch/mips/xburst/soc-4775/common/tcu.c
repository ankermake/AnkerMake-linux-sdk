...
static int tcu_set_fun_pin(struct tcu_device *tcu)
{
	...
	switch (tcu->id) {
	case 0:
		ret = jzgpio_set_func(GPIO_PORT_E, GPIO_FUNC_0, BIT(00));
		break;
	case 1:
		ret = jzgpio_set_func(GPIO_PORT_E, GPIO_FUNC_0, BIT(01));
		break;
	case 2:
		ret = jzgpio_set_func(GPIO_PORT_E, GPIO_FUNC_0, BIT(02));
		break;
	case 3:
		ret = jzgpio_set_func(GPIO_PORT_E, GPIO_FUNC_0, BIT(03));
		break;
	default:
	...
	if ( ret )
		pr_warn("TCU:%d, jzgpio_set_func() failed(%d), Please check your code\n",
			tcu->id, ret);
	return ret;
}
 ...
 * the TCU controller designed shortage, only TCU1 mode can
 * set default output level, but TCU2 doesn't. Only for M200 and X1000*/
static int tcu_set_pin_to_default_level(struct tcu_device *tcu, int inverse)
{
	int ret = -EIO;
	enum gpio_function pin_level;
	unsigned int init_level; /*used in pwm output mode*/

	init_level = tcu->init_level;

	if (inverse)
		init_level = !init_level;

	pin_level = init_level ? GPIO_OUTPUT1 : GPIO_OUTPUT0;
	...
	switch (tcu->id) {
	case 0:
		ret = jzgpio_set_func(GPIO_PORT_E, pin_level, BIT(0));
		break;
	case 1:
		ret = jzgpio_set_func(GPIO_PORT_E, pin_level, BIT(1));
		break;
	case 2:
		ret = jzgpio_set_func(GPIO_PORT_E, pin_level, BIT(2));
		break;
	case 3:
		ret = jzgpio_set_func(GPIO_PORT_E, pin_level, BIT(3));
		break;
	default:
		pr_warn("TCU:%d, has not default pin,"
				" Please check your code\n",
	...
	if ( ret )
		pr_warn("TCU:%d, jzgpio_set_func() GPIO_OUTPUTx failed(%d), Please check your code\n",
			tcu->id, ret);
	return ret;
}
...
static void set_tcu_full_half_value_wait_period_end(struct tcu_device *tcu)
{
	if (tcu->full_num == tcu->half_num) {
		tcu_set_pin_to_default_level(tcu, 0);
		tcu->half_num -= 1;
		//return;
	}
	else if (tcu->half_num < 2 ) {
		tcu_set_pin_to_default_level(tcu, 1); /* inverse pwm gpio */
		//return;
	}
	else {
		tcu_set_fun_pin(tcu);
...
#if 0
	if (tcu->using) {
		unsigned long timeout_1s;
		unsigned int counter;
		unsigned int full;

		full = regr(CH_TDFR(tcu->id));
		timeout_1s = jiffies + HZ;
		/* wait counter end, time cost is (0 ~ pwm period). */
		do {
			if (time_after(jiffies, timeout_1s)) {/* 1 second timeout*/
				pr_warn("TCU:%d, %s() 1 second timeout\n", tcu->id, __func__);
				break;
			}
			counter = regr(CH_TCNT(tcu->id));
		} while (counter < (full -1));
	}
#endif
...
{
/*
	printk("tcu %d: flag=%x, init_level=%d, using=%d, shutdown=%d,\
		div=%d, full=%d, half=%d, count=%d\n",
	       tcu->id, tcu->pwm_flag, tcu->init_level, tcu->using, tcu->pwm_shutdown,
	       tcu->divi_ratio, tcu->full_num, tcu->half_num, tcu->count_value);
*/
	...
	if(tcu->pwm_flag) {
		/* update half/full */
		//set_tcu_full_half_value(tcu->id,tcu->full_num,tcu->half_num);
		set_tcu_full_half_value_wait_period_end(tcu);
	}
