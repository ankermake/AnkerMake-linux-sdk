	...
	struct gpio_reg_func *rfunc, unsigned int pins)
{
	...
	if(comp == pins) {
		writel(pins, chip->reg + PXPAT1S);
	}
	comp = pins & rfunc->save[3];
	if(comp == pins) {
		writel(pins, chip->reg + PXPAT0S);
	}

	comp = pins & (~rfunc->save[0]);
	if(comp == pins) {
		writel(pins, chip->reg + PXINTC);
	}
	comp = pins & (~rfunc->save[1]);
	if(comp == pins) {
		writel(pins, chip->reg + PXMSKC);
	}
	comp = pins & (~rfunc->save[2]);
	if(comp == pins) {
		writel(pins, chip->reg + PXPAT1C);
	}
	comp = pins & (~rfunc->save[3]);
	if(comp == pins) {
		writel(pins, chip->reg + PXPAT0C);
	}

	comp = pins & rfunc->save[4];
	if(comp == pins) {
		writel(pins, chip->reg + PXPENS);
	}
	comp = pins & (~rfunc->save[4]);
	if(comp == pins) {
		writel(pins, chip->reg + PXPENC);
	}
	spin_unlock_irqrestore(&chip->gpio_lock,flags);
}
...
void gpio_suspend_set(struct jzgpio_chip *jz)
{
	...
	pxint &= jz->sleep_state.pxignore;
	pxmsk &= jz->sleep_state.pxignore;
	pxpat1 &= jz->sleep_state.pxignore;
	pxpat0 &= jz->sleep_state.pxignore;
	pxpen &= jz->sleep_state.pxignore;

	pxint |= (jz->sleep_state.pxint);
	pxmsk |= (jz->sleep_state.pxmsk);
	pxpat1 |= (jz->sleep_state.pxpat1);
	pxpat0 |= (jz->sleep_state.pxpat0);
	pxpen |= (jz->sleep_state.pxpen);
...
int __init gpio_ss_check(void)
{
	...
	for(i = 0; gpio_ss_table[i][1] != GSS_TABLET_END;i++) {
		...
		if(gpio_type == -1) {
			...
			jz_gpio_chips[group].sleep_state.pxpen  &= ~(1 << index);
			jz_gpio_chips[group].sleep_state.pxint  &= ~(1 << index);
			jz_gpio_chips[group].sleep_state.pxmsk  &= ~(1 << index);
			jz_gpio_chips[group].sleep_state.pxpat1 &= ~(1 << index);
			jz_gpio_chips[group].sleep_state.pxpat0 &= ~(1 << index);
		} else{
