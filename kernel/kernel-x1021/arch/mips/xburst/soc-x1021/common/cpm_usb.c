...
#define USBPCR_UTMI_RST0	        BIT(6)														/*0*/
...
#define USBRDT_UTMI_RST_BIT		27
#define USBRDT_UTMI_RST			BIT(USBRDT_UTMI_RST_BIT)
#define USBRDT_USBRDT_MSK		(0x5)
...
#define USBPCR1_REFCLKSEL_MSK		(0x3 << 26)
#define USBPCR1_REFCLKDIV_MSK	        (0x3 << 23)
#define USBPCR1_REFCLKDIV_24M	        (0x5 << 23)
#define USBPCR1_PORT_RST		BIT(21)
#define USBPCR1_WORD_IF_16BIT	        BIT(19)
#define USBPCR1_TXRISETUNE_BIT          6
#define USBPCR1_TXRISETUNE_MSK          (0x3 << USBPCR1_TXRISETUNE_BIT)
#define USBPCR1_TXRISETUNE(x)           (((x) << USBPCR1_TXRISETUNE_BIT) & USBPCR1_TXRISETUNE_MSK)
#define USBPCR1_TXRESTUNE_BIT           4
#define USBPCR1_TXRESTUNE_MSK           (0x3 << USBPCR1_TXRISETUNE_BIT)
#define USBPCR1_TXRESTUNE(x)            (((x) << USBPCR1_TXRISETUNE_BIT) & USBPCR1_TXRISETUNE_MSK)
#define USBPCR1_TXPREEMPAMPTUNE_BIT     2
#define USBPCR1_TXPREEMPAMPTUNE_MSK     (0x3 << USBPCR1_TXPREEMPAMPTUNE_BIT)
#define USBPCR1_TXPREEMPAMPTUNE(x)      (((x) << USBPCR1_TXPREEMPAMPTUNE_BIT) & USBPCR1_TXPREEMPAMPTUNE_BIT)
#define USBPCR1_TXPREEMPAMP             BIT(1)
#define USBPCR1_UHCEN                   BIT(0)
...
{
	cpm_set_bit(SRBC_USB_SR, CPM_SRBC0);
	udelay(10);
	cpm_clear_bit(SRBC_USB_SR, CPM_SRBC0);
}
EXPORT_SYMBOL(jz_otg_ctr_reset);

void jz_otg_phy_init(otg_mode_t mode)
{
	unsigned int usbpcr1, usbpcr;

	/*phy clk*/
	cpm_clear_bit(OPCR_GATE_USBPHY_CLK_BIT, CPM_OPCR);

	usbpcr1 = cpm_inl(CPM_USBPCR1);
	usbpcr1 &= ~((USBPCR1_REFCLKDIV_MSK)|USBPCR1_DMPD|USBPCR1_DPPD);
	usbpcr1 |= BIT(30) | BIT(8) | BIT(9) | USBPCR1_REFCLKDIV_24M;
	usbpcr = USBPCR_IDPULLUP_OTG | USBPCR_COMMONONN | USBPCR_VBUSVLDEXT | USBPCR_VBUSVLDEXTSEL |
		USBPCR_COMPDISTUNE(0) | USBPCR_OTGTUNE(0) | USBPCR_SQRXTUNE(7) | USBPCR_TXFSLSTUNE(0) |
		USBPCR_TXHSXVTUNE(1) | USBPCR_TXVREFTUNE(10);
	...
	cpm_outl(USBVBFIL_IDDIGFIL(0xff)|USBVBFIL_USBVBFIL(0x80), CPM_USBVBFIL);

	/*WARNING: when USBRDT BIT(1) set, the phy id always be detect low level, BUG ?*/
	cpm_outl(USBRDT_USBRDT(0x4), CPM_USBRDT);

	cpm_clear_bit(USBRDT_UTMI_RST_BIT, CPM_USBRDT);
	...
	udelay(501);
	cpm_set_bit(USBRDT_UTMI_RST_BIT, CPM_USBRDT);
	udelay(1);
	usbpcr1 |= USBPCR1_DMPD|USBPCR1_DPPD;
	cpm_outl(usbpcr1, CPM_USBPCR1);
	return;
}
...
{
	return (!(cpm_test_bit(OPCR_SPENDN_BIT, CPM_OPCR)));
}
