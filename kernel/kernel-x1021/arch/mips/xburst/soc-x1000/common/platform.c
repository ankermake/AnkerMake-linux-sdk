...
#if (defined(CONFIG_SND_ASOC_JZ_AIC_SPDIF_V13) || defined(CONFIG_SND_ASOC_JZ_EXTCODEC) || defined(CONFIG_JZ_EXTERNAL_CODEC_V13))
	I2S_PORTB,
#endif
...
#if defined(CONFIG_USB_DWC2_DRVVBUS_PIN) || (!defined(CONFIG_USB_DWC2) && !defined(CONFIG_USB_JZ_DWC2))
	OTG_DRVVUS,
#endif
...
static struct resource jz_mcu_res[] = {
	[0] = {
		.name  = "pdma",
		.flags = IORESOURCE_MEM,
		.start = PDMA_IOBASE,
	...
	[1] = {
		...
		.flags = IORESOURCE_MEM,
		.start = INTC_IOBASE,
		.end = INTC_IOBASE + 0x1000 - 1,
	},
	[2] = {
		.name  = "gpio",
		.flags = IORESOURCE_MEM,
		.start = GPIO_IOBASE,
		.end = GPIO_IOBASE + 0x10000 - 1,
	},
	[3] = {
		.name  = "pdmam",
		.flags = IORESOURCE_IRQ,
...
static struct jzmcu_platform_data jzmcu_pdata = {
	.irq_base = IRQ_MCU_BASE,
	.irq_end = IRQ_MCU_END,
};

struct platform_device jz_mcu_device = {
	.name = "jz-mcu",
	.id = -1,
	.dev = {
		.platform_data = &jzmcu_pdata,
	},
	.resource = jz_mcu_res,
	.num_resources = ARRAY_SIZE(jz_mcu_res),
};
#endif
