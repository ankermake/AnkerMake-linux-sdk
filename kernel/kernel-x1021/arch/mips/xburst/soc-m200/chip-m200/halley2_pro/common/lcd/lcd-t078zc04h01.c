...
int visionox_t078zc04h01_init(struct lcd_device *lcd)
{
    int ret = 0;
...
int visionox_t078zc04h01_reset(struct lcd_device *lcd)
{
...
int visionox_t078zc04h01_power_on(struct lcd_device *lcd, int enable)
{
    if(visionox_t078zc04h01_init(lcd))
        return -EFAULT;
    gpio_direction_output(GPIO_MIPI_PWR, enable); /* 2.8v en */
    msleep(2);
    return 0;
}
struct lcd_platform_data visionox_t078zc04h01_data = {
	.reset = visionox_t078zc04h01_reset,
	.power_on= visionox_t078zc04h01_power_on,
};

struct mipi_dsim_lcd_device visionox_t078zc04h01_device={
	.name		= "visionox_t078zc04h01-lcd",
	.id = 0,
	.platform_data = &visionox_t078zc04h01_data,
};
