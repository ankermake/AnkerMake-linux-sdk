			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	struct jz_dma_pcm  *jz_pcm = snd_soc_platform_get_drvdata(platform);
	ucontrol->value.integer.value[0] = jz_pcm->dma_mute;
	return 0;
}
...
static int jz_pcm_platform_probe(struct platform_device *pdev)
{
	...
	if(snd_dma_data)
	{
		if(snd_dma_data->dma_soft_mute == SUPPLY_DMA_SOFT_MUTE_AMIXER){
		...
		dev_warn(&pdev->dev,"Don't have platform data\n");
	}
