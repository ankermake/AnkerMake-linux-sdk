...
static struct workqueue_struct *i2s_queue=NULL;
static struct work_struct i2s_start_work;
static struct work_struct i2s_stop_work;
...
#if defined(CONFIG_SOC_X1021) || defined(CONFIG_SOC_X1630)
		...
		struct snd_soc_dai *dai)
{
	...
	I2S_DEBUG_MSG("start set AIC register....\n");
...
struct snd_soc_dai *g_dai;

static void jz_i2s_timer_start_substream(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct jz_i2s *jz_i2s = dev_get_drvdata(dai->dev);
	struct device *aic = jz_i2s->aic;
	int timeout = 150000;
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		dev_dbg(dai->dev, "codec fifo level2 %x\n", jz_aic_read_reg(aic, AICSR));
		...
		struct snd_soc_dai *dai)
{
	...
	g_dai = dai;
	schedule_work(&i2s_start_work);
}

static void jz_is2_timer_stop_substream(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	struct jz_i2s *jz_i2s = dev_get_drvdata(dai->dev);
	struct device *aic = jz_i2s->aic;
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (__i2s_transmit_dma_is_enable(aic)) {
			...
			while((!__aic_test_tur(aic)) && time_before(jiffies, timeout_jiffies));
		...
		if (__i2s_receive_dma_is_enable(aic)) {
			...
			__aic_clear_ror(aic);
			/*step 3: make sure no data transfer on ahb2 bus*/
			while(!__aic_test_ror(aic) && time_before(jiffies, timeout_jiffies));
			I2S_DEBUG_MSG("i2s record stop ok\n");
			/*step 4: safely stop record function*/
			if (IS_ERR_OR_NULL(jz_i2s->i2s_enable))
				__i2s_disable_record(aic);
			else
				clk_disable(jz_i2s->i2s_enable);	/*aec stop function*/
		...
		struct snd_soc_dai *dai)
{
	...
	schedule_work(&i2s_stop_work);
}

static void i2s_start_work_handler(struct work_struct *data)
{
	jz_i2s_timer_start_substream(g_substream, g_dai);
}

static void i2s_stop_work_handler(struct work_struct *data)
{
	jz_is2_timer_stop_substream(g_substream, g_dai);
}
...
static int jz_i2s_platfrom_probe(struct platform_device *pdev)
{
	...
	i2s_queue = create_singlethread_workqueue("hello world");
	if (!i2s_queue) {
		printk("create workqueue failed.\n");
	}
	INIT_WORK(&i2s_start_work, i2s_start_work_handler);
	INIT_WORK(&i2s_stop_work, i2s_stop_work_handler);
