 ...
 *	 used for X1021 X1630 and so on
 *
		...
		enum snd_soc_bias_level level)
{
	...
	printk(KERN_DEBUG "%s enter set level %d\n", __func__, level);
	switch (level) {
	...
	case SND_SOC_BIAS_STANDBY:
		if (!icdc_d4->power_on) {
			...
			snd_soc_write(codec, ICDC_CDC1R, ICDC_CDCR1_DACVALDALEN_16BIT|ICDC_CDCR1_DACI2SMODE_I2S);
			...
			snd_soc_update_bits(codec, ICDC_CCR, ICDC_CCR_DACPERCHARGE_N, ICDC_CCR_DACPERCHARGE_N);
	...
	case SND_SOC_BIAS_OFF:
		if (icdc_d4->power_on) {
			...
			snd_soc_update_bits(codec, ICDC_CHCR, ICDC_CHCR_HPOUTPOP_MSK, ICDC_CHCR_HPOUTPOP_PERCHARGE);
			snd_soc_update_bits(codec, ICDC_CCR, ICDC_CCR_DACCHAGESEL_MSK, ICDC_CCR_DACCHAGESEL(0));
                ...
                struct snd_soc_dai *dai)
{
	...
	printk(KERN_DEBUG "%s enter sample rate %d\n", __func__, speed);
...
#define	AUDIO_SOURCE_SINE	1
#define AUDIO_SOURCE_SQUARE	2
#define AUDIO_SOURCE_ZERO	3
...
static int hpout_volsw_put(struct snd_kcontrol * kcontrol, struct snd_ctl_elem_value * ucontrol)
{
	...
	if (icdc_d4->hpout_vol == 0 && icdc_d4->audio_source != AUDIO_SOURCE_ZERO) {
		icdc_d4->audio_source = AUDIO_SOURCE_ZERO;
		snd_soc_update_bits(codec, ICDC_CASR, ICDC_CASR_DACLECHSRC_MSK, ICDC_CASR_DACLECHSRC_ZERO);
	} else if (icdc_d4->hpout_vol != 0 && icdc_d4->audio_source == AUDIO_SOURCE_ZERO) {
		icdc_d4->audio_source = AUDIO_SOURCE_I2S;
	...
	if (icdc_d4->dac_enabled) {
		...
		snd_soc_update_bits(codec, ICDC_CAFR, BIT(6), BIT(6));
		snd_soc_update_bits(codec, ICDC_CAFR, (0x7 << 3), (0x5 << 3));
		snd_soc_update_bits(codec, ICDC_CGAINR, BIT(5), BIT(5));
		} else*/
...
static const char *audio_source_mode[] = {"I2S", "SINE", "SQUARE" ,"ZERO"};
static const struct soc_enum audio_source_enum[] = {
	SOC_ENUM_SINGLE(SND_SOC_NOPM, 0, ARRAY_SIZE(audio_source_mode), audio_source_mode),
};

static int audio_source_put(struct snd_kcontrol * kcontrol, struct snd_ctl_elem_value * ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct icdc_d4 *icdc_d4 = snd_soc_codec_get_drvdata(codec);
	struct snd_soc_card *card = codec->card;
	...
	icdc_d4->audio_source = ucontrol->value.enumerated.item[0] & 0x3;
	snd_soc_update_bits(codec, ICDC_CASR, ICDC_CASR_DACLECHSRC_MSK, ICDC_CASR_DACLECHSRC(icdc_d4->audio_source));

	mutex_unlock(&card->dapm_mutex);
	return 0;
}

static int audio_source_get(struct snd_kcontrol * kcontrol, struct snd_ctl_elem_value * ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct icdc_d4 *icdc_d4 = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.enumerated.item[0] = icdc_d4->audio_source;
	return 0;
}
...
static const struct snd_kcontrol_new icdc_d4_snd_controls[] = {
	SOC_SINGLE_EXT_TLV("Master Playback Volume", SND_SOC_NOPM,
			0, (ICDC_CHCR_HPOUTGAIN_MSK >> ICDC_CHCR_HPOUTGAIN_SFT), 0,
			hpout_volsw_get, hpout_volsw_put,
			hpout_tlv),
	SOC_SINGLE_EXT_TLV("Mic Volume", SND_SOC_NOPM,
			0, (ICDC_CACR2_ALCGAIN_MSK >> ICDC_CACR2_ALCGAIN_SFT), 0,
			alc_volsw_get, alc_volsw_put,
			alc_tlv),
	SOC_ENUM_EXT("Digital Audio Source", audio_source_enum[0], audio_source_get, audio_source_put),
