	...
	if (icdc_d2_writable(codec, reg)) {
		if (!icdc_d2_volatile(codec,reg)) {
			if((reg == DLV_REG_GCR_DACL)||(reg == DLV_REG_GCR_DACR)){
				tmp_val = (tmp_val != 0x3f) ? (~tmp_val) & 0x3f : 0x3f;
			}
...
static const struct snd_kcontrol_new icdc_d2_snd_controls[] = {
	/* Volume controls */
	SOC_DOUBLE_R_SX_TLV("Digital Playback Volume", DLV_REG_GCR_DACL ,DLV_REG_GCR_DACR, 0, 0x1f, 0x20, dac_tlv),
...
static int icdc_d2_suspend(struct snd_soc_codec *codec)
{
	return 0;
}
...
static int icdc_d2_probe(struct snd_soc_codec *codec)
{
	...
	/* enable speaker */
	snd_soc_update_bits(codec, DLV_REG_CR_HP, 0x3, 0x3);
	snd_soc_update_bits(codec, DLV_REG_CR_LO, 0x3, 0x3);
