...
#include "../welcome.h"
struct snd_tips_data __attribute__((weak)) tips_data = {
	.data		= wel_buf,
	.length		= 50688,
	.vol		= CONFIG_XBURST_SOUND_TIPS_VOL,
	.channels	= 2,
	.fmt		= 16,
	.rate		= 22050,
};
...
static void i2s_match_codec(struct i2s_device *i2s_dev, char *name)
{
	...
	list_for_each_safe(list,tmp,&codecs_head) {
		...
		if (!strcmp(codec_info->name,name)) {
			...
			printk("%s: current codec %p [%s:%s]\n", __func__, codec_info,
					codec_info->name, codec_info->codec_mode ? "Slava" : "Master");
			return;
		}
	}
}
...
static long i2s_ioctl_2(struct snd_dev_data *ddata, unsigned int cmd, unsigned long arg)
{
	...
	case SND_DSP_GET_HP_DETECT:
		...
		*(int*)arg = jz_get_hp_switch_state(i2s_dev);
		...
		break;
		/*
		 * There is no flush_work() in the four case below.
...
static long do_ioctl_work(struct i2s_device *i2s_dev, unsigned int cmd, unsigned long arg)
{
	...
	case SND_DSP_FLUSH_SYNC:
		/*FIXME: Compatible with DSP interface*/
		break;
	default:
...
static int i2s_global_init(struct platform_device *pdev, struct snd_switch_data *switch_data)
{
	...
	*(volatile unsigned int*)0xb0000070 = 0x0;
	...
	dev_info(&pdev->dev, "I2S init success.\n");
	ret = codec_ctrl(i2s_dev->cur_codec, CODEC_INIT,0);
	if (ret < 0)
		dev_err(&pdev->dev, "Codec init fail.\n");
	dev_info(&pdev->dev, "Codec init success.\n");
	return 0;
