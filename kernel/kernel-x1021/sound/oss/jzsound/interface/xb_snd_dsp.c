				   ...
				   enum dma_transfer_direction direction)
{
	...
	if (direction == DMA_DEV_TO_MEM && dp->sg_len >= 3 && dp->mod_timer) {
		atomic_set(&dp->watchdog_avail,0);
#ifdef CONFIG_HIGH_RES_TIMERS
		hrtimer_start(&dp->transfer_watchdog,
				ktime_set(0, dp->watchdog_mdelay * 1000000), HRTIMER_MODE_REL);
...
static void do_replay_watch_func(struct dsp_pipe *dp)
{
	...
	mdelay = (dp->buffersize - node_offset - 1 ) /
		(dp->channels * (dp->samplerate / 1000) * (dp->bits/ 8));

#ifdef CONFIG_HIGH_RES_TIMERS
	hrtimer_start(&dp->transfer_watchdog,
			ktime_set(0, mdelay * 1000000), HRTIMER_MODE_REL);
#else
	mod_timer(&dp->transfer_watchdog,jiffies+msecs_to_jiffies(mdelay));
#endif
...
#else
	mod_timer(&dp->transfer_watchdog,
			jiffies+msecs_to_jiffies(mdelay));
#endif
 ...
 * others
\********************************************************/
static int init_pipe(struct dsp_pipe *dp,struct device *dev,enum dma_transfer_direction direction)
{
	int i = 0;
	...
	{
		printk("SOUDND ERROR: %s,%d, fragsize error...\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	{
		printk("SOUDND ERROR: %s,%d, fragcnt error...\n", __func__, __LINE__);
		return -EINVAL;
	}
			...
			struct snd_dev_data *ddata)
{
	...
	if (dp->is_first_start == true) {
		dp->watchdog_mdelay = dp->buffersize /
			(dp->samplerate / 1000) * dp->channels * (dp->bits / 8);
...
static size_t xb_snd_get_bits(int format)
{
...
int xb_snd_dsp_probe(struct snd_dev_data *ddata)
{
	...
	if ((dp = endpoints->out_endpoint) != NULL) {
		ret = init_pipe(dp , ddata->dev, DMA_MEM_TO_DEV);
	}
	if (ret)
		goto error1;
	if ((dp = endpoints->in_endpoint) != NULL) {
		ret = init_pipe(dp , ddata->dev, DMA_DEV_TO_MEM);
		if (ret)
			goto error2;
	}
