...
{
        dev->lcd_power = 1;
	if (dev->pdata->gpio_lcd_pw_en) {
		gpio_direction_output(dev->pdata->gpio_lcd_pw_en, 1);
	} else {
...
static void kfm701a21_1a_off(struct kfm701a21_1a_data *dev)
{
        dev->lcd_power = 0;
	if (dev->pdata->gpio_lcd_pw_en) {
		gpio_direction_output(dev->pdata->gpio_lcd_pw_en, 0);
	} else {
...
static int kfm701a21_1a_probe(struct platform_device *pdev)
{
	...
	if (dev->pdata->gpio_lcd_pw_en) {
		gpio_request(dev->pdata->gpio_lcd_pw_en, "lcd power en");
	} else {
		dev->lcd_vcc_reg = regulator_get(NULL, dev->pdata->lcd_vcc_name);
