...
static int kd035hvfmd057_probe(struct platform_device *pdev)
{
	...
	props.brightness = 120;
	dev->bd = backlight_device_register("pwm-backlight.0", dev->dev, dev, (struct backlight_ops*)dev->ctrl->pdata, &props);
	if (IS_ERR(dev->bd)) {
		dev_err(dev->dev, "failed to register 'pwm-backlight.0'.\n");
