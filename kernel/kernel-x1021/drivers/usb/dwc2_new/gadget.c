				...
				const struct usb_endpoint_descriptor *desc)
{
	...
	mps = dwc2_max_packet(__le16_to_cpu(desc->wMaxPacketSize));
	mc = dwc2_hb_mult(__le16_to_cpu(desc->wMaxPacketSize));
				      ...
				      int is_selfpowered)
{
	...
	spin_lock_irqsave(&hsotg->lock, flags);
	hsotg->is_selfpowered = !!is_selfpowered;
	spin_unlock_irqrestore(&hsotg->lock, flags);
...
static int dwc2_hsotg_wakeup(struct usb_gadget *gadget)
{
	...
	usb_status = dwc2_readl(hsotg, GOTGCTL);
	if (usb_status & GOTGCTL_BSESVLD && hsotg->connected) {
		if (hsotg->remote_wakeup_allowed) {
			...
			dev_info(hsotg->dev, "Remote Wakeup is disabled\n");
		}
	}
