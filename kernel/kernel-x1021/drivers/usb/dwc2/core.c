...
void calculate_fifo_size(struct dwc2 *dwc) {
	...
	dev_rx_fifo_size = (4 * 1 + 6) + 1 + 3 * (1024 / 4 + 1) + MAX_EPS_CHANNELS + 1;
	host_rx_fifo_size = 2 * (1024 / 4 + 2) + MAX_EPS_CHANNELS;

	rx_fifo_size = max(dev_rx_fifo_size, host_rx_fifo_size);
	rx_fifo_size = (rx_fifo_size + 3) & ~3;
	...
	dev_in_ep_tx_fifo_size0 = 2 * (64 / 4);
	host_non_periodic_tx_fifo_size = 2 * (512 / 4);
	...
	for (i = 0; i < num_in_eps; i++) {
		if (i < (num_in_eps - DWC_NUMBER_OF_HB_EP)) {
			txfifosize.b.depth = (2 * (512 / 4));
		} else {
			txfifosize.b.depth = (2 * (1024 / 4));
		}
...
int dwc2_core_init(struct dwc2 *dwc)
{
	...
	} else if (dwc->otg_mode == HOST_ONLY) {
		usbcfg.b.force_host_mode = 1;
	} else if (dwc->otg_mode == DEVICE_ONLY) {
		usbcfg.b.force_dev_mode = 1;
#endif
	}
	...
	if (!IS_ENABLED(CONFIG_USB_DWC2_HOST_OVERCURRENT_DET)) {
 ...
 */
void dwc2_gadget_disconnect(struct dwc2 *dwc)
{
