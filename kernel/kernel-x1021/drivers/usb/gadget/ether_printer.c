...
#define DRIVER_DESC		"Ethernet and Printer Gadget"
...
static int __init eth_do_config(struct usb_configuration *c)
{
	...
	if (IS_ERR(f_printer)) {
		printk("%s: usb get printer function error %ld\n", __FUNCTION__, PTR_ERR(f_printer));
	} else {
		status = usb_add_function(c, f_printer);
		if (status < 0) {
			printk("%s: usb add printer function error %d\n", __FUNCTION__, status);
			usb_put_function(f_geth);
		}
	}
...
static int __init eth_printer_bind(struct usb_composite_dev *cdev)
{
	struct usb_gadget	*gadget = cdev->gadget;
	struct f_eem_opts	*eem_opts = NULL;
	struct f_ecm_opts	*ecm_opts = NULL;
	struct f_gether_opts	*geth_opts = NULL;
	struct f_printer_opts *opts = NULL;
...
static int __exit eth_printer_unbind(struct usb_composite_dev *cdev)
{
...
static __refdata struct usb_composite_driver eth_printer_driver = {
	.name		= "g_ether_printer",
	.dev		= &device_desc,
	.strings	= dev_strings,
	.max_speed	= USB_SPEED_SUPER,
	.bind		= eth_printer_bind,
	.unbind		= __exit_p(eth_printer_unbind),
};
...
{
	return usb_composite_probe(&eth_printer_driver);
}
module_init(init);

static void __exit cleanup(void)
{
	usb_composite_unregister(&eth_printer_driver);
}
module_exit(cleanup);
