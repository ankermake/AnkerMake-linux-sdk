...
#include "f_hid.c"
#endif
#ifdef CONFIG_USB_ANDROID_PRINTER
#include "f_printer.c"
#endif
...
struct printer_function_config {
	struct usb_function *f_printer;
	struct usb_function_instance *f_printer_inst;
};

static int printer_function_init(struct android_usb_function *f,
		struct usb_composite_dev *cdev)
{
	int ret;
	struct f_printer_opts *opts;
	struct printer_function_config *config;

	config = kzalloc(sizeof(struct printer_function_config), GFP_KERNEL);
	if (!config)
		return -ENOMEM;
	f->config = config;

	config->f_printer_inst = usb_get_function_instance("printer");
	if (IS_ERR(config->f_printer_inst)) {
		ret = PTR_ERR(config->f_printer_inst);
		kfree(f->config);
		f->config = NULL;
		return ret;
	}

	opts = container_of(config->f_printer_inst, struct f_printer_opts, func_inst);
	opts->minor = 0;
	opts->q_len = QLEN;
	opts->pnp_string = pnp_string;

	config->f_printer = usb_get_function(config->f_printer_inst);
	if (IS_ERR(config->f_printer)) {
		ret = PTR_ERR(config->f_printer);
		usb_put_function_instance(config->f_printer_inst);
		kfree(f->config);
		f->config = NULL;
		return ret;
	}
...
static void printer_function_cleanup(struct android_usb_function *f)
{
	struct printer_function_config *config = f->config;

	usb_put_function(config->f_printer);
	usb_put_function_instance(config->f_printer_inst);
	kfree(f->config);
	f->config = NULL;
}

static int printer_function_bind_config(struct android_usb_function *f,
		struct usb_configuration *c)
{
	int ret = 0;
	struct printer_function_config *config = f->config;

	ret = usb_add_function(c, config->f_printer);
	if (ret) {
		pr_err("Could not bind printer config\n");
		return ret;
	}
...
static struct android_usb_function printer_function	= {
	.name		= "printer",
	.init		= printer_function_init,
	.cleanup	= printer_function_cleanup,
	.bind_config	= printer_function_bind_config,
};
#endif
