...
static ssize_t f_printer_opts_q_len_show(struct f_printer_opts *opts, char *page)
{
	int result;
...
static struct f_printer_opts_attribute f_printer_opts_attr_pnp_string =
	__CONFIGFS_ATTR(pnp_string, S_IRUGO | S_IWUSR, f_printer_opts_pnp_string_show, f_printer_opts_pnp_string_store);

static struct f_printer_opts_attribute f_printer_opts_attr_q_len =
	__CONFIGFS_ATTR(q_len, S_IRUGO | S_IWUSR, f_printer_opts_q_len_show, f_printer_opts_q_len_store);

static struct configfs_attribute *printer_attrs[] = {
	&f_printer_opts_attr_pnp_string.attr,
	&f_printer_opts_attr_q_len.attr,
	NULL,
};
...
static struct usb_function_driver printer_usb_func = {
	.name = "printer",
	.mod  = THIS_MODULE,
...
static int __init printer_mod_init(void)
{
	int status = 0;
	...
	if (status)
		return status;
	return usb_function_register(&printer_usb_func);
}

static void __exit printer_mod_exit(void)
{
	usb_function_unregister(&printer_usb_func);
	gprinter_cleanup();
}

module_init(printer_mod_init);
module_exit(printer_mod_exit)

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Craig Nadler");
MODULE_ALIAS("usbfunc:printer");
