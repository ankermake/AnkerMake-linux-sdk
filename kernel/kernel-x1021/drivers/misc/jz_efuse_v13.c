...
static void efuse_vddq_set(unsigned long is_on)
{
	...
	/* wait AVDEFUSE_2V5 setting up for xx ms */
	if (is_on) {
		dev_dbg(efuse->dev, "%s() efuse->write_delay: %dms\n", __func__, efuse->write_delay);
		mdelay(efuse->write_delay);
	}
#endif
}
...
int jz_efuse_read(uint32_t seg_id, uint32_t data_length, uint32_t *buf)
{
	...
	switch(seg_id){
		...
		case RANDOM_ID:
			...
			break;
		case USER_ID:
			if(bit_num > 128) { /* read must word align */
				dev_err(efuse->dev, "read segment %d data must length %d = 128 bit", seg_id, bit_num);
				return -1;
			}
			val = efuse_readl(EFUSE_STATE);
			if(val & CUSTID_PRT) {
				dev_dbg(efuse->dev, "segment user_id is protected, EFUSE_STATE= %x\n", val);
			}
			break;
		case TRIM3_ID:
			if(bit_num != 16) {
				dev_err(efuse->dev, "read segment %d data must length %d = 16 bit", seg_id, bit_num);
				return -1;
			}
			break;
		case PROTECT_ID:
			if(bit_num > 16) { /* read must word align */
...
static int inline merge_old_buf_to_new(unsigned char *new, unsigned char *old, int length)
{
	while(length--)
		*new++ |= *old++;
	return 0;
...
static int jz_efuse_write(uint32_t seg_id, uint32_t data_length, uint32_t *buf)
{
	...
	unsigned int offset;
	uint32_t data_length_w;
	uint32_t *buf2;
	uint32_t buf2_array[8];

#ifndef CONFIG_JZ_EFUSE_WRITE
	return -EACCES;
#endif
	buf2 = &buf2_array[0];
	...
	switch(seg_id){
		case CHIP_ID:
			...
			val = efuse_readl(EFUSE_STATE);
			if(val & CHIP_PRT) {
				dev_err(efuse->dev, "segment chip_id is protected, EFUSE_STATE= %x\n", val);
				return -1;
			}
			break;
		case RANDOM_ID:
		...
		case USER_ID:
			...
			val = efuse_readl(EFUSE_STATE);
			if(val & CUSTID_PRT) {
				dev_err(efuse->dev, "segment user_id is protected, EFUSE_STATE= %x\n", val);
				return -1;
			}
			break;
		case PROTECT_ID:
			if(bit_num > 16) { /* write must word align */
	...
	/* read segment back and merge */
	memset((void*)buf2, 0, 8*sizeof(uint32_t));
	if (jz_efuse_read(seg_id, data_length, buf2)<0) {
		return -1;
	}

	merge_old_buf_to_new((unsigned char *)buf, (unsigned char *)buf2, data_length);

	addr = efuse->id2addr[seg_id];
	word_num = data_length / 4;
	word_num += data_length % 4 ? 1 : 0;
	data_length_w = data_length - 1;

	efuse->write_delay = 1;	/* initial write_delay 1ms */
...
write_efuse:
	 ...
	 */
	val =  addr << 21 | data_length_w << 16 | 1 << 15;
	efuse_writel(val, EFUSE_CTRL);
	...
	/* read and check */
	memset((void*)buf2, 0, 8*sizeof(uint32_t));
	jz_efuse_read(seg_id, data_length, buf2);

	if ( strncmp((char *)buf, (char *)buf2, data_length) ) {
		dev_info(efuse->dev, "%s() efuse_check failed. seg_id: 0x%x, data_length: 0x%x, efuse->write_delay=%d ms\n",
			 __func__, seg_id, data_length, efuse->write_delay);
		/* write efuse again */
		if (efuse->write_delay < 200) {	  /* 200 ms */
			efuse->write_delay += 10; /* 10 */
			goto write_efuse;
		}
		else {
			return -EIO;
...
static long efuse_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	//printk("%s() %x, %x, %p\n", __func__, efuse->wr_info->seg_id, efuse->wr_info->data_length, efuse->wr_info->buf);
	switch (cmd) {
	case CMD_GET_CHIPID:
	case CMD_GET_RANDOM:
	case CMD_GET_USERID:
...
static int efuse_read_chip_id_proc(struct seq_file *m, void *v)
{
	...
	len = seq_printf(m,"--------> chip id(16 Bytes):\n");
	pc = (unsigned char *)&buf[0];
	for(i = 0; i < 16; i++)
		len += seq_printf(m," %02x", *pc++);
	len += seq_printf(m,"\n");
...
static int efuse_read_user_id_proc(struct seq_file *m, void *v)
{
	...
	jz_efuse_read(USER_ID, 16,buf);
	len = seq_printf(m,"--------> user id(16 Bytes):\n");
	pc = (unsigned char *)&buf[0];
	for(i = 0; i < 16; i++)
		len += seq_printf(m," %02x", *pc++);
	len += seq_printf(m,"\n");
...
static int efuse_read_soc_id_proc(struct seq_file *m, void *v)
{
	...
	len = seq_printf(m,"SOC ID(2 Bytes):\n");
