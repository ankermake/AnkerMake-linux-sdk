...
void jz_spi_clk_enable(struct jz_spi *hw)
{
	if(hw->clk_flag == 0)
		return;
...
void jz_spi_clk_disable(struct jz_spi *hw) {
	if(hw->clk_flag)
		return;
...
static unsigned long jz_spi_clk_get_rate(struct spi_device *spi)
{
	struct jz_spi *hw = spi_master_get_devdata(spi->master);
	unsigned long rate;
	u16 cgv;
...
static int jz_spi_set_clk(struct spi_device *spi, unsigned long rate)
{
	struct jz_spi *hw = spi_master_get_devdata(spi->master);
	...
	cur_rate = jz_spi_clk_get_rate(spi);
	if(cur_rate == rate)
		return 0;
	spin_lock(&hw->lock);
	src_rate = clk_get_rate(hw->clk);
...
static int jz_spi_setupxfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	if((ret = jz_spi_set_clk(spi, hz)))
		return ret;

	dev_dbg(&spi->dev, "The real SPI CLK is %ld Hz\n", jz_spi_clk_get_rate(spi));
...
static int __init jz_spi_probe(struct platform_device *pdev)
{
	...
	hw->clk = clk_get(&pdev->dev, "cgu_spi");
	if (IS_ERR(hw->clk) || IS_ERR(hw->clk_gate)) {
	...
	}
	{
		...
		p = clk_get(&pdev->dev, "ext1");
#else
		p = clk_get(&pdev->dev, "cgu_sfc");
#endif
		if(IS_ERR(p)) {
			dev_err(&pdev->dev, "Cannot get spi parent clock\n");
			goto err_no_clk;
		}
		clk_set_parent(hw->clk, p);
	}
