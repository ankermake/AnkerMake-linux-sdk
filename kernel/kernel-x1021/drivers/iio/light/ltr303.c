...
static int ltr303_gpio_irq(struct ltr303_data *ltr303)
{
	...
	rc = gpio_request(ltr303->gpio_int_no, "ltr 303 int");
	if (rc) {
		dev_err(&ltr303->i2c_client->dev, "can's request gpio for ltr 303 int\n");
		goto out1;
	}
...
//(Linux RTOS)<
	...
	ret = ltr303_irq_init(ltr303);
	if (ret < 0) {
                dev_err(&ltr303->i2c_client->dev, "irq init failed\n");
                return ret;
        }
