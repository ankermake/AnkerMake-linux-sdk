...
#define BLOCK_SIZE CONFIG_MTD_BLOCK_BBT_RO_BLOCK_SIZE
				  ...
				  unsigned long block, char *buf)
{
	...
	int index = mtd_div_by_eb(((uint64_t)block * BLOCK_SIZE), mtd);
	int delta = mtd_mod_by_eb(((uint64_t)block * BLOCK_SIZE), mtd);
	int err;
	loff_t off;
	size_t retlen;
	...
	err = mtd_read(dev->mtd, off, BLOCK_SIZE, &retlen, buf);
	if (err) {
			/*
			 ...
			 */
		if (mtd_is_bitflip(err) && (BLOCK_SIZE == retlen))
			return 0;
		else
			return 1;
	}
