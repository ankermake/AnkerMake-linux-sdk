...
static int cw_bat_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = cw_bat_gpio_init(cw_bat->plat_data->bat_low_pin, cw_bat->plat_data->bat_low_func);
	if (ret) {
		dev_err(&cw_bat->client->dev, "cw_bat_gpio_init bat_low_pin error\n");
		return ret;
	}
	ret = cw_bat_gpio_init(cw_bat->plat_data->chg_ok_pin, cw_bat->plat_data->chg_ok_func);
	if (ret) {
		dev_err(&cw_bat->client->dev, "cw_bat_gpio_init chg_ok_pin error\n");
		return ret;
	}

	ret = cw_bat_gpio_init(cw_bat->plat_data->chg_iset_pin, cw_bat->plat_data->chg_iset_func);
	if (ret) {
		dev_err(&cw_bat->client->dev, "cw_bat_gpio_init chg_iset_pin error\n");
		return ret;
	}
