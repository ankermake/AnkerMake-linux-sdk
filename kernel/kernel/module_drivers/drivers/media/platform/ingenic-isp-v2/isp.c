...
int isp_set_wdr(struct isp_device *isp)
{
	...
	wdr_size = tsize;
	wdr_vaddr = dma_alloc_noncoherent(isp->ispcam->dev, tsize, &wdr_paddr, DMA_BIDIRECTIONAL, GFP_KERNEL);
...
int isp_set_mdns(struct isp_device *isp)
{
	...
	isp->buf_info[MDNS_BUF].vaddr = dma_alloc_noncoherent(isp->ispcam->dev, tsize, &isp->buf_info[MDNS_BUF].paddr, DMA_BIDIRECTIONAL, GFP_KERNEL);
...
int isp_set_dualsensor_buf(struct isp_device *isp)
{
	...
	if(!dualsensor_vaddr && !dualsensor_paddr) {
		dualsensor_vaddr = dma_alloc_noncoherent(isp->ispcam->dev, tsize, &dualsensor_paddr, DMA_BIDIRECTIONAL, GFP_KERNEL);
		if(!dualsensor_vaddr || !dualsensor_paddr)
			return -ENOMEM;
	}
...
#ifndef CONFIG_INGENIC_ISP_V2_SENSOR_NUM_ONE
	if(dualsensor_vaddr && dualsensor_paddr) {
		dma_free_noncoherent(g_isp_device[MAIN_ISP_INDEX]->ispcam->dev, dualsensor_size, dualsensor_vaddr, dualsensor_paddr, DMA_BIDIRECTIONAL);
	}
#endif
set_dualsensor_buf_err:
	...
	if(isp->sensor_info->wdr_en && wdr_vaddr && wdr_paddr){
		dma_free_noncoherent(isp->ispcam->dev, wdr_size, wdr_vaddr, wdr_paddr, DMA_BIDIRECTIONAL);
	}
set_wdr_err:
	wdr_vaddr = 0;
	wdr_paddr = 0;
	if(isp->buf_info[MDNS_BUF].vaddr && isp->buf_info[MDNS_BUF].paddr)
		dma_free_noncoherent(isp->ispcam->dev, isp->buf_info[MDNS_BUF].size, isp->buf_info[MDNS_BUF].vaddr, isp->buf_info[MDNS_BUF].paddr, DMA_BIDIRECTIONAL);
set_mdns_err:
