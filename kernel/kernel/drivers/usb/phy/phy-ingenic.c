		...
		const char *buf, size_t count)
{
	...
	if (!strcmp(usb_phy->hsotg_sw_switch, "device")) {
		if(usb_phy->sw_switch_mode == FORCE_DEVICE){
			printk("OTG aleady in Device Mode\n");
			return count;
		}
		printk("---Forced switching Device mode!\n");

		usb_phy->sw_switch_mode = FORCE_DEVICE;

		if (usb_phy->drvvbus_gpiod){
			gpiod_set_value(usb_phy->drvvbus_gpiod,0);
		}
		...
		if(usb_phy->sw_switch_mode == FORCE_HOST){
			printk("OTG aleady in Host Mode\n");
			return count;
		}
		printk("---Forced switching Host mode!\n");

		usb_phy->sw_switch_mode = FORCE_HOST;
