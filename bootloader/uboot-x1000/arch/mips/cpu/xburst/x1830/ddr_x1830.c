...
static inline void ddr_send_cmd(enum ddr_cmd cmd, enum ddr_cmd_reg reg, unsigned int value)
{
	unsigned int lmr = 
		(1 << 0) |
		(cmd << 3) |
...
void ddr_inno_phy_init()
{
	...
	reg = phy_readl(INNO_CHANNEL_EN);
	reg = (reg & ~0x6) | 0x00;
	phy_writel(reg, INNO_CHANNEL_EN);
	mdelay(1);
...
#else
	// precharge all banks
	ddr_send_cmd(CMD_precharge_banks, 0, 1 << 10);

	// EMRS(2) full array self refresh
	ddr_send_cmd(CMD_load_mode_reg, reg_EMRS2, 0);

	// EMRS(3) nothing to do
	ddr_send_cmd(CMD_load_mode_reg, reg_EMRS3, 0);

	// EMRS(1) enable dll
	ddr_send_cmd(CMD_load_mode_reg, reg_EMRS1, 0);

	// MRS dll reset
	ddr_send_cmd(CMD_load_mode_reg, reg_MRS, 1 << 8);

	// precharge all banks
	ddr_send_cmd(CMD_precharge_banks, 0, 1 << 10);

	// auto refresh
	ddr_send_cmd(CMD_auto_refresh, 0, 1 << 10);
	ddr_send_cmd(CMD_auto_refresh, 0, 1 << 10);
	udelay(10*1000);

	// MRS, parameters and not resset dll
	ddr_send_cmd(CMD_load_mode_reg, reg_MRS, DDR_MR0_VALUE);
...
{
	writel(delay_l,X1830_DQS_DELAY_L);
	writel(delay_h,X1830_DQS_DELAY_H);

	printf("X1830_DQS_DELAY_L: %x\n",readl(X1830_DQS_DELAY_L));
	printf("X1830_DQS_DELAY_H: %x\n",readl(X1830_DQS_DELAY_H));
}
/*
...
void phy_calibration(int al8_1x,int ah8_1x,int al8_2x,int ah8_2x)
{
	...
	m=(0x1<<1);
	phy_writel(m,INNO_TRAINING_CTRL);
	//printf("X1830_REG02: %x\n", phy_readl(INNO_TRAINING_CTRL));
	int x=readl(X1830_REG46);
	int y=readl(X1830_REG56);
	x=(x&(0x83)+(al8_1x<<3)+(al8_2x<<4));
	y=(y&(0x83)+(ah8_1x<<3)+(ah8_2x<<4));
	//debug for t30
	writel(30, X1830_REG46);
	writel(27, X1830_REG56);
	printf("X1830_REG46: %x\n", readl(X1830_REG46));
	printf("X1830_REG56: %x\n", readl(X1830_REG56));
#endif
...
#ifdef CONFIG_FPGA
	//ddr_writel(ddr_readl(DDR_DSGCR)&~(1<<4),DDR_DSGCR);
#endif
