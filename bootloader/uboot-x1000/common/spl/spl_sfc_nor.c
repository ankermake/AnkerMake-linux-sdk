...
static char *spl_nor_os_boot(void)
{
	char *cmdargs = NULL;
...
static unsigned int get_part_offset_by_name(struct norflash_partitions *partition, const char *name)
{
	int i = 0;

	for(i = 0; i < partition->num_partition_info; i++) {
		if (!strncmp(partition->nor_partition[i].name, name, strlen(name))) {
...
static char *spl_nor_os_ota_boot(void)
{
	...
	if (img_addr != 0) {
		...
		sfc_read_data(img_addr, sizeof(buf), (unsigned int)buf);
		buf[sizeof(buf) - 1] = 0;
...
static void spl_nor_rtos_boot(void)
{
	struct mini_spi_nor_info *spi_nor_info = nor_info + sizeof(struct spinor_flashinfo);
	...
	if (rtos_check_header(&rtos))
		hang();
	sfc_read_data(CONFIG_RTOS_OFFSET, rtos.img_end - rtos.img_start, (unsigned int *)rtos.img_start);
	rtos_start(&rtos);
}
#endif

#ifdef CONFIG_SPL_RTOS_OTA_BOOT
static void spl_nor_rtos_ota_boot(void)
{
	struct mini_spi_nor_info *spi_nor_info = nor_info + sizeof(struct spinor_flashinfo);
	struct rtos_header rtos;
	...
	sfc_read_data(CONFIG_RTOS_OTA_TAG_OFFSET, sizeof(ota), (unsigned int *)&ota);
	id = rtos_ota_check_header(&ota);
	if (id < 0)
		hang();

	os_offset = id ? CONFIG_RTOS_OTA_OS1_OFFSET : CONFIG_RTOS_OTA_OS0_OFFSET;
	sfc_read_data(os_offset, sizeof(rtos), (unsigned int *)&rtos);
	if (rtos_check_header(&rtos))
		hang();
	sfc_read_data(os_offset, rtos.img_end - rtos.img_start, (unsigned int *)rtos.img_start);
	rtos_start(&rtos);
}
#endif
...
#if defined(CONFIG_SPL_OS_OTA_BOOT)
	return spl_nor_os_ota_boot();
#elif defined(CONFIG_SPL_OS_BOOT)
	cmdargs = spl_nor_os_boot();
#elif defined(CONFIG_SPL_RTOS_BOOT)
	spl_nor_rtos_boot();
#elif defined(CONFIG_SPL_RTOS_OTA_BOOT)
	spl_nor_rtos_ota_boot();
